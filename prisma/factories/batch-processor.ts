...
export class BatchProcessor {
    ...
    async processBatchEnhanced<T, R>(
        ...
        console.log(`🚀 Enhanced batch processing: ${data.length} items in ${totalBatches} batches`);
        console.log(`📊 Configuration: batchSize=${opts.batchSize}, maxConcurrency=${opts.maxConcurrency}`);
        ...
        try {
            ...
            const memoryUsage = opts.enableMemoryMonitoring ? this.getCurrentMemoryUsage() : undefined;

            console.log(`✅ Enhanced batch processing completed in ${duration}ms`);
            console.log(`📈 Results: ${results.length} processed, ${errors.length} errors`);
    ...
    async createInBatchesEnhanced<T>(
        ...
        console.log(`🏗️  Creating ${dataArray.length} ${entityName} records in batches...`);
        ...
        return this.processBatchEnhanced(
            ...
            async (batch, batchIndex) => {
                const createPromises = batch.map((data, itemIndex) => {
                    ...
                    const createPromise = (this.prisma as any)[entityName].create({ data });
                        ...
                        createPromise.then((result: any) => {
                            this.registerRollbackOperation(async () => {
                                try {
                                    ...
                                    console.warn(`Failed to rollback ${entityName} ${result.id}:`, error);
                                }
                            });
                        });
                    }
                ...
                return await this.processConcurrently(createPromises, opts.maxConcurrency);
            },
            opts
        );
    }
    ...
    async createInBatches<T>(
        ...
        const result = await this.createInBatchesEnhanced<T>(entityName, dataArray, options);
    ...
    async updateInBatchesEnhanced<T>(
        ...
        console.log(`🔄 Updating ${updates.length} ${entityName} records in batches...`);
            ...
            async (batch) => {
                const updatePromises = batch.map(({ where, data }) => {
                    ...
                    const updatePromise = opts.enableRollback
                        ? (this.prisma as any)[entityName].findUnique({ where }).then((original: any) => {
                            originalData = original;
                            return (this.prisma as any)[entityName].update({ where, data });
                        })
                    ...
                    if (opts.enableRollback) {
                        updatePromise.then(() => {
                            this.registerRollbackOperation(async () => {
                                if (originalData) {
                                    try {
                                        const { id, createdAt, updatedAt, ...restoreData } = originalData;
                                        await (this.prisma as any)[entityName].update({
                                            where,
                                            data: restoreData
                                        });
                                    } catch (error) {
                                        console.warn(`Failed to rollback ${entityName} update:`, error);
                                    }
                                }
                            });
                        });
                ...
                return await this.processConcurrently(updatePromises, opts.maxConcurrency);
            },
            opts
        );
    }
    ...
    async updateInBatches<T>(
        ...
        const result = await this.updateInBatchesEnhanced<T>(entityName, updates, options);
        return result.results;
    }
    ...
    async upsertInBatchesEnhanced<T>(
        ...
        console.log(`🔀 Upserting ${upserts.length} ${entityName} records in batches...`);

        return this.processBatchEnhanced(
            upserts,
            async (batch) => {
                const upsertPromises = batch.map(({ where, create, update }) => {
                    const upsertPromise = (this.prisma as any)[entityName].upsert({ where, create, update });
                        ...
                        upsertPromise.then((result: any) => {
                            this.registerRollbackOperation(async () => {
                                try {
                                    ...
                                    const existing = await (this.prisma as any)[entityName].findUnique({ where });
                                    if (existing) {
                                        // If it existed, we need to restore the original data
                                        // This is complex and might require additional tracking
                                        console.warn(`Rollback for upsert ${entityName} not fully implemented`);
                                    } else {
                                    ...
                                    console.warn(`Failed to rollback ${entityName} upsert:`, error);
                                }
                            });
                        });
                    }
                ...
                return await this.processConcurrently(upsertPromises, opts.maxConcurrency);
            },
            opts
        );
    }
    ...
    async upsertInBatches<T>(
        ...
        const result = await this.upsertInBatchesEnhanced<T>(entityName, upserts, options);
        return result.results;
    }
    ...
    private async processBatchItems<T, R>(
        ...
        const promises = batch.map((item, index) => processor(item, startIndex + index));
    ...
    generatePerformanceReport(result: BatchOperationResult<any>): string {
        ...
        const errorRate = totalProcessed > 0 ? ((totalErrors / totalProcessed) * 100).toFixed(2) : '0.00';
    ...
    async optimizeForBatchOperations(): Promise<void> {
        try {
            ...
            console.log('💡 Tip: Consider increasing DATABASE_CONNECTION_LIMIT for large batch operations');
    ...
    async validateBatchIntegrityEnhanced(
        ...
        const details: Array<{ check: string; passed: boolean; description: string }> = [];
    ...
    async validateBatchIntegrity(
        ...
        const result = await this.validateBatchIntegrityEnhanced(entityName, expectedCount, businessId);
        return {
    ...
    async rollbackBatchEnhanced(
        ...
        try {
            ...
            console.log(`🔄 Rolling back ${recordsToDelete.length} ${entityName} records...`);

            if (options?.dryRun) {
                console.log(`🧪 Dry run: Would delete ${recordsToDelete.length} records`);
                return {
            ...
            for (let i = 0; i < totalBatches; i++) {
                ...
                const batchEnd = Math.min(batchStart + batchSize, recordsToDelete.length);
                ...
                try {
                    ...
                    console.log(`🔄 Rollback batch ${i + 1}/${totalBatches}: ${result.count} records deleted`);
            ...
            const duration = Date.now() - startTime;
            console.log(`✅ Rollback completed: ${deletedCount} ${entityName} records deleted in ${duration}ms`);
    ...
    async rollbackBatch(
        ...
        const result = await this.rollbackBatchEnhanced(entityName, businessId, createdAfter);
     ...
     * Initialize progress tracking
     */
    private initializeProgressTracker(totalItems: number, totalBatches: number): void {
    ...
    private updateProgress(processedItems: number, currentBatch: number): void {
        ...
        this.progressTracker.estimatedTimeRemaining = remainingItems / this.progressTracker.itemsPerSecond;
    }
    ...
    private logProgress(): void {
        ...
        console.log(`📊 Progress: ${progress.processedItems}/${progress.totalItems} (${percentage}%)`);
    ...
    private startMemoryMonitoring(thresholdMB: number): void {
        this.memoryMonitor = setInterval(() => {
            ...
            if (heapUsedMB > thresholdMB) {
                console.warn(`⚠️  Memory usage high: ${Math.round(heapUsedMB)}MB (threshold: ${thresholdMB}MB)`);
    ...
    private async executeRollback(): Promise<void> {
        console.log(`🔄 Executing ${this.rollbackOperations.length} rollback operations...`);
        ...
        try {
            for await (const item of dataGenerator) {
                ...
                if (batch.length >= opts.batchSize) {
                    try {
                        const batchResults = await this.processBatchItems(batch, processor, opts.maxConcurrency, processed);
                        results.push(...batchResults);
                        ...
                        if (opts.enableProgressLogging && processed % (opts.logInterval! * opts.batchSize) === 0) {
                            console.log(`🌊 Streamed ${processed} items (${Math.round(processed / ((Date.now() - startTime) / 1000))} items/sec)`);
                        }
            ...
            console.log(`✅ Enhanced streaming completed: ${processed} items in ${duration}ms`);
