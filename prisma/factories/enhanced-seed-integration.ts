...
export class EnhancedSeedOrchestrator {
  ...
  async runEnhancedSeed(options: EnhancedSeedOptions): Promise<SeedResult> {
    ...
    try {
      ...
      const scenario = await this.loadScenarioConfig(
        options.scenario || 'demo'
      );
      console.log(
        `📋 Using scenario: ${scenario.name} - ${scenario.description}`
      );

      // Validate configuration
      const configValidation = this.resetManager.validateConfiguration(
      ...
      if (options.resetBeforeSeed) {
        ...
        const resetResult = await this.resetManager.cleanReset(
          options.businessId,
          {
      ...
      const seedResults = await this.executeSeedingProcess(
        options.businessId,
        scenario,
        options.verbose
      );
      ...
      if (options.validateAfterSeed) {
        ...
        validationResult = await this.resetManager.validateDataIntegrity(
          options.businessId
        );
        ...
        console.log(
          `✅ Validation completed: ${validationResult.isValid ? 'PASSED' : 'FAILED'}`
        );
      }
   ...
   */
  private async loadScenarioConfig(
    scenarioName: string
  ): Promise<SeedScenario> {
    const scenario = this.resetManager.getScenario(scenarioName);

    if (!scenario) {
      const availableScenarios = this.resetManager
        .getPredefinedScenarios()
        .map(s => s.name);
      throw new Error(
        `Scenario '${scenarioName}' not found. Available: ${availableScenarios.join(', ')}`
      );
    }
  ...
  private async executeSeedingProcess(
    ...
    try {
      ...
      if (verbose) {
        ...
        console.log(
          `  Historical months: ${config.appointments.historicalMonths}`
        );
        console.log(
          `  Service categories: ${config.services.categories.length}`
        );
      }
