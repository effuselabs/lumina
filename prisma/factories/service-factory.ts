...
import { ServiceCategory, ServiceDefinition, ServicePackage, ValidationResult } from './types';
...
export class ServiceFactory extends BaseFactory<Service> {
     ...
     */
    async generateAllServices(options: ServiceFactoryOptions = {}): Promise<Service[]> {
        ...
        for (const category of this.serviceCategories) {
            console.log(`  Creating ${category.name} services (${category.services.length} services)...`);

            for (const serviceDefinition of category.services) {
                const service = await this.createService(serviceDefinition, category.name, options);
        ...
        if (options.includeSeasonalServices !== false) {
            ...
            for (const seasonalService of this.seasonalServices) {
                const service = await this.createSeasonalService(seasonalService, options);
                services.push(service);
            }
        }

        console.log(`✅ Generated ${services.length} services across ${this.serviceCategories.length} categories`);
    ...
    private async createService(
        ...
        if (!validation.isValid) {
            throw new Error(`Invalid service data: ${validation.errors.map(e => e.message).join(', ')}`);
        }
    ...
    private async createSeasonalService(
        ...
        const isCurrentlyAvailable = seasonalService.availableMonths.includes(currentMonth);
     ...
     */
    getPricingTiers(basePrice: number): { junior: number; senior: number; master: number } {
        return {
    ...
    getServicesByCategory(category: string): ServiceDefinition[] {
        const categoryData = this.serviceCategories.find(cat => cat.name === category);
     ...
     */
    calculatePackagePrice(packageDef: ServicePackage, services: Service[]): number {
