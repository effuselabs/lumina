...
import { BookingPatterns, BookingSource, ValidationResult } from './types';
...
export class AppointmentFactory extends BaseFactory<Appointment> {
  ...
  async generate(options?: AppointmentFactoryOptions): Promise<Appointment> {
    ...
    if (!validation.isValid) {
      throw new Error(
        `Appointment validation failed: ${validation.errors.map(e => e.message).join(', ')}`
      );
    }
   ...
   */
  private generateAppointmentProfile(
    options?: AppointmentFactoryOptions
  ): AppointmentProfile {
    ...
    const { startTime, endTime } = this.generateAppointmentTime(
      appointmentDate,
      staff
    );
    ...
    const { depositAmount, depositPaid } = this.generateDepositInfo(
      serviceIds,
      status
    );
    ...
    const modifications =
      status === 'SCHEDULED'
        ? []
        : this.generateAppointmentModifications(status);
  ...
  private generateAppointmentTime(
    ...
    if (staffSchedule && staffSchedule.startTime && staffSchedule.endTime) {
      ...
      const businessHours =
        this.patterns.peakHours.length > 0
          ? this.patterns.peakHours
          : [9, 10, 11, 14, 15, 16, 17];
      ...
      if (complementaryServices.length > 0) {
        ...
        if (remainingServices.length > 0) {
          selectedServices.push(
            faker.helpers.arrayElement(remainingServices).id
          );
        }
      }
    }
  ...
  private getComplementaryServices(
    ...
    const complementaryCategories = complementaryPairs[primaryCategory] || [
      primaryCategory,
    ];
   ...
   */
  private generateLeadTime(
    source: BookingSource,
    appointmentDate: Date
  ): number {
    const now = new Date();
    const maxLeadTime = Math.floor(
    ...
    return faker.number.int({
      min: minLeadTime,
      max: Math.max(minLeadTime, maxLeadTimeAdjusted),
    });
  }
   ...
   */
  private generateAppointmentModifications(
    status: AppointmentStatus
    ...
    const modificationType = faker.helpers.arrayElement([
      'RESCHEDULE',
      'TIME_CHANGE',
      'SERVICE_CHANGE',
    ]);
  ...
  async generateHistoricalAppointments(
    ...
    console.log(
      `Generating ${targetCount} historical appointments from ${startDate.toDateString()} to ${endDate.toDateString()}...`
    );

    const appointments: Appointment[] = [];
    const options: AppointmentFactoryOptions = {
      startDate,
      endDate,
      targetCount,
    };
