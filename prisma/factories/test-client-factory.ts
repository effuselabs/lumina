...
async function testClientFactory() {
  ...
  try {
    ...
    console.log(
      `✅ Generated client: ${singleClient.firstName} ${singleClient.lastName}`
    );
    ...
    const batchClients = await clientFactory.generateClients(
      5,
      {},
      (processed, total) => {
    ...
    sampleClients.forEach((client, index) => {
      console.log(
        `   ${index + 1}. ${client.firstName} ${client.lastName} - ${client.email}`
      );
      console.log(
        `      ${client.city}, ${client.state} | Marketing: Email=${client.emailMarketing}, SMS=${client.smsMarketing}`
      );
    });
