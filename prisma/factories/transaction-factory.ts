...
export class TransactionFactory extends BaseFactory<Transaction> {
  ...
  async initialize(): Promise<void> {
    ...
    this.financialContext.appointments = appointmentsData.map(apt => ({
      ...
      staff: {
        ...
        commissionRate: apt.staff.commissionRate
          ? Number(apt.staff.commissionRate)
          : undefined,
        chairRentalAmount: apt.staff.chairRentalAmount
          ? Number(apt.staff.chairRentalAmount)
          : undefined,
      },
    }));
    ...
    this.financialContext.staff = staffData.map(staff => ({
      ...
      chairRentalAmount: staff.chairRentalAmount
        ? Number(staff.chairRentalAmount)
        : undefined,
    }));

    console.log(
      `  Loaded ${this.financialContext.appointments.length} completed appointments`
    );
   ...
   */
  private generateTransactionProfile(
  ...
  /**
   * Staff eligible for a given payroll-style transaction type.
   *
   * COMMISSION and CHAIR_RENTAL transactions only make sense for staff on
   * those employment arrangements, and the seeded staff mix is random — a run
   * can legitimately produce a salon where nobody rents a chair.
   */
  private staffEligibleFor(type: TransactionType) {
    switch (type) {
      case 'COMMISSION':
        return this.financialContext.staff.filter(
          s =>
            s.employmentType === 'COMMISSION' || s.employmentType === 'HYBRID'
        );
      case 'CHAIR_RENTAL':
        return this.financialContext.staff.filter(
          s =>
            s.employmentType === 'CHAIR_RENTAL' || s.employmentType === 'HYBRID'
        );
      default:
        return this.financialContext.staff;
    }
  }
   ...
   *
   * Types whose prerequisites are not present are dropped before the draw
   * rather than after. This used to pick blindly and then hand off to a
   * generator that called `faker.helpers.arrayElement` on an empty filter
   * result, which throws "Cannot get value from empty dataset" and fails the
   * whole seed.
   *
   * Because the staff employment mix is randomised, whether the seed
   * succeeded came down to a dice roll — it passed locally and in the
   * migrations job for days, then failed in CI. A seed that works most of
   * the time is the failure mode this rebuild exists to remove: no seed
   * means no working local app, which means nothing gets verified.
   */
  private generateTransactionType(): TransactionType {
    const distribution: Array<{ type: TransactionType; weight: number }> = [
      { type: 'PAYMENT', weight: 70 },
      { type: 'TIP', weight: 15 },
      { type: 'REFUND', weight: 2 },
      { type: 'COMMISSION', weight: 8 },
      { type: 'CHAIR_RENTAL', weight: 3 },
      { type: 'DEPOSIT', weight: 2 },
    ];

    const available = distribution.filter(
      ({ type }) => this.staffEligibleFor(type).length > 0
    );

    // PAYMENT needs only an appointment, so this cannot be empty in a
    // context that has any staff at all.
    if (available.length === 0) {
      return 'PAYMENT';
    }

    return this.weightedRandom(
      available.map(d => d.type),
      available.map(d => d.weight)
    );
  }
   ...
   */
  private generatePaymentTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const appointment = faker.helpers.arrayElement(
      this.financialContext.appointments
    );
    ...
    const paymentId =
      paymentMethod === 'CASH'
        ? undefined
        : this.generatePaymentId(paymentMethod);
    ...
    return {
      ...
      chairRentalApplicable:
        appointment.staff.employmentType === 'CHAIR_RENTAL' ||
        appointment.staff.employmentType === 'HYBRID',
   ...
   */
  private generateTipTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const appointment = faker.helpers.arrayElement(
      this.financialContext.appointments
   ...
   */
  private generateRefundTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const appointment = faker.helpers.arrayElement(
      this.financialContext.appointments
    ...
    return {
      ...
      staffEmploymentType: appointment.staff.employmentType,
      commissionAmount: this.calculateCommissionAdjustment(
        refundAmount,
        appointment.staff
      ),
   ...
   */
  private generateCommissionTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const eligible = this.staffEligibleFor('COMMISSION');
    if (eligible.length === 0) {
      // Not reachable through generateTransactionType, which filters this
      // type out — but a direct caller deserves a legible error rather
      // than faker's "Cannot get value from empty dataset".
      throw new Error(
        'Cannot generate a COMMISSION transaction: no staff on COMMISSION or HYBRID employment'
      );
    }

    const staff = faker.helpers.arrayElement(eligible);
    ...
    if (staffAppointments.length === 0) {
      ...
      const anyStaffAppointment = this.financialContext.appointments.find(
    ...
    const totalRevenue = staffAppointments.reduce(
      (sum, apt) =>
        sum + apt.services.reduce((svcSum, svc) => svcSum + svc.price, 0),
      0
    );
   ...
   */
  private generateChairRentalTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const eligible = this.staffEligibleFor('CHAIR_RENTAL');
    if (eligible.length === 0) {
      throw new Error(
        'Cannot generate a CHAIR_RENTAL transaction: no staff on CHAIR_RENTAL or HYBRID employment'
      );
    }
   ...
   */
  private generateDepositTransaction(
    options?: TransactionFactoryOptions
  ): TransactionProfile {
    const appointment = faker.helpers.arrayElement(
      this.financialContext.appointments
  ...
  protected validate(data: any): ValidationResult {
    ...
    if (data.type === 'CHAIR_RENTAL' && data.amount > 0) {
      errors.push({
        field: 'amount',
        message: 'Chair rental amount must be negative (expense)',
        code: 'INVALID_VALUE',
      });
    }

    if (
      ['PAYMENT', 'TIP', 'COMMISSION', 'DEPOSIT'].includes(data.type) &&
      data.amount < 0
    ) {
      errors.push({
        field: 'amount',
        message: `${data.type} amount must be positive`,
        code: 'INVALID_VALUE',
      });
    }
  ...
  async generateComprehensiveTransactions(
    ...
    console.log(
      `Generating ${targetCount} comprehensive financial transactions...`
    );
    ...
    console.log(
      `✅ Generated ${transactions.length} comprehensive financial transactions`
    );
  ...
  private logTransactionSummary(transactions: Transaction[]): void {
    ...
    transactions.forEach(tx => {
      ...
      if (tx.paymentMethod) {
        summary.byPaymentMethod[tx.paymentMethod] =
          (summary.byPaymentMethod[tx.paymentMethod] || 0) + 1;
      }
