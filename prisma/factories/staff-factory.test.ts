...
jest.mock('@faker-js/faker', () => ({
    faker: {
        ...
        datatype: {
            boolean: (probability?: number) => probability ? probability > 0.5 : true,
        },
        location: {
...
describe('StaffFactory', () => {
    ...
    describe('generate', () => {
        ...
        it('should create a staff member with chair rental employment type', async () => {
            const mockUser = { id: 'user-2', email: 'test2@example.com', name: 'Test User 2' };
            const mockStaff = {
                id: 'staff-2',
                businessId,
                userId: 'user-2',
        ...
        it('should create a staff member with hybrid employment type', async () => {
            const mockUser = { id: 'user-3', email: 'test3@example.com', name: 'Test User 3' };
            const mockStaff = {
                id: 'staff-3',
                businessId,
                userId: 'user-3',
        ...
        it('should generate realistic working hours', async () => {
            const mockUser = { id: 'user-5', email: 'test5@example.com', name: 'Test User 5' };
            ...
            const createCall = (mockPrisma.staff.create as jest.Mock).mock.calls[0][0];
        ...
        it('should validate staff data correctly', async () => {
            const mockUser = { id: 'user-6', email: 'test6@example.com', name: 'Test User 6' };
            ...
            await expect(invalidStaffFactory.generate()).rejects.toThrow('Staff validation failed: Test error');
        });
    });

    describe('generateBatch', () => {
        it('should generate multiple staff members', async () => {
            const mockUser = { id: 'user-batch', email: 'batch@example.com', name: 'Batch User' };
        ...
        it('should handle batch processing with progress callback', async () => {
            const mockUser = { id: 'user-progress', email: 'progress@example.com', name: 'Progress User' };
            const mockStaff = { id: 'staff-progress', businessId, userId: 'user-progress' };
            ...
            await staffFactory.generateBatch(2, {}, {
                batchSize: 1,
                maxConcurrency: 1,
                progressCallback,
            });
    ...
    describe('employment type distribution', () => {
        it('should respect specialty employment type preferences', async () => {
            const mockUser = { id: 'user-specialty', email: 'specialty@example.com', name: 'Specialty User' };
            const mockStaff = { id: 'staff-specialty', businessId, userId: 'user-specialty' };

            (mockPrisma.user.create as jest.Mock).mockResolvedValue(mockUser);
            (mockPrisma.staff.create as jest.Mock).mockResolvedValue(mockStaff);

            // Find a chair rental specialty
            const chairRentalSpecialty = specialties.find(s => s.employmentType === 'CHAIR_RENTAL');
