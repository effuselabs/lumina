...
import { getEnvironmentConfig, loadSeedConfig, mergeConfigs, validateSeedConfig } from './seed-config';
...
export interface ValidationError {
    type: 'referential_integrity' | 'business_logic' | 'data_consistency' | 'configuration';
    entity: string;
    field?: string;
    message: string;
...
export class DataResetManager {
    ...
    getPredefinedScenarios(): SeedScenario[] {
        ...
        return [
            {
                name: 'demo',
                description: 'Full demo environment with comprehensive data for sales demonstrations',
                config: baseConfig,
                resetOptions: {
                    preserveUsers: true,
            ...
            },
            {
                ...
                config: mergeConfigs(baseConfig, {
                    clients: { count: 200, demographics: baseConfig.clients.demographics },
                    staff: { count: 12, specialties: baseConfig.staff.specialties },
                    appointments: { historicalMonths: 12, patternsConfig: baseConfig.appointments.patternsConfig },
                }),
                resetOptions: {
                    preserveUsers: true,
                    preserveBusiness: true,
            ...
            },
            {
                ...
                config: mergeConfigs(baseConfig, {
                    ...
                    staff: { count: 2, specialties: baseConfig.staff.specialties.slice(0, 2) },
                    appointments: { historicalMonths: 1, patternsConfig: baseConfig.appointments.patternsConfig },
                }),
                resetOptions: {
                    preserveUsers: true,
                    preserveBusiness: true,
    ...
    getScenario(name: string): SeedScenario | null {
        return this.getPredefinedScenarios().find(scenario => scenario.name === name) || null;
    }

    /**
     * Clean reset with referential integrity maintenance
     */
    async cleanReset(businessId: string, options: ResetOptions = {}): Promise<ResetResult> {
        ...
        try {
            ...
            `;
                        deletedCounts['loyaltyTransactions'] = Number(loyaltyTransactionsDeleted);
                        if (options.verbose) console.log(`  Deleted ${loyaltyTransactionsDeleted} loyalty transactions`);
                    } catch (error) {
            ...
            `;
                        deletedCounts['loyaltyMemberships'] = Number(loyaltyMembershipsDeleted);
                        if (options.verbose) console.log(`  Deleted ${loyaltyMembershipsDeleted} loyalty memberships`);
                    } catch (error) {
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${loyaltyProgramsDeleted} loyalty programs`);
                    } catch (error) {
            ...
            `;
                        deletedCounts['campaignRecipients'] = Number(campaignRecipientsDeleted);
                        if (options.verbose) console.log(`  Deleted ${campaignRecipientsDeleted} campaign recipients`);
                    } catch (error) {
            ...
            `;
                        deletedCounts['marketingCampaigns'] = Number(marketingCampaignsDeleted);
                        if (options.verbose) console.log(`  Deleted ${marketingCampaignsDeleted} marketing campaigns`);
                    } catch (error) {
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${promotionUsageDeleted} promotion usage records`);
                    } catch (error) {
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${promotionsDeleted} promotions`);
                    } catch (error) {
            ...
            `;
                        deletedCounts['giftCardRedemptions'] = Number(giftCardRedemptionsDeleted);
                        if (options.verbose) console.log(`  Deleted ${giftCardRedemptionsDeleted} gift card redemptions`);
                    } catch (error) {
                        warnings.push('Gift card redemptions table not found or accessible');
                    }
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${giftCardsDeleted} gift cards`);
                    } catch (error) {
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${productSalesDeleted} product sales`);
                    } catch (error) {
            ...
            `;
                        ...
                        if (options.verbose) console.log(`  Deleted ${productsDeleted} products`);
                    } catch (error) {
                        warnings.push('Products table not found or accessible');
                    }

                } catch (error) {
                    warnings.push(`Some business operations data could not be deleted: ${error}`);
                }
                ...
                if (options.verbose) console.log(`  Deleted ${appointmentServicesDeleted.count} appointment services`);
                ...
                if (options.verbose) console.log(`  Deleted ${transactionsDeleted.count} transactions`);
                ...
                if (options.verbose) console.log(`  Deleted ${appointmentsDeleted.count} appointments`);
                ...
                if (options.verbose) console.log(`  Deleted ${staffServicesDeleted.count} staff services`);
                ...
                if (options.verbose) console.log(`  Deleted ${servicesDeleted.count} services`);
                ...
                if (options.verbose) console.log(`  Deleted ${staffDeleted.count} staff members`);
                ...
                if (options.verbose) console.log(`  Deleted ${clientsDeleted.count} clients`);
                ...
                if (!options.preserveBusinessUsers) {
                    ...
                    if (options.verbose) console.log(`  Deleted ${businessUsersDeleted.count} business user relationships`);
                }
     ...
     */
    private async getEntityCounts(businessId: string): Promise<Record<string, number>> {
        const counts: Record<string, number> = {};

        try {
            // Core entities
            counts.clients = await this.prisma.client.count({ where: { businessId } });
            counts.staff = await this.prisma.staff.count({ where: { businessId } });
            counts.services = await this.prisma.service.count({ where: { businessId } });
            counts.appointments = await this.prisma.appointment.count({ where: { businessId } });
            counts.transactions = await this.prisma.transaction.count({ where: { businessId } });
            ...
            // Business users
            counts.businessUsers = await this.prisma.businessUser.count({ where: { businessId } });
    ...
    async validateDataIntegrity(businessId: string): Promise<ValidationResult> {
        ...
        try {
            ...
            const counts = await this.getEntityCounts(businessId);
            totalEntities = Object.values(counts).reduce((sum, count) => sum + count, 1); // +1 for business
