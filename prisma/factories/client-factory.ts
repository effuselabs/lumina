...
export interface ClientProfile {
    ...
    visitFrequency: 'weekly' | 'biweekly' | 'monthly' | 'quarterly' | 'occasional';
    averageSpend: number;
    firstVisitDate: Date;
    lastVisitDate?: Date;
    totalVisits: number;
...
export class ClientFactory extends BaseFactory<Client> {
    ...
    async generate(options?: ClientFactoryOptions): Promise<Client> {
        ...
        if (!validation.isValid) {
            throw new Error(`Client validation failed: ${validation.errors.map(e => e.message).join(', ')}`);
        }
    ...
    private generateClientProfile(options?: ClientFactoryOptions): ClientProfile {
        ...
        const { emailMarketing, smsMarketing } = this.generateMarketingPreferences(communicationMethod);
        ...
        const { firstVisitDate, lastVisitDate, totalVisits } = this.generateVisitHistory(
            visitFrequency,
            options?.historicalMonths || 6
        );
    ...
    private generateGender(): 'female' | 'male' | 'nonBinary' {
        const genders: ('female' | 'male' | 'nonBinary')[] = ['female', 'male', 'nonBinary'];
        const weights = [
    ...
    private generatePreferredServices(): string[] | undefined {
        ...
        return faker.helpers.arrayElements(this.serviceIds, Math.min(preferredCount, this.serviceIds.length));
    }
     ...
     */
    private generateMarketingPreferences(communicationMethod: CommunicationMethod): {
        emailMarketing: boolean;
        smsMarketing: boolean;
    } {
     ...
     */
    private generateAverageSpend(ageRange: AgeRange, loyaltyTier: LoyaltyTier): number {
    ...
    private generateVisitHistory(
        ...
        const expectedVisits = Math.round(visitsPerMonth[frequency] * historicalMonths);
        const actualVisits = Math.max(1, Math.round(expectedVisits * faker.number.float({ min: 0.7, max: 1.3 })));
    ...
    private formatClientNotes(profile: ClientProfile): string {
        ...
        notes.push(`Age range: ${profile.ageRange}, Loyalty: ${profile.loyaltyTier} (${profile.loyaltyPoints} pts)`);
        notes.push(`Visit frequency: ${profile.visitFrequency}, Avg spend: $${profile.averageSpend}`);
