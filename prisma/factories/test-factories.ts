...
async function testFactoryInfrastructure() {
  ...
  try {
    ...
    console.log(
      `✅ Configuration loaded: ${config.clients.count} clients, ${config.staff.count} staff`
    );
    ...
    console.log(
      `✅ Batch processor test completed: processed ${results.length} items`
    );
    ...
    console.log(
      `✅ Data integrity check completed: ${integrityResult.summary.passed}/${integrityResult.summary.total} checks passed`
    );

    // Test financial integrity
    const financialIntegrity =
      await integrityValidator.validateFinancialIntegrity();
    console.log(
      `✅ Financial integrity check: ${financialIntegrity.isValid ? 'PASSED' : 'FAILED'} (${financialIntegrity.errors.length} errors, ${financialIntegrity.warnings.length} warnings)`
    );

    // Test appointment integrity
    const appointmentIntegrity =
      await integrityValidator.validateAppointmentIntegrity();
    console.log(
      `✅ Appointment integrity check: ${appointmentIntegrity.isValid ? 'PASSED' : 'FAILED'} (${appointmentIntegrity.errors.length} errors, ${appointmentIntegrity.warnings.length} warnings)`
    );

    // Test staff-service integrity
    const staffServiceIntegrity =
      await integrityValidator.validateStaffServiceIntegrity();
    console.log(
      `✅ Staff-service integrity check: ${staffServiceIntegrity.isValid ? 'PASSED' : 'FAILED'} (${staffServiceIntegrity.errors.length} errors, ${staffServiceIntegrity.warnings.length} warnings)`
    );
