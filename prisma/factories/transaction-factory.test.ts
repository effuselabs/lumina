...
describe('TransactionFactory', () => {
  ...
  describe('transaction generation', () => {
    ...
    it('should generate a payment transaction', async () => {
      ...
      expect(transaction.type).toMatch(
        /^(PAYMENT|TIP|REFUND|COMMISSION|CHAIR_RENTAL|DEPOSIT)$/
      );
    ...
    it('should generate commission amounts for commission-based staff', async () => {
      ...
      const commissionTransactions = transactions.filter(
        t =>
          t.staffEmploymentType === 'COMMISSION' ||
          t.staffEmploymentType === 'HYBRID'
      );
  ...
  describe('comprehensive transaction generation', () => {
    ...
    it('should generate the target number of transactions', async () => {
      ...
      const transactions =
        await factory.generateComprehensiveTransactions(targetCount);
    ...
    it('should generate diverse transaction types', async () => {
      ...
      expect(typeDistribution['PAYMENT']).toBeGreaterThan(
        typeDistribution['TIP'] || 0
      );
    });
    ...
    it('should generate transactions with realistic payment method distribution', async () => {
      ...
      expect(paymentMethodDistribution['CARD']).toBeGreaterThan(
        paymentMethodDistribution['CASH'] || 0
      );
    });
    ...
    it('should call progress callback during generation', async () => {
      ...
      await factory.generateComprehensiveTransactions(
        30,
        undefined,
        (processed, total) => {
  ...
  describe('validation', () => {
    it('should validate required fields', () => {
      ...
      expect(result.errors.some((e: any) => e.field === 'businessId')).toBe(
        true
      );
    ...
    it('should validate refund amounts are negative', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((e: any) => e.message.includes('negative'))
      ).toBe(true);
    });
    ...
    it('should validate commission rates are within valid range', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((e: any) => e.field === 'commissionRate')).toBe(
        true
      );
    });
    ...
    it('should validate positive amounts for payment types', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(
        result.errors.some((e: any) => e.message.includes('positive'))
      ).toBe(true);
    });
  });
  ...
  describe('financial calculations', () => {
    ...
    it('should calculate commission amounts correctly', async () => {
      ...
      const paymentTransactions = transactions.filter(
        t => t.type === 'PAYMENT' && t.commissionAmount && t.commissionRate
      );
