    ...
    try {
      ...
      if (staff.workingHours) {
        ...
        if (daySchedule?.openTime && daySchedule?.closeTime) {
          ...
          if (
            appointmentStart < daySchedule.openTime ||
            appointmentEnd > daySchedule.closeTime
          ) {
            return false;
          }
        }
...
export class ScheduleValidator {
  ...
  async validateAppointment(
    appointment: AppointmentBookingData
    ...
    const respectsHours = await this.respectsBusinessHours(
      this.businessId,
    ...
    const totalServiceDuration =
      services.reduce((total, service) => total + service.duration, 0) * 60000;
...
export class ServiceCompatibilityValidator {
  ...
  async validateStaffServiceCompatibility(
    ...
    try {
      ...
      const staffCannotPerformIds = serviceIds.filter(
        id => !staffCanPerformIds.includes(id)
      );

      if (staffCannotPerformIds.length > 0) {
        const cannotPerformServices = services.filter(s =>
          staffCannotPerformIds.includes(s.id)
        );
        errors.push({
          field: 'serviceIds',
      ...
      if (categories.length > 3) {
        warnings.push({
          field: 'serviceIds',
          message:
            'Multiple service categories selected - consider splitting into separate appointments',
   ...
   */
  async getRecommendedStaffForServices(
    serviceIds: string[]
  ): Promise<string[]> {
    try {
      const staffServices = await this.prisma.staffService.findMany({
      ...
      const staffServiceCounts = staffServices.reduce(
        (acc, ss) => {
...
export class FinancialIntegrityValidator {
   ...
   */
  async validateTransactionAmounts(
    ...
    try {
      ...
      const paymentTransactions = appointment.transactions.filter(
      ...
      const duplicateAmounts = paymentAmounts.filter(
        (amount, index) => paymentAmounts.indexOf(amount) !== index
      );
      ...
      const refundTransactions = appointment.transactions.filter(
        t => t.type === 'REFUND'
      );
  ...
  async validateCommissionCalculations(
    ...
    try {
      ...
      const totalRevenue = appointments.reduce((total, apt) => {
        const aptRevenue = apt.transactions.reduce(
...
export class BusinessLogicValidator {
  ...
  constructor(prisma: PrismaClient, businessId: string) {
    ...
    this.serviceCompatibilityValidator = new ServiceCompatibilityValidator(
      prisma,
      businessId
    );
    this.financialIntegrityValidator = new FinancialIntegrityValidator(
      prisma,
      businessId
    );
  ...
  async validateAppointmentBooking(
    ...
    try {
      ...
      const scheduleResult =
        await this.scheduleValidator.validateAppointment(appointment);
      ...
      const serviceResult =
        await this.serviceCompatibilityValidator.validateStaffServiceCompatibility(
          appointment.staffId,
