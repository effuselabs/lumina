...
export class StaffFactory extends BaseFactory<Staff> {
  ...
  async generate(options?: {
    ...
    const specialty =
      options?.specialty || faker.helpers.arrayElement(this.staffSpecialties);
    ...
    if (!validation.isValid) {
      throw new Error(
        `Staff validation failed: ${validation.errors.map(e => e.message).join(', ')}`
      );
    }
    ...
    console.log(
      `✅ Created staff member: ${profile.displayName} (${profile.title})`
    );
    return staff;
  }
  ...
  private generateStaffProfile(
    ...
    const experienceLevel =
      options?.experienceLevel || this.generateExperienceLevel();

    // Generate employment details based on type
    const employmentDetails = this.generateEmploymentDetails(
      employmentType,
      experienceLevel
    );

    // Generate work schedule
    const workingHours = this.generateWorkSchedule(
      employmentType,
      experienceLevel
    );
   ...
   */
  private generateEmploymentDetails(
    employmentType: EmploymentType,
    experienceLevel: ExperienceLevel
  ) {
    ...
    switch (employmentType) {
      case 'COMMISSION':
        ...
        details.baseSalary = faker.datatype.boolean(0.3)
          ? this.generateBaseSalary()
          : undefined;
        break;

      case 'CHAIR_RENTAL':
        details.chairRentalAmount =
          this.generateChairRentalAmount(experienceLevel);
        details.chairRentalPeriod = this.generateRentalPeriod();
        break;

      case 'HYBRID':
        details.commissionRate = this.generateCommissionRate(
          experienceLevel,
          true
        );
        details.chairRentalAmount = this.generateChairRentalAmount(
          experienceLevel,
          true
        );
        details.chairRentalPeriod = this.generateRentalPeriod();
   ...
   */
  private generateCommissionRate(
    experienceLevel: ExperienceLevel,
   ...
   */
  private generateChairRentalAmount(
    experienceLevel: ExperienceLevel,
    isHybrid: boolean = false
  ): number {
    const baseAmounts = {
   ...
   */
  private generateWorkSchedule(
    employmentType: EmploymentType,
    experienceLevel: ExperienceLevel
  ): WeeklySchedule {
    const isFullTime = this.determineFullTimeStatus(
      employmentType,
      experienceLevel
    );
   ...
   */
  private determineFullTimeStatus(
    employmentType: EmploymentType,
    experienceLevel: ExperienceLevel
  ): boolean {
  ...
  private generateFullTimeSchedule(): WeeklySchedule {
    ...
    const additionalDays = faker.helpers.arrayElements(
      remainingDays,
      workDays - 3
    );
  ...
  private generatePartTimeSchedule(): WeeklySchedule {
    ...
    const selectedPeakDays = faker.helpers.arrayElements(
      peakDays,
      Math.min(2, workDays)
    );
   ...
   */
  private generateStaffExperience(
    specialty: StaffSpecialty,
    experienceLevel: ExperienceLevel
  ): StaffExperience {
    ...
    const previousSalons =
      yearsInIndustry > 2
        ? faker.number.int({ min: 1, max: Math.floor(yearsInIndustry / 2) })
        : 0;
   ...
   */
  private generateSpecializations(
    specialty: StaffSpecialty,
    experienceLevel: ExperienceLevel
  ): string[] {
    ...
    const certificationPool = [
      {
        ...
        org: 'State Board of Cosmetology',
        required: true,
      },
      {
      ...
      {
        name: 'Balayage Certification',
        org: "L'Oreal Professional",
        required: false,
      },
      {
        name: 'Keratin Treatment Specialist',
        org: 'Brazilian Blowout',
        required: false,
      },
      {
        name: 'Bridal Hair Specialist',
        org: 'Wedding Beauty Association',
        required: false,
      },
      {
        name: "Men's Grooming Certification",
        org: 'American Crew',
        required: false,
      },
      {
      ...
      },
      {
        ...
        org: 'PhiBrows Academy',
        required: false,
      },
      {
        name: 'Nail Technology License',
        org: 'State Board of Cosmetology',
        required: false,
      },
      {
  ...
  private createCertification(
    ...
    if (hasExpiration) {
      ...
      expirationDate.setFullYear(
        expirationDate.getFullYear() + faker.number.int({ min: 2, max: 5 })
      );
   ...
   */
  private generateTrainingHistory(
    specialty: StaffSpecialty,
    experience: StaffExperience
  ): TrainingRecord[] {
    const trainingPool = [
      {
        ...
        provider: 'Vidal Sassoon Academy',
        hours: 16,
      },
      {
        name: 'Color Theory Masterclass',
        provider: 'Redken Education',
        hours: 8,
      },
      { name: 'Balayage Workshop', provider: 'Wella Professionals', hours: 12 },
      {
        name: 'Client Consultation Skills',
        provider: 'Beauty Industry Training',
        hours: 4,
      },
      {
        name: 'Chemical Safety Training',
        provider: 'OSHA Compliance',
        hours: 6,
      },
      {
        name: 'Texture Services Workshop',
        provider: 'Matrix Education',
        hours: 10,
      },
      {
        name: 'Bridal Styling Intensive',
        provider: 'Wedding Hair Academy',
        hours: 20,
      },
      {
        name: "Men's Grooming Trends",
        provider: 'American Crew Education',
        hours: 8,
      },
      {
        name: 'Salon Business Management',
        provider: 'Professional Beauty Association',
        hours: 12,
      },
      {
        name: 'Customer Service Excellence',
        provider: 'Service Industry Training',
        hours: 6,
      },
    ];
    ...
    return selectedTraining.map(training => {
      const yearsAgo = faker.number.float({
        min: 0.1,
        max: experience.yearsInIndustry,
      });
      const dateCompleted = new Date();
      dateCompleted.setFullYear(
        dateCompleted.getFullYear() - Math.floor(yearsAgo)
      );
  ...
  private generateSkillsLearned(courseName: string): string[] {
    const skillsMap: Record<string, string[]> = {
      ...
      'Texture Services Workshop': [
        'Perm techniques',
        'Relaxer application',
        'Curl patterns',
      ],
      'Bridal Styling Intensive': [
        'Updo techniques',
        'Hair accessories',
        'Long-lasting styles',
      ],
  ...
  protected validate(data: StaffProfile): ValidationResult {
    ...
    if (data.employmentType === 'COMMISSION' && !data.commissionRate) {
      errors.push({
        ...
        message: 'Commission rate required for commission employees',
        code: 'REQUIRED',
      });
    }
    ...
    if (
      ...
      errors.push({
        field: 'chairRentalAmount',
        message: 'Chair rental amount required for rental employees',
        code: 'REQUIRED',
      });
    }
    ...
    if (data.chairRentalAmount && data.chairRentalAmount < 0) {
      errors.push({
        field: 'chairRentalAmount',
        message: 'Chair rental amount must be positive',
        code: 'RANGE',
      });
    }
    ...
    if (!data.workingHours || Object.keys(data.workingHours).length === 0) {
      errors.push({
        field: 'workingHours',
        message: 'Working hours must be specified',
        code: 'REQUIRED',
      });
    }
