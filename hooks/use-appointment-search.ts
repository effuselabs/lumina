...
import { createSearchResult, extractHighlightTerms, filterAppointments, sortByRelevance, validateFilters } from '@/lib/search-utils';
import { AppointmentFilters, FilterState, SearchResult } from '@/types/appointment-filters';
...
export function useSearchHistory(maxHistory: number = 10) {
    ...
    const addToHistory = useCallback((filters: AppointmentFilters) => {
...
export function useSearchAnalytics() {
    ...
    const recordSearch = useCallback((
        ...
        setAnalytics(prev => {
            ...
            const totalResults = prev.averageResultCount * (prev.totalSearches - 1) + resultCount;
            newAnalytics.averageResultCount = totalResults / newAnalytics.totalSearches;

            // Track search times (keep last 100)
            newAnalytics.searchTimes = [...prev.searchTimes, searchTime].slice(-100);
    ...
    const getAverageSearchTime = useCallback(() => {
        ...
        return analytics.searchTimes.reduce((sum, time) => sum + time, 0) / analytics.searchTimes.length;
