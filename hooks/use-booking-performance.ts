...
export function useBookingPerformance() {
        ...
        for (const entry of list.getEntries()) {
          ...
          if (entry.entryType === 'measure') {
            ...
            if (measureEntry.name.startsWith('booking-')) {
              setMetrics(prev => ({
                ...prev,
                [measureEntry.name.replace('booking-', '')]:
  ...
  const optimizeImageSrc = useCallback(
  ...
  const getRecommendations = useCallback((): string[] => {
    ...
    if (metrics.stepTransitionTime && metrics.stepTransitionTime > 500) {
      recommendations.push(
        'Step transitions are slow. Consider lazy loading or code splitting.'
      );
    }

    if (metrics.apiResponseTime && metrics.apiResponseTime > 1000) {
      recommendations.push(
        'API responses are slow. Consider caching or optimizing backend.'
      );
    }

    if (networkState.isSlowConnection) {
      recommendations.push(
        'Slow network detected. Using optimized images and aggressive caching.'
      );
    }

    if (metrics.renderTime && metrics.renderTime > 16) {
      recommendations.push(
        'Rendering is slow. Consider memoization or virtualization.'
      );
    }
