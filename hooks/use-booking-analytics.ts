...
} from '@/lib/analytics/booking-analytics';
import {
  BookingError,
  bookingErrorTracker,
} from '@/lib/monitoring/booking-error-tracker';
...
export function useBookingAnalytics({
  ...
  const trackTimeSelected = useCallback(
    (timeSlot: string, staffId: string) => {
  ...
  const trackBookingCompleted = useCallback(
    (appointmentId: string, totalPrice: number) => {
  ...
  const trackAvailabilityError = useCallback(
