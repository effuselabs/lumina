...
export function useNetworkResilience(
  ...
  /*
   * Hold the caller's callbacks in refs.
   *
   * The connectivity effect below used to depend on [onConnectionChange,
   * onSlowConnection]. Callers pass inline arrow functions — ServiceSelection
   * does — which are new on every render, so the effect re-ran every render,
   * called setNetworkState, triggered another render, and looped:
   * "Maximum update depth exceeded". The booking form never settled.
   *
   * Fixing it here rather than memoising at each call site: a hook should not
   * require its callers to know that an inline callback will melt it.
   */
  ...
  useEffect(() => {
    ...
    if ('connection' in navigator) {
      ...
      const updateConnectionInfo = () => {
        ...
        onSlowConnectionRef.current?.(isSlowConnection);
      };
    ...
    // Empty deps on purpose: the callbacks are read from refs, so this
    // subscribes once instead of re-subscribing on every render.
  }, []);
  ...
  const shouldShowOfflineMessage = useCallback(() => {
    return (
      ...
      (networkState.retryCount >= config.maxRetries &&
        networkState.lastFailedAt)
    );
