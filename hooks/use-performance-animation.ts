...
export function useAnimation(
    ...
    const createAnimation = useCallback(() => {
        ...
        const actualDuration = respectReducedMotion ? getAnimationDuration(duration) : duration;
        const actualEasing = respectReducedMotion ? getEasingFunction(easing) : easing;
        ...
        animation.addEventListener('finish', () => {
            ...
            if (monitorPerformance && animationIdRef.current) {
                const finalMetrics = animationMonitor.stopMonitoring(animationIdRef.current);
        ...
        return animation;
    }, [keyframes, duration, easing, delay, iterations, fill, respectReducedMotion, monitorPerformance]);
...
export function useHoverAnimation(
    ...
    options: UseAnimationOptions = {}
): [React.RefObject<HTMLElement>, { onMouseEnter: () => void; onMouseLeave: () => void }] {
    const elementRef = useRef<HTMLElement>(null);
    const hoverAnimationRef = useRef<{ enter: () => Animation; leave: () => Animation } | null>(null);
...
export function useLoadingAnimation(
    ...
    options: UseAnimationOptions = {}
): [React.RefObject<HTMLElement>, { start: () => void; stop: () => void; isLoading: boolean }] {
    const elementRef = useRef<HTMLElement>(null);
    const animationRef = useRef<Animation | null>(null);
    ...
    const controls = {
        start: useCallback(() => {
            ...
            animationRef.current = createLoadingAnimation(elementRef.current, type, duration);
            setIsLoading(true);
...
export function useFocusAnimation(
    ...
    const handlers = {
        onFocus: useCallback(() => {
            ...
            focusAnimationRef.current = createFocusAnimation(elementRef.current, duration);
        }, [duration]),
...
export function useAnimationMetrics(): {
    ...
    const getAverageFPS = useCallback(() => {
        ...
        const fpsValues = Array.from(metrics.values()).map(m => m.averageFPS).filter(fps => fps > 0);
    ...
    const getSlowAnimations = useCallback(() => {
        const metrics = animationMonitor.getAllMetrics();
        return Array.from(metrics.values()).filter(m => m.averageFPS < 55 && m.averageFPS > 0);
    }, []);
