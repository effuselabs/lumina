...
export function useBulkOperations({
  ...
  const cancelAppointments = useCallback(
    async (
      ...
      try {
        const results = await executeWithProgress('cancel', async () => {
          const results = await service.cancelAppointments(
            appointmentIds,
            reason,
            notifyClients
          );
  ...
  const updateAppointmentStatus = useCallback(
    async (
      ...
      try {
        const results = await executeWithProgress('status_update', async () => {
          const results = await service.updateAppointmentStatus(
            appointmentIds,
            newStatus,
            notes
          );
  ...
  const rescheduleAppointments = useCallback(
    async (
      appointmentIds: string[],
      newDateTime: Date,
      ...
      try {
        const results = await executeWithProgress('reschedule', async () => {
          const results = await service.rescheduleAppointments(
            appointmentIds,
            newDateTime,
            preserveStaff
          );
  ...
  const validateOperation = useCallback(
    async (
      ...
      return await service.validateBulkOperation(
        operation,
        appointmentIds,
        params
      );
  ...
  const getTimeSlotConflicts = useCallback(
    async (appointmentIds: string[], newDateTime: Date) => {
