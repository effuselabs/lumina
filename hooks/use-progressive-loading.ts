...
export function useProgressiveLoading({
     ...
     */
    const generateChunks = useCallback((centerDate: Date, totalChunks: number): LoadingChunk[] => {
        const chunks: LoadingChunk[] = [];
        const chunkDays = chunkSize;

        for (let i = -Math.floor(totalChunks / 2); i <= Math.floor(totalChunks / 2); i++) {
     ...
     */
    const loadAppointmentsForRange = useCallback(async (
        start: Date,
        end: Date
        ...
        const response = await fetch(`/api/appointments?businessId=${businessId}&start=${start.toISOString()}&end=${end.toISOString()}`);
    ...
    const loadChunk = useCallback(async (chunk: LoadingChunk): Promise<void> => {
        ...
        if (activeLoads.current.has(rangeKey) || loadedRanges.current.has(rangeKey)) {
            return;
        }
        ...
        try {
            ...
            setAppointments(prev => {
                ...
                const newAppointments = chunkAppointments.filter(apt => !existingIds.has(apt.id));
     ...
     */
    const loadChunks = useCallback(async (chunks: LoadingChunk[]): Promise<void> => {
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            setLoadingState(prev => ({
                ...prev,
                error: error instanceof Error ? error.message : 'Failed to load more appointments'
            }));
        } finally {
            setLoadingState(prev => ({ ...prev, isLoadingMore: false }));
        }
    }, [loadingState.isLoadingMore, loadingState.hasMore, initialDate, generateChunks, loadChunks]);
     ...
     */
    const loadDateRange = useCallback(async (start: Date, end: Date): Promise<void> => {
        try {
            ...
            setAppointments(prev => {
                ...
                return [...filtered, ...appointments].sort((a, b) =>
                    a.startTime.getTime() - b.startTime.getTime()
                );
            });
        } catch (error) {
            setLoadingState(prev => ({
                ...prev,
                error: error instanceof Error ? error.message : 'Failed to load date range'
            }));
        }
    }, [loadAppointmentsForRange]);
    ...
    useEffect(() => {
        ...
        const unloadedChunks = newChunks.filter(newChunk =>
            !loadingChunks.current.some(existingChunk =>
                existingChunk.dateRange.start.getTime() === newChunk.dateRange.start.getTime()
            )
        );
