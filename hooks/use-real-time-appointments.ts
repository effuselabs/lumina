...
export interface UseRealTimeAppointmentsReturn {
    ...
    // Actions
    updateAppointment: (appointmentId: string, updates: Partial<DashboardAppointment>) => Promise<string>;
    deleteAppointment: (appointmentId: string) => Promise<string>;
    createAppointment: (appointment: Omit<DashboardAppointment, 'id'>) => Promise<string>;
...
export function useRealTimeAppointments(
     ...
     */
    const updateAppointmentsList = useCallback((appointment: DashboardAppointment, isOptimistic = false) => {
     ...
     */
    const removeAppointmentFromList = useCallback((appointmentId: string, isOptimistic = false) => {
     ...
     */
    const handleConflictDetected = useCallback((conflict: ConflictResolution) => {
        addNotification(
    ...
    useEffect(() => {
        ...
        syncServiceRef.current = new RealTimeSyncService(businessId, userId, syncCallbacks);

        // Initialize WebSocket service if enabled
        if (enableWebSocket) {
            wsServiceRef.current = createWebSocketService(businessId, userId, wsCallbacks);
    ...
    const updateAppointment = useCallback(async (
        ...
        if (enableOptimisticUpdates) {
            return syncServiceRef.current.applyOptimisticUpdate(appointmentId, updates, 'update');
        } else {
     ...
     */
    const deleteAppointment = useCallback(async (appointmentId: string): Promise<string> => {
        if (!syncServiceRef.current) {
            throw new Error('Sync service not initialized');
        }

        if (enableOptimisticUpdates) {
            return syncServiceRef.current.applyOptimisticUpdate(appointmentId, {}, 'delete');
        } else {
            // Direct API call without optimistic updates
            const response = await fetch(`/api/appointments/${appointmentId}`, {
                method: 'DELETE',
    ...
    const createAppointment = useCallback(async (
        ...
        const appointmentWithId = { ...appointment, id: tempId } as DashboardAppointment;

        if (enableOptimisticUpdates) {
            return syncServiceRef.current.applyOptimisticUpdate(tempId, appointmentWithId, 'create');
        } else {
            // Direct API call without optimistic updates
