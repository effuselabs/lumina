...
export function SafeComponentWrapper({
  ...
  React.useEffect(() => {
    const checkStylesLoaded = () => {
      ...
      if (!hasStyles) {
        console.warn(
          `Styles may not be loaded for ${componentName}, applying fallbacks`
        );
  ...
  const defaultFallback = (
    <div className="ds-fallback-card" role="alert" aria-live="polite">
      <p className="text-sm text-gray-600">
        {componentName} is temporarily unavailable. Please try refreshing the
        page.
      </p>
    </div>
  );
  ...
  const SafeComponent = (props: P) => {
    ...
    return (
      <SafeComponentWrapper
        ...
        fallback={
          options?.fallback ? <options.fallback {...props} /> : undefined
        }
      >
        <Component {...props} />
      </SafeComponentWrapper>
    ...
    <>
      <noscript>
        <div className="ds-fallback-card">
          <h2 className="mb-2 text-lg font-semibold">JavaScript Required</h2>
