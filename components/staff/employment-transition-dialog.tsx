...
export function EmploymentTransitionDialog({
  ...
  const [step, setStep] = useState<
    'validation' | 'configuration' | 'confirmation'
  >('validation');
  ...
  const getTransitionValidation = (
    ...
    if (toType === 'HYBRID') {
      warnings.push(
        'Hybrid model requires both commission and rental components'
      );
              ...
              <CardContent className="space-y-4">
                ...
                {form.getValues('reason') && (
                  <div>
                    ...
                    <p className="mt-1 text-sm">{form.getValues('reason')}</p>
                  </div>
                )}
              </CardContent>
            </Card>
