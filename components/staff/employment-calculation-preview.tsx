...
export function EmploymentCalculationPreview({
  ...
  const typeInfo = config.employmentType
    ? employmentTypeDescriptions[
        config.employmentType as keyof typeof employmentTypeDescriptions
      ]
    : null;
        ...
        {calculations.preview && calculations.preview.length > 0 && (
          <div className="mt-4 rounded-lg border border-green-200 bg-white p-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              {(() => {
                const result =
                  calculations.preview[1]?.result ||
                  calculations.preview[0]?.result;
