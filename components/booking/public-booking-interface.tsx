...
export function PublicBookingInterface({
  ...
  if (error && !businessData) {
    return <BookingErrorHandler error={error} onRetry={loadBusinessData} />;
  }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="mx-auto max-w-4xl px-4 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600">{businessData?.business?.address}</p>
            </div>
          </div>
        </div>
      </header>
      ...
      >
        ...
        {error && currentStep !== 'confirmation' && (
          <BookingErrorHandler error={error} onRetry={() => setError(null)} />
        )}

        {/* Loading states */}
        {isLoading && <BookingLoadingState type="booking" />}
      </main>
