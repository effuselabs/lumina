    ...
    >
      ...
      <header className="sticky top-0 z-50 bg-white shadow-sm">
        ...
        {networkState.isSlowConnection && networkState.isOnline && (
          <div className="bg-yellow-50 px-4 py-2 text-center">
            <div className="flex items-center justify-center gap-2 text-sm text-yellow-700">
              ...
              <span>
                Slow connection detected - optimizing for your network
              </span>
            </div>
          </div>
        )}
        ...
        <div className="px-4 py-3">
          ...
          {/*
           * Title only — navigation lives in the bottom footer.
           *
           * This header used to carry its own Back/Next pair on top of
           * the footer's Back/Continue, so every mobile step showed two
           * competing sets of controls bound to the same handlers, with
           * different labels ("Next"/"Book" against
           * "Continue"/"Complete Booking"). The footer is the one to
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-4">
          <Card className="min-h-[60vh] p-4">{children}</Card>
        </div>
      </main>
