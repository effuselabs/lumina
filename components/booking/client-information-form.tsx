...
import { AlertCircle, Loader2, Mail, MessageSquare, Phone, User } from 'lucide-react';
...
export default function ClientInformationForm({
    ...
    const performClientLookup = async () => {
        ...
        try {
            const response = await fetch(`/api/public/booking/${businessId}/client-lookup`, {
                method: 'POST',
                headers: {
            ...
            if (data.clientExists && data.clientData) {
                // Pre-fill form with existing client data
                setIsNewClient(false);
                setLookupPerformed(true);

                if (data.clientData.firstName) setValue('firstName', data.clientData.firstName);
                if (data.clientData.lastName) setValue('lastName', data.clientData.lastName);
                if (data.clientData.email) setValue('email', data.clientData.email);
                if (data.clientData.phone) setValue('phone', data.clientData.phone);
                if (data.clientData.marketingOptIn !== undefined) {
                    setValue('marketingOptIn', data.clientData.marketingOptIn);
                }
            ...
            setLookupError('Unable to check client information. Please continue with your details.');
            setIsNewClient(true);
        } finally {
            ...
            <CardHeader>
                ...
                {!isNewClient && (
                    <Alert>
                        ...
                        <AlertDescription>
                            Welcome back! We've pre-filled your information from your previous visits.
            ...
            <CardContent>
                <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        >
                            I'd like to receive appointment reminders and special offers via email
                        </Label>
                    </div>
