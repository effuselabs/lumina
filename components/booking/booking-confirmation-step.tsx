...
import { BookingLoadingState, MultiStepProgress, NetworkStatusIndicator } from './booking-loading-states';
...
export function BookingConfirmationStep({
    ...
    const [bookingError, setBookingError] = useState<PublicBookingError | Error | null>(null);
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <p className="font-semibold text-lg">{createdAppointment.business.name}</p>
                                {createdAppointment.business.address && (
                                    <p className="text-sm text-gray-600">{createdAppointment.business.address}</p>
                                )}
                            </div>

                            <div>
                                <div className="text-sm font-medium text-gray-600 mb-1">Services</div>
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center justify-between font-semibold">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            <Badge className="flex items-center space-x-1">
                                                <DollarSign className="h-3 w-3" />
                                                <span>{formatPrice(createdAppointment.totalPrice)}</span>
                                            </Badge>
                                            <Badge variant="outline" className="flex items-center space-x-1">
                                                <Clock className="h-3 w-3" />
                                                <span>{formatDuration(createdAppointment.totalDuration)}</span>
                                            </Badge>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <p className="font-semibold">{format(createdAppointment.dateTime, 'EEEE, MMMM d, yyyy')}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="font-semibold">
                                    {format(createdAppointment.dateTime, 'h:mm a')} - {format(new Date(createdAppointment.dateTime.getTime() + createdAppointment.totalDuration * 60000), 'h:mm a')}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <p className="font-semibold">{createdAppointment.staff.displayName}</p>
                                {createdAppointment.staff.title && (
                                    <p className="text-sm text-gray-600">{createdAppointment.staff.title}</p>
                                )}
                            </div>
                        ...
                        {createdAppointment.notes && (
                            <div>
                                ...
                                <p className="text-sm bg-gray-50 p-3 rounded-md">{createdAppointment.notes}</p>
                            </div>
                        )}
                    </CardContent>
                </Card>
    ...
    if (isBooking) {
        const steps = [
            { id: 'validate', title: 'Validating time slot', status: bookingProgress >= 40 ? 'completed' as const : bookingProgress >= 20 ? 'active' as const : 'pending' as const },
            { id: 'create', title: 'Creating appointment', status: bookingProgress >= 80 ? 'completed' as const : bookingProgress >= 60 ? 'active' as const : 'pending' as const },
            { id: 'confirm', title: 'Sending confirmation', status: bookingProgress >= 100 ? 'completed' as const : bookingProgress >= 80 ? 'active' as const : 'pending' as const },
        ];
        ...
        return (
            <div className="max-w-2xl mx-auto space-y-6">
                ...
                <BookingLoadingState
                    type="booking"
                    message={networkState.isSlowConnection ? "Creating your appointment (slow connection detected)..." : "Creating your appointment..."}
                ...
                <CardContent className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200 flex justify-between items-center">
                            ...
                            <div className="text-right">
                                <p className="font-semibold text-lg">{formatPrice(selectedTimeSlot.totalPrice)}</p>
                                <p className="text-sm text-gray-600">{formatDuration(selectedTimeSlot.totalDuration)}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="p-3 bg-gray-50 rounded-lg">
                            <div className="flex items-center space-x-2 mb-1">
                                ...
                                <span className="font-medium">{format(selectedTimeSlot.startTime, 'EEEE, MMMM d, yyyy')}</span>
                            </div>
                            <div className="flex items-center space-x-2 mb-1">
                                <Clock className="h-4 w-4 text-gray-600" />
                                <span>{format(selectedTimeSlot.startTime, 'h:mm a')} - {format(selectedTimeSlot.endTime, 'h:mm a')}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                    ...
                    <div>
                        ...
                        <div className="p-3 bg-gray-50 rounded-lg space-y-2">
                            <p><span className="font-medium">Name:</span> {clientData.firstName} {clientData.lastName}</p>
                            <p><span className="font-medium">Email:</span> {clientData.email}</p>
                            <p><span className="font-medium">Phone:</span> {clientData.phone}</p>
                            {clientData.notes && (
                                <p><span className="font-medium">Notes:</span> {clientData.notes}</p>
                            )}
                        </div>
                    </div>
