...
import { Calendar as CalendarIcon, Clock, Edit, Settings, User } from 'lucide-react'
...
export function StaffAvailabilityManager({ businessId, staffId }: StaffAvailabilityManagerProps) {
    ...
    const handleToggleOnlineBooking = async (staffId: string, enabled: boolean) => {
        try {
            // Update staff online booking status
            setStaff(prev => prev.map(s =>
                s.id === staffId
                    ? { ...s, acceptsOnlineBookings: enabled }
                    : s
            ))

            if (selectedStaff?.id === staffId) {
                setSelectedStaff(prev => prev ? { ...prev, acceptsOnlineBookings: enabled } : null)
            }
        } catch (err) {
    ...
    const handleUpdateWorkingHours = async (staffId: string, workingHours: WorkingHours) => {
        try {
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-3">
                            {staff.map((staffMember) => (
                                <div
                                    key={staffMember.id}
                                    className={`p-3 rounded-lg border cursor-pointer transition-colors ${selectedStaff?.id === staffMember.id
                    ...
                    {selectedStaff ? (
                        <div className="space-y-6">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <Label htmlFor="online-booking">Accept Online Bookings</Label>
                                        ...
                                        <Dialog open={editingHours} onOpenChange={setEditingHours}>
                                            ...
                                            <DialogContent className="max-w-md">
                                                <DialogHeader>
                                                    ...
                                                    <DialogDescription>
                                                        Set the working hours for {selectedStaff.displayName}
                                                    </DialogDescription>
                                                </DialogHeader>
                                                <WorkingHoursEditor
                                                    workingHours={selectedStaff.workingHours}
                                                    onSave={(hours) => handleUpdateWorkingHours(selectedStaff.id, hours)}
                                ...
                                <CardContent>
                                    <div className="space-y-3">
                                        {[1, 2, 3, 4, 5, 6, 0].map((dayIndex) => {
                                            ...
                                            return (
                                                <div key={dayIndex} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                        ...
                        <Card>
                            <CardContent className="py-8 text-center">
                                <p className="text-gray-600">Select a staff member to view their availability settings</p>
                            </CardContent>
                        </Card>
                    )}
                </div>
...
function WorkingHoursEditor({ workingHours, onSave, onCancel }: WorkingHoursEditorProps) {
