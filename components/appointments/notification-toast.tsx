...
export const realTimeToasts = {
  ...
  sync: {
    ...
    pending: (count: number) =>
      toast.warning(`${count} changes pending sync`, {
  ...
  appointment: {
    created: (clientName: string) =>
      toast.success(`Appointment created for ${clientName}`, {
        icon: <CheckCircle className="h-4 w-4" />,
        duration: 4000,
      }),

    updated: (clientName: string) =>
      toast.info(`Appointment updated for ${clientName}`, {
        icon: <Info className="h-4 w-4" />,
        duration: 3000,
      }),

    deleted: (clientName: string) =>
      toast.info(`Appointment cancelled for ${clientName}`, {
        icon: <Info className="h-4 w-4" />,
        duration: 4000,
      }),

    conflict: (clientName: string) =>
      toast.warning(`Conflict detected for ${clientName}'s appointment`, {
        icon: <AlertCircle className="h-4 w-4" />,
        duration: 6000,
        action: {
          label: 'Resolve',
  ...
  optimistic: {
    applied: (action: string) =>
      toast.info(`${action} applied - syncing...`, {
        icon: <RefreshCw className="h-4 w-4 animate-spin" />,
        duration: 2000,
      }),
