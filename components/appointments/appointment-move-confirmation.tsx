...
export function AppointmentMoveConfirmation({
  ...
  const isStaffChange =
    fromSlot.staffId !== toSlot.staffId && toSlot.staffId !== '';
  const isDateChange =
    fromSlot.startTime.toDateString() !== toSlot.startTime.toDateString();
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-color-background-muted rounded-lg p-4">
            <h3 className="text-color-secondary mb-3 font-medium">
              Appointment Details
            </h3>
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <User className="text-color-foreground-muted h-4 w-4" />
              ...
              <div className="text-color-foreground-muted text-sm">
                Duration: {Math.floor(appointment.totalDuration / 60)}h{' '}
                {appointment.totalDuration % 60}m
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="rounded-lg border border-red-200 bg-red-50 p-4">
              <h4 className="mb-2 font-medium text-red-800">From</h4>
            ...
            <div className="rounded-lg border border-green-200 bg-green-50 p-4">
              <h4 className="mb-2 font-medium text-green-800">To</h4>
              <div className="space-y-1 text-sm">
                <div className="flex items-center space-x-2">
                  <Calendar className="h-4 w-4 text-green-600" />
                  <span>{formatDate(toSlot.startTime)}</span>
                  {isDateChange && (
                    <Badge variant="secondary">Date Change</Badge>
                  )}
                </div>
                <div className="flex items-center space-x-2">
                ...
                {toSlot.staffId && (
                  <div className="flex items-center space-x-2">
                    ...
                    {isStaffChange && (
                      <Badge variant="secondary">Staff Change</Badge>
                    )}
                  </div>
                )}
              </div>
          ...
          {hasConflicts && validation && (
            <div className="space-y-3">
              <h3 className="text-color-secondary flex items-center space-x-2 font-medium">
              ...
              {validation.conflicts.map((conflict, index) => (
                <Alert
                  key={index}
                  variant={
                    getConflictColor(conflict.severity) as
                      'default' | 'destructive'
                  }
                >
          ...
          {suggestedAlternatives.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-color-secondary font-medium">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {suggestedAlternatives.slice(0, 4).map((slot, index) => (
                  <button
                    ...
                    className="border-color-border hover:bg-color-background-muted rounded-lg border p-3 text-left transition-colors"
                  >
