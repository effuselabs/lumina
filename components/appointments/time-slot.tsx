...
export function TimeSlot({
  ...
  return (
    <div
      className={cn(
        'border-color-border relative border-b transition-colors',
        getSlotHeight(),
        {
    ...
    >
      ...
      {view === 'day' && (
        <div className="text-color-foreground-muted absolute left-2 top-1 text-xs">
          ...
          {slot.appointments.map((appointment, index) => (
            <AppointmentBlock
              ...
              className={cn('flex-shrink-0', {
                'z-10': index === 0,
                '-mt-1 scale-95 transform opacity-90': index > 0, // Stack overlapping appointments
              })}
            />
          ))}
        </div>
      ...
      {!slot.isAvailable && !hasAppointments && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-color-foreground-muted text-xs">Unavailable</div>
        </div>
      )}
    </div>
  );
