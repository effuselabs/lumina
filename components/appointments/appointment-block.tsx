...
import { AppointmentBlockProps, AppointmentStatus } from '@/types/dashboard-appointments';
...
export function AppointmentBlock({
    ...
    return (
        <div
            className={cn(
                'rounded border-l-4 cursor-pointer transition-all hover:shadow-sm relative group',
                getStatusColor(appointment.status),
                getBlockSize(),
                {
                    'opacity-50 scale-95': isDragging,
                    'shadow-lg ring-2 ring-lumina-coral/20': isDragging,
                    'shadow-md': isHovered && appointment.canReschedule,
                    'cursor-grab': appointment.canReschedule && !isDragging && !isSelectionMode,
                    'cursor-grabbing': appointment.canReschedule && isDragging,
                    'ring-2 ring-blue-500 ring-offset-1': isSelected,
                    'hover:ring-1 hover:ring-blue-300': isSelectionMode && enableSelection,
                },
                className
            )}
            onClick={handleClick}
        ...
        >
            <div className="flex flex-col space-y-1 min-w-0">
                ...
                {appointment.canReschedule && showDetails && !isSelectionMode && (
                    <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <GripVertical className="h-3 w-3 text-color-foreground-muted" />
                    </div>
                )}
                    ...
                    <>
                        ...
                        <div className="flex items-center space-x-2 text-xs opacity-75">
                            ...
                            <div className="flex items-center space-x-1">
                                ...
                                <span className="truncate">{appointment.staff.displayName}</span>
                            </div>
                        </div>
                    </>
                )}
                ...
                {showMinimalInfo && (
                    <div className="text-xs opacity-90 truncate">
                        {formatTime(appointment.startTime)} • {appointment.staff.displayName}
                    </div>
                )}
            ...
            {appointment.canReschedule && !showDetails && !isSelectionMode && (
                <div className="absolute bottom-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <GripVertical className="h-2 w-2 text-color-foreground-muted" />
                </div>
            )}
