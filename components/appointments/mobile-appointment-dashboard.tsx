...
export interface MobileAppointmentDashboardProps {
  ...
  onAppointmentSave: (
    appointment: Partial<DashboardAppointment>
  ) => Promise<void>;
...
export function MobileAppointmentDashboard({
  ...
  const handleAppointmentDrop = useCallback(
    async (appointmentId: string, newSlot: unknown) => {
  ...
  const handleAppointmentSave = useCallback(
    async (appointmentData: Partial<DashboardAppointment>) => {
  ...
  const containerClasses = cn('flex flex-col h-full bg-color-background', {
    'min-h-screen': isMobile,
    'h-screen': !isMobile,
  });

  const contentClasses = cn('flex-1 flex flex-col overflow-hidden', {
  ...
  return (
    <div className={containerClasses}>
      ...
      {isMobile && (
        <div className="fixed bottom-6 right-6 z-40">
          <button
            ...
            className="flex h-14 w-14 touch-manipulation items-center justify-center rounded-full bg-lumina-radiant text-white shadow-lg transition-all duration-200 hover:shadow-xl"
      ...
      {isMobile && (
        <div className="bg-color-background-muted/50 flex-shrink-0 px-4 py-2 text-center">
