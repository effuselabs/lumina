...
export function BulkOperationHistory({
  ...
  const [selectedEntry, setSelectedEntry] =
    useState<BulkOperationHistoryEntry | null>(null);
  ...
  const getOperationIcon = (
    operation: BulkOperationHistoryEntry['operation']
  ) => {
    switch (operation) {
      case 'cancel':
  ...
  const filteredHistory = mockHistory.filter(entry => {
    ...
    const daysDiff = Math.floor(
      (new Date().getTime() - entry.performedAt.getTime()) /
        (1000 * 60 * 60 * 24)
    );
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            View and audit all bulk appointment operations performed in your
            salon.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 rounded-lg bg-gray-50 p-3">
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      {selectedEntry.performedBy.name}
                    </div>
                    <div className="text-xs text-gray-500">
                      {selectedEntry.performedBy.email}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm font-medium">Results</div>
                    <div className="text-sm">
                      {selectedEntry.successCount}/
                      {selectedEntry.appointmentCount} successful
                    </div>
                  </div>
                </div>

                <div>
                  <h4 className="mb-2 font-medium">Affected Clients</h4>
                ...
                {selectedEntry.details.errors &&
                  selectedEntry.details.errors.length > 0 && (
                    <div>
