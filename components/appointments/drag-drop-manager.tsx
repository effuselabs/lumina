...
function DragDropManagerInner({
  ...
  const {
    state,
    undoHistory,
    onAppointmentMove: contextOnMove,
  } = useDragDrop();
  const { currentOperation, showNotification, hideNotification } =
    useUndoNotification();
  ...
  const handleAppointmentDrop = useCallback(
    async (appointmentId: string, newSlot: CalendarSlot) => {
      ...
      const alternatives = validation.isValid
        ? []
        : findAlternativeSlots(
            appointment,
            newSlot.startTime,
  ...
  const handleUndo = useCallback(async () => {
    ...
    try {
      await onAppointmentMove(
        currentOperation.appointmentId,
        currentOperation.originalSlot
      );
      hideNotification();
    } catch (error) {
...
export function DragDropManager(props: DragDropManagerProps) {
  const handleUndoOperation = async (operation: UndoOperation) => {
    await props.onAppointmentMove(
      operation.appointmentId,
      operation.originalSlot
    );
  };
