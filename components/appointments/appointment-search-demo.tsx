...
const generateMockAppointments = (): DashboardAppointment[] => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const staff =
      mockStaffMembers[Math.floor(Math.random() * mockStaffMembers.length)];
    const service =
    ...
    appointments.push({
      ...
      client: {
        id: `client-${i + 1}`,
        firstName: ['John', 'Jane', 'Bob', 'Alice', 'Charlie', 'Diana'][
          Math.floor(Math.random() * 6)
        ],
        lastName: ['Smith', 'Johnson', 'Brown', 'Davis', 'Wilson', 'Miller'][
          Math.floor(Math.random() * 6)
        ],
