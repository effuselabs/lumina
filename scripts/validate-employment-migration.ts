...
async function validateStaffEmploymentConfigurations(
...
async function validateDatabaseConstraints(
  ...
  try {
    ...
    for (const test of constraintTests) {
      const result_query = await prisma.$queryRawUnsafe<
        Array<{ count: bigint }>
      >(test.query);
    ...
    result.errors.push(
      `Database constraint validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}

async function validatePaymentCalculationsTable(
  result: ValidationResult
): Promise<void> {
  ...
  try {
    ...
    if (missingColumns.length > 0) {
      result.errors.push(
        `Payment calculations table missing columns: ${missingColumns.join(', ')}`
      );
    }
    ...
    if (missingIndexes.length > 0) {
      result.warnings.push(
        `Payment calculations table missing indexes: ${missingIndexes.join(', ')}`
      );
    }
    ...
    result.errors.push(
      `Payment calculations validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
...
async function validateDataConsistency(
  ...
  try {
    ...
    if (orphanedTransactions > 0) {
      result.warnings.push(
        `Found ${orphanedTransactions} transactions with invalid staff references`
      );
    }

    // Check for inconsistent employment types in transactions
    const inconsistentTransactions = await prisma.$queryRaw<
      Array<{ count: bigint }>
    >`
      SELECT COUNT(*) as count
      FROM transactions t
    ...
    if (inconsistentCount > 0) {
      result.warnings.push(
        `Found ${inconsistentCount} transactions with inconsistent employment types`
      );
    }
    ...
    result.errors.push(
      `Data consistency validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
