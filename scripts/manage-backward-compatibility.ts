...
async function showCandidates(compatService: BackwardCompatibilityService) {
  ...
  if (candidates.businesses.length === 0 && candidates.staff.length === 0) {
    console.log(
      '✅ No migration candidates found - all entities are using structured format!'
    );
    return;
  }
  ...
  if (candidates.businesses.length > 0) {
    console.log(
      `🏢 Businesses needing migration (${candidates.businesses.length}):`
    );
  ...
  if (candidates.staff.length > 0) {
    ...
    candidates.staff.forEach((staff, index) => {
      console.log(
        `  ${index + 1}. ${staff.displayName} (ID: ${staff.id}, Business: ${staff.businessId})`
      );
    });
    console.log();
  }
...
async function migrateBusiness(
  compatService: BackwardCompatibilityService,
  businessId: string
) {
  ...
  if (result.success) {
    console.log(
      `✅ Successfully migrated ${result.migratedHours} business hours`
    );
  } else {
...
async function migrateStaff(
  ...
  if (result.success) {
    console.log(
      `✅ Successfully migrated ${result.migratedSlots} availability slots`
    );
  } else {
    console.log(`❌ Migration failed: ${result.error}`);
    process.exit(1);
...
async function migrateAll(compatService: BackwardCompatibilityService) {
  ...
  const totalCandidates =
    candidates.businesses.length + candidates.staff.length;
...
async function testFallback(
  ...
  try {
    const staffAvailability =
      await compatService.getStaffAvailability(entityId);
    ...
    console.log(
      `  Availability slots: ${staffAvailability.availability.length}`
    );
    ...
    if (staffAvailability.availability.length > 0) {
      ...
      staffAvailability.availability.forEach(slot => {
        ...
        console.log(
          `    ${dayName}: ${slot.startTime} - ${slot.endTime} (Recurring: ${slot.isRecurring})`
        );
      });
    }
