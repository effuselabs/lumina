...
function displayContextSummary(contextState: any, verbose: boolean = false) {
  ...
  const validationEmojis = {
    essentialDocsAccessible: contextState.validationResults
      .essentialDocsAccessible
      ? '✅'
      : '❌',
    ...
    recentDecisionsLoaded: contextState.validationResults.recentDecisionsLoaded
      ? '✅'
      : '❌',
    developmentStandardsAvailable: contextState.validationResults
  ...
  console.log(
    `  ${validationEmojis.essentialDocsAccessible} Essential Documents`
  );
  ...
  console.log(
    `  ${validationEmojis.developmentStandardsAvailable} Development Standards`
  );
  ...
  console.log(
    `  ⚠️  Current Issues: ${contextState.projectStateSnapshot.currentIssues.length}`
  );
...
function displayVerboseInfo(contextState: any) {
  ...
  if (contextState.projectStateSnapshot.activeFeatures.length > 0) {
    ...
    contextState.projectStateSnapshot.activeFeatures.forEach((feature: any) => {
      ...
      const statusEmoji =
        statusEmojiMap[feature.status as keyof typeof statusEmojiMap] || '❓';

      console.log(
        `  ${statusEmoji} ${feature.name} (${feature.completionPercentage || 0}%)`
      );
    });
  }
  ...
  if (contextState.projectStateSnapshot.recentDecisions.length > 0) {
    console.log('\n📋 Recent Decisions:');
    contextState.projectStateSnapshot.recentDecisions
      .slice(0, 3)
      .forEach((decision: any) => {
...
async function generateContextReport(contextState: any, outputPath?: string) {
  ...
  const reportPath =
    ...
    join(
      ...
      'project-management',
      'context-reports',
      `ai-context-report-${new Date().toISOString().split('T')[0]}.md`
    );
...
function validateContext(contextState: any) {
  ...
  if (contextState.contextCompleteness < 70) {
    issues.push(
      `Low context completeness: ${contextState.contextCompleteness}%`
    );
  }
  ...
  if (
    contextState.projectStateSnapshot.currentIssues.some(
