...
async function addDecision(args: CLIArgs) {
  if (!args.title) {
    console.error(
      '❌ Title is required for adding a decision. Use --title "Decision Title"'
    );
    process.exit(1);
  }
  ...
  if (args.interactive) {
    ...
    console.log(
      '🔄 Interactive mode not implemented yet. Creating template decision.'
    );
  }
  ...
  const decision: Omit<ArchitecturalDecision, 'id'> = {
    ...
    status: 'proposed',
    context:
      'Please update this context with the actual situation that led to this decision.',
    ...
    impact:
      'Please describe the impact of this decision on the system and development process.',
    relatedIssues: [],
  };
  ...
  console.log(
    `📝 Please edit docs/project-management/decision-log.md to complete the decision details`
  );
}
...
async function listDecisions(args: CLIArgs) {
  ...
  for (const decision of decisions) {
    ...
    console.log(
      `   Status: ${decision.status} | Date: ${decision.date.toDateString()}`
    );
    console.log(
...
async function updateDecision(args: CLIArgs) {
  ...
  if (!args.status) {
    console.error(
      '❌ Status is required for updating. Use --status [proposed|accepted|deprecated|superseded]'
    );
    process.exit(1);
  }
...
async function createTemplate(args: CLIArgs) {
  if (!args.title) {
    console.error(
      '❌ Title is required for creating a template. Use --title "Decision Title"'
    );
    process.exit(1);
  }
