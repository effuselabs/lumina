...
async function showMigrationStatus(migrationService: DataMigrationService) {
  ...
  console.log(
    `  With structured data: ${status.businesses.withStructuredData}`
  );
  ...
  const totalNeedsMigration =
    status.businesses.needsMigration + status.staff.needsMigration;
...
async function performDryRun(migrationService: DataMigrationService) {
  ...
  console.log(
    `  Business hours to migrate: ${status.businesses.needsMigration}`
  );
  console.log(
    `  Staff availability to migrate: ${status.staff.needsMigration}`
  );

  if (
    status.businesses.needsMigration === 0 &&
    status.staff.needsMigration === 0
  ) {
  ...
  if (status.businesses.needsMigration > 0) {
    console.log(
      `  1. Migrate ${status.businesses.needsMigration} business operating hours from JSON to business_hours table`
    );
  ...
  if (status.staff.needsMigration > 0) {
    console.log(
      `  2. Migrate ${status.staff.needsMigration} staff working hours from JSON to staff_availability table`
    );
    console.log('     - Validate JSON structure');
    console.log('     - Convert day names to numeric values');
...
async function performMigration(migrationService: DataMigrationService) {
  ...
  if (result.success) {
    ...
    if (result.rollbackData) {
      ...
      await fs.writeFile(
        rollbackFile,
        JSON.stringify(result.rollbackData, null, 2)
      );
    ...
    if (validation.isValid) {
      ...
      console.log(
        `⚠️  Migration integrity validation found ${validation.errors.length} issues:`
      );
      validation.errors.forEach((error, index) => {
        console.log(
          `  ${index + 1}. [${error.type}:${error.id}:${error.field}] ${error.message}`
        );
      });
    }
  } else {
...
async function performRollback(migrationService: DataMigrationService) {
  ...
  try {
    ...
    console.log(
      `  Business hours records to remove: ${rollbackData.businessHours?.length || 0}`
    );
    console.log(
      `  Staff availability records to remove: ${rollbackData.staffAvailability?.length || 0}`
    );
    ...
    console.log(
      `  Staff availability removed: ${result.staffAvailabilityRemoved}`
    );
    console.log(`  Errors: ${result.errors.length}`);
    ...
    console.log(
      `❌ Failed to read rollback data: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
    process.exit(1);
  }
}
