...
class ResponsiveDesignTester {
      ...
      mediaQueryMatches.forEach(match => {
        ...
        if (match.includes('max-width') && !match.includes('min-width')) {
          this.addIssue(
            filePath,
            0,
            'Using max-width media queries - consider mobile-first approach with min-width',
            'info',
            'mobile-first'
          );
        }
      ...
      if (line.includes('grid-template-columns')) {
        if (!line.includes('repeat') && !line.includes('minmax')) {
          this.addIssue(
            filePath,
            index + 1,
            'Grid layout may not be responsive - consider using repeat() with minmax()',
            'warning',
            'grid'
          );
        }
  ...
  private testTSXFile(
      ...
      if (line.includes('className') && line.includes('grid')) {
        ...
        if (!hasResponsiveBreakpoints) {
          this.addIssue(
            filePath,
            index + 1,
            'Grid layout missing responsive breakpoints',
            'warning',
            'grid'
          );
        }
      ...
      if (line.includes('className')) {
        ...
        if (line.includes('hidden') && !/(sm:|md:|lg:|xl:)/.test(line)) {
          this.addIssue(
            filePath,
            index + 1,
            'Hidden element without responsive breakpoints',
            'warning',
            'mobile-first'
          );
        }
        ...
        const textSizeMatch = line.match(
          /text-(xs|sm|base|lg|xl|2xl|3xl|4xl|5xl|6xl)/g
        );
        if (
          textSizeMatch &&
          textSizeMatch.length > 0 &&
          !/(sm:|md:|lg:|xl:)/.test(line)
        ) {
          this.addIssue(
            filePath,
            index + 1,
            'Text size without responsive breakpoints',
            'info',
            'breakpoints'
          );
        }
    ...
    if (filePath.includes('layout.tsx') && !content.includes('viewport')) {
      this.addIssue(
        filePath,
        0,
        'Layout missing viewport meta tag for responsive design',
        'warning',
        'layout'
      );
    }
    ...
    if (content.includes('container') && !content.includes('mx-auto')) {
      this.addIssue(
        filePath,
        0,
        'Container usage without centering - may cause layout issues',
        'info',
        'layout'
      );
    }
    ...
    lines.forEach((line, index) => {
      if (
        ...
        this.addIssue(
          filePath,
          index + 1,
          'Table without overflow handling - may break on mobile',
          'warning',
          'mobile-first'
        );
      }
      ...
      if (line.includes('className') && line.includes('flex')) {
        if (line.includes('flex-row') && !/(sm:|md:|lg:|xl:)/.test(line)) {
          this.addIssue(
            filePath,
            index + 1,
            'Flex row layout without responsive breakpoints',
            'info',
            'breakpoints'
          );
        }
  ...
  private generateSummary() {
    return {
      ...
      mobileFirst: this.issues.filter(i => i.category === 'mobile-first')
        .length,
  ...
  private printResults(): void {
    ...
    if (summary.grid > 0) {
      console.log(
        `   - Add responsive breakpoints to ${summary.grid} grid layouts`
      );
    }
    if (summary.mobileFirst > 0) {
      console.log(
        `   - Review ${summary.mobileFirst} mobile-first design issues`
      );
    }
    if (summary.breakpoints > 0) {
      console.log(
        `   - Add responsive breakpoints to ${summary.breakpoints} elements`
      );
    }
    if (summary.layout > 0) {
      console.log(
        `   - Fix ${summary.layout} layout-related responsive issues`
      );
    }
  }
  ...
  async generateResponsiveReport(): Promise<void> {
    ...
    const reportPath =
      '.kiro/specs/design-system-consistency/responsive-design-test-report.md';
    await fs.writeFile(reportPath, reportContent);

    console.log(`📄 Responsive design test report saved to: ${reportPath}`);
  }

  private generateDetailedResponsiveReport(
