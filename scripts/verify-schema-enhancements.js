...
async function verifySchemaEnhancements() {
  console.log(
    '🔍 Verifying Appointment Booking Engine Schema Enhancements...\n'
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ✓ AppointmentStatusHistory model accessible (${statusHistoryCount} records)`
      );
    } catch (error) {
      console.log(
        `   ❌ AppointmentStatusHistory model not accessible: ${error.message}`
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `   ✓ AppointmentPreferences model accessible (${preferencesCount} records)`
      );
    } catch (error) {
      console.log(
        `   ❌ AppointmentPreferences model not accessible: ${error.message}`
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `   ❌ Enhanced Appointment fields not accessible: ${error.message}`
      );
      return false;
    }
    ...
    try {
      ...
      console.log(
        `   ❌ Enhanced AppointmentService fields not accessible: ${error.message}`
      );
      return false;
    }
    ...
    if (queryTime < 1000) {
      ...
      console.log(
        `   ⚠️  Query took ${queryTime}ms (may indicate missing index)`
      );
    }
    ...
    try {
      ...
      console.log(
        `   ❌ Appointment relationships not working: ${error.message}`
      );
      return false;
    }
    ...
    console.log(
      '   • Enhanced AppointmentService model with ordering and staff assignment'
    );
