...
function listScenarios() {
  ...
  scenarios.forEach(scenario => {
    ...
    console.log(
      `   Historical months: ${scenario.config.appointments?.historicalMonths || 'default'}`
    );
    console.log('');
  });
}
    ...
    if (verbose) {
      ...
      if (result.errors.length > 0) {
        ...
        result.errors.forEach(error => {
          console.log(
            `  [${error.severity.toUpperCase()}] ${error.entity}${error.field ? `.${error.field}` : ''}: ${error.message}`
          );
        });
      }
      ...
      if (result.warnings.length > 0) {
        ...
        result.warnings.forEach(warning => {
          console.log(
            `  ${warning.entity}${warning.field ? `.${warning.field}` : ''}: ${warning.message}`
          );
        });
      }
    }
...
async function seedData(
  ...
  console.log(
    `🌱 Seeding data for business: ${businessId} with scenario: ${scenarioName}\n`
  );
  ...
  try {
    ...
    console.log(
      '⚠️  Note: Actual seeding integration would be implemented here'
    );
    ...
    switch (options.command) {
      ...
      case 'validate':
        if (!options.businessId) {
          ...
          console.log(
            'Usage: npm run manage-seed-data validate -b <business-id>'
          );
          process.exit(1);
        }
      ...
      case 'seed':
        if (!options.businessId) {
          ...
          console.log(
            'Usage: npm run manage-seed-data seed -b <business-id> -s <scenario>'
          );
          process.exit(1);
        }
        if (!options.scenario) {
      ...
      case 'report':
        if (!options.businessId) {
          ...
          console.log(
            'Usage: npm run manage-seed-data report -b <business-id>'
          );
          process.exit(1);
        }
