...
class ThemeSwitchingTester {
  ...
  private testCSSFile(
    ...
    lines.forEach((line, index) => {
      ...
      const hslColorMatch = line.match(
      ...
      if (hexColorMatch || rgbColorMatch || hslColorMatch) {
        ...
        if (
          !filePath.includes('design-tokens') &&
          !filePath.includes('colors.css')
        ) {
          this.hardcodedColorCount++;
          this.addIssue(
            filePath,
            index + 1,
            'Hardcoded color found in CSS - should use CSS custom properties',
            'warning'
          );
        }
      }
    ...
    if (
      ...
      this.addIssue(
        filePath,
        0,
        'Using Tailwind dark mode instead of data-theme approach',
        'info'
      );
    }
  }
  ...
  private testTSXFile(
    ...
    lines.forEach((line, index) => {
      if (line.includes('className') && /#[0-9a-fA-F]{3,6}/.test(line)) {
        ...
        this.addIssue(
          filePath,
          index + 1,
          'Hardcoded color in className - should use design tokens',
          'warning'
        );
      }
    });
    ...
    if (filePath.includes('layout.tsx')) {
      if (
        ...
        !content.includes('theme-provider')
      ) {
        this.missingThemeSupportCount++;
        this.addIssue(
          filePath,
          0,
          'Layout missing ThemeProvider - theme switching will not work',
          'error'
        );
      }
    }
    ...
    if (content.includes('useTheme') || content.includes('theme-provider')) {
      ...
      this.addIssue(
        filePath,
        0,
        'Using Tailwind dark mode classes instead of data-theme approach',
        'info'
      );
    }
      ...
      styleObjectMatches.forEach(match => {
        if (/#[0-9a-fA-F]{3,6}/.test(match)) {
          ...
          this.addIssue(
            filePath,
            0,
            'Hardcoded color in style object - should use CSS custom properties',
            'warning'
          );
        }
      });
  ...
  private printResults(): void {
    ...
    console.log(
      `🌙 Files missing theme support: ${this.missingThemeSupportCount}`
    );
    ...
    if (this.hardcodedColorCount > 0) {
      console.log(
        `   - Replace ${this.hardcodedColorCount} hardcoded colors with CSS custom properties`
      );
    }
    if (this.missingThemeSupportCount > 0) {
      console.log(
        `   - Add ThemeProvider to ${this.missingThemeSupportCount} layout files`
      );
    }
    if (this.cssVariableUsageCount > 0) {
      console.log(
        `   ✅ Good: ${this.cssVariableUsageCount} CSS variables already in use`
      );
    }
  }
  ...
  async generateThemeReport(): Promise<void> {
    ...
    const reportPath =
      '.kiro/specs/design-system-consistency/theme-switching-test-report.md';
