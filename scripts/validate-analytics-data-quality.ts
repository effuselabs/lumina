...
class AnalyticsDataQualityValidator {
    ...
    async validateDataQuality(): Promise<ValidationSummary> {
        console.log('🔍 Starting Analytics and Reporting Data Quality Validation...\n');

        // Check if seed data exists
        await this.checkSeedDataExists();

        // Run test suites
        await this.runTestSuite('Analytics Data Quality', '__tests__/analytics/data-quality-validation.test.ts');
        await this.runTestSuite('Dashboard API Endpoints', '__tests__/api/dashboard-endpoints.test.ts');
        await this.runTestSuite('Report Types Validation', '__tests__/reports/report-types-validation.test.ts');
        await this.runTestSuite('Calculation Accuracy', '__tests__/analytics/calculation-accuracy.test.ts');
    ...
    private async checkSeedDataExists(): Promise<void> {
        ...
        try {
            ...
            if (business.clients.length < 10) {
                console.warn(`⚠️  Warning: Only ${business.clients.length} clients found. Recommend at least 50 for comprehensive testing.`);
            }

            if (business.staff.length < 3) {
                console.warn(`⚠️  Warning: Only ${business.staff.length} staff found. Recommend at least 6 for comprehensive testing.`);
            }

            if (business.services.length < 10) {
                console.warn(`⚠️  Warning: Only ${business.services.length} services found. Recommend at least 30 for comprehensive testing.`);
            }

            if (business.appointments.length < 50) {
                console.warn(`⚠️  Warning: Only ${business.appointments.length} appointments found. Recommend at least 500 for comprehensive testing.`);
            }
            ...
            throw new Error('Please run comprehensive seed data generation before validation');
        }
    }
    ...
    private async validateDataDistribution(businessId: string): Promise<void> {
        ...
        try {
            ...
            if (daysWithRevenue === 0) {
                ...
                console.log(`✅ Revenue data: ${daysWithRevenue}/${revenueData.length} days with revenue`);
            }

            // Check client metrics
            const clientMetrics = await dataService.getClientMetrics(dateRange);
            console.log(`✅ Client metrics: ${clientMetrics.totalClients} total, ${clientMetrics.newClients} new, ${clientMetrics.returningClients} returning`);

            // Check staff performance
            const staffPerformance = await dataService.getStaffPerformance(dateRange);
            const activeStaff = staffPerformance.filter(s => s.appointmentCount > 0).length;
            console.log(`✅ Staff performance: ${activeStaff}/${staffPerformance.length} staff with appointments`);

            // Check service analytics
            const serviceAnalytics = await dataService.getServiceAnalytics(dateRange);
            const popularServices = serviceAnalytics.filter(s => s.bookingCount > 0).length;
            console.log(`✅ Service analytics: ${popularServices}/${serviceAnalytics.length} services with bookings`);
    ...
    private printDataQualityAssessment(summary: ValidationSummary): void {
        ...
        if (passRate === 100) {
            console.log('🎉 EXCELLENT: All analytics and reporting data quality tests passed!');
            ...
            console.log('   • All report types (P&L, commission, client, service) work correctly');
