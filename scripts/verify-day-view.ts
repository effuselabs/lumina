...
function verifyTimeSlotGeneration() {
    ...
    console.log(`✅ Business hours: ${todayHours.openTime} - ${todayHours.closeTime}`);
...
function verifyAppointmentFiltering() {
    ...
    const staff1Appointments = mockAppointments.filter(apt => apt.staffId === 'staff-1');
    const staff2Appointments = mockAppointments.filter(apt => apt.staffId === 'staff-2');
...
function verifyConflictDetection() {
    ...
    const staff1Appointments = conflictingAppointments.filter(apt => apt.staffId === 'staff-1');
    let conflicts = 0;
...
function verifyBusinessHoursHandling() {
    ...
    const sundayHours = mockBusinessHours.find(h => h.dayOfWeek === sundayDate.getDay());
    ...
    const mondayHours = mockBusinessHours.find(h => h.dayOfWeek === mondayDate.getDay());

    if (mondayHours && !mondayHours.isClosed && mondayHours.openTime && mondayHours.closeTime) {
    ...
    if (isToday) {
        ...
        if (todayHours && !todayHours.isClosed && todayHours.openTime && todayHours.closeTime) {
            ...
            if (isWithinBusinessHours) {
                ...
                console.log(`✅ Current time indicator position: ${percentage.toFixed(2)}%`);
            }
        }
    }
