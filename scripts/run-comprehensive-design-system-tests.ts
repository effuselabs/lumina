...
const testSuites: TestSuite[] = [
  {
    ...
    description:
      'Screenshot-based testing for visual consistency across themes and breakpoints',
  ...
  },
  {
    ...
    description:
      'Testing layout and functionality across different viewport sizes',
...
class ComprehensiveTestRunner {
  ...
  private displayTestPlan(): void {
    ...
    const totalEstimatedTime = testSuites
      ...
      .reduce((total, suite) => {
        const max = parseInt(
          suite.estimatedTime.split('-')[1] || suite.estimatedTime
        );
        return total + max;
      }, 0);

    console.log(
      `Total estimated time: ${totalEstimatedTime}-${totalEstimatedTime + 10} minutes\n`
    );
  }
  ...
  private buildComprehensiveReport(): string {
    ...
    const visualResult = this.results.find(
      r => r.suite === 'Visual Regression'
    );
    if (visualResult) {
    ...
    const browserResult = this.results.find(
      r => r.suite === 'Cross-Browser Compatibility'
    );
    if (browserResult) {
    ...
    report += `\n### Responsive Design\n`;
    const responsiveResult = this.results.find(
      r => r.suite === 'Responsive Design'
    );
    if (responsiveResult) {
    ...
    const a11yResult = this.results.find(
      r => r.suite === 'Accessibility Compliance'
    );
    if (a11yResult) {
...
`;
  ...
  private getTestSummary() {
    ...
    const requiredPassed = this.results.filter(
      r => r.passed && testSuites.find(s => s.name === r.suite)?.required
    ).length;
  ...
  private displaySummary(): void {
    ...
    console.log(`\n⏱️  Performance:`);
    console.log(
      `   Total Duration: ${Math.round(totalDuration / 1000)}s (${Math.round(totalDuration / 60000)}m)`
    );

    console.log(`\n🎯 Overall Assessment:`);
    if (summary.successRate >= 90) {
      console.log(
        '   🎉 EXCELLENT - Design system meets high quality standards'
      );
    } else if (summary.successRate >= 75) {
      console.log(
        '   ✅ GOOD - Design system meets acceptable standards with minor issues'
      );
    } else {
      console.log(
        '   ⚠️  NEEDS IMPROVEMENT - Design system requires attention'
      );
    }
