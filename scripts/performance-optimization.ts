...
class PerformanceOptimizer {
    ...
    private async analyzeFile(filePath: string): Promise<void> {
        try {
            ...
            this.addIssue(filePath, 0, `Failed to read file: ${error}`, 'error', 'bundle', 'high');
        }
    }

    private analyzeCSSFile(filePath: string, content: string, lines: string[]): void {
        // Check for unused CSS selectors
        const selectors = content.match(/\.[a-zA-Z][a-zA-Z0-9_-]*\s*{/g);
        if (selectors && selectors.length > 100) {
            this.addIssue(filePath, 0, 'Large CSS file with many selectors - consider splitting or tree-shaking', 'warning', 'css', 'medium');
        }
        ...
        lines.forEach((line, index) => {
            ...
            if (line.includes('* {') || line.includes('*,')) {
                this.addIssue(filePath, index + 1, 'Universal selector can impact performance', 'warning', 'css', 'medium');
            }
            ...
            if (selectorComplexity > 4 && line.includes('{')) {
                this.addIssue(filePath, index + 1, 'Complex CSS selector may impact performance', 'info', 'css', 'low');
            }

            // Check for expensive properties
            if (line.includes('box-shadow') && line.includes('blur')) {
                this.addIssue(filePath, index + 1, 'Blur box-shadow can be expensive - consider alternatives', 'info', 'css', 'low');
            }

            // Check for layout-triggering properties in animations
            if (line.includes('@keyframes') || line.includes('transition')) {
                if (line.includes('width') || line.includes('height') || line.includes('top') || line.includes('left')) {
                    this.addIssue(filePath, index + 1, 'Animation uses layout-triggering properties - use transform instead', 'warning', 'animation', 'high');
                }
            }
        });
        ...
        lines.forEach((line, index) => {
            ...
            if (trimmed && trimmed.includes(':') && !trimmed.startsWith('/*')) {
                ...
                if (count > 2) {
                    this.addIssue(filePath, index + 1, 'Duplicate CSS rule - consider consolidating', 'info', 'css', 'low');
                }
            }
        });
    }

    private analyzeTSXFile(filePath: string, content: string, lines: string[]): void {
        // Check for large components
        const componentLines = lines.length;
        if (componentLines > 300) {
            this.addIssue(filePath, 0, 'Large component file - consider splitting into smaller components', 'warning', 'rendering', 'medium');
        }

        // Check for missing React.memo or useMemo
        if (content.includes('export default function') && !content.includes('memo(') && componentLines > 100) {
            this.addIssue(filePath, 0, 'Large component without memoization - consider React.memo', 'info', 'rendering', 'medium');
        }

        // Check for expensive operations in render
        lines.forEach((line, index) => {
            // Check for array operations in JSX
            if (line.includes('.map(') && line.includes('key=') && !line.includes('useMemo')) {
                this.addIssue(filePath, index + 1, 'Array mapping in render without memoization', 'info', 'rendering', 'low');
            }

            // Check for object creation in JSX
            if (line.includes('style={{') || line.includes('className={`')) {
                this.addIssue(filePath, index + 1, 'Object/string creation in render - consider memoization', 'info', 'rendering', 'low');
            }

            // Check for inline functions
            if (line.includes('onClick={() =>') || line.includes('onChange={() =>')) {
                this.addIssue(filePath, index + 1, 'Inline function in JSX - consider useCallback', 'info', 'rendering', 'low');
            }

            // Check for heavy imports
            if (line.includes('import') && (line.includes('lodash') || line.includes('moment'))) {
                this.addIssue(filePath, index + 1, 'Heavy library import - consider tree-shaking or alternatives', 'warning', 'bundle', 'high');
            }

            // Check for dynamic imports
            if (line.includes('import(') && !line.includes('lazy')) {
                this.addIssue(filePath, index + 1, 'Dynamic import without lazy loading - consider React.lazy', 'info', 'bundle', 'medium');
            }
        });
        ...
        imports.forEach(importStatement => {
            ...
            if (importName && !content.includes(importName.replace('import ', '').replace(/[{}]/g, ''))) {
                this.addIssue(filePath, 0, `Potentially unused import: ${importStatement}`, 'info', 'bundle', 'low');
            }
        });
    }
    ...
    private async analyzeBundleSize(): Promise<void> {
        ...
        try {
            ...
            const heavyPackages = [
                ...
                'antd', 'semantic-ui-react', 'react-bootstrap'
            ];

            Object.keys(dependencies).forEach(dep => {
                if (heavyPackages.some(heavy => dep.includes(heavy))) {
                    this.addIssue('package.json', 0, `Heavy dependency detected: ${dep}`, 'warning', 'bundle', 'high');
            ...
            const duplicates = depNames.filter(dep =>
                depNames.some(other => other !== dep && other.includes(dep.split('-')[0]))
            );

            duplicates.forEach(dup => {
                this.addIssue('package.json', 0, `Potential duplicate dependency: ${dup}`, 'info', 'bundle', 'medium');
            });
    ...
    private async analyzeCSSOptimization(): Promise<void> {
        ...
        try {
            ...
            if (!tailwindConfig.includes('purge') && !tailwindConfig.includes('content')) {
                this.addIssue('tailwind.config.ts', 0, 'Tailwind CSS purging not configured - bundle may include unused styles', 'warning', 'css', 'high');
            }

            // Check for CSS-in-JS usage
            const cssInJsFiles = await glob('**/*.{tsx,ts}', { ignore: ['node_modules/**'] });
            let cssInJsUsage = 0;

            for (const file of cssInJsFiles.slice(0, 20)) { // Sample check
                const content = await fs.readFile(file, 'utf-8');
                if (content.includes('styled-components') || content.includes('emotion') || content.includes('makeStyles')) {
                    cssInJsUsage++;
                }
            }

            if (cssInJsUsage > 5) {
                this.addIssue('multiple files', 0, 'Heavy CSS-in-JS usage detected - consider static CSS for better performance', 'info', 'css', 'medium');
            }
    ...
    private async analyzeAnimationPerformance(): Promise<void> {
        ...
        const animationFiles = await glob('**/*.{css,tsx,ts}', { ignore: ['node_modules/**'] });

        for (const file of animationFiles.slice(0, 30)) { // Sample check
            try {
                const content = await fs.readFile(file, 'utf-8');

                // Check for 60fps animations
                if (content.includes('animation') || content.includes('transition')) {
                    if (content.includes('width') || content.includes('height') || content.includes('top') || content.includes('left')) {
                        this.addIssue(file, 0, 'Animation may cause layout thrashing - use transform/opacity instead', 'warning', 'animation', 'high');
                    }
                }

                // Check for will-change usage
                if (content.includes('transform') && !content.includes('will-change')) {
                    this.addIssue(file, 0, 'Consider adding will-change property for better animation performance', 'info', 'animation', 'low');
                }

                // Check for excessive animations
                const animationCount = (content.match(/@keyframes|animation:|transition:/g) || []).length;
                if (animationCount > 10) {
                    this.addIssue(file, 0, 'Many animations in single file - consider performance impact', 'info', 'animation', 'medium');
                }
        ...
        this.recommendations.push('Use transform and opacity for smooth 60fps animations');
        this.recommendations.push('Add will-change property for elements that will be animated');
        this.recommendations.push('Remove animations on low-end devices using prefers-reduced-motion');
    }

    private addIssue(file: string, line: number, issue: string, severity: 'error' | 'warning' | 'info', category: 'bundle' | 'css' | 'animation' | 'rendering', impact: 'high' | 'medium' | 'low'): void {
    ...
    async generatePerformanceReport(): Promise<void> {
        ...
        const reportPath = '.kiro/specs/design-system-consistency/performance-optimization-report.md';
        await fs.writeFile(reportPath, reportContent);

        console.log(`\n📄 Performance optimization report saved to: ${reportPath}`);
    }

    private generateDetailedPerformanceReport(result: PerformanceOptimizationResult): string {
...
${result.issues.filter(i => i.impact === 'high').map(issue =>
            `- **${issue.file}:${issue.line}** - ${issue.issue} (${issue.category})`
        ).join('\n')}

## Medium Impact Optimizations

${result.issues.filter(i => i.impact === 'medium').slice(0, 15).map(issue =>
            `- **${issue.file}:${issue.line}** - ${issue.issue} (${issue.category})`
        ).join('\n')}
