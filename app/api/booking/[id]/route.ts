    ...
    try {
            ...
            try {
                ...
                if (updatedAppointment.client?.email && emailType) {
                    const emailResult = await emailService.sendBookingConfirmation({
                        ...
                        staffName: updatedAppointment.staff.displayName || updatedAppointment.staff.user.name || '',
                        appointmentDate: format(new Date(updatedAppointment.startTime), 'EEEE, MMMM d, yyyy'),
                    ...
                    if (emailResult.success) {
                        console.log(`${emailType} email sent successfully:`, emailResult.messageId)
                    }
                }
            } catch (emailError) {
        ...
        return NextResponse.json({
            ...
            appointment: {
                ...
                staff: {
                    name: updatedAppointment.staff.displayName || updatedAppointment.staff.user.name,
                },
                service: {
    ...
    try {
        ...
        if (appointment.client?.email) {
            try {
                ...
                console.log('Appointment cancelled, should send cancellation email to:', appointment.client.email)
            } catch (emailError) {
