    ...
    try {
        ...
        if (type === 'single') {
            ...
            return NextResponse.json(
                { error: 'Invalid conversion type. Must be "single", "slot", or "batch"' },
                { status: 400 }
            )
        }
    } catch (error) {
...
async function handleSingleTimeConversion(body: ConversionRequest) {
    ...
    try {
        ...
        const sourceDST = TimeZoneHandler.handleDSTTransition(time, time, date, fromTimezone)
        ...
        return NextResponse.json({
            ...
            dstInfo: {
                ...
                warnings: [
                    ...(sourceDST.dstTransition ? [`Source timezone DST transition: ${sourceDST.transitionType}`] : []),
                    ...(targetDST.dstTransition ? [`Target timezone DST transition: ${targetDST.transitionType}`] : [])
                ]
            }
        })
    } catch (error) {
...
async function handleTimeSlotConversion(body: TimeSlotConversionRequest) {
    ...
    if (!startTime || !endTime || !date || !fromTimezone || !toTimezone) {
        return NextResponse.json(
            { error: 'Missing required fields: startTime, endTime, date, fromTimezone, toTimezone' },
            { status: 400 }
        )
    }
    ...
    try {
        ...
        const utcSlot = TimeZoneHandler.timeSlotToUTC(startTime, endTime, date, fromTimezone)

        // Convert to target timezone
        const localSlot = TimeZoneHandler.timeSlotToLocal(utcSlot.start, utcSlot.end, toTimezone)

        // Check for DST transitions
        const sourceDST = TimeZoneHandler.handleDSTTransition(startTime, endTime, date, fromTimezone)
        const targetDST = TimeZoneHandler.handleDSTTransition(
            localSlot.start,
            localSlot.end,
            localSlot.date,
        ...
        // Calculate duration
        const originalDuration = DateTime.fromFormat(`${date} ${endTime}`, 'yyyy-MM-dd HH:mm', { zone: fromTimezone })
            .diff(DateTime.fromFormat(`${date} ${startTime}`, 'yyyy-MM-dd HH:mm', { zone: fromTimezone }), 'minutes').minutes

        const convertedDuration = utcSlot.end.diff(utcSlot.start, 'minutes').minutes
        ...
        return NextResponse.json({
            ...
            dstInfo: {
                ...
                warnings: [
                    ...(sourceDST.dstTransition ? [`Source timezone DST transition: ${sourceDST.transitionType}`] : []),
                    ...(targetDST.dstTransition ? [`Target timezone DST transition: ${targetDST.transitionType}`] : []),
                    ...(originalDuration !== convertedDuration ? ['Duration changed due to timezone conversion'] : [])
                ]
            }
        })
    } catch (error) {
...
async function handleBatchConversion(body: { conversions: ConversionRequest[] }) {
...
export async function GET(request: NextRequest) {
    try {
        ...
        if (action === 'current-time') {
            ...
            const currentTime = TimeZoneHandler.getCurrentTimeInBusinessZone(timezone)
            const timezoneInfo = TimeZoneHandler.getTimeZoneInfo(timezone, currentTime)
        ...
        return NextResponse.json(
            { error: 'Invalid action. Supported actions: supported-timezones, current-time' },
            { status: 400 }
        )
    } catch (error) {
