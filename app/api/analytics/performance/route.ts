...
const PerformanceDataSchema = z.object({
    ...
    performanceData: z.object({
        ...
        userInteractions: z.array(z.object({
            type: z.string(),
            timestamp: z.number(),
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('Performance metrics received:', {
            ...
            networkType: validatedData.performanceData.networkConditions.effectiveType,
        });
...
function analyzePerformanceIssues(data: z.infer<typeof PerformanceDataSchema>): string[] {
    const issues: string[] = [];
    const { performanceData } = data;

    // Check Core Web Vitals
    if (performanceData.firstContentfulPaint > 1800) {
        issues.push(`Slow First Contentful Paint: ${performanceData.firstContentfulPaint.toFixed(0)}ms`);
    }

    if (performanceData.largestContentfulPaint > 2500) {
        issues.push(`Slow Largest Contentful Paint: ${performanceData.largestContentfulPaint.toFixed(0)}ms`);
    }

    if (performanceData.cumulativeLayoutShift > 0.1) {
        issues.push(`High Cumulative Layout Shift: ${performanceData.cumulativeLayoutShift.toFixed(3)}`);
    }
    ...
    Object.entries(performanceData.apiResponseTimes).forEach(([api, time]) => {
        ...
        if (time > threshold) {
            issues.push(`Slow ${api} API: ${time.toFixed(0)}ms (threshold: ${threshold}ms)`);
        }
    });
    ...
    if (performanceData.networkConditions.effectiveType === 'slow-2g' ||
...
export async function GET(request: NextRequest) {
    try {
        ...
        const performanceAnalytics = await getPerformanceAnalytics(businessId, timeRange);
