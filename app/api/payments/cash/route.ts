...
export async function POST(request: NextRequest) {
  try {
    ...
    const { appointmentId, amount, tipAmount, description } =
      cashPaymentSchema.parse(body);
    ...
    const paymentTransaction = await createTransaction({
      ...
      description:
        description ||
        `Cash payment for appointment with ${appointment.staff.displayName}`,
    ...
    if (
      paymentTransaction.commissionAmount &&
      paymentTransaction.commissionAmount > 0
    ) {
