...
async function calculateRevenue(businessId: string, startDate: Date, endDate: Date) {
...
async function calculateGrowth(current: number, previous: number): Promise<number> {
    if (previous === 0) return current > 0 ? 100 : 0;
    return Math.round(((current - previous) / previous) * 100);
}

async function generateTrend(businessId: string, days: number): Promise<number[]> {
    const trend: number[] = [];
...
export async function GET(request: NextRequest) {
    try {
        ...
        const revenueYesterday = await calculateRevenue(businessId, yesterday, today);
        const revenueThisWeek = await calculateRevenue(businessId, weekStart, now);
        const revenueLastWeek = await calculateRevenue(businessId, lastWeekStart, weekStart);
        const revenueThisMonth = await calculateRevenue(businessId, monthStart, now);
        const revenueLastMonth = await calculateRevenue(businessId, lastMonthStart, monthStart);
        const revenueYearToDate = await calculateRevenue(businessId, yearStart, now);
