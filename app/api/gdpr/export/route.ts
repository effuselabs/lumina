...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const { clientId, businessId, format, includeTransactions, includeCommunications } =
        ...
        const validation = await businessContextSecurity.validateBusinessContext(
            ...
            createSecurityMetadata(securityContext, { action: 'gdpr_export', clientId })
        )
        ...
        // Export client data
        const exportData = await dataProtection.exportClientData(clientId, businessId)
...
function convertToCSV(exportData: any): string {
    ...
    exportData.appointments.forEach((appointment: any) => {
        const services = appointment.services.map((s: any) => s.serviceName).join('; ')
        lines.push([
            appointment.id,
