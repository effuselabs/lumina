...
const DeleteRequestSchema = z.object({
    ...
    acknowledgeIrreversible: z.boolean().refine(val => val === true, 'Must acknowledge that deletion is irreversible')
})
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const { clientId, businessId, reason, confirmationCode, acknowledgeIrreversible } =
        ...
        return NextResponse.json({
            ...
            notice: 'Some business records have been anonymized rather than deleted to comply with financial and legal requirements.'
        })
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const validation = await businessContextSecurity.validateBusinessContext(
            ...
            user.id,
            ['OWNER'],
            createSecurityMetadata(securityContext, { action: 'gdpr_delete_confirmation', clientId })
        )

        if (!validation.isValid) {
            return NextResponse.json(
                { error: 'Unauthorized: Only business owners can request deletion confirmation codes' },
                { status: 403 }
            )
        }
        ...
        const clientValidation = await businessContextSecurity.validateClientAccess(
            ...
            businessId,
            user.id,
            createSecurityMetadata(securityContext, { action: 'gdpr_delete_confirmation' })
        )
        ...
        return NextResponse.json({
            ...
            warning: 'This deletion is irreversible. Please ensure you have exported any needed data before proceeding.'
        })
 ...
 */
function generateConfirmationCode(clientId: string, businessId: string): string {
