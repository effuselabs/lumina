...
import { validateAppointmentSchema } from '@/lib/validations/appointment';
  ...
  try {
    ...
    const validationResult = {
      ...
      validations: {
        basicValidation: {
          passed: true,
          details: 'Basic data validation passed',
        },
        serviceValidation: {
          passed: false,
          details: '',
    ...
    try {
      ...
      const serviceValidation =
        await multiServiceCoordinator.validateMultiServiceBooking(
      ...
      validationResult.validations.serviceValidation = {
        ...
        details: serviceValidation.isValid
          ? 'Multi-service booking validation passed'
    ...
    try {
      ...
      validationResult.validations.availabilityValidation = {
        ...
        details: availabilityResult.isAvailable
          ? 'Time slot is available'
          : availabilityResult.reason || 'Time slot not available',
        alternatives:
      ...
      if (!availabilityResult.isAvailable) {
        ...
        if (
          ...
          validationResult.recommendations.push(
            `${availabilityResult.alternatives.length} alternative time slots available`
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      validationResult.validations.conflictValidation = {
        ...
        details: conflictResult.hasConflicts
          ? `${conflictResult.conflicts.length} conflicts detected`
    ...
    try {
      ...
      validationResult.validations.durationValidation = {
        ...
        details: durationResult.isValid
          ? 'Service duration validation passed'
          : durationResult.reason || 'Duration validation failed',
      };

      if (!durationResult.isValid) {
        validationResult.isValid = false;
        validationResult.errors.push(
          'Service duration does not match time slot'
        );
      }
    } catch (error) {
      validationResult.isValid = false;
    ...
    try {
      ...
      validationResult.validations.businessRulesValidation = {
        ...
        details:
          validatedData.startTime > new Date()
    ...
    if (validationResult.isValid) {
      validationResult.recommendations.push(
        'Appointment data is valid and ready for booking'
      );
    } else {
      validationResult.recommendations.push(
        'Please address the validation errors before booking'
      );
    }

    if (validationResult.warnings.length > 0) {
      validationResult.recommendations.push(
        'Consider the warnings for optimal scheduling'
      );
    }
