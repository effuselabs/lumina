...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      successRate:
        recentPerformanceMetrics.length > 0
          ? recentPerformanceMetrics.filter(m => m.success).length /
            recentPerformanceMetrics.length
          : 1,
      averageResponseTime:
        recentPerformanceMetrics.length > 0
          ? recentPerformanceMetrics.reduce((sum, m) => sum + m.duration, 0) /
            recentPerformanceMetrics.length
          : 0,
      cacheHitRatio:
        recentCacheMetrics.length > 0
          ? recentCacheMetrics.filter(m => m.operation === 'hit').length /
            recentCacheMetrics.filter(
    ...
    const performanceByOperation = recentPerformanceMetrics.reduce(
      (acc, metric) => {
    ...
    const operationMetrics = Object.entries(performanceByOperation).map(
      ([operation, data]) => ({
        operation,
        count: data.count,
    ...
    const alertSummary = filteredAlerts.reduce(
      (acc, alert) => {
        if (!acc[alert.type]) {
          acc[alert.type] = {
            count: 0,
  ...
  try {
    ...
    switch (action) {
      case 'update_thresholds':
        ...
        availabilityLogger.log(
          LogLevel.INFO,
          'Monitoring thresholds updated via API',
          {
...
function calculateSystemHealth(
  ...
  const unhealthyServices = Array.from(serviceHealth.values()).filter(
