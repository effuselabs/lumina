...
const reportQuerySchema = z.object({
    ...
    reportType: z.enum(['revenue', 'staff', 'employment', 'detailed']).optional().default('detailed'),
...
export async function GET(request: NextRequest) {
    try {
        ...
        const { businessId, startDate, endDate, reportType, staffId, employmentType } = query;
        ...
        switch (reportType) {
            case 'revenue':
                report = { ...report, ...(await generateRevenueReport(businessId, startDateTime, endDateTime, employmentType)) };
                break;

            case 'staff':
                if (!staffId) {
                    return NextResponse.json({ error: 'Staff ID is required for staff reports' }, { status: 400 });
                }
                report = { ...report, ...(await generateStaffReport(staffId, startDateTime, endDateTime)) };
                break;

            case 'employment':
                report = { ...report, ...(await generateEmploymentReport(businessId, startDateTime, endDateTime)) };
                break;

            case 'detailed':
            default:
                report = { ...report, ...(await generateDetailedReport(businessId, startDateTime, endDateTime, staffId, employmentType)) };
                break;
        }
...
async function generateRevenueReport(
    ...
    const dailyRevenue = await getDailyRevenueBreakdown(businessId, startDate, endDate);
 ...
 */
async function generateStaffReport(staffId: string, startDate: Date, endDate: Date) {
    const staffEarnings = await calculateStaffEarnings(staffId, startDate, endDate);
 ...
 */
async function generateEmploymentReport(businessId: string, startDate: Date, endDate: Date) {
    ...
    const employmentBreakdown = await Promise.all([
        calculateBusinessRevenue(businessId, startDate, endDate, { employmentType: 'COMMISSION' }),
        calculateBusinessRevenue(businessId, startDate, endDate, { employmentType: 'CHAIR_RENTAL' }),
        calculateBusinessRevenue(businessId, startDate, endDate, { employmentType: 'HYBRID' }),
    ]);
    ...
    return {
        employmentBreakdown: {
            commission: {
                ...
                averagePerStaff: staff.filter(s => s.employmentType === 'COMMISSION').length > 0
                    ? commissionData.totalRevenue / staff.filter(s => s.employmentType === 'COMMISSION').length
                    : 0,
            },
            chairRental: {
                staffCount: staff.filter(s => s.employmentType === 'CHAIR_RENTAL').length,
                ...
                averagePerStaff: staff.filter(s => s.employmentType === 'CHAIR_RENTAL').length > 0
                    ? chairRentalData.totalRevenue / staff.filter(s => s.employmentType === 'CHAIR_RENTAL').length
                    : 0,
            },
            hybrid: {
                ...
                averagePerStaff: staff.filter(s => s.employmentType === 'HYBRID').length > 0
                    ? hybridData.totalRevenue / staff.filter(s => s.employmentType === 'HYBRID').length
                    : 0,
            },
        },
    ...
    return {
        ...
        summary: {
            ...
            activeEmploymentTypes: [
                ...(employmentReport.employmentBreakdown.commission.staffCount > 0 ? ['COMMISSION'] : []),
                ...(employmentReport.employmentBreakdown.chairRental.staffCount > 0 ? ['CHAIR_RENTAL'] : []),
                ...(employmentReport.employmentBreakdown.hybrid.staffCount > 0 ? ['HYBRID'] : []),
            ],
            totalBusinessRetention: revenueReport.revenue.net - (
 ...
 */
async function getDailyRevenueBreakdown(businessId: string, startDate: Date, endDate: Date) {
