...
export class DecisionLog {
    ...
    constructor(rootPath: string = process.cwd()) {
        this.logPath = join(rootPath, 'docs', 'onboarding', 'DECISION_LOG.md');
    }

    /**
     * Add a new architectural decision to the log
     */
    async addDecision(decision: Omit<ArchitecturalDecision, 'id'>): Promise<string> {
        ...
        const updatedContent = this.insertDecisionIntoLog(existingContent, decisionMarkdown);
     ...
     */
    async getRecentDecisions(days: number = 30): Promise<ArchitecturalDecision[]> {
        const content = await this.readLogContent();
     ...
     */
    async getAllDecisions(status?: ArchitecturalDecision['status']): Promise<ArchitecturalDecision[]> {
        const content = await this.readLogContent();
        const decisions = this.parseDecisionsFromContent(content);
...
`;
    ...
    private generateNextId(content: string): string {
        ...
        const numbers = existingIds.map(id => parseInt(id.match(/\d+/)![0]));
    ...
    private formatDecisionAsMarkdown(decision: ArchitecturalDecision): string {
        const alternatives = decision.alternatives.map((alt, index) =>
            `${index + 1}. **${alt.name}**: ${alt.description}\n   - ${alt.rejectionReason}`
        ).join('\n');

        const relatedIssues = decision.relatedIssues.length > 0
...
---`;
     ...
     */
    private insertDecisionIntoLog(content: string, decisionMarkdown: string): string {
        ...
        if (templateIndex !== -1) {
            return content.slice(0, templateIndex) +
                decisionMarkdown + '\n\n' +
    ...
    private parseDecisionsFromContent(content: string): ArchitecturalDecision[] {
        ...
        while ((match = decisionRegex.exec(content)) !== null) {
            const [, id, title, dateStr, status, context, decision, rationale, alternativesStr, impact, relatedIssuesStr] = match;

            // Parse alternatives
            const alternatives = this.parseAlternatives(alternativesStr);

            // Parse related issues
            const relatedIssues = relatedIssuesStr.trim() === '[No related issues]'
                ? []
     ...
     */
    private parseAlternatives(alternativesStr: string): ArchitecturalDecision['alternatives'] {
...
`;
    ...
    private extractThemes(decisions: ArchitecturalDecision[]): string[] {
        ...
        const keywords = decisions.flatMap(decision =>
            [...decision.title.toLowerCase().split(' '), ...decision.context.toLowerCase().split(' ')]
        );

        const keywordCounts = keywords.reduce((acc, keyword) => {
            if (keyword.length > 3) { // Filter out short words
