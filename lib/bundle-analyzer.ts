...
export class BundleAnalyzer {
  ...
  estimateComponentSize(componentName: string): number {
    return (
      COMPONENT_SIZE_ESTIMATES[
        componentName as keyof typeof COMPONENT_SIZE_ESTIMATES
      ] || 2000
    );
  }
  ...
  findDuplicates(): DuplicateModule[] {
    const moduleOccurrences = new Map<
      string,
      { count: number; size: number; chunks: Set<string> }
    >();

    this.modules.forEach(module => {
      const baseName = this.getBaseName(module.name);
      const existing = moduleOccurrences.get(baseName) || {
        count: 0,
        size: 0,
        chunks: new Set(),
      };
  ...
  analyze(): BundleAnalysis {
    ...
    const totalGzippedSize = modules.reduce(
      (sum, module) => sum + (module.gzippedSize || module.size * 0.3),
      0
    );
  ...
  private generateRecommendations(
    ...
    const syncChunks = modules.filter(
      m => !m.isAsync && !m.isEntry && m.size > 20000
    );
    ...
    if (modules.reduce((sum, m) => sum + m.size, 0) > 500000) {
      recommendations.push(
        'Total bundle size exceeds 500KB, consider optimization'
      );
    }
...
export class ComponentUsageTracker {
  private usage = new Map<
    string,
    { count: number; lastUsed: number; size: number }
  >();

  trackUsage(componentName: string): void {
    const existing = this.usage.get(componentName) || {
      count: 0,
      lastUsed: 0,
      size: 0,
    };
...
export function checkPerformanceBudget(
  ...
  if (analysis.totalSize > budget.maxBundleSize) {
    violations.push(
      `Bundle size ${Math.round(analysis.totalSize / 1024)}KB exceeds budget ${Math.round(budget.maxBundleSize / 1024)}KB`
    );
  }
  ...
  const largeModules = analysis.modules.filter(
    m => m.size > budget.maxModuleSize
  );
  if (largeModules.length > 0) {
    violations.push(
  ...
  if (analysis.duplicates.length > budget.maxDuplicates) {
    violations.push(
      `Too many duplicate modules: ${analysis.duplicates.length} > ${budget.maxDuplicates}`
    );
  }
