...
export class SteeringImpactAnalyzer {
  ...
  async loadSteeringFiles(): Promise<SteeringFile[]> {
    ...
    try {
      const files = readdirSync(this.steeringDir).filter(f =>
        f.endsWith('.md')
      );
      ...
      for (const file of files) {
        ...
        if (frontmatterMatch) {
          try {
            ...
            const patternMatch = yamlContent.match(
              /fileMatchPattern:\s*\[(.*?)\]/s
            );

            if (inclusionMatch) {
              frontmatter.inclusion = inclusionMatch[1] as
                | 'always'
                | 'fileMatch'
                | 'manual';
            }
        ...
        const affectedFileTypes = this.extractAffectedFileTypes(
          content,
          frontmatter
        );
  ...
  async analyzeSteeringImpact(): Promise<Map<string, SteeringImpact>> {
    ...
    for (const steeringFile of this.steeringFiles) {
      ...
      const estimatedReworkHours = this.estimateReworkEffort(
        affectedTasks,
        steeringFile
      );
   ...
   */
  async createTaskSteeringMappings(): Promise<
    Map<string, TaskSteeringMapping>
  > {
    await this.loadSteeringFiles();
    await this.loadDevelopmentTasks();
    ...
    for (const task of this.developmentTasks) {
      ...
      const implementationGuidance = this.extractImplementationGuidance(
        task,
        applicableSteering
      );
   ...
   */
  async generateDeveloperNotifications(
    changedSteeringFiles: string[]
    ...
    for (const steeringFileName of changedSteeringFiles) {
      ...
      for (const developer of impact.affectedDevelopers) {
        ...
        const message =
          `Steering file ${steeringFileName}.md has been updated. ` +
  ...
  private extractPatternsFromContent(content: string): string[] {
    ...
    const pathMatches =
      content.match(/`[^`]*\.(ts|tsx|js|jsx|md|json)`/g) || [];
    patterns.push(...pathMatches.map(p => p.replace(/`/g, '')));

    return patterns;
  }

  private extractAffectedFileTypes(
    content: string,
    frontmatter: any
  ): string[] {
  ...
  private extractRequirementsFromTask(taskContent: string): string[] {
    ...
    if (
      ...
      taskContent.toLowerCase().includes('interface')
    )
      requirements.push('UI/UX');
    if (taskContent.toLowerCase().includes('security'))
      requirements.push('Security');
    if (
      taskContent.toLowerCase().includes('payment') ||
      taskContent.toLowerCase().includes('stripe')
    )
  ...
  private getSteeringFileFocus(steeringFile: SteeringFile): string[] {
    ...
    if (steeringFile.name.includes('security'))
      focus.push('Security', 'Authentication');
    if (steeringFile.name.includes('ui'))
      focus.push('UI', 'Frontend', 'Components');
    if (steeringFile.name.includes('coding'))
      focus.push('Code Quality', 'Standards');
  ...
  private calculateImpactLevel(
    ...
    if (affectedTaskCount >= 5 || steeringFile.name.includes('security'))
      return 'high';
  ...
  private determineChangeType(
    steeringFile: SteeringFile
  ): 'breaking' | 'enhancement' | 'clarification' {
    if (
      steeringFile.name.includes('security') ||
      steeringFile.name.includes('database')
    ) {
      return 'breaking';
    }
  ...
  private estimateReworkEffort(
    affectedTasks: DevelopmentTask[],
    steeringFile: SteeringFile
  ): number {
    const baseRework = affectedTasks.reduce(
      (sum, task) => sum + task.estimatedEffort * 0.3,
      0
    );

    // Adjust based on steering file type
    if (steeringFile.name.includes('security'))
      return Math.ceil(baseRework * 1.5);
    if (steeringFile.name.includes('database'))
      return Math.ceil(baseRework * 1.3);

    return Math.ceil(baseRework);
  }

  private identifyAffectedDevelopers(
    affectedTasks: DevelopmentTask[]
  ): string[] {
    ...
    for (const task of affectedTasks) {
      if (
        task.requirements.includes('Authentication') ||
        task.requirements.includes('Security')
      ) {
  ...
  private findApplicableSteeringFiles(task: DevelopmentTask): SteeringFile[] {
    return this.steeringFiles.filter(steeringFile => {
      ...
      if (
        steeringFile.inclusion === 'fileMatch' &&
        steeringFile.fileMatchPattern
      ) {
        for (const pattern of steeringFile.fileMatchPattern) {
          for (const affectedFile of task.affectedFiles) {
            if (this.matchesPattern(affectedFile, pattern)) {
  ...
  private generateComplianceChecks(
    ...
    applicableSteering: SteeringFile[]
  ): ComplianceCheck[] {
    ...
    for (const steeringFile of applicableSteering) {
      ...
      for (const line of lines) {
        if (line.startsWith('- **') || line.startsWith('* **')) {
          const guidancePoint = line.replace(
            /^[-*]\s*\*\*([^*]+)\*\*:?\s*/,
            '$1: '
          );
...
export async function analyzeSteeringImpact(
  changedFiles: string[] = []
): Promise<string> {
...
export async function getTaskSteeringMappings(): Promise<
  Map<string, TaskSteeringMapping>
> {
  const analyzer = new SteeringImpactAnalyzer();
...
export async function notifyAffectedDevelopers(
  changedFiles: string[]
): Promise<Map<string, string[]>> {
  const analyzer = new SteeringImpactAnalyzer();
