...
export class ProjectStateTracker {
   ...
   */
  private async analyzeFeatureSpec(
    name: string,
    path: string
    ...
    try {
      ...
      try {
        ...
        lastUpdated = new Date(
          Math.max(lastUpdated.getTime(), requirementsStat.mtime.getTime())
        );

        const requirementsContent = await readFile(requirementsPath, 'utf-8');
        const introMatch = requirementsContent.match(
          /## Introduction\s*\n\n([^#]+)/
        );
        if (introMatch) {
      ...
      try {
        ...
        lastUpdated = new Date(
          Math.max(lastUpdated.getTime(), tasksStat.mtime.getTime())
        );

        const tasksContent = await readFile(tasksPath, 'utf-8');
        const { status: taskStatus, completion } =
          this.analyzeTasksContent(tasksContent);
        status = taskStatus;
   ...
   */
  private analyzeTasksContent(content: string): {
  ...
  async getRecentDecisions(
    days: number = 30
  ): Promise<ProjectState['recentDecisions']> {
    try {
   ...
   */
  private extractIssuesFromDailyStatus(
    content: string,
    filename: string
  ): ProjectIssue[] {
    const issues: ProjectIssue[] = [];
  ...
  async getRecentActivity(
    days: number = 7
  ): Promise<ProjectState['recentActivity']> {
