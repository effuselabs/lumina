...
export class HybridModelPerformanceMonitor {
  ...
  private recordCalculationError(error: any, calculationType: string): void {
    ...
    const existingError = this.accuracyMetrics.commonErrors.find(
    ...
    this.accuracyMetrics.commonErrors = this.accuracyMetrics.commonErrors.slice(
      0,
      10
    );
  ...
  private checkPerformanceThresholds(executionTime: number): void {
    ...
    if (executionTime > this.performanceThresholds.maxCalculationTime) {
      this.createPerformanceAlert({
        ...
        metadata: {
          executionTime,
          threshold: this.performanceThresholds.maxCalculationTime,
        },
      });
    }
    ...
    if (
      this.accuracyMetrics.totalValidations > 0 &&
      this.accuracyMetrics.accuracyPercentage <
        this.performanceThresholds.minAccuracyPercentage
    ) {
      this.createPerformanceAlert({
        type: 'accuracy_issue',
        severity: 'high',
    ...
    if (failureRate > this.performanceThresholds.maxFailureRate) {
      this.createPerformanceAlert({
        ...
        metadata: {
          failureRate,
          threshold: this.performanceThresholds.maxFailureRate,
        },
      });
    }
  }
  ...
  private updateExecutionTimeMetrics(): void {
    ...
    this.performanceMetrics.averageCalculationTime =
      sum / this.calculationTimes.length;
  ...
  getPerformanceSummary(): {
    ...
    if (
      ...
      recommendations.push(
        'Immediate attention required for calculation system'
      );
    }

    if (this.performanceMetrics.averageCalculationTime > 3000) {
      recommendations.push(
        'Calculation execution times are above optimal thresholds'
      );
    }

    if (this.accuracyMetrics.accuracyPercentage < 99) {
      recommendations.push(
        'Review calculation logic for accuracy improvements'
      );
    }

    if (this.accuracyMetrics.commonErrors.length > 0) {
      recommendations.push(
        `Address common calculation errors: ${this.accuracyMetrics.commonErrors[0].errorType}`
      );
    }
...
export const hybridModelPerformanceMonitor =
  new HybridModelPerformanceMonitor();
