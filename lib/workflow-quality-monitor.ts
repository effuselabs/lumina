...
export interface WorkflowAlert {
  id: string;
  type:
    | 'documentation_sync_failure'
    | 'steering_compliance_violation'
    | 'linear_sync_error'
...
export class WorkflowQualityMonitor {
   ...
   */
  recordExecution(
    type: 'documentation_sync' | 'steering_compliance' | 'linear_sync',
    success: boolean,
  ...
  async checkDocumentationCompliance(): Promise<DocumentationComplianceReport> {
    ...
    if (
      mockReport.compliancePercentage <
      this.complianceThresholds.documentationCompliance
    ) {
      this.createAlert({
  ...
  getSuccessRate(): number {
    ...
    return (
      (this.metrics.successfulExecutions / this.metrics.agentHookExecutions) *
      100
    );
  }
  ...
  generateHealthReport(): {
    ...
    if (successRate >= 98 && criticalAlerts === 0 && highAlerts === 0) {
      ...
      recommendations.push(
        'Consider investigating recurring Agent Hook failures'
      );
    } else {
      overallHealth = 'poor';
      recommendations.push(
        'Immediate attention required for workflow automation issues'
      );
    ...
    if (
      ...
      this.complianceThresholds.maxExecutionTime
    ) {
      recommendations.push(
        'Agent Hook execution times are above optimal thresholds'
      );
    }
  ...
  private checkPerformanceThresholds(executionTime: number): void {
    if (executionTime > this.complianceThresholds.maxExecutionTime) {
      this.createAlert({
        ...
        metadata: {
          executionTime,
          threshold: this.complianceThresholds.maxExecutionTime,
        },
      });
    }
  }
  ...
  private createAlert(
    alertData: Omit<WorkflowAlert, 'id' | 'timestamp'>
  ): void {
