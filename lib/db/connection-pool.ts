...
const getConnectionPoolConfig = (): ConnectionPoolConfig => {
  ...
  return {
    ...
    enableQueryLogging:
      !isProduction || process.env.ENABLE_QUERY_LOGGING === 'true',
...
class ConnectionPoolManager {
  ...
  private buildOptimizedConnectionString(): string {
    ...
    const poolParams = new URLSearchParams({
      ...
      pool_timeout: Math.floor(
        this.config.connectionTimeoutMs / 1000
      ).toString(),

      // Query optimization
      statement_timeout:
        Math.floor(this.config.statementTimeoutMs / 1000).toString() + 's',
      idle_in_transaction_session_timeout:
        Math.floor(this.config.idleTimeoutMs / 1000).toString() + 's',
  ...
  getMetrics(): {
    ...
    const lastHealthCheck =
      this.connectionMetrics.lastHealthCheck?.getTime() || now;
    ...
    return {
      ...
      performance: {
        queriesPerSecond:
          timeDiffSeconds > 0
            ? this.connectionMetrics.totalQueries / timeDiffSeconds
            : 0,
        errorRate:
