...
interface QueryRecommendation {
  type:
    'INDEX' | 'QUERY_STRUCTURE' | 'PAGINATION' | 'CACHING' | 'BUSINESS_LOGIC';
...
export class QueryOptimizer {
  ...
  private generateRecommendations(
    ...
    if (duration > this.SLOW_QUERY_THRESHOLD) {
      recommendations.push({
        ...
        suggestedAction:
          'Review and optimize database indexes for this query pattern',
    ...
    if (rowsReturned > 100) {
      recommendations.push({
        ...
        suggestedAction:
          'Implement cursor-based pagination to reduce memory usage',
    ...
    if (queryName.includes('find_appointments_by_business')) {
      if (duration > 300) {
        recommendations.push({
          ...
          priority: 'HIGH',
          description:
            'Business appointment queries should use composite index',
          suggestedAction:
            'Ensure (business_id, start_time) index exists and is being used',
    ...
    if (queryName.includes('conflict_detection')) {
      if (duration > 250) {
        recommendations.push({
          ...
          priority: 'HIGH',
          description:
            'Conflict detection queries need time-range optimization',
          suggestedAction:
            'Use partial index on active appointments with time columns',
  ...
  getPerformanceStats(): {
    ...
    const totalDuration = this.queryHistory.reduce(
    ...
    for (const query of this.queryHistory) {
      ...
      breakdown.averageDuration =
        (breakdown.averageDuration * (breakdown.count - 1) + query.duration) /
        breakdown.count;
      breakdown.slowestDuration = Math.max(
        breakdown.slowestDuration,
        query.duration
      );
    ...
    const allRecommendations = this.queryHistory.flatMap(
      q => q.recommendations
    );
    const recommendationCounts = new Map<
      string,
      { recommendation: QueryRecommendation; count: number }
    >();
    ...
    switch (queryPattern.toLowerCase()) {
        ...
        recommendations.push(
          {
            ...
            priority: 'HIGH',
            description:
              'Optimize appointment creation with proper foreign key indexes',
            suggestedAction:
              'Ensure indexes exist on business_id, staff_id, client_id columns',
          ...
          },
          {
            ...
            priority: 'MEDIUM',
            description:
              'Batch validation operations to reduce database roundtrips',
            suggestedAction:
              'Validate business context, staff, and client in single query',
        ...
        recommendations.push(
          {
            ...
            suggestedAction:
              'Create index on (business_id, start_time, status) for common search patterns',
        ...
        recommendations.push(
          {
            ...
            suggestedAction:
              'Create partial index on active appointments with time columns',
          ...
          },
          {
            ...
            suggestedAction:
              'Structure time range queries to utilize index scans',
        ...
        recommendations.push(
          {
            ...
            suggestedAction:
              'Implement Redis caching for statistics with appropriate TTL',
          ...
          },
          {
            ...
            suggestedAction:
              'Create indexes that include all columns needed for aggregation',
      ...
      default:
        recommendations.push({
          ...
          suggestedAction:
            'Ensure proper indexing, limit result sets, and use efficient joins',
   ...
   */
  async generateIndexUsageReport(
    prisma: PrismaClient
...
export async function withQueryOptimization<T>(
  ...
  const { result, analysis } = await optimizer.analyzeQuery(
    prisma,
    queryName,
    queryFn
  );
  ...
  if (analysis.duration > 500) {
    console.warn(
      `Slow query detected: ${queryName} (${analysis.duration.toFixed(2)}ms)`
    );
