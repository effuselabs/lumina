...
class AppointmentSearchIndex {
   ...
   */
  private toSearchableAppointment(
    appointment: DashboardAppointment
  ): SearchableAppointment {
  ...
  private calculateScore(
    searchable: SearchableAppointment,
    query: string
  ): number {
    ...
    if (searchText.includes(queryLower)) {
      ...
      const wordBoundaryMatch = new RegExp(`\\b${queryLower}\\b`).test(
        searchText
      );
  ...
  search(
    ...
    const businessAppointmentIds =
      this.businessIndexes.get(businessId) || new Set();
    ...
    if (filters.serviceIds && filters.serviceIds.length > 0) {
      ...
      filters.serviceIds.forEach(serviceId => {
        const serviceAppointments =
          this.serviceIndex.get(serviceId) || new Set();
