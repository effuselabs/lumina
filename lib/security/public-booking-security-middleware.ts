...
import {
  ...
  validatePublicBookingCSRF,
} from './public-booking-csrf';
import {
...
interface SecurityMiddlewareConfig {
  requireCSRF: boolean;
  rateLimiter:
    'general' | 'availability' | 'clientLookup' | 'bookingCreation' | 'strict';
...
export class PublicBookingSecurityMiddleware {
  ...
  async validateRequest(
    ...
    try {
      ...
      if (config.validateBusiness && businessId) {
        const businessValidation = await this.validateBusinessAccess(
          businessId,
          req
        );
      ...
      if (businessId) {
        await auditPublicBooking.businessAccess(businessId, req, responseTime, {
      ...
      const requestBody =
        req.method !== 'GET'
          ? await req
              ...
              .catch(() => null)
          : null;
      const isSuspicious = publicBookingAbuseDetector.detectSuspiciousActivity(
        req,
        requestBody
      );
          ...
          response: NextResponse.json(
            {
              error: {
                ...
                userMessage:
                  'Your request appears suspicious. Please contact us directly if you need assistance.',
              },
            },
            {
              status: 403,
  ...
  private async sanitizeInput(
    ...
    SecurityValidationResult & { sanitizedData?: any; violations?: string[] }
  > {
    try {
      ...
      if (sanitizationResult.violations.length > 0 && businessId) {
        await auditPublicBooking.inputSanitization(businessId, req, {
          field: 'request_body',
      ...
      const criticalViolations = sanitizationResult.violations.filter(
        v =>
          v.includes('suspicious') ||
          v.includes('invalid format') ||
          v.includes('required')
      );
...
export async function securePublicBookingGET(
  ...
  return publicBookingSecurityMiddleware.validateRequest(
    req,
    {
      requireCSRF: false,
      rateLimiter: 'general',
...
export function createSecurePublicBookingResponse(
  ...
  return publicBookingSecurityMiddleware.createSecureResponse(
    data,
    req,
    options
  );
