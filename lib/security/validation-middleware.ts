...
interface SecurityOptions {
    ...
    requireCSRF?: boolean
    rateLimiter?: 'general' | 'create' | 'statusUpdate' | 'conflictCheck' | 'validation'
    checkAbuse?: boolean
...
export class ValidationMiddleware {
    ...
    static async validateRequest<T>(
        ...
        try {
            ...
            if (options.requireBusinessContext) {
                const businessResult = await this.validateBusinessContext(req, body, context)
    ...
    private static sanitizeRequestBody(body: any): any {
        ...
        if (sanitized.clientName || sanitized.clientEmail || sanitized.clientPhone) {
...
export async function validateAppointmentRequest<T>(
    ...
    const result = await ValidationMiddleware.validateRequest(req, schema, defaultOptions)
