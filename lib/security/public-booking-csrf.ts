...
const defaultConfig: PublicCSRFConfig = {
    secret: process.env.PUBLIC_BOOKING_CSRF_SECRET || 'default-public-csrf-secret',
...
export class PublicBookingCSRFProtection {
    private config: PublicCSRFConfig;
     ...
     */
    addTokenToResponse(response: NextResponse, sessionIdentifier?: string): NextResponse {
        const token = this.generateToken(sessionIdentifier);

        // Add token to cookie
        response.cookies.set(this.config.cookieName, token, this.config.cookieOptions);
 ...
 */
export async function validatePublicBookingCSRF(req: NextRequest): Promise<NextResponse | null> {
 ...
 */
export function addCSRFTokenToResponse(response: NextResponse, req: NextRequest): NextResponse {
 ...
 */
export function generatePublicBookingCSRFToken(sessionIdentifier?: string): string {
