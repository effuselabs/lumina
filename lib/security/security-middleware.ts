 ...
 */
export function extractRequestSecurityContext(request: NextRequest): RequestSecurityContext {
 ...
 */
export function extractServerClientIP(headersList: Headers): string | undefined {
...
export function sanitizeUrl(url: string): string {
    try {
        ...
        const sensitiveParams = [
            ...
            'session', 'jwt', 'api_key', 'access_token'
        ]
...
export function detectSuspiciousActivity(
    ...
    if (securityContext.userAgent) {
        ...
        if (suspiciousUserAgents.some(pattern => userAgentLower.includes(pattern))) {
