...
export class DataProtectionService {
  ...
  private get encryptionKey(): Buffer {
    ...
    if (key.length !== this.encryptionConfig.keyLength) {
      throw new Error(
        `Encryption key must be ${this.encryptionConfig.keyLength} bytes`
      );
    }
  ...
  maskPhone(phone: string): string {
    ...
    const masked =
      digits.charAt(0) + '*'.repeat(digits.length - 3) + digits.slice(-2);
    return masked;
  }
  ...
  async exportClientData(
    ...
    try {
      ...
      throw new Error(
        `Failed to export client data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
    ...
    try {
      ...
      await prisma.$transaction(async tx => {
        ...
        for (const membership of loyaltyMemberships) {
          ...
          deletedRecords.loyaltyTransactions =
            (deletedRecords.loyaltyTransactions || 0) +
            deletedLoyaltyTxns.count;
        }
        ...
        for (const appointment of appointments) {
          ...
          retainedRecords.transactions =
            (retainedRecords.transactions || 0) +
      ...
      throw new Error(
        `Failed to delete client data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
    ...
    try {
      await prisma.$transaction(async tx => {
        ...
        const commCutoff = new Date(
          now.getTime() -
            retentionPolicy.communicationData * 24 * 60 * 60 * 1000
        );
        const deletedComms = await tx.communicationHistory.deleteMany({
          where: {
            businessId,
        ...
        const auditCutoff = new Date(
          now.getTime() - retentionPolicy.auditLogs * 24 * 60 * 60 * 1000
        );
        ...
        const securityCutoff = new Date(
          now.getTime() - retentionPolicy.securityLogs * 24 * 60 * 60 * 1000
        );
      ...
      throw new Error(
        `Failed to anonymize expired data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
