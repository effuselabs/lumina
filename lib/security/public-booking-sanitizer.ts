...
export class PublicBookingSanitizer {
     ...
     */
    private sanitizeName(name: string, fieldName: string): {
        value: string;
        violations: string[];
    } {
        const violations: string[] = [];
        ...
        if (sanitized.length > this.config.maxLength.name) {
            violations.push(`${fieldName} exceeds maximum length of ${this.config.maxLength.name} characters`);
    ...
    private sanitizeEmail(email: string): {
        ...
        if (sanitized.length > this.config.maxLength.email) {
            violations.push(`Email exceeds maximum length of ${this.config.maxLength.email} characters`);
    ...
    private sanitizePhone(phone: string): {
        ...
        if (sanitized.length > this.config.maxLength.phone) {
            violations.push(`Phone number exceeds maximum length of ${this.config.maxLength.phone} characters`);
    ...
    private sanitizeNotes(notes: string): {
        ...
        if (sanitized.length > this.config.maxLength.notes) {
            violations.push(`Notes exceed maximum length of ${this.config.maxLength.notes} characters`);
     ...
     */
    private sanitizeId(id: any, fieldName: string): {
        value: string;
        violations: string[];
    } {
        const violations: string[] = [];
    ...
    private isSuspiciousEmail(email: string): boolean {
        const suspiciousPatterns = [
            ...
            /test|spam|fake|temp|throwaway/i,
...
export const publicBookingSchemas = {
    ...
    bookingRequest: z.object({
        services: z.array(z.string().regex(/^c[a-z0-9]{24}$/)).min(1).max(5),
