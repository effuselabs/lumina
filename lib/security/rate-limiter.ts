...
export const appointmentRateLimiters = {
  ...
  create: new RateLimiter({
    ...
    message:
      'Too many appointment creation attempts. Please try again in 5 minutes.',
  }),
...
export class AbuseDetector {
  ...
  detectSuspiciousBooking(req: NextRequest, appointmentData: any): boolean {
    ...
    if (
      ...
      this.isSuspiciousEmail(appointmentData.clientEmail)
    ) {
      return true;
    }
...
export class InputSanitizer {
  ...
  static sanitizeClientInfo(info: {
    name?: string;
    email?: string;
    phone?: string;
  }) {
    return {
      name: info.name
        ? this.sanitizeString(info.name).substring(0, 100)
        : undefined,
      email: info.email ? info.email.toLowerCase().trim() : undefined,
      phone: info.phone
        ? info.phone.replace(/[^\d\s\-\(\)\+]/g, '')
        : undefined,
    };
  }

  // Validate and sanitize business context
  static validateBusinessContext(
    businessId: string,
