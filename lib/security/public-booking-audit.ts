...
export class PublicBookingAuditLogger {
    ...
    private calculateRiskScore(entry: PublicBookingAuditEntry): number {
        ...
        if (entry.responseTime && entry.responseTime > 5000) { // > 5 seconds
            score += 15;
        }

        // Suspicious event data patterns
        if (entry.eventData) {
            if (entry.eventData.violations && Array.isArray(entry.eventData.violations)) {
     ...
     */
    private async triggerCriticalAlert(entry: PublicBookingAuditEntry, riskScore: number): Promise<void> {
...
export const auditPublicBooking = {
    businessAccess: (businessId: string, req: NextRequest, responseTime?: number, eventData?: Record<string, any>) =>
        publicBookingAuditLogger.logBusinessAccess(businessId, req, responseTime, eventData),

    availabilityCheck: (businessId: string, req: NextRequest, eventData: any, responseTime?: number) =>
        publicBookingAuditLogger.logAvailabilityCheck(businessId, req, eventData, responseTime),

    clientLookup: (businessId: string, req: NextRequest, eventData: any, responseTime?: number) =>
        publicBookingAuditLogger.logClientLookup(businessId, req, eventData, responseTime),

    bookingCompleted: (businessId: string, req: NextRequest, eventData: any, responseTime?: number) =>
        publicBookingAuditLogger.logBookingCompleted(businessId, req, eventData, responseTime),

    bookingFailed: (businessId: string, req: NextRequest, eventData: any, error?: Error, responseTime?: number) =>
        publicBookingAuditLogger.logBookingFailed(businessId, req, eventData, error, responseTime),

    securityViolation: (businessId: string, req: NextRequest, violationType: PublicBookingAuditEvent, eventData: any, securityFlags?: string[]) =>
        publicBookingAuditLogger.logSecurityViolation(businessId, req, violationType, eventData, securityFlags),
