...
export function calculateHybridEarnings(
    ...
    const commission = calculateCommissionEarnings(grossRevenue, commissionRate, baseSalary);
    const chairRental = calculateChairRental(chairRentalAmount, chairRentalPeriod, periodsWorked, grossRevenue);

    // For hybrid model, total earnings is commission minus rental due
    const totalEarnings = Math.max(0, commission.totalEarnings - chairRental.totalRental);
...
export function calculateBusinessRetention(
    ...
    const retentionPercentage = totalRevenue > 0 ? (netRetention / totalRevenue) * 100 : 0;
...
export function calculateStaffPayment(
    ...
    switch (employmentType) {
        case 'commission': {
            ...
            baseSalaryAmount = calculation.baseSalary > 0 ? calculation.baseSalary : undefined;
            netEarnings = calculation.totalEarnings;
            break;
        }

        case 'chair_rental': {
            if (!employmentConfig.chairRentalAmount || !employmentConfig.chairRentalPeriod) {
                throw new Error('Chair rental amount and period are required for chair rental contractors');
            }

            const calculatedPeriods = periodsWorked ?? calculateRentalPeriods(
                period.start,
                period.end,
        ...
        case 'hybrid': {
            if (!employmentConfig.commissionRate || !employmentConfig.chairRentalAmount || !employmentConfig.chairRentalPeriod) {
                throw new Error('Commission rate, chair rental amount, and period are required for hybrid employees');
            }
            ...
            baseSalaryAmount = calculation.commission.baseSalary > 0 ? calculation.commission.baseSalary : undefined;
            netEarnings = calculation.totalEarnings;
            break;
        }
...
export function calculateMixedEmploymentSummary(
    ...
    const commissionEmployees = paymentCalculations.filter(calc => calc.employmentType === 'commission');
    const chairRentalContractors = paymentCalculations.filter(calc => calc.employmentType === 'chair_rental');
    const hybridStaff = paymentCalculations.filter(calc => calc.employmentType === 'hybrid');

    const totalBusinessRetention = paymentCalculations.reduce((sum, calc) => sum + calc.businessRetention, 0);
    ...
    return {
        ...
        commissionEmployees: {
            ...
            totalEarnings: commissionEmployees.reduce((sum, calc) => sum + calc.netEarnings, 0),
            averageEarnings: commissionEmployees.length > 0
                ? commissionEmployees.reduce((sum, calc) => sum + calc.netEarnings, 0) / commissionEmployees.length
                : 0,
        },
        chairRentalContractors: {
            count: chairRentalContractors.length,
            totalRental: chairRentalContractors.reduce((sum, calc) => sum + (calc.chairRentalDue || 0), 0),
            averageRental: chairRentalContractors.length > 0
                ? chairRentalContractors.reduce((sum, calc) => sum + (calc.chairRentalDue || 0), 0) / chairRentalContractors.length
                : 0,
        },
        hybridStaff: {
            count: hybridStaff.length,
            totalEarnings: hybridStaff.reduce((sum, calc) => sum + calc.netEarnings, 0),
            averageEarnings: hybridStaff.length > 0
                ? hybridStaff.reduce((sum, calc) => sum + calc.netEarnings, 0) / hybridStaff.length
                : 0,
        },
...
export function validateCalculationInputs(
    ...
    switch (employmentType) {
        case 'commission':
            ...
            if (employmentConfig.commissionRate < 0 || employmentConfig.commissionRate > 100) {
                throw new Error('Commission rate must be between 0 and 100');
            }
            break;
...
export function compareEmploymentProfitability(
    ...
    const chairRental = calculateChairRental(chairRentalAmount, chairRentalPeriod, periodsWorked, grossRevenue);
    const hybrid = calculateHybridEarnings(grossRevenue, commissionRate, chairRentalAmount, chairRentalPeriod, periodsWorked);
