 ...
 */
export async function createTransaction(params: CreateTransactionParams): Promise<TransactionWithDetails> {
    const {
        businessId,
        appointmentId,
        staffId,
    ...
    if (staffId && staffEmploymentType && type === 'PAYMENT') {
        try {
            ...
            if (staff) {
                const effectiveCommissionRate = commissionRate || staff.commissionRate?.toNumber();

                if (effectiveCommissionRate && (staff.employmentType === 'COMMISSION' || staff.employmentType === 'HYBRID')) {
                    calculatedCommissionAmount = (amount * effectiveCommissionRate) / 100;
                }

                chairRentalApplicable = staff.employmentType === 'CHAIR_RENTAL' || staff.employmentType === 'HYBRID';
            }
        } catch (error) {
 ...
 */
export async function getTransactionByPaymentId(paymentId: string): Promise<TransactionWithDetails | null> {
...
export async function getBusinessTransactions(
    ...
    if (filters?.employmentType) where.staffEmploymentType = filters.employmentType;
...
export async function calculateBusinessRevenue(
    ...
    const totalRevenue = payments.reduce((sum, t) => sum + t.amount.toNumber(), 0);
...
export async function calculateStaffEarnings(
    ...
    const totalCommissions = transactions.reduce((sum, t) => sum + (t.commissionAmount?.toNumber() || 0), 0);
    const transactionCount = transactions.length;

    const totalCommissionRate = transactions.reduce((sum, t) => sum + (t.commissionRate?.toNumber() || 0), 0);
    const averageCommissionRate = transactionCount > 0 ? totalCommissionRate / transactionCount : 0;
