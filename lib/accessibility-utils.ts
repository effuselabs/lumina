...
export class DocumentStructureManager {
  ...
  static validateHeadingHierarchy(container: HTMLElement = document.body): {
    ...
    headings.forEach((heading, index) => {
      ...
      if (level > previousLevel + 1) {
        issues.push(
          `Heading level jumps from H${previousLevel} to H${level}. Levels should increment by 1.`
        );
      }
   ...
   */
  static createBreadcrumbNavigation(
...
export const AccessibilityUtils = {
  ...
  createTableHeaders: (headers: string[]): string => {
    return headers
      .map(
        (header, index) => `<th scope="col" id="header-${index}">${header}</th>`
      )
      .join('');
  },
