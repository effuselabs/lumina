...
import { AppointmentFilters, SearchHighlight, SearchResult } from '@/types/appointment-filters';
...
export function filterAppointments(
    ...
    if (filters.clientName) {
        ...
        filtered = filtered.filter(appointment => {
            const fullName = `${appointment.client.firstName} ${appointment.client.lastName}`.toLowerCase();
...
export function extractHighlightTerms(filters: AppointmentFilters): string[] {
    ...
    if (filters.searchTerm) {
        ...
        terms.push(...filters.searchTerm.split(/\s+/).filter(term => term.length > 0));
    }

    if (filters.clientName) {
        terms.push(...filters.clientName.split(/\s+/).filter(term => term.length > 0));
    }
 ...
 */
export function highlightText(text: string, searchTerms: string[]): SearchHighlight[] {
...
function calculateRelevanceScore(
    ...
    searchTerms.forEach(term => {
        ...
        if (appointment.client.firstName.toLowerCase().includes(termLower) ||
        ...
        if (appointment.services.some(s => s.name.toLowerCase().includes(termLower))) {
            score += 6;
        }
...
export function validateFilters(filters: AppointmentFilters): {
    ...
    if (filters.searchTerm) {
        if (filters.searchTerm.length < 2) {
            warnings.push('Search term is very short, consider using more specific terms');
        }
    ...
    if (filters.staffIds && filters.staffIds.length > 20) {
        warnings.push('Many staff members selected, consider narrowing the selection');
    }
