...
import { AppointmentCacheManager, CacheKeyGenerator } from './appointment-cache'
...
export class CacheWarmingManager {
    ...
    async warmBusinessCache(businessId: string): Promise<{
        ...
        try {
            ...
            if (this.config.warmingStrategies.upcomingAppointments) {
                warmingPromises.push(this.warmUpcomingAppointments(businessId, staff.map(s => s.id)))
            }

            if (this.config.warmingStrategies.staffSchedules) {
                warmingPromises.push(this.warmStaffSchedules(businessId, staff.map(s => s.id)))
            }
            ...
            if (this.config.warmingStrategies.popularTimeSlots) {
                warmingPromises.push(this.warmPopularTimeSlots(businessId, staff.map(s => s.id)))
            }

            // Execute with concurrency control
            const results = await this.executeConcurrently(warmingPromises, this.config.concurrency)
            itemsWarmed = results.reduce((sum, count) => sum + count, 0)

            const duration = Date.now() - startTime
            this.updateWarmingStats(itemsWarmed, duration, errors.length)

            console.log(`Cache warming completed for business ${businessId}: ${itemsWarmed} items in ${duration}ms`)
     ...
     */
    private async warmUpcomingAppointments(businessId: string, staffIds: string[]): Promise<number> {
        let itemsWarmed = 0

        try {
            const dateRange = {
                startDate: new Date(),
                endDate: new Date(Date.now() + this.config.warmingPeriods.upcomingDays * 24 * 60 * 60 * 1000)
            }
            ...
            const businessAppointments = await this.repository.findByBusinessOptimized(businessId, {
                startDate: dateRange.startDate,
                endDate: dateRange.endDate,
                status: [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED],
                limit: 100
            ...
            await this.cacheManager.setAppointmentList(businessKey, businessAppointments)
            itemsWarmed++

            // Warm individual staff schedules
            for (const staffId of staffIds) {
                const staffKey = CacheKeyGenerator.appointmentsByStaff(staffId, businessId, dateRange)
     ...
     */
    private async warmStaffSchedules(businessId: string, staffIds: string[]): Promise<number> {
        let itemsWarmed = 0
        ...
        try {
            const dateRanges = [
                ...
                // Next week
                {
                    startDate: this.getStartOfWeek(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)),
                    endDate: this.getEndOfWeek(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000))
                }
            ]
    ...
    private async warmBusinessStatistics(businessId: string): Promise<number> {
        ...
        try {
            const statisticsRanges = [
                ...
                // Last month
                {
                    startDate: this.getStartOfMonth(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
                    endDate: this.getEndOfMonth(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))
                },
                // Last 30 days
                {
            ...
            for (const dateRange of statisticsRanges) {
                const statistics = await this.repository.getAppointmentStatsOptimized(businessId, dateRange)
                await this.cacheManager.setStatistics(businessId, dateRange, statistics)
                itemsWarmed++
            }
    ...
    private async warmFrequentClientData(businessId: string): Promise<number> {
        ...
        try {
            ...
            const frequentClients = await prisma.client.findMany({
                where: {
                    ...
                    appointments: {
                        some: {
                            createdAt: {
                                gte: new Date(Date.now() - this.config.warmingPeriods.historicalDays * 24 * 60 * 60 * 1000)
                            }
                        }
                    }
                },
            ...
            for (const client of frequentClients) {
                const clientAppointments = await this.repository.findByBusinessOptimized(businessId, {
                    clientId: client.id,
                    limit: 50
                })
                ...
                await this.cacheManager.setAppointmentList(cacheKey, clientAppointments)
                itemsWarmed++
            }
     ...
     */
    private async warmPopularTimeSlots(businessId: string, staffIds: string[]): Promise<number> {
        let itemsWarmed = 0
     ...
     */
    private updateWarmingStats(itemsWarmed: number, duration: number, errors: number): void {
        this.warmingStats = {
    ...
    async warmAllBusinessCaches(): Promise<{
        ...
        try {
            ...
            console.log(`Cache warming completed: ${businessesProcessed} businesses, ${totalItemsWarmed} items, ${totalDuration}ms`)
     ...
     */
    getWarmingStats(): typeof this.warmingStats & { isWarmingInProgress: boolean } {
        return {
            ...this.warmingStats,
    ...
    isWarmingNeeded(): boolean {
        ...
        const hoursSinceLastWarming = (now.getTime() - lastWarming.getTime()) / (1000 * 60 * 60)
