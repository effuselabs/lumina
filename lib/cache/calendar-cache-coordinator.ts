...
export class CalendarCacheCoordinator extends EventEmitter {
   ...
   */
  private async handleAppointmentDeleted(
    event: CacheInvalidationEvent
  ): Promise<void> {
   ...
   */
  private async handleAvailabilityChanged(
    event: CacheInvalidationEvent
  ): Promise<void> {
  ...
  private async invalidateAppointmentCaches(
    ...
    try {
      ...
      await this.appointmentCache.invalidateBusinessAppointments(
        event.businessId
      );

      // Invalidate staff appointment lists
      await this.appointmentCache.invalidateStaffAppointments(
        event.staffId,
        event.businessId
      );
    } catch (error) {
   ...
   * Invalidate availability-related caches
   */
  private async invalidateAvailabilityCaches(
    event: CacheInvalidationEvent
  ): Promise<void> {
    try {
      ...
      console.log(
        `Requested availability cache invalidation for staff ${event.staffId}`
      );
    } catch (error) {
   ...
   */
  private async invalidateTimeSlotCaches(
    event: CacheInvalidationEvent
  ): Promise<void> {
   ...
   * Invalidate conflict detection caches
   */
  private async invalidateConflictCaches(
    event: CacheInvalidationEvent
  ): Promise<void> {
    try {
      ...
      console.log(
        `Would invalidate conflict caches with pattern: ${conflictPattern}`
      );
    } catch (error) {
  ...
  private async coordinateWithCalendarInfrastructure(
    ...
    try {
      ...
      console.log(
        `Coordinated with calendar infrastructure for event: ${event.type}`
      );
    } catch (error) {
   ...
   */
  private async retryCoordination(
    event: CacheInvalidationEvent,
    attempt: number = 1
  ): Promise<void> {
    if (attempt > this.config.retryAttempts) {
      console.error(
        `Failed to coordinate with calendar infrastructure after ${this.config.retryAttempts} attempts`
      );
      return;
    }
    ...
    for (const [key, events] of batches) {
      try {
        ...
        for (const chunk of chunks) {
          await Promise.all(
            chunk.map(event => this.processInvalidationEvent(event))
          );
        }

        console.log(
          `Processed ${events.length} invalidation events for ${key}`
        );
      } catch (error) {
  ...
  getCoordinationStats(): {
    ...
    const queuedEvents = Array.from(this.invalidationQueue.values()).reduce(
...
export async function notifyAppointmentCreated(
  ...
  await calendarCacheCoordinator.onAppointmentCreated(
    appointmentId,
    businessId,
    staffId,
    timeSlot,
...
export async function notifyAppointmentUpdated(
  ...
  await calendarCacheCoordinator.onAppointmentUpdated(
    appointmentId,
    businessId,
    staffId,
    timeSlot,
...
export async function notifyAppointmentDeleted(
  ...
  await calendarCacheCoordinator.onAppointmentDeleted(
    appointmentId,
    businessId,
    staffId,
    timeSlot
...
export async function notifyAvailabilityChanged(
  ...
  await calendarCacheCoordinator.onAvailabilityChanged(
    businessId,
    staffId,
    timeSlot
  );
