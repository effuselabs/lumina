...
class BookingPerformanceMonitor {
  ...
  private observeWebVitals() {
    ...
    window.addEventListener('load', () => {
      ...
      setTimeout(() => {
        ...
        this.recordMetric(
          'time-to-interactive',
          this.performanceData.timeToInteractive
        );
      }, 100);
    });
  }
  ...
  getRecommendations(): string[] {
    ...
    if (data.cumulativeLayoutShift > 0.1) {
      recommendations.push(
        'Layout shifts detected. Ensure proper sizing for dynamic content.'
      );
    }

    const slowApis = Object.entries(data.apiResponseTimes).filter(
      ([_, time]) => time > 1000
    );
    if (slowApis.length > 0) {
      recommendations.push(
        `Slow API responses detected: ${slowApis.map(([name]) => name).join(', ')}`
      );
    }

    if (data.errors.length > 0) {
      recommendations.push(
        `${data.errors.length} errors detected. Check console for details.`
      );
    }
...
export function useBookingPerformance(businessId?: string) {
  ...
  return {
    recordMetric: bookingPerformanceMonitor.recordMetric.bind(
      bookingPerformanceMonitor
    ),
    startMeasurement: bookingPerformanceMonitor.startMeasurement.bind(
      bookingPerformanceMonitor
    ),
    measureAsync: bookingPerformanceMonitor.measureAsync.bind(
      bookingPerformanceMonitor
    ),
    getPerformanceData: bookingPerformanceMonitor.getPerformanceData.bind(
      bookingPerformanceMonitor
    ),
    getRecommendations: bookingPerformanceMonitor.getRecommendations.bind(
      bookingPerformanceMonitor
    ),
  };
}
