...
export interface SuggestedAlternative {
  type:
    'time_slot' | 'staff_member' | 'service_option' | 'date_range' | 'action';
  title: string;
  description: string;
  data?: {
    timeSlot?: TimeSlot;
...
export class BusinessClosedError extends AvailabilityError {
  constructor(
    ...
    const dayName = requestedTime.toLocaleDateString('en-US', {
      weekday: 'long',
    });
...
export class SchedulingConflictError extends AvailabilityError {
  constructor(
    conflictType:
      'overlapping_appointment' | 'double_booking' | 'resource_conflict',
    conflictDetails: {
      existingAppointment?: {
        id: string;
        startTime: Date;
    ...
    if (conflictDetails.existingAppointment) {
      const timeString =
        conflictDetails.existingAppointment.startTime.toLocaleTimeString(
          'en-US',
          {
            hour: '2-digit',
            minute: '2-digit',
      ...
      userMessage =
        'This time slot is no longer available due to a scheduling conflict.';
    }
...
export class InvalidTimeSlotError extends AvailabilityError {
  constructor(
    reason:
      'past_time' | 'invalid_format' | 'end_before_start' | 'too_far_future',
    requestedTime?: Date,
    suggestedAlternatives: SuggestedAlternative[] = []
  ) {
    let userMessage: string;
    switch (reason) {
      case 'past_time':
        userMessage =
          'You cannot book appointments in the past. Please select a future time.';
        break;
      case 'invalid_format':
      ...
      case 'end_before_start':
        userMessage =
          'The appointment end time cannot be before the start time.';
        break;
      case 'too_far_future':
        userMessage =
...
export class AvailabilityErrorFactory {
  static createBusinessClosedError(
    ...
    const suggestedAlternatives: SuggestedAlternative[] = alternativeSlots.map(
      slot => ({
        type: 'time_slot',
    ...
    return new BusinessClosedError(
      requestedTime,
      businessHours,
      suggestedAlternatives
    );
  ...
  static createStaffUnavailableError(
    ...
    const suggestedAlternatives: SuggestedAlternative[] =
      alternativeStaff.flatMap(staff =>
    ...
    return new StaffUnavailableError(
      staffName,
      requestedTime,
      reason,
      suggestedAlternatives
  ...
  static createInsufficientDurationError(
    ...
    const suggestedAlternatives: SuggestedAlternative[] = longerSlots.map(
      slot => ({
        type: 'time_slot',
    ...
    return new InsufficientDurationError(
      requiredDuration,
      availableDuration,
      serviceNames,
      suggestedAlternatives
...
export function isBusinessClosedError(
  error: any
): error is BusinessClosedError {
  return error instanceof BusinessClosedError;
}

export function isStaffUnavailableError(
  error: any
