...
export class AppointmentCancellationError extends AppointmentError {
  constructor(
    appointmentId: string,
    reason:
      'too_late' | 'already_started' | 'payment_required' | 'policy_violation',
    policyDetails?: {
    ...
    switch (reason) {
      ...
      case 'payment_required':
        userMessage =
          'A cancellation fee may apply. Please contact us to cancel this appointment.';
        break;
      case 'policy_violation':
        userMessage =
          'This appointment cannot be cancelled due to business policy restrictions.';
        break;
      default:
  ...
  constructor(
    ...
    switch (errorType) {
      ...
      case 'duration_exceeded':
        const maxHours = details.maxDuration
          ? Math.floor(details.maxDuration / 60)
          : 4;
...
export class AppointmentErrorFactory {
  static createAppointmentNotFoundError(
    ...
    const suggestedAlternatives: SuggestedAlternative[] =
      alternativeAppointments.map(apt => ({
        type: 'action',
  ...
  static createInvalidStatusTransitionError(
    ...
    const suggestedAlternatives: SuggestedAlternative[] = validTransitions.map(
      status => ({
        type: 'action',
  ...
  static createMultiServiceBookingError(
    ...
    return new MultiServiceBookingError(
      errorType,
      serviceIds,
      serviceNames,
      details,
...
export function isAppointmentNotFoundError(
  error: any
): error is AppointmentNotFoundError {
...
export function isMultiServiceBookingError(
  error: any
): error is MultiServiceBookingError {
...
export function isCalendarIntegrationFailureError(
  error: any
): error is CalendarIntegrationFailureError {
  return error instanceof CalendarIntegrationFailureError;
}

// Get error severity for appointment errors
export function getAppointmentErrorSeverity(
