...
export interface BookingAnalyticsEvent {
    businessId: string;
    eventType: 'booking_started' | 'service_selected' | 'time_selected' | 'form_submitted' | 'booking_completed' | 'booking_abandoned';
    sessionId: string;
    timestamp: Date;
...
export class BookingAnalyticsService {
    ...
    async getConversionMetrics(
        ...
        sessionMap.forEach(sessionEvents => {
            const hasCompleted = sessionEvents.some(e => e.eventType === 'booking_completed');

            if (hasCompleted) {
                completedBookings++;

                // Calculate completion time
                const startEvent = sessionEvents.find(e => e.eventType === 'booking_started');
                const endEvent = sessionEvents.find(e => e.eventType === 'booking_completed');

                if (startEvent && endEvent) {
                    const completionTime = endEvent.timestamp.getTime() - startEvent.timestamp.getTime();
        ...
        return {
            totalSessions,
            completedBookings,
            conversionRate: totalSessions > 0 ? (completedBookings / totalSessions) * 100 : 0,
            averageTimeToComplete: completedBookings > 0 ? totalCompletionTime / completedBookings : 0,
            abandonmentPoints,
        };
    }
    ...
    async getPerformanceMetrics(
        ...
        return {
            ...
            averageApiResponseTime: apiResponseTimes.length > 0
                ...
                : 0,
            errorRate: performanceEvents.length > 0
                ? (errorEvents.length / performanceEvents.length) * 100
                : 0,
            mobileUsagePercentage: performanceEvents.length > 0
