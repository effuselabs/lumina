...
export class AppointmentAnalyticsService {
    ...
    async getAppointmentAnalytics(
        ...
        const [
            ...
            this.getVolumeMetrics(businessId, startDate, endDate, compareWithPrevious),
            this.getRevenueMetrics(businessId, startDate, endDate, compareWithPrevious),
    ...
    private async getVolumeMetrics(
        ...
        if (compareWithPrevious) {
            ...
            growthRate = previousAppointments > 0
    ...
    private async getRevenueMetrics(
        ...
        const total = appointments.reduce((sum, appointment) =>
            sum + Number(appointment.totalPrice), 0
        );

        const averageBookingValue = appointments.length > 0
    ...
    private async getStaffUtilizationMetrics(
        ...
        const staffMetrics = staffAppointments.reduce((acc, appointment) => {
        ...
        return Object.values(staffMetrics).map((staff: any) => {
            ...
            const averageServiceTime = staff.completedAppointments > 0
    ...
    private async getServicePopularityMetrics(
        ...
        const serviceMetrics = serviceAppointments.reduce((acc, appointmentService) => {
    ...
    private async getPeakHourMetrics(
        ...
        const hourMetrics = appointments.reduce((acc, appointment) => {
        ...
        return Object.values(hourMetrics).map((hourData: any) => ({
            ...
            utilizationRate: hourData.appointmentCount > 0
    ...
    private async getClientRetentionMetrics(
        ...
        const newClients = appointments.filter(appointment =>
    ...
    private async getTrendMetrics(
        ...
        const dailyMetrics = appointments.reduce((acc, appointment) => {
        ...
        const dailyVolume: DailyVolumeMetric[] = Object.values(dailyMetrics).map((day: any) => ({
            date: day.date,
            appointments: day.appointments,
            revenue: day.revenue,
            successRate: day.appointments > 0 ? (day.completed / day.appointments) * 100 : 0,
        }));
    ...
    async getStaffEfficiencyReport(
        ...
        return Object.values(staffMetrics).map((staff: any) => {
            ...
            const appointmentsPerDay = periodDays > 0 ? staff.appointments.length / periodDays : 0;
            const totalHours = staff.totalDuration / 60;
            const revenuePerHour = totalHours > 0 ? staff.totalRevenue / totalHours : 0;

            // Calculate punctuality rate (simplified - assumes on-time if completed)
            const punctualityRate = staff.appointments.length > 0
