...
export class StaffAvailabilityRepository {
        ...
        for (const availability of dayAvailability) {
          ...
          const isEffective =
            (!availability.effectiveDate ||
              availability.effectiveDate <= currentDate) &&
            (!availability.expiryDate ||
              availability.expiryDate >= currentDate);
  ...
  async getAvailabilityWithFallback(
    ...
    const structuredAvailability = await this.getAvailability(
      staffId,
      dateRange
    );
