...
export class AppointmentRepository {
   ...
   */
  async create(request: CreateAppointmentRequest): Promise<AppointmentWithRelations> {
    ...
    if (request.clientId) {
      await this.validateClientBusinessContext(request.clientId, request.businessId)
    }
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw new Error('Appointment creation failed due to constraint violation')
        }
        if (error.code === 'P2003') {
          throw new Error('Invalid reference: staff, client, or service not found')
        }
      }
   ...
   */
  async findById(id: string, businessId: string): Promise<AppointmentWithRelations | null> {
    try {
      ...
      throw new Error(`Failed to find appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async findByBusiness(
    ...
    try {
      ...
      return {
        ...
        nextOffset: offset + appointments.length < total ? offset + limit : undefined
      }
    } catch (error) {
      throw new Error(`Failed to find appointments: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  private async validateBusinessContext(staffId: string, businessId: string): Promise<void> {
   ...
   */
  private async validateClientBusinessContext(clientId: string, businessId: string): Promise<void> {
 ...
 */
  async findByStaff(
    ...
    try {
      ...
      throw new Error(`Failed to find appointments by staff: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async findByClient(
    ...
    try {
      ...
      throw new Error(`Failed to find appointments by client: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async addServices(
    ...
    const validation = await multiServiceAppointmentService.validateMultiServiceAppointment(
      businessId,
    ...
    try {
      ...
      const maxOrder = existingServices.length > 0 ? existingServices[0].serviceOrder : 0
      ...
      throw new Error(`Failed to add services to appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async removeServices(
    ...
    try {
      ...
      throw new Error(`Failed to remove services from appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async reorderServices(
    ...
    try {
      ...
      const reorderedServices = existingAppointment.services
        .map(s => {
          const newOrderInfo = serviceOrders.find(so => so.serviceId === s.serviceId)
          return {
            ...s,
      ...
      throw new Error(`Failed to reorder services: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  async getQualifiedStaffForServices(businessId: string, serviceIds: string[]) {
    return multiServiceAppointmentService.getQualifiedStaff(businessId, serviceIds)
  }
