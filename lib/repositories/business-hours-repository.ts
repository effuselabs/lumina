...
export class BusinessHoursRepository {
  ...
  async isBusinessOpen(businessId: string, dateTime: Date): Promise<boolean> {
    ...
    const { businessHours, holiday } = await this.getBusinessHoursForDate(
      businessId,
      date
    );
    ...
    if (holiday) {
      ...
      if (holiday.openTime && holiday.closeTime) {
        return (
          timeString >= holiday.openTime && timeString <= holiday.closeTime
        );
      }
    }
    ...
    return (
      timeString >= businessHours.openTime &&
      timeString <= businessHours.closeTime
    );
  }
   ...
   */
  async getBusinessHoursWithFallback(
    businessId: string
  ): Promise<BusinessHours[]> {
    ...
    if (
      ...
      typeof business.operatingHours === 'object'
    ) {
