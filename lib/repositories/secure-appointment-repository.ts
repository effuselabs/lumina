...
export class SecureAppointmentRepository extends AppointmentRepository {
  ...
  override async create(
    ...
    if (!validation.isValid) {
      throw new Error(
        'Unauthorized: Cannot create appointment in this business context'
      );
    }
    ...
    if (request.clientId) {
      const clientValidation =
        await businessContextSecurity.validateClientAccess(
          request.clientId,
          request.businessId,
          userId,
  ...
  override async delete(
    ...
    if (!canDelete) {
      ...
      throw new Error(
        'Unauthorized: Only owners and managers can delete appointments'
      );
    }
  ...
  override async updateStatus(
    ...
    try {
      const updatedAppointment = await super.updateStatus(
        id,
        businessId,
        status
      );
  ...
  async findByBusinessSecure(
    ...
    if (!validation.isValid) {
      throw new Error(
        'Unauthorized: Cannot access appointments for this business'
      );
    }
    ...
    const secureFilters = canViewAll
      ? filters
      : {
          ...filters,
  ...
  private validateStatusUpdatePermission(
    ...
    if (!isAssignedStaff) {
      return {
        allowed: false,
        reason: 'Can only update status of your own appointments',
      };
    }

    // Define allowed transitions for staff
    const allowedStaffTransitions: Record<
      AppointmentStatus,
      AppointmentStatus[]
    > = {
