...
export class TimeOffRequestRepository {
     ...
     */
    async getTimeOffRequestById(requestId: string): Promise<TimeOffRequestWithDetails | null> {
     ...
     */
    async denyTimeOffRequest(requestId: string, approverId: string, denialReason?: string): Promise<TimeOffRequest> {
        // Validate that the approver exists and has permission
        const approver = await prisma.staff.findUnique({
            where: { id: approverId },
            select: { businessId: true }
     ...
     */
    async cancelTimeOffRequest(requestId: string, staffId: string): Promise<TimeOffRequest> {
     ...
     */
    async checkTimeOffConflicts(staffId: string, startDate: Date, endDate: Date): Promise<TimeOffConflict[]> {
        ...
        const overlappingTimeOff = await this.getOverlappingTimeOffRequests(staffId, startDate, endDate, [TimeOffStatus.APPROVED])
     ...
     */
    async getApprovedTimeOff(staffId: string, startDate: Date, endDate: Date): Promise<TimeOffRequest[]> {
        return await prisma.timeOffRequest.findMany({
            where: {
                staffId,
                status: TimeOffStatus.APPROVED,
     ...
     */
    async getPendingTimeOffRequests(businessId: string): Promise<TimeOffRequestWithDetails[]> {
        return await this.getTimeOffRequests(businessId, { status: TimeOffStatus.PENDING })
    }

    /**
     * Get time-off statistics for a business
     */
    async getTimeOffStatistics(businessId: string, year?: number): Promise<{
     ...
     */
    async hasApprovedTimeOffOnDate(staffId: string, date: Date): Promise<boolean> {
     ...
     */
    async bulkApproveTimeOffRequests(requestIds: string[], approverId: string): Promise<TimeOffRequest[]> {
        const results: TimeOffRequest[] = []

        for (const requestId of requestIds) {
            try {
                const approved = await this.approveTimeOffRequest(requestId, approverId)
