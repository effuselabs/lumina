...
export class EnhancedBusinessHoursRepository {
    ...
    async getBusinessHours(query: BusinessHoursQuery): Promise<BusinessHour[]> {
        const result = await this.backwardCompatService.getBusinessHours(query.businessId);

        // Log deprecation warning if using JSON fallback
        if (result.source === 'json' && result.deprecationWarning) {
            this.backwardCompatService.logDeprecationWarning('business', query.businessId, 'operatingHours');
        }
     ...
     */
    async setBusinessHours(businessId: string, hours: BusinessHour[]): Promise<void> {
     ...
     */
    async getBusinessHoursForDay(businessId: string, dayOfWeek: number): Promise<BusinessHour | null> {
        const businessHours = await this.getBusinessHours({ businessId });
    ...
    async needsMigration(businessId: string): Promise<boolean> {
        const status = await this.backwardCompatService.hasBusinessHours(businessId);
    ...
    async validateBusinessHours(businessId: string): Promise<{
        ...
        try {
            ...
            for (const hour of businessHours) {
                if (!hour.isClosed && hour.openTime && hour.closeTime) {
                    if (hour.openTime >= hour.closeTime) {
                        result.errors.push(`Day ${hour.dayOfWeek}: Open time (${hour.openTime}) is not before close time (${hour.closeTime})`);
                        result.isValid = false;
                    }
                }

                if (hour.isClosed && (hour.openTime || hour.closeTime)) {
                    result.warnings.push(`Day ${hour.dayOfWeek}: Marked as closed but has open/close times`);
                }
            }
            ...
            if (status.needsMigration) {
                result.warnings.push('Business is using deprecated JSON format. Migration recommended.');
            }
