...
class PerformanceMonitor {
  ...
  async measureQuery<T>(
    ...
    try {
      ...
      const duration = performance.now() - startTime;
      console.error(
        `Query failed: ${operation} after ${duration.toFixed(2)}ms`,
        error
      );
      throw error;
    }
...
class OptimizedQueryBuilder {
   ...
   */
  static buildAppointmentInclude(
    options: {
  ...
  static buildOptimizedWhere(
    ...
    const { status, startDate, endDate, clientId, staffId, search } = filters;
  ...
  async findByBusinessOptimized(
    ...
    return this.performanceMonitor.measureQuery(
      ...
      async () => {
        ...
        const where = OptimizedQueryBuilder.buildOptimizedWhere(
          businessId,
          filterOptions
        );
        ...
        const nextCursor = hasMore
          ? resultAppointments[resultAppointments.length - 1].id
          : undefined;

        return {
          appointments:
            resultAppointments as unknown as AppointmentWithRelations[],
          hasMore,
          nextCursor,
        };
      }
      ...
      async () => {
        ...
        const byStaff = staffStats.map(stat => {
          ...
          return {
            ...
            staffName:
              staff?.displayName || staff?.user?.name || 'Unknown Staff',
    ...
    return this.performanceMonitor.measureQuery(
      ...
      async () => {
        ...
        try {
          ...
          const invalidIds = appointmentIds.filter(
...
export const optimizedAppointmentRepository =
  new OptimizedAppointmentRepository();
