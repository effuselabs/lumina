...
export class EnhancedStaffAvailabilityRepository {
     ...
     */
    async getStaffAvailability(query: StaffAvailabilityQuery): Promise<StaffAvailabilitySlot[]> {
        const result = await this.backwardCompatService.getStaffAvailability(query.staffId);

        // Log deprecation warning if using JSON fallback
        if (result.source === 'json' && result.deprecationWarning) {
            this.backwardCompatService.logDeprecationWarning('staff', query.staffId, 'workingHours');
        }
     ...
     */
    async setStaffAvailability(staffId: string, businessId: string, availability: StaffAvailabilitySlot[]): Promise<void> {
     ...
     */
    async getStaffAvailabilityForDay(staffId: string, dayOfWeek: number): Promise<StaffAvailabilitySlot[]> {
        const availability = await this.getStaffAvailability({ staffId });
    ...
    async getAvailableStaff(businessId: string, dateTime: Date, serviceDuration: number): Promise<string[]> {
        ...
        for (const staffMember of staff) {
            const availability = await this.getStaffAvailability({ staffId: staffMember.id });
            const dayAvailability = availability.filter(a => a.dayOfWeek === dayOfWeek);

            // Check if any slot can accommodate the service duration
            const canAccommodate = dayAvailability.some(slot =>
    ...
    async needsMigration(staffId: string): Promise<boolean> {
        const status = await this.backwardCompatService.hasStaffAvailability(staffId);
     ...
     */
    async getEffectiveAvailability(staffId: string, date: Date): Promise<StaffAvailabilitySlot[]> {
        ...
        try {
            ...
            for (const slot of availability) {
                if (slot.startTime >= slot.endTime) {
                    result.errors.push(`Day ${slot.dayOfWeek}: Start time (${slot.startTime}) is not before end time (${slot.endTime})`);
                    result.isValid = false;
                }
            }

            // Check for overlapping slots on the same day
            const dayGroups = availability.reduce((groups, slot) => {
            ...
            if (status.needsMigration) {
                result.warnings.push('Staff is using deprecated JSON format. Migration recommended.');
            }
