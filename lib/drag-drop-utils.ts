...
export function validateAppointmentDrop(
  ...
  appointmentEndTime.setMinutes(
    appointmentEndTime.getMinutes() + appointmentDuration
  );
  ...
  const conflictingAppointments = allAppointments.filter(apt => {
    ...
    if (apt.staffId !== targetSlot.staffId && targetSlot.staffId !== '')
      return false;

    // Check for time overlap
    return (
      apt.startTime < appointmentEndTime && apt.endTime > targetSlot.startTime
    );
  });
  ...
  if (conflictingAppointments.length > 0) {
    conflicts.push({
      ...
      affectedAppointments: [
        appointment.id,
        ...conflictingAppointments.map(apt => apt.id),
      ],
    });
  }
  ...
  if (appointment.staffId !== targetSlot.staffId && targetSlot.staffId !== '') {
    warnings.push(
      'This will reassign the appointment to a different staff member'
    );
  }
...
export function findAlternativeSlots(
  ...
  const daySlots = allSlots.filter(
    slot => slot.startTime.toDateString() === targetDate.toDateString()
  );
  ...
  alternatives.sort((a, b) => {
    const aDistance = Math.abs(
      a.startTime.getTime() - appointment.startTime.getTime()
    );
    const bDistance = Math.abs(
      b.startTime.getTime() - appointment.startTime.getTime()
    );
