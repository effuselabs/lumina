...
export class BookingErrorTracker {
  ...
  async getErrorSummary(
    ...
    errors.forEach(error => {
      typeGroups.set(
        error.errorType,
        (typeGroups.get(error.errorType) || 0) + 1
      );
    });

    const errorsByType = Array.from(typeGroups.entries()).map(
      ([type, count]) => ({
        type,
        count,
      })
    ...
    errors.forEach(error => {
      severityGroups.set(
        error.severity,
        (severityGroups.get(error.severity) || 0) + 1
      );
    });

    const errorsBySeverity = Array.from(severityGroups.entries()).map(
      ([severity, count]) => ({
        severity,
        count,
      })
