...
export class BusinessMetricsTracker {
    ...
    async trackAppointmentCreated(
        ...
        try {
            ...
            for (const serviceId of appointmentData.serviceIds) {
                await this.trackServiceUsage(businessId, serviceId, appointmentData.startTime);
            }

            // Track staff utilization
            await this.trackStaffUtilization(businessId, appointmentData.staffId, appointmentData.startTime);
    ...
    async trackAppointmentStatusChange(
        ...
        try {
            ...
            switch (newStatus) {
                case 'CONFIRMED':
                    await this.incrementMetric(businessId, dateKey, 'APPOINTMENT_CONFIRMED');
                    break;
                case 'COMPLETED':
                    await this.incrementMetric(businessId, dateKey, 'APPOINTMENT_COMPLETED');
                    break;
                case 'CANCELLED':
                    await this.incrementMetric(businessId, dateKey, 'APPOINTMENT_CANCELLED');
                    break;
                case 'NO_SHOW':
                    await this.incrementMetric(businessId, dateKey, 'APPOINTMENT_NO_SHOW');
                    break;
            }
     ...
     */
    private async updateSuccessRate(businessId: string, date: Date): Promise<void> {
        const metrics = await prisma.businessMetric.findMany({
            where: {
                businessId,
                date,
                type: {
                    in: ['APPOINTMENT_COMPLETED', 'APPOINTMENT_CANCELLED', 'APPOINTMENT_NO_SHOW'],
                },
            },
        });

        const completed = metrics.find(m => m.type === 'APPOINTMENT_COMPLETED')?.value || 0;
        const cancelled = metrics.find(m => m.type === 'APPOINTMENT_CANCELLED')?.value || 0;
        const noShow = metrics.find(m => m.type === 'APPOINTMENT_NO_SHOW')?.value || 0;
    ...
    async getBusinessMetrics(
        ...
        return Object.entries(metricsByDate).map(([dateStr, dayMetrics]) => {
            ...
            const appointmentVolume = dayMetrics.find(m => m.type === 'APPOINTMENT_CREATED')?.value || 0;
            const completed = dayMetrics.find(m => m.type === 'APPOINTMENT_COMPLETED')?.value || 0;
            const cancelled = dayMetrics.find(m => m.type === 'APPOINTMENT_CANCELLED')?.value || 0;
            const noShow = dayMetrics.find(m => m.type === 'APPOINTMENT_NO_SHOW')?.value || 0;

            const revenueMetric = dayMetrics.find(m => m.type === 'APPOINTMENT_CREATED');
            const totalRevenue = revenueMetric?.metadata?.totalRevenue || 0;
            const averageBookingValue = appointmentVolume > 0 ? totalRevenue / appointmentVolume : 0;

            const staffUtilizationMetric = dayMetrics.find(m => m.type === 'STAFF_UTILIZATION');
            const staffUtilization = staffUtilizationMetric?.metadata?.staff || {};

            const serviceUsageMetric = dayMetrics.find(m => m.type === 'SERVICE_USAGE');
