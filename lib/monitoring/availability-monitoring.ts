...
class AvailabilityMonitoring {
  ...
  private checkPerformanceThresholds(metric: PerformanceMetric): void {
    ...
    if (
      metric.operation === 'availability_check' &&
      metric.duration > this.thresholds.availabilityQueryTime
    ) {
      this.generateAlert({
        type: AlertType.PERFORMANCE_DEGRADATION,
        severity:
          metric.duration > this.thresholds.availabilityQueryTime * 2
    ...
    if (
      metric.operation === 'conflict_detection' &&
      metric.duration > this.thresholds.conflictDetectionTime
    ) {
      this.generateAlert({
        type: AlertType.CONFLICT_DETECTION_SLOW,
        severity:
          metric.duration > this.thresholds.conflictDetectionTime * 3
            ? AlertSeverity.CRITICAL
            : AlertSeverity.WARNING,
  ...
  private updateCacheMetrics(metric: CacheMetric): void {
    ...
    const aggregation =
      this.metricsBuffer.get(key) || this.createEmptyAggregation();
    ...
    const totalCacheOperations =
      aggregation.cacheHits + aggregation.cacheMisses;
    if (totalCacheOperations >= 10) {
      ...
      if (hitRatio < this.thresholds.cacheHitRatio) {
        this.generateAlert({
          ...
          severity:
            hitRatio < this.thresholds.cacheHitRatio * 0.5
              ? AlertSeverity.CRITICAL
              : AlertSeverity.WARNING,
  ...
  private performPeriodicChecks(): void {
    try {
      ...
      availabilityLogger.log(
        LogLevel.ERROR,
        'Error in periodic monitoring checks',
        {
  ...
  private updateAggregatedMetrics(metric: PerformanceMetric): void {
    ...
    const totalResponseTime =
      aggregation.averageResponseTime * (aggregation.totalRequests - 1) +
      metric.duration;
    aggregation.averageResponseTime =
      totalResponseTime / aggregation.totalRequests;
