...
export class BookingPerformanceMonitor {
  ...
  async getPerformanceInsights(
    ...
    const pageLoadEvents = events.filter(
      e => e.eventType === 'page_load' && e.duration
    );
    const apiEvents = events.filter(
      e => e.eventType === 'api_response' && e.duration
    );
    ...
    const averagePageLoadTime =
      pageLoadEvents.length > 0
        ? pageLoadEvents.reduce((sum, e) => sum + (e.duration || 0), 0) /
          pageLoadEvents.length
        : 0;

    const averageApiResponseTime =
      apiEvents.length > 0
        ? apiEvents.reduce((sum, e) => sum + (e.duration || 0), 0) /
          apiEvents.length
        : 0;
