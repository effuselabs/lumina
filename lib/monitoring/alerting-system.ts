...
export class AlertingSystem {
    ...
    async getActiveAlerts(limit: number = 50): Promise<AlertNotification[]> {
        ...
        return alerts.map(alert => ({
            ...
            message: this.generateMessageFromData(alert.type, alert.data as Record<string, any>),
     ...
     */
    private generateMessageFromData(type: string, data: Record<string, any>): string {
        switch (type) {
            case 'slow-response-time':
