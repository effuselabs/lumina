...
class AppointmentPerformanceMonitor {
  ...
  private checkThresholds(metric: PerformanceMetric): void {
    ...
    if (thresholdKey && metric.value > this.thresholds[thresholdKey]) {
      console.warn(
        `Performance threshold violation: ${metric.name} took ${metric.value}ms (threshold: ${this.thresholds[thresholdKey]}ms)`
      );
  ...
  private emitPerformanceWarning(
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent('appointment-performance-warning', {
          detail: {
            metric: metric.name,
            value: metric.value,
            threshold,
  ...
  generateReport(timeWindow: number = 24 * 60 * 60 * 1000): PerformanceReport {
    ...
    const metrics = {
      ...
      averageSearchResponseTime: this.getAverageMetric(
        'appointment-search',
        timeWindow
      ),
      averageDragDropResponseTime: this.getAverageMetric(
        'appointment-drag-drop',
        timeWindow
      ),
      averageRealTimeUpdateLatency: this.getAverageMetric(
        'appointment-realtime-update',
        timeWindow
      ),
      peakMemoryUsage: Math.max(
        ...this.getMetrics(startTime, now, 'appointment-memory-usage').map(
          m => m.value
        )
      ),
  ...
  private calculateCacheHitRate(timeWindow: number): number {
    const hits = this.getMetrics(
      Date.now() - timeWindow,
      Date.now(),
      'appointment-cache-hit'
    ).length;
    const misses = this.getMetrics(
      Date.now() - timeWindow,
      Date.now(),
      'appointment-cache-miss'
    ).length;
  ...
  private generateRecommendations(
    ...
    if (metrics.averageCalendarLoadTime > this.thresholds.calendarLoadTime) {
      recommendations.push(
        'Consider implementing virtual scrolling for large appointment datasets'
      );
      recommendations.push(
        'Optimize calendar rendering with React.memo and useMemo'
      );
    }

    if (
      metrics.averageSearchResponseTime > this.thresholds.searchResponseTime
    ) {
    ...
    if (violations.length > 10) {
      recommendations.push(
        'Performance is consistently below thresholds - consider infrastructure scaling'
      );
    }
...
// Singleton instance
export const appointmentPerformanceMonitor =
  new AppointmentPerformanceMonitor();
...
export function usePerformanceMonitoring() {
  return {
    ...
    startTiming: (name: string) =>
      appointmentPerformanceMonitor.startTiming(name),
