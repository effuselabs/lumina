...
import { AvailabilityError, ErrorSeverity, getErrorSeverity } from '@/lib/errors/availability-errors'
...
export interface BusinessMetric {
    type: 'availability_check' | 'booking_attempt' | 'conflict_detected' | 'cache_usage'
    businessId: string
    timestamp: Date
    value: number
    metadata?: Record<string, any>
...
class AvailabilityLogger {
    ...
    log(level: LogLevel, message: string, context: LogEntry['context'], metadata?: Record<string, any>): void {
    ...
    logPerformance(metric: PerformanceMetric): void {
        ...
        // Log slow operations
        if (metric.duration > 200) { // 200ms threshold
            this.log(LogLevel.WARN, `Slow operation detected: ${metric.operation}`, {
    ...
    logAvailabilityCheck(businessId: string, staffId: string, duration: number, success: boolean, errorCode?: string): void {
        this.logPerformance({
    ...
    logConflictDetection(businessId: string, conflictCount: number, duration: number): void {
        this.logPerformance({
    ...
    logBookingAttempt(businessId: string, success: boolean, errorCode?: string): void {
        this.logBusinessMetric({
    ...
    logCacheHitRatio(businessId: string, hits: number, total: number): void {
        ...
        if (ratio < 0.7 && total > 10) { // Less than 70% hit ratio with sufficient samples
            this.log(LogLevel.WARN, `Low cache hit ratio detected: ${(ratio * 100).toFixed(1)}%`, {
    ...
    private consoleLog(entry: LogEntry): void {
        ...
        switch (entry.level) {
            case LogLevel.DEBUG:
                console.debug(`[${timestamp}] DEBUG: ${entry.message}\nContext: ${context}`)
                break
            case LogLevel.INFO:
                console.info(`[${timestamp}] INFO: ${entry.message}\nContext: ${context}`)
                break
            case LogLevel.WARN:
                console.warn(`[${timestamp}] WARN: ${entry.message}\nContext: ${context}`)
                break
            case LogLevel.ERROR:
                console.error(`[${timestamp}] ERROR: ${entry.message}\nContext: ${context}`)
                if (entry.stack) {
                    console.error(`Stack: ${entry.stack}`)
                }
                break
            case LogLevel.FATAL:
                console.error(`[${timestamp}] FATAL: ${entry.message}\nContext: ${context}`)
                if (entry.stack) {
                    console.error(`Stack: ${entry.stack}`)
                }
                break
...
export function withPerformanceLogging<T>(
    ...
    return fn()
        ...
        .catch(error => {
            const duration = Date.now() - startTime
            const errorCode = error instanceof AvailabilityError ? error.code : 'UNKNOWN_ERROR'
            ...
            if (error instanceof AvailabilityError) {
                ...
                availabilityLogger.log(LogLevel.ERROR, `Unexpected error in ${operation}`, {
                    operation,
                    businessId,
    ...
    return fn()
        ...
        .catch(error => {
            ...
            availabilityLogger.logCache({
                ...
                businessId,
                duration,
                errorCode: error instanceof AvailabilityError ? error.code : 'CACHE_ERROR'
            })
            throw error
        })
}