...
export class DataRetentionCleanupService {
        ...
        try {
            ...
            for (const business of businesses) {
                ...
                try {
                    ...
                    console.log(`Completed cleanup for business ${business.name} (${business.id})`)
                } catch (error) {
                    totalErrors++
                    ...
                    console.error(`Failed to cleanup business ${business.name} (${business.id}):`, error)
                }
            }
            ...
            const summary: CleanupJobSummary = {
                ...
                totalErrors,
                totalAnonymizedRecords: results.reduce((sum, r) =>
                    sum + Object.values(r.anonymizedRecords).reduce((s, v) => s + v, 0), 0
                ),
                totalDeletedRecords: results.reduce((sum, r) =>
                    sum + Object.values(r.deletedRecords).reduce((s, v) => s + v, 0), 0
                ),
                results,
                startTime,
                endTime,
    ...
    async cleanupBusinessData(
        ...
        try {
            ...
            const { anonymizedRecords, deletedRecords } = await dataProtection.anonymizeExpiredData(
                businessId,
                retentionPolicy
            )
    ...
    private async performAdditionalCleanup(
        ...
        try {
            ...
            const cacheCleanupDate = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000) // 7 days
            ...
            const invitationCleanupDate = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000) // 90 days
            ...
            const statusHistoryCleanupDate = new Date(now.getTime() - retentionPolicy.appointmentData * 24 * 60 * 60 * 1000)
            ...
            const campaignArchiveDate = new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000) // 1 year
     ...
     */
    private async logCleanupJobCompletion(summary: CleanupJobSummary): Promise<void> {
        try {
     ...
     */
    validateRetentionPolicy(policy: DataRetentionPolicy): { isValid: boolean; errors: string[] } {
        const errors: string[] = []
        ...
        Object.entries(minimumRetention).forEach(([key, minDays]) => {
            ...
            if (policyValue < minDays) {
                errors.push(`${key} retention period (${policyValue} days) is below minimum requirement (${minDays} days)`)
            }
        })
        ...
        Object.entries(maximumRetention).forEach(([key, maxDays]) => {
            ...
            if (policyValue > maxDays) {
                errors.push(`${key} retention period (${policyValue} days) exceeds maximum limit (${maxDays} days)`)
            }
        })
