...
export const appointmentServiceSchema = z.object({
    ...
    serviceOrder: z.number().min(1, 'Service order must be at least 1').default(1),
    startOffset: z.number().min(0, 'Start offset must be non-negative').default(0),
...
export const createAppointmentSchema = z.object({
    ...
    (data) => {
        ...
        return data.clientId || (data.clientName && (data.clientEmail || data.clientPhone))
    },
    {
        message: 'Either client ID or client contact information (name and email/phone) is required',
        path: ['clientId']
    }
)
...
export const appointmentFiltersSchema = z.object({
    ...
    // Sorting
    sortBy: z.enum(['startTime', 'endTime', 'createdAt', 'updatedAt', 'status']).default('startTime'),
...
export const removeServicesSchema = z.object({
    serviceIds: z.array(z.string().cuid('Invalid service ID format')).min(1, 'At least one service ID is required')
})
...
export const validateAppointmentSchema = z.object({
    ...
    endTime: z.coerce.date(),
    services: z.array(z.string().cuid('Invalid service ID format')).min(1, 'At least one service is required'),
    clientId: z.string().cuid('Invalid client ID format').optional(),
    excludeAppointmentId: z.string().cuid('Invalid appointment ID format').optional()
}).refine(
    (data) => data.endTime > data.startTime,
    {
        message: 'End time must be after start time',
...
export type ValidateAppointmentRequest = z.infer<typeof validateAppointmentSchema>
