...
export class RealTimeSyncService {
  ...
  rollbackOptimisticUpdate(updateId: string): void {
    const updateIndex = this.updateQueue.findIndex(
    ...
    const pendingIndex = this.syncState.pendingUpdates.findIndex(
      u => u.id === updateId
    );
    if (pendingIndex !== -1) {
  ...
  handleServerDelete(appointmentId: string): void {
    const conflictingUpdate = this.updateQueue.find(
      update =>
        update.appointmentId === appointmentId && update.type !== 'delete'
    );
  ...
  resolveConflict(
    ...
    const conflictIndex = this.syncState.conflicts.findIndex(
  ...
  private async syncSingleUpdate(update: OptimisticUpdate): Promise<void> {
    ...
    const response = await fetch(endpoint, {
      ...
      body:
        update.type !== 'delete'
          ? JSON.stringify(update.optimisticData)
          : undefined,
    });
  ...
  private attemptAutoMerge(
    ...
    const hasConflictingChanges = systemFields.some(field => {
      return (
        local[field as keyof DashboardAppointment] !==
        server[field as keyof DashboardAppointment]
      );
    });
