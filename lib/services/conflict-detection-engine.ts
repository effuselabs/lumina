...
export class ConflictDetectionEngine {
     ...
     * Requirements: 4.1, 4.2, 4.3, 4.4
     */
    static async detectConflicts(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            ...
            const staffAvailabilityConflict = await this.checkStaffAvailability(request)
            if (staffAvailabilityConflict) {
                conflicts.push(staffAvailabilityConflict)
            }

            // Check overlapping appointments
            const overlappingConflicts = await this.checkOverlappingAppointments(request)
            ...
            for (const conflict of conflicts) {
                if (conflict.severity === ConflictSeverity.ERROR) {
                    conflict.suggestedResolutions = await this.generateResolutions(request, conflict)
                }
            }
            ...
            if (error instanceof Error && error.message === 'Invalid business context') {
                throw error
            }
    ...
    static async validateAppointmentSlot(
        ...
        try {
            ...
            const conflicts = allConflicts.filter(c => c.severity === ConflictSeverity.ERROR)
     ...
     */
    static async checkOverlappingAppointments(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            const conflicts: Conflict[] = []
     ...
     */
    private static async checkBusinessHours(request: AppointmentRequest): Promise<Conflict | null> {
        try {
            const validation = await TimeSlotAnalysisEngine.validateBusinessHoursBoundaries(
            ...
            if (!validation.isValid) {
                ...
                const businessHours = await this.getBusinessHoursForDay(request.businessId, dayOfWeek)
     ...
     * Private helper method
     */
    private static async checkStaffAvailability(request: AppointmentRequest): Promise<Conflict | null> {
        try {
            const dayOfWeek = request.startTime.getDay()
            ...
            const requestStartMinutes = request.startTime.getUTCHours() * 60 + request.startTime.getUTCMinutes()
            const requestEndMinutes = request.endTime.getUTCHours() * 60 + request.endTime.getUTCMinutes()
     ...
     * Private helper method
     */
    private static async checkTimeOffConflicts(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            const conflicts: Conflict[] = []
     ...
     * Private helper method
     */
    private static async checkServiceDurationFit(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            const conflicts: Conflict[] = []
            ...
            const totalRequiredDuration = services.reduce((sum, service) => sum + service.duration, 0)
        ...
        try {
            switch (conflict.type) {
                ...
                case ConflictType.BUSINESS_HOURS_VIOLATION:
                    ...
                    const businessHoursSlots = await this.findSlotsWithinBusinessHours(request)
     ...
     * Private helper method
     */
    private static async findAlternativeSlots(request: AppointmentRequest): Promise<TimeSlot[]> {
        try {
            ...
            for (let dayOffset = 0; dayOffset < this.SEARCH_DAYS_AHEAD && alternatives.length < this.MAX_ALTERNATIVE_SLOTS; dayOffset++) {
                ...
                if (dayOffset === 0) {
                    ...
                    alternatives.push(...laterSlots.slice(0, this.MAX_ALTERNATIVE_SLOTS - alternatives.length))
                } else {
                    ...
                    alternatives.push(...daySlots.slice(0, this.MAX_ALTERNATIVE_SLOTS - alternatives.length))
                }
            }
     ...
     * Private helper method
     */
    private static async findAlternativeStaff(request: AppointmentRequest): Promise<Array<{
        staffId: string
        staffName: string
        availableSlots: TimeSlot[]
    }>> {
        try {
            ...
            const qualifiedStaff = capableStaff.filter(staff => {
                ...
                return request.serviceIds.every(serviceId => staffServiceIds.includes(serviceId))
            })
     ...
     * Private helper method
     */
    private static async findSlotsWithinBusinessHours(request: AppointmentRequest): Promise<TimeSlot[]> {
        try {
            const duration = Math.floor(
                (request.endTime.getTime() - request.startTime.getTime()) / (1000 * 60)
            )
     ...
     * Private helper method
     */
    private static async findLongerSlots(request: AppointmentRequest): Promise<TimeSlot[]> {
        try {
            if (request.serviceIds.length === 0) {
                return []
            }
            ...
            const requiredDuration = services.reduce((sum, service) => sum + service.duration, 0)
    ...
    static async validateAppointmentSlotAdvanced(
        ...
        try {
            ...
            if (checkTimeOff) {
                const timeOffConflicts = await this.checkAdvancedTimeOffConflicts(request)
                allConflicts.push(...timeOffConflicts)
            }

            if (checkServiceDuration) {
                const durationConflicts = await this.checkAdvancedServiceDuration(request)
            ...
            if (generateResolutions) {
                for (const conflict of allConflicts) {
                    if (conflict.severity === ConflictSeverity.ERROR && !conflict.suggestedResolutions) {
                        conflict.suggestedResolutions = await this.generateResolutions(request, conflict)
                    }
                }
            }
            ...
            // Separate errors from warnings
            const conflicts = allConflicts.filter(c => c.severity === ConflictSeverity.ERROR)
            const warnings = allConflicts.filter(c => c.severity !== ConflictSeverity.ERROR)
    ...
    static async checkOverlappingAppointmentsAdvanced(
        ...
        try {
            ...
            const bufferedStartTime = new Date(request.startTime.getTime() - bufferMinutes * 60000)
            const bufferedEndTime = new Date(request.endTime.getTime() + bufferMinutes * 60000)
            ...
            for (const appointment of overlappingAppointments) {
                ...
                const severity = bufferMinutes > 0 && overlapMinutes <= bufferMinutes
     ...
     */
    private static async checkAdvancedTimeOffConflicts(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            const conflicts: Conflict[] = []
            ...
            for (const timeOff of timeOffRequests) {
                ...
                const isFullDayTimeOff = this.isFullDayTimeOff(timeOff.startDate, timeOff.endDate)
                ...
                if (isFullDayTimeOff) {
                    ...
                    if (hasTimeOverlap) {
                        const overlapStart = new Date(Math.max(
                            request.startTime.getTime(),
                            timeOff.startDate.getTime()
                        ))
                        const overlapEnd = new Date(Math.min(
                            request.endTime.getTime(),
                            timeOff.endDate.getTime()
                        ))
                        const overlapMinutes = Math.floor(
                            (overlapEnd.getTime() - overlapStart.getTime()) / (1000 * 60)
                        )
     ...
     * Private helper method
     */
    private static async checkAdvancedServiceDuration(request: AppointmentRequest): Promise<Conflict[]> {
        try {
            const conflicts: Conflict[] = []
            ...
            const totalServiceDuration = validServices.reduce((sum, service) => sum + service.duration, 0)
            const bufferTime = Math.max(0, (validServices.length - 1) * BUFFER_TIME_MINUTES)
     ...
     * Private helper method
     */
    private static async checkSchedulingWarnings(request: AppointmentRequest): Promise<Conflict[]> {
        ...
        try {
            ...
            if (adjacentAppointments.length > 0) {
                warnings.push({
                    type: ConflictType.STAFF_UNAVAILABLE,
                    severity: ConflictSeverity.WARNING,
                    message: 'Back-to-back appointments scheduled - no buffer time between clients',
                    details: {}
                })
            }
    ...
    private static async getContinuousTimeSlotsForStaff(
        ...
        try {
            ...
            const continuousSlots: Array<{ startTime: Date; endTime: Date; duration: number }> = []
            ...
            for (const slot of availability) {
                ...
                if (slotConflicts.length === 0) {
                    ...
                    const duration = Math.floor((slotEnd.getTime() - slotStart.getTime()) / (1000 * 60))
                    ...
                    for (const conflict of slotConflicts) {
                        if (currentStart < conflict.startTime) {
                            const duration = Math.floor(
                                (conflict.startTime.getTime() - currentStart.getTime()) / (1000 * 60)
                            )
                        ...
                        currentStart = new Date(Math.max(currentStart.getTime(), conflict.endTime.getTime()))
                    }
