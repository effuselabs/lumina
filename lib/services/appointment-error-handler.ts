...
class AppointmentErrorHandler {
  ...
  private errorCounts: Map<string, { count: number; lastReset: Date }> =
    new Map();
  ...
  private updateErrorCounts(operation: string, severity: ErrorSeverity): void {
    ...
    if (!existing || now.getTime() - existing.lastReset.getTime() > 60000) {
      // Reset every minute
  ...
  private checkAlertThresholds(
    ...
    if (
      errorRate &&
      errorRate.count > this.monitoringConfig.alertThresholds.errorRate
    ) {
    ...
    if (severity === ErrorSeverity.CRITICAL) {
      ...
      if (
        criticalCount &&
        criticalCount.count >
          this.monitoringConfig.alertThresholds.criticalErrors
      ) {
  ...
  getErrorStatistics(): {
    ...
    for (const [key, data] of this.errorCounts.entries()) {
      ...
      stats.errorsByOperation[operation] =
        (stats.errorsByOperation[operation] || 0) + data.count;
      stats.errorsBySeverity[severity as ErrorSeverity] =
        (stats.errorsBySeverity[severity as ErrorSeverity] || 0) + data.count;
