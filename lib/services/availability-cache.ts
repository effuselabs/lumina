...
export class AvailabilityCache {
  ...
  static async get(options: CacheOptions): Promise<AvailabilitySlot[] | null> {
    ...
    try {
      ...
      return this.reviveSlotDates(cacheEntry.availableSlots);
    } catch (error) {
   ...
   * Requirements: 6.5
   */
  static async invalidateStaffAvailability(
    staffId: string,
    businessId: string
  ): Promise<void> {
    try {
  ...
  /**
   * Restore Date objects on slots read back from the cache.
   *
   * Slots are stored as JSON, so `startTime` and `endTime` come back as ISO
   * strings. Callers receive them typed as Date and call `.getTime()` on
   * them, which throws `slot.startTime.getTime is not a function` — every
   * slot then fails validation and the booking page offers nothing.
   *
   * This stayed hidden while the cache was broken: generateCacheKey threw on
   * every call, so nothing was ever stored or served and callers only ever
   * saw freshly built Date objects. Repairing the cache exposed it. A cache
   * must return the same shape it was given.
   */
  private static reviveSlotDates(stored: unknown): AvailabilitySlot[] {
