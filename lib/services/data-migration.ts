...
interface MigrationResult {
    ...
    rollbackData?: {
        ...
        staffAvailability: Array<{ id: string; staffId: string; dayOfWeek: number }>;
    };
}
...
export class DataMigrationService {
            ...
            await this.prisma.$transaction(async (tx) => {
                ...
                if (result.errors.length > 0) {
                    const criticalErrors = result.errors.filter(e =>
                        e.error.includes('validation failed') || e.error.includes('constraint violation')
                    );

                    if (criticalErrors.length > 0) {
                        throw new Error(`Migration failed with ${criticalErrors.length} critical errors`);
                    }
                }
            });
    ...
    private async migrateBusinessHours(tx: any): Promise<{
        ...
        const result = {
            ...
            rollbackData: [] as Array<{ id: string; businessId: string; dayOfWeek: number }>,
        };
        ...
        for (const business of businesses) {
            try {
                ...
                const operatingHours = OperatingHoursSchema.parse(business.operatingHours);
    ...
    private async migrateStaffAvailability(tx: any): Promise<{
        ...
        const result = {
            ...
            rollbackData: [] as Array<{ id: string; staffId: string; dayOfWeek: number }>,
        };
    ...
    private async validateBusinessHoursIntegrity(): Promise<{
        errors: Array<{ type: 'business'; id: string; field: string; message: string }>;
    }> {
        const errors: Array<{ type: 'business'; id: string; field: string; message: string }> = [];
        ...
        for (const business of businessesWithoutStructuredData) {
            errors.push({
                ...
                id: business.id,
                field: 'businessHours',
                message: 'Business has JSON operating hours but no structured business hours data',
            });
        }
        ...
        for (const businessHour of logicalInconsistencies) {
            if (businessHour.openTime && businessHour.closeTime && businessHour.openTime >= businessHour.closeTime) {
                errors.push({
                    type: 'business',
                    id: businessHour.businessId,
                    field: 'timeLogic',
    ...
    private async validateStaffAvailabilityIntegrity(): Promise<{
        errors: Array<{ type: 'staff'; id: string; field: string; message: string }>;
    }> {
        const errors: Array<{ type: 'staff'; id: string; field: string; message: string }> = [];
        ...
        for (const staff of staffWithoutStructuredData) {
            errors.push({
                ...
                message: 'Staff has JSON working hours but no structured availability data',
            });
        }
     ...
     */
    async rollbackMigration(rollbackData: MigrationResult['rollbackData']): Promise<{
        success: boolean;
        ...
        try {
            await this.prisma.$transaction(async (tx) => {
                ...
                if (rollbackData.staffAvailability.length > 0) {
                    const staffAvailabilityIds = rollbackData.staffAvailability.map(sa => sa.id);
