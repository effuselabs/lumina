...
class GracefulDegradationManager {
    ...
    static getInstance(strategy?: DegradationStrategy): GracefulDegradationManager {
        if (!GracefulDegradationManager.instance) {
            GracefulDegradationManager.instance = new GracefulDegradationManager(strategy)
        }
    ...
    async executeWithDegradation<T>(
        ...
        if (circuitState === CircuitState.OPEN) {
            availabilityLogger.log(LogLevel.WARN, `Circuit breaker open for ${serviceName}, using fallback`, {
                operation: context?.operation || 'unknown',
                businessId: context?.businessId
            })

            if (fallbackOperation) {
                return await this.executeFallback(serviceName, fallbackOperation, context)
            } else {
                throw new CacheFailureError(`Service ${serviceName} is unavailable`, false)
            }
        }
        ...
        for (let attempt = 1; attempt <= this.strategy.maxRetries; attempt++) {
            try {
                ...
                availabilityLogger.log(LogLevel.WARN, `Attempt ${attempt} failed for ${serviceName}`, {
                    operation: context?.operation || 'unknown',
                    businessId: context?.businessId,
                    errorCode: error instanceof AvailabilityError ? error.code : 'UNKNOWN_ERROR'
                ...
                if (attempt === this.strategy.maxRetries || this.shouldOpenCircuit(serviceName)) {
                    break
                }
        ...
        if (fallbackOperation && this.strategy.enableFallback) {
            availabilityLogger.log(LogLevel.ERROR, `All attempts failed for ${serviceName}, using fallback`, {
                operation: context?.operation || 'unknown',
                businessId: context?.businessId
            })
        ...
        if (lastError instanceof AvailabilityError) {
            ...
            throw new CacheFailureError(`Service ${serviceName} failed after ${this.strategy.maxRetries} attempts`, false)
        }
    }
    ...
    private async executeFallback<T>(
        ...
        try {
            ...
            availabilityLogger.log(LogLevel.INFO, `Fallback successful for ${serviceName}`, {
                operation: context?.operation || 'unknown',
                businessId: context?.businessId
            })
            ...
            availabilityLogger.log(LogLevel.ERROR, `Fallback failed for ${serviceName}`, {
                operation: context?.operation || 'unknown',
                businessId: context?.businessId,
                errorCode: error instanceof AvailabilityError ? error.code : 'FALLBACK_ERROR'
            })

            throw new CacheFailureError(`Both primary and fallback operations failed for ${serviceName}`, true)
        }
    }
    ...
    async setCachedAvailabilityWithFallback(
        ...
        try {
            ...
            availabilityLogger.log(LogLevel.WARN, `Failed to cache availability data for key ${cacheKey}`, {
                operation: 'set_cached_availability',
                businessId,
                errorCode: error instanceof AvailabilityError ? error.code : 'CACHE_SET_ERROR'
            })
        }
    }
    ...
    private recordFailure(serviceName: string): void {
        ...
        if (health.failureCount >= this.strategy.circuitBreakerThreshold) {
            ...
            availabilityLogger.log(LogLevel.ERROR, `Circuit breaker opened for ${serviceName}`, {
                operation: 'circuit_breaker',
                errorCode: 'CIRCUIT_BREAKER_OPEN'
            }, { failureCount: health.failureCount, threshold: this.strategy.circuitBreakerThreshold })
        }
    }
    ...
    private async performHealthChecks(): Promise<void> {
        for (const [serviceName, health] of this.serviceHealth.entries()) {
            ...
            if (circuitState === CircuitState.OPEN) {
                ...
                availabilityLogger.log(LogLevel.INFO, `Testing recovery for ${serviceName}`, {
    ...
    resetCircuitBreaker(serviceName: string): void {
        ...
        availabilityLogger.log(LogLevel.INFO, `Circuit breaker reset for ${serviceName}`, {
