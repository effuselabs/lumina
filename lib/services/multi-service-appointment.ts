...
export class MultiServiceAppointmentService {
  ...
  async validateMultiServiceAppointment(
    ...
    const unauthorizedServices = serviceIds.filter(
      id => !staffServiceIds.includes(id)
    );
    ...
    result.totalDuration = services.reduce(
   ...
   */
  private orderServicesByDependencies(
    services: ServiceSelection[]
  ...
  async getQualifiedStaff(
    businessId: string,
    serviceIds: string[]
  ): Promise<StaffServiceCapability[]> {
    const staffServices = await prisma.staffService.findMany({
      where: {
        serviceId: { in: serviceIds },
        staff: {
   ...
   */
  validateServiceSequence(services: ServiceSelection[]): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
      ...
      if (service.dependencies) {
        for (const depId of service.dependencies) {
          if (!serviceIds.has(depId)) {
            errors.push(
              `Service ${service.serviceName} depends on service ${depId} which is not in the appointment`
            );
          }
        }
      }
...
export const multiServiceAppointmentService =
  new MultiServiceAppointmentService();
