...
export class TimeSlotAnalysisEngine {
    ...
    static async findSuitableSlots(
        ...
        try {
            const { businessId, serviceId, staffId, date, includeUnavailable = false } = options
            ...
            const requiredDuration = serviceDuration.duration + (serviceDuration.bufferTime || 0)
            ...
            const targetStaffIds = staffId ? [staffId] : await this.getServiceStaff(businessId, serviceId)
    ...
    static async findMultiServiceSlots(
        ...
        try {
            const { businessId, services, date, staffId, includeUnavailable = false } = options

            // Calculate total required duration
            const totalDuration = await ServiceDurationValidator.getMinimumSlotDuration(services, businessId)
    ...
    static async validateContinuousTimeSlot(
        ...
        try {
            ...
            const isValid = continuousDuration >= requiredDuration && gaps.length === 0
    ...
    static async validateBusinessHoursBoundaries(
        ...
        try {
            ...
            // Get time components from the slot times (ignoring date/timezone)
            const slotStartHour = timeSlot.startTime.getUTCHours()
            const slotStartMinute = timeSlot.startTime.getUTCMinutes()
            const slotEndHour = timeSlot.endTime.getUTCHours()
            const slotEndMinute = timeSlot.endTime.getUTCMinutes()

            // Convert to minutes for easier comparison
            const businessOpenMinutes = openHour * 60 + openMinute
            const businessCloseMinutes = closeHour * 60 + closeMinute
        ...
        try {
            ...
            for (const availability of staffAvailability) {
                ...
                const gaps = conflicts.filter(conflict =>
                    conflict.startTime < availabilityEnd && conflict.endTime > availabilityStart
                )

                if (gaps.length === 0) {
                    // No gaps - entire period is continuous
                    const duration = Math.floor(
                        (availabilityEnd.getTime() - availabilityStart.getTime()) / (1000 * 60)
                    )
                    ...
                    const sortedGaps = gaps.sort((a, b) => a.startTime.getTime() - b.startTime.getTime())
                    ...
                    for (const gap of sortedGaps) {
                        ...
                        currentStart = new Date(Math.max(currentStart.getTime(), gap.endTime.getTime()))
                    }
    ...
    private static async detectAvailabilityGaps(
        ...
        try {
            ...
            for (const appointment of appointments) {
                const gapStart = new Date(Math.max(startTime.getTime(), appointment.startTime.getTime()))
                const gapEnd = new Date(Math.min(endTime.getTime(), appointment.endTime.getTime()))
                const duration = Math.floor((gapEnd.getTime() - gapStart.getTime()) / (1000 * 60))
            ...
            for (const timeOff of timeOffRequests) {
                const gapStart = new Date(Math.max(startTime.getTime(), timeOff.startDate.getTime()))
                const gapEnd = new Date(Math.min(endTime.getTime(), timeOff.endDate.getTime()))
                const duration = Math.floor((gapEnd.getTime() - gapStart.getTime()) / (1000 * 60))
    ...
    private static async getConflictsForDay(
        ...
        for (const timeOff of timeOffRequests) {
            conflicts.push({
                startTime: new Date(Math.max(startOfDay.getTime(), timeOff.startDate.getTime())),
                endTime: new Date(Math.min(endOfDay.getTime(), timeOff.endDate.getTime()))
            })
        }
    ...
    private static async getServiceDuration(
        serviceId: string,
        staffId?: string
    ): Promise<{ serviceId: string; duration: number; bufferTime?: number } | null> {
        try {
     ...
     * Private helper method
     */
    private static async getServiceStaff(businessId: string, serviceId: string): Promise<string[]> {
    ...
    private static async getMultiServiceCapableStaff(
        ...
        for (const [staffId, staffServices] of staffServiceMap) {
            const canPerformAll = serviceIds.every(serviceId => staffServices.has(serviceId))
            if (canPerformAll) {
