...
export class AppointmentStatusManager {
     ...
     */
    private getTimestampUpdates(newStatus: AppointmentStatus): Record<string, Date | null> {
        const now = new Date();
     ...
     */
    private async handleStatusSpecificEvents(event: StatusChangeEvent): Promise<void> {
     ...
     */
    private async handleConfirmationEvents(event: StatusChangeEvent): Promise<void> {
        // Placeholder for confirmation-specific events
        console.log(`Appointment ${event.appointmentId} confirmed - triggering confirmation events`);
    ...
    private async handleStartEvents(event: StatusChangeEvent): Promise<void> {
        ...
        console.log(`Appointment ${event.appointmentId} started - triggering start events`);
     ...
     */
    private async handleCompletionEvents(event: StatusChangeEvent): Promise<void> {
        // Placeholder for completion-specific events
        console.log(`Appointment ${event.appointmentId} completed - triggering completion events`);
     ...
     */
    private async handleCancellationEvents(event: StatusChangeEvent): Promise<void> {
        // Placeholder for cancellation-specific events
        console.log(`Appointment ${event.appointmentId} cancelled/no-show - triggering cancellation events`);
    ...
    async validateStatusUpdateWithBusinessRules(
        ...
        try {
            ...
            if (appointment.startTime < now && newStatus !== AppointmentStatus.COMPLETED) {
                if (newStatus === AppointmentStatus.CANCELLED) {
                    warnings.push('Cancelling a past appointment - consider marking as NO_SHOW instead');
                } else {
                    errors.push('Cannot modify past appointments except to mark as completed');
                }
            }
            ...
            if (newStatus === AppointmentStatus.CANCELLED) {
                ...
                if (hoursUntilAppointment < 24) {
                    warnings.push('Cancellation within 24 hours - cancellation policy may apply');
                }
            }
            ...
            if (appointment.status === AppointmentStatus.IN_PROGRESS &&
                ...
                warnings.push('Cancelling an in-progress appointment - ensure proper handling');
            }
