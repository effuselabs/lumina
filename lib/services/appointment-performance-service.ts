...
class AppointmentPerformanceService {
  ...
  async initialize(businessId: string): Promise<void> {
    const stopTiming = this.monitoring.startTiming(
      'appointment-performance-init'
    );
  ...
  async updateAppointment(
    ...
    try {
      // Update via API
      const updatedAppointment = await this.updateAppointmentAPI(
        appointmentId,
        updates
      );
  ...
  private async initializeSearchIndex(businessId: string): Promise<void> {
    ...
    try {
      const appointments = await this.fetchAppointmentsFromAPI(businessId, {
        start: oneMonthAgo,
        end: now,
      });
          ...
          switch (key) {
            case 'staffIds':
              return Array.isArray(value)
                ? value.includes(appointment.staffId)
                : true;
            case 'serviceIds':
              return Array.isArray(value)
                ? appointment.services.some(s => value.includes(s.id))
                : true;
            case 'status':
              return Array.isArray(value)
...
export const appointmentPerformanceService =
  new AppointmentPerformanceService();
