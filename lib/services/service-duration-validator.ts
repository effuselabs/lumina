...
export class ServiceDurationValidator {
  ...
  static async validateServiceFit(
    ...
    try {
      ...
      const requiredDuration =
        ...
        (serviceDuration.bufferTime || this.DEFAULT_BUFFER_TIME);
  ...
  static async validateMultiServiceBooking(
    ...
    try {
      const requiredDuration = await this.getMinimumSlotDuration(
        services,
        businessId
      );
  ...
  private static async findAlternativeSlots(
    ...
    try {
      ...
      for (
        let dayOffset = 0;
        dayOffset < 7 && alternatives.length < maxAlternatives;
        dayOffset++
      ) {
