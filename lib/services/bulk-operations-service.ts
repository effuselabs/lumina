...
export class BulkOperationsService {
    ...
    async validateBulkOperation(
        ...
        conflicts: Array<{
            appointmentId: string;
            type: 'staff_unavailable' | 'time_overlap' | 'business_closed' | 'invalid_status';
            message: string;
        ...
        try {
            ...
            return data.conflicts.map((conflict: any) => ({
                ...
                conflicts: conflict.conflicts.map((c: any) => ({
                    ...c,
                    suggestedAlternatives: c.suggestedAlternatives.map((alt: string) => new Date(alt)),
                })),
            }));
        } catch (error) {
...
export function getBulkOperationsService(businessId: string): BulkOperationsService {
    if (!bulkOperationsServices.has(businessId)) {
        bulkOperationsServices.set(businessId, new BulkOperationsService(businessId));
    }
