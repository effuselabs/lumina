...
export async function calculateAndStoreStaffPayment(
  ...
  if (existingCalculation) {
    ...
    return await createPaymentCalculation({
      business: {
        connect: { id: paymentCalculation.businessId },
      },
      staff: {
        connect: { id: paymentCalculation.staffId },
      },
...
export async function generatePayrollReport(
  ...
  const summary = calculateMixedEmploymentSummary(
    businessId,
    period,
    paymentCalculations
  );
...
export async function getAverageEarningsByEmploymentType(
  ...
  Record<EmploymentType, { averageEarnings: number; staffCount: number }>
> {
  const calculations = await getBusinessPaymentCalculations(businessId, period);

  const result: Record<
    EmploymentType,
    { averageEarnings: number; staffCount: number }
  > = {
  ...
  const groupedCalculations = calculations.reduce(
    (acc, calc) => {
      const employmentType =
        calc.employmentType.toLowerCase() as EmploymentType;
  ...
  Object.entries(groupedCalculations).forEach(([type, calcs]) => {
    ...
    const totalEarnings = calcs.reduce(
      (sum, calc) => sum + calc.netEarnings.toNumber(),
      0
    );
...
export async function autoCalculatePaymentsFromTransactions(
  ...
  const revenueByStaff = await calculateStaffRevenueForPeriod(
    businessId,
    period
  );
