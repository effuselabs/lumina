...
class EnhancedErrorHandler {
  ...
  async convertConflictsToErrors(
    ...
    for (const conflict of conflicts) {
      try {
        const error = await this.convertSingleConflictToError(
          conflict,
          context,
          alternativeSlots
        );
        ...
        availabilityLogger.log(
          LogLevel.ERROR,
          `Failed to convert conflict to error: ${conflict.type}`,
          context,
          {
            conflict,
            conversionError:
  ...
  private async convertSingleConflictToError(
    ...
    const suggestions = await this.generateSuggestionsForConflict(
      conflict,
      context,
      alternativeSlots
    );
    ...
    switch (conflict.type) {
      ...
      case ConflictType.OVERLAPPING_APPOINTMENT:
        return this.createSchedulingConflictError(
          conflict,
          context,
          suggestions
        );
      ...
      case ConflictType.INSUFFICIENT_DURATION:
        return this.createInsufficientDurationError(
          conflict,
          context,
          suggestions
        );
      ...
      default:
        availabilityLogger.log(
          LogLevel.WARN,
          `Unknown conflict type: ${conflict.type}`,
          context
        );
        return null;
    }
  ...
  private createStaffUnavailableError(
    ...
    let reason: 'no_availability' | 'time_off' | 'not_scheduled' =
      'no_availability';
  ...
  private createInsufficientDurationError(
    ...
    if (!serviceDuration) {
      throw new Error(
        'Insufficient duration conflict missing duration details'
      );
    }
  ...
  private async generateSuggestionsForConflict(
    ...
    try {
      ...
      switch (conflict.type) {
        case ConflictType.BUSINESS_HOURS_VIOLATION:
          await this.addBusinessHoursSuggestions(
            suggestions,
            conflict,
            context
          );
        ...
        case ConflictType.STAFF_UNAVAILABLE:
          await this.addStaffAlternativeSuggestions(
            suggestions,
            conflict,
            context
          );
      ...
      availabilityLogger.log(
        LogLevel.WARN,
        `Failed to generate suggestions for conflict: ${conflict.type}`,
        context,
        {
  ...
  handleUnexpectedError(
    ...
    availabilityLogger.log(
      LogLevel.ERROR,
      `Unexpected error in ${context.operation}: ${message}`,
      context,
      {
        error: message,
        stack,
...
export async function withEnhancedErrorHandling<T>(
  ...
  try {
    ...
    if (error instanceof AvailabilityError) {
      ...
      const availabilityError = enhancedErrorHandler.handleUnexpectedError(
        error,
        context
      );
