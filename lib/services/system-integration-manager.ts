...
export class SystemIntegrationManager {
    ...
    async createAppointment(
        ...
        try {
            ...
            // Record metrics
            await this.recordOperationMetrics('create', Date.now() - startTime, result.success)
    ...
    async updateAppointment(
        ...
        try {
            ...
            // Record metrics
            await this.recordOperationMetrics('update', Date.now() - startTime, result.success)

            return result

        } catch (error) {
            await this.recordOperationMetrics('update', Date.now() - startTime, false)
            throw error
        }
    }
    ...
    async cancelAppointment(
        ...
        try {
            ...
            // Record metrics
            await this.recordOperationMetrics('cancel', Date.now() - startTime, result.success)

            return result

        } catch (error) {
            await this.recordOperationMetrics('cancel', Date.now() - startTime, false)
            throw error
        }
    }
    ...
    private async checkAppointmentBookingEngine(): Promise<SystemStatus> {
        ...
        try {
            await this.appointmentService.getAppointments('health-check', { limit: 1 })
     ...
     */
    private determineChangeType(updates: any, previousData?: any): 'updated' | 'rescheduled' | 'status_changed' {
    ...
    private async recordOperationMetrics(
        ...
        try {
            ...
            console.log(`Operation metrics: ${operation}, ${responseTime}ms, success: ${success}`)
