...
import { AppointmentFilters, AppointmentRepository, CreateAppointmentRequest, DateRange, UpdateAppointmentRequest } from '@/lib/repositories/appointment-repository'
...
export class AppointmentService {
     ...
     */
    async createAppointment(request: CreateAppointmentServiceRequest): Promise<AppointmentServiceResult> {
        ...
        try {
            ...
            const serviceValidation = await this.multiServiceCoordinator.validateMultiServiceBooking(
                request.services,
            ...
            const availabilityResult = await CalendarIntegration.checkAvailability(availabilityCheck)

            if (!availabilityResult.isAvailable) {
                result.errors.push('Selected time slot is not available')
                if (availabilityResult.alternatives) {
                    result.warnings.push(`Alternative times available: ${availabilityResult.alternatives.length} options`)
                }
                return result
            }
            ...
            const conflictResult = await CalendarIntegration.detectConflicts(conflictCheck)
            ...
            const durationResult = await CalendarIntegration.validateServiceDuration(durationValidation)
            ...
            const appointment = await this.appointmentRepository.create(createRequest)

            // 7. Invalidate availability cache
            await this.invalidateRelatedCaches(appointment.businessId, appointment.staffId, {
            ...
            result.errors.push(`Failed to create appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return result
        }
    }
    ...
    async updateAppointment(
        ...
        try {
            ...
            const existingAppointment = await this.appointmentRepository.findById(appointmentId, businessId)
            ...
            if (updates.startTime || updates.endTime || updates.services) {
                ...
                const newServices = updates.services || existingAppointment.services.map(s => ({
                ...
                if (updates.services) {
                    ...
                    const totalDuration = await this.multiServiceCoordinator.calculateTotalDuration(
                    ...
                    await this.updateAppointmentServices(appointmentId, businessId, newServices)
                }
            }
            ...
            await this.invalidateRelatedCaches(businessId, updatedAppointment.staffId, {
            ...
            result.errors.push(`Failed to update appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return result
        }
    }
    ...
    async getAppointments(
        ...
        try {
            ...
            const result = await this.appointmentRepository.findByBusiness(businessId, filters)
            ...
            if (includeConflicts || validateAvailability) {
                for (const appointment of result.appointments) {
                    ...
                    if (validateAvailability) {
                        ...
                        const availabilityResult = await CalendarIntegration.checkAvailability(availabilityCheck)
                            ; (appointment as any).isCurrentlyAvailable = availabilityResult.isAvailable
                    }
                }
            }
    ...
    async cancelAppointment(
        ...
        try {
            ...
            const validation = await this.statusManager.validateStatusUpdateWithBusinessRules(
                appointmentId,
            ...
            if (!statusResult.success) {
                result.errors.push(statusResult.error || 'Failed to cancel appointment')
                return result
            }
            ...
            await this.invalidateRelatedCaches(businessId, cancelledAppointment.staffId, {
            ...
            result.errors.push(`Failed to cancel appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return result
        }
    }
    ...
    private async updateAppointmentServices(
        ...
        try {
            ...
            const existingServiceIds = existingAppointment.services.map(s => s.serviceId)
            if (existingServiceIds.length > 0) {
                await this.appointmentRepository.removeServices(appointmentId, businessId, existingServiceIds)
            }
            ...
            await this.appointmentRepository.addServices(appointmentId, businessId, serviceRequests)

        } catch (error) {
            throw new Error(`Failed to update appointment services: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async getAppointmentsByStaff(
        ...
        try {
            ...
            throw new Error(`Failed to get appointments by staff: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async getAppointmentsByClient(
        ...
        try {
            ...
            throw new Error(`Failed to get appointments by client: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async searchAppointmentsByClient(
        ...
        try {
            ...
            throw new Error(`Failed to search appointments by client: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async getAppointmentStatistics(
        ...
        try {
            return await this.appointmentRepository.getAppointmentStats(businessId, dateRange)
        } catch (error) {
            throw new Error(`Failed to get appointment statistics: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async findConflictingAppointments(
        ...
        try {
            ...
            throw new Error(`Failed to find conflicting appointments: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async rescheduleAppointment(
        ...
        try {
            ...
            const updateResult = await this.updateAppointment(appointmentId, businessId, {
            ...
            if (options.reason) {
                ...
                console.log(`Appointment ${appointmentId} rescheduled: ${options.reason}`)
            }
            ...
            result.errors.push(`Failed to reschedule appointment: ${error instanceof Error ? error.message : 'Unknown error'}`)
            return result
        }
    }
    ...
    async bulkUpdateAppointmentStatus(
        ...
        try {
            ...
            throw new Error(`Failed to bulk update appointment status: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
     ...
     * Requirements: 2.3, 3.1, 3.2
     */
    async getAppointmentsNeedingStatusUpdate(
        businessId: string
    ): Promise<{
        ...
        try {
            return await this.statusManager.getAppointmentsNeedingStatusUpdate(businessId)
        } catch (error) {
            throw new Error(`Failed to get appointments needing status update: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
}
