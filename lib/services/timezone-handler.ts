...
export class TimeZoneHandler {
    ...
    static handleDSTTransition(
        ...
        const transitionType = dayEndOffset > dayStartOffset ? 'spring_forward' : 'fall_back'
...
export class MultiLocationTimeZoneManager {
    ...
    getLocationTimeZone(locationId?: string): string {
        if (locationId && this.businessConfig.locations) {
            const location = this.businessConfig.locations.find(l => l.id === locationId)
    ...
    normalizeAvailabilityAcrossLocations(
        ...
        return availability.map(slot => {
            ...
            const utcStart = TimeZoneHandler.localToUTC(slot.startTime, slot.date, timezone)
            const utcEnd = TimeZoneHandler.localToUTC(slot.endTime, slot.date, timezone)
