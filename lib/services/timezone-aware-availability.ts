...
export class TimeZoneAwareAvailabilityCalculator {
    ...
    for (const slot of baseResult.slots) {
      try {
        ...
        if (typeof request.date === 'string') {
          ...
          dateStr = (request.date as any).toISOString().split('T')[0];
        }
        const startTimeStr =
        ...
        const endTimeStr =
          slot.endTime instanceof Date
            ...
            : slot.endTime;
        const utcStart = this.parseSlotTime(
          startTimeStr,
          dateStr,
          businessTimezone
        );
        const utcEnd = this.parseSlotTime(
          endTimeStr,
          dateStr,
          businessTimezone
        );
  ...
  async validateAppointmentTime(
    ...
    if (
      !TimeZoneHandler.isValidTimeInZone(appointmentStart, date, clientTimezone)
    ) {
      errors.push(
        `Invalid appointment start time in timezone ${clientTimezone}: ${appointmentStart}`
      );
    }

    if (
      !TimeZoneHandler.isValidTimeInZone(appointmentEnd, date, clientTimezone)
    ) {
      errors.push(
        `Invalid appointment end time in timezone ${clientTimezone}: ${appointmentEnd}`
      );
    }
    ...
    if (staffAvailability.length === 0) {
      ...
      const isWithinStaffHours = staffAvailability.some(availability => {
        return (
          availability.utcOpenTime &&
          availability.utcCloseTime &&
  ...
  private async getBusinessHoursWithTimeZone(
    ...
    const allBusinessHours = await this.businessHoursRepo.getBusinessHours({
      businessId,
    });
    ...
    if (businessHours.openTime && businessHours.closeTime) {
      try {
        utcOpenTime = TimeZoneHandler.localToUTC(
          businessHours.openTime,
          date,
          timezone
        );
        utcCloseTime = TimeZoneHandler.localToUTC(
   ...
   */
  private async getBusinessTimeZone(
    businessId: string,
    locationId?: string
  ): Promise<string> {
...
export class MultiLocationAvailabilityManager {
  ...
  > {
    const locations = this.timeZoneManager['businessConfig'].locations || [
      {
        id: 'default',
        name: 'Main Location',
        timezone: this.timeZoneManager['businessConfig'].timezone,
      },
    ];
    ...
    for (const location of locations) {
      try {
        ...
        console.error(
          `Error getting availability for location ${location.id}:`,
          error
        );
        results.push({
          locationId: location.id,
  ...
  async findBestTimeAcrossLocations(
    ...
    const preferredDateTime = DateTime.fromFormat(
      `${date} ${preferredTime}`,
      'yyyy-MM-dd HH:mm',
      {
      ...
      for (const slot of location.slots) {
        if (slot.duration >= duration) {
          ...
          const timeDifference = Math.abs(
            slotDateTime.diff(preferredDateTime, 'minutes').minutes
          );
