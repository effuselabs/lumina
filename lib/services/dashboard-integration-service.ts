...
export interface NotificationPayload {
    type: 'appointment_created' | 'appointment_updated' | 'appointment_cancelled' | 'appointment_rescheduled'
...
export class DashboardIntegrationService {
    ...
    async getDashboardAppointments(
        ...
        try {
            ...
            const enhancedAppointments = await Promise.all(
                result.appointments.map(appointment => this.enhanceAppointmentForDashboard(appointment, businessId))
            )
            ...
            throw new Error(`Failed to get dashboard appointments: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async createDashboardAppointment(
        ...
        try {
            ...
            return {
                success: false,
                errors: [`Failed to create appointment: ${error instanceof Error ? error.message : 'Unknown error'}`],
                warnings: []
            }
        }
    }
    ...
    async updateDashboardAppointment(
        ...
        try {
            ...
            return {
                success: false,
                errors: [`Failed to update appointment: ${error instanceof Error ? error.message : 'Unknown error'}`],
                warnings: []
            }
        }
    }
    ...
    async getStaffAvailability(
        ...
        try {
            ...
            for (const staffId of staffIds) {
                ...
                availability[staffId] = {
                    ...
                    busySlots: appointments.appointments.map(apt => ({
                        ...
                        clientName: apt.clientName || `${apt.client?.firstName} ${apt.client?.lastName}`.trim()
                    })),
            ...
            throw new Error(`Failed to get staff availability: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
     ...
     * Requirements: 8.3
     */
    async getClientData(clientId: string, businessId: string): Promise<{
        id: string
        firstName: string
        lastName: string
        email: string
        ...
        try {
            ...
            const appointments = await this.appointmentService.getAppointments(businessId, {
                clientId,
                limit: 10,
                orderBy: 'startTime',
                sortDirection: 'desc'
            ...
            throw new Error(`Failed to get client data: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async searchClients(
        ...
        try {
            const clients = await ClientService.searchClients(businessId, query, limit)
     ...
     * Requirements: 8.4
     */
    async getServicesForDashboard(businessId: string): Promise<Array<{
        id: string
        name: string
        description?: string
        duration: number
     ...
     * Requirements: 8.5
     */
    async getStaffForDashboard(businessId: string): Promise<Array<{
        id: string
        firstName: string
        lastName: string
        displayName: string
        ...
        try {
            ...
            throw new Error(`Failed to get staff data: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async sendAppointmentNotification(payload: NotificationPayload): Promise<void> {
        try {
            ...
            if (payload.clientId) {
                ...
                console.log(`Notification sent to client ${payload.clientId} for ${payload.type}`)
            }
    ...
    async subscribeToAppointmentUpdates(
        ...
        try {
            ...
            throw new Error(`Failed to subscribe to updates: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    async getSystemIntegrationStatus(): Promise<SystemIntegrationStatus> {
        try {
            ...
            throw new Error(`Failed to check system status: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    private async enhanceAppointmentForDashboard(
        ...
        try {
            ...
            const canEdit = appointment.status !== AppointmentStatus.COMPLETED && appointment.status !== AppointmentStatus.CANCELLED
            const canCancel = appointment.status === AppointmentStatus.SCHEDULED || appointment.status === AppointmentStatus.CONFIRMED
            ...
            return {
                ...
                client: {
                    ...
                    firstName: appointment.client?.firstName || appointment.clientName?.split(' ')[0] || '',
                    lastName: appointment.client?.lastName || appointment.clientName?.split(' ').slice(1).join(' ') || '',
