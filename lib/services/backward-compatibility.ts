...
export class BackwardCompatibilityService {
    ...
    try {
      ...
      if (business?.operatingHours) {
        try {
          ...
          this.triggerBusinessHoursMigration(businessId, jsonHours).catch(
            error => {
              console.warn(
                `Failed to auto-migrate business hours for ${businessId}:`,
                error
              );
            }
          );
          ...
          return {
            ...
            deprecationWarning:
              'Business hours are being read from deprecated JSON format. Migration to structured format is recommended.',
          };
        } catch (error) {
          console.warn(
            `Invalid JSON operating hours for business ${businessId}:`,
            error
          );
        }
      }
  ...
  async getStaffAvailability(
    ...
    try {
      ...
      const structuredAvailability =
        await this.prisma.staffAvailability.findMany({
          where: { staffId },
          orderBy: { dayOfWeek: 'asc' },
        });
      ...
      if (staff?.workingHours) {
        try {
          ...
          this.triggerStaffAvailabilityMigration(
            staffId,
            staff.businessId,
            jsonHours
          ).catch(error => {
            console.warn(
              `Failed to auto-migrate staff availability for ${staffId}:`,
              error
            );
          });
          ...
          return {
            ...
            migrationNeeded: true,
            deprecationWarning:
              'Staff availability is being read from deprecated JSON format. Migration to structured format is recommended.',
          };
        } catch (error) {
          console.warn(
            `Invalid JSON working hours for staff ${staffId}:`,
            error
          );
        }
      }
  ...
  private convertJsonToBusinessHours(
    ...
    for (const [dayName, dayHours] of Object.entries(jsonHours)) {
      const dayOfWeek =
        this.dayMapping[dayName as keyof typeof this.dayMapping];
  ...
  async getDeprecationStatistics(): Promise<{
    ...
    return {
      ...
      migrationProgress: {
        businesses:
          totalBusinesses > 0
            ? Math.round((businessesWithStructured / totalBusinesses) * 100)
            : 0,
        staff:
          totalStaff > 0
            ? Math.round((staffWithStructured / totalStaff) * 100)
            : 0,
      },
    };
  }
  ...
  async forceMigrateStaff(staffId: string): Promise<{
    ...
    try {
      ...
      const jsonHours = WorkingHoursSchema.parse(staff.workingHours);
      await this.triggerStaffAvailabilityMigration(
        staffId,
        staff.businessId,
        jsonHours
      );

      const availableSlots = Object.values(jsonHours).filter(
        h => h.isAvailable
      ).length;
