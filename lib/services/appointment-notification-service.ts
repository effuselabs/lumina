...
export interface AppointmentNotificationData {
    ...
    changeType: 'created' | 'updated' | 'cancelled' | 'rescheduled' | 'status_changed'
    changedBy?: {
        id: string
        name: string
...
export class AppointmentNotificationService {
    ...
    async sendAppointmentNotifications(
        ...
        try {
            ...
            for (const recipient of recipients) {
                await this.sendNotificationToRecipient(recipient, notificationData, result)
            }
    ...
    async sendRealTimeNotifications(
        ...
        try {
            ...
            const realtimePayload = {
                ...
                data: {
                    appointment: {
                        ...
                        clientName: appointment.clientName || `${appointment.client?.firstName} ${appointment.client?.lastName}`.trim(),
            ...
            result.errors.push(`Real-time notification failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
    ...
    private async sendNotificationToRecipient(
        ...
        try {
            ...
            const template = await this.generateNotificationTemplate(recipient, notificationData)
            ...
            result.errors.push(`Failed to notify ${recipient.name}: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
        ...
        try {
            ...
            if (appointment.client) {
                recipients.push({
                    ...
                    type: 'client',
                    email: appointment.client.email || appointment.clientEmail || undefined,
                    phone: appointment.client.phone || appointment.clientPhone || undefined,
    ...
    private async generateNotificationTemplate(
        ...
        const clientName = appointment.clientName || `${appointment.client?.firstName} ${appointment.client?.lastName}`.trim()
        ...
        switch (changeType) {
            ...
            case 'updated':
            case 'rescheduled':
                return this.generateUpdatedTemplate(templateData, recipient.type, notificationData)

            case 'cancelled':
                return this.generateCancelledTemplate(templateData, recipient.type)

            case 'status_changed':
                return this.generateStatusChangedTemplate(templateData, recipient.type, appointment.status)
     ...
     */
    private generateCreatedTemplate(data: any, recipientType: string): NotificationTemplate {
     ...
     */
    private generateUpdatedTemplate(data: any, recipientType: string, notificationData: AppointmentNotificationData): NotificationTemplate {
     ...
     */
    private generateCancelledTemplate(data: any, recipientType: string): NotificationTemplate {
        if (recipientType === 'client') {
            return {
     ...
     */
    private generateStatusChangedTemplate(data: any, recipientType: string, status: AppointmentStatus): NotificationTemplate {
     ...
     */
    private generateDefaultTemplate(data: any, recipientType: string): NotificationTemplate {
        return {
    ...
    private async sendPushNotification(
        ...
        try {
            ...
            result.errors.push(`Push notification failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
