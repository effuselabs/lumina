...
import { AvailabilityCalculator, AvailabilityQuery, AvailabilitySlot } from './availability-calculator'
import { AppointmentRequest, Conflict, ConflictDetectionEngine, ValidationResult } from './conflict-detection-engine'
import { MultiServiceBooking, ServiceDurationValidator, TimeSlot } from './service-duration-validator'
...
export interface CacheInvalidationRequest {
    ...
    type: 'staff_availability' | 'business_hours' | 'appointments' | 'client_appointments' | 'appointment_details' | 'all'
}
...
export class CalendarIntegration {
     ...
     * Requirements: 4.1, 4.2, 4.3, 1.2, 1.3
     */
    static async checkAvailability(request: AvailabilityCheckRequest): Promise<AvailabilityCheckResult> {
        ...
        try {
            ...
            const conflictRequest: AppointmentRequest = {
                businessId: request.businessId,
                staffId: request.staffId,
            ...
            const requestedSlot = availabilityResult.slots.find(slot =>
                slot.startTime.getTime() === request.startTime.getTime() &&
                slot.endTime.getTime() === request.endTime.getTime()
            )

            const isAvailable = requestedSlot?.isAvailable === true && validationResults.isValid
     ...
     */
    static async detectConflicts(request: ConflictCheckRequest): Promise<ConflictCheckResult> {
        try {
            // Validate input parameters
            ...
            const suggestedResolutions = conflicts
                .filter(c => c.suggestedResolutions && c.suggestedResolutions.length > 0)
            ...
            return {
                ...
                suggestedResolutions: suggestedResolutions.length > 0 ? suggestedResolutions : undefined
            }
     ...
     * Requirements: 4.1, 4.2, 4.3, 1.1, 1.5
     */
    static async validateServiceDuration(request: DurationValidationRequest): Promise<DurationValidationResult> {
        try {
            // Validate input parameters
            ...
            if (request.serviceIds.length === 0) {
                return {
                    ...
                    availableDuration: Math.floor(
                        (request.timeSlot.endTime.getTime() - request.timeSlot.startTime.getTime()) / (1000 * 60)
                    )
                }
            }
            ...
            const multiServiceBookings: MultiServiceBooking[] = request.serviceIds.map(serviceId => ({
                serviceId,
                staffId: request.staffId
            }))
     ...
     */
    static async invalidateAvailabilityCache(request: CacheInvalidationRequest): Promise<void> {
        const startTime = Date.now()
        ...
        try {
            ...
            switch (request.type) {
                case 'staff_availability':
                    if (request.staffId) {
                        await Promise.all([
                            AvailabilityCache.invalidateStaffAvailability(request.staffId, request.businessId),
                            AvailabilityCalculator.invalidateStaffAvailabilityCache(request.staffId, request.businessId),
                            this.invalidateAppointmentCache(`staff:${request.staffId}:${request.businessId}`)
                        ])
                    }
                    break
                ...
                case 'business_hours':
                    await Promise.all([
                        ...
                        AvailabilityCalculator.invalidateBusinessHoursCache(request.businessId),
                ...
                case 'appointments':
                    ...
                    if (request.staffId && request.dateRange) {
                        ...
                        while (currentDate <= endDate) {
                            ...
                            invalidationPromises.push(
                                this.invalidateAppointmentCache(`appointments:${request.staffId}:${dateKey}`)
                            )
                ...
                case 'client_appointments':
                    if (request.clientId) {
                        await this.invalidateAppointmentCache(`client:${request.clientId}:${request.businessId}`)
                    }
                    break
                ...
                case 'appointment_details':
                    if (request.appointmentId) {
                        await Promise.all([
                            this.invalidateAppointmentCache(`appointment:${request.appointmentId}`),
                            // Also invalidate related staff and client caches
                            request.staffId ? this.invalidateAppointmentCache(`staff:${request.staffId}:${request.businessId}`) : Promise.resolve(),
                            request.clientId ? this.invalidateAppointmentCache(`client:${request.clientId}:${request.businessId}`) : Promise.resolve()
                        ])
                    }
                    break
                ...
                case 'all':
                    await Promise.all([
                        AvailabilityCache.invalidateBusinessHours(request.businessId),
                        request.staffId ? AvailabilityCache.invalidateStaffAvailability(request.staffId, request.businessId) : Promise.resolve(),
                        request.staffId ? AvailabilityCalculator.invalidateStaffAvailabilityCache(request.staffId, request.businessId) : Promise.resolve(),
                        this.invalidateAppointmentCache(`business:${request.businessId}`)
                    ])
                    break
            ...
            switch (request.warmingType) {
                case 'availability':
                    if (request.staffIds && request.staffIds.length > 0) {
                        ...
                        const batches = this.batchArray(request.staffIds, this.CACHE_WARMING_BATCH_SIZE)

                        for (const batch of batches) {
                            warmingPromises.push(
                                this.warmAvailabilityCache(request.businessId, batch, request.dateRange)
                            )

                            // Add delay between batches for low priority requests
                            if (request.priority === 'low') {
                                await new Promise(resolve => setTimeout(resolve, this.CACHE_WARMING_DELAY_MS))
                            }
                        }
                    }
                    break
                ...
                case 'appointments':
                    if (request.staffIds && request.staffIds.length > 0) {
                        for (const staffId of request.staffIds) {
                            warmingPromises.push(
                                this.warmAppointmentCache(request.businessId, staffId, request.dateRange)
                            )
                        }
                    }
                    break
    ...
    static getCacheMetrics() {
        try {
            ...
            return {
                availability: { hitRate: 0, totalRequests: 0, totalHits: 0, totalMisses: 0, averageQueryTime: 0 },
                cache: { hitRate: 0, totalRequests: 0, totalHits: 0, totalMisses: 0, averageQueryTime: 0 },
                coordination: this.cacheMetrics
            }
        }
    }
    ...
    static async cleanupExpiredCache(): Promise<number> {
        try {
            const availabilityCleanup = await AvailabilityCalculator.cleanupExpiredCache()
    ...
    private static async withRetry<T>(
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                console.warn(`${operationName} attempt ${attempt} failed, retrying...`, lastError.message)
            }
        }
            ...
            for (const slot of availableSlots) {
                ...
                if (slotDuration >= requestedDuration) {
                    alternatives.push({
                        ...
                        endTime: new Date(slot.startTime.getTime() + requestedDuration * 60000),
     ...
     * Private helper method
     */
    private static validateAvailabilityRequest(request: AvailabilityCheckRequest): void {
     ...
     * Private helper method
     */
    private static validateDurationRequest(request: DurationValidationRequest): void {
        if (!Array.isArray(request.serviceIds)) {
            throw new Error('Service IDs must be an array')
        }
        if (!request.timeSlot) {
     ...
     * Private helper method
     */
    private static validateCacheInvalidationRequest(request: CacheInvalidationRequest): void {
        if (!request.businessId) {
            throw new Error('Business ID is required')
        }

        const validTypes = ['staff_availability', 'business_hours', 'appointments', 'client_appointments', 'appointment_details', 'all']
        if (!validTypes.includes(request.type)) {
            throw new Error('Invalid cache invalidation type')
        }

        if (request.type === 'staff_availability' && !request.staffId) {
            throw new Error('Staff ID is required for staff availability cache invalidation')
        }

        if (request.type === 'client_appointments' && !request.clientId) {
            throw new Error('Client ID is required for client appointments cache invalidation')
        }

        if (request.type === 'appointment_details' && !request.appointmentId) {
            throw new Error('Appointment ID is required for appointment details cache invalidation')
        }
    }
     ...
     * Private helper method
     */
    private static validateCacheWarmingRequest(request: CacheWarmingRequest): void {
        if (!request.businessId) {
            throw new Error('Business ID is required')
        }
        ...
        const validTypes = ['availability', 'appointments', 'client_history', 'all']
     ...
     * Private helper method
     */
    private static async invalidateAppointmentCache(cacheKey: string): Promise<void> {
        try {
            ...
            console.error(`Error invalidating appointment cache for key ${cacheKey}:`, error)
        }
    }
    ...
    private static async warmAppointmentCache(
        ...
        try {
            console.log(`Warming appointment cache for staff ${staffId} in business ${businessId}`)
            ...
            console.error(`Error warming appointment cache for staff ${staffId}:`, error)
        }
    }
     ...
     * Private helper method
     */
    private static async warmClientAppointmentCache(businessId: string, clientId: string): Promise<void> {
        try {
            console.log(`Warming client appointment cache for client ${clientId} in business ${businessId}`)
            ...
            console.error(`Error warming client appointment cache for client ${clientId}:`, error)
        }
    }
    ...
    private static updateAverageResponseTime(executionTime: number): void {
        const totalOperations = this.cacheMetrics.invalidationCount + this.cacheMetrics.warmingCount
        ...
        if (totalOperations === 1) {
            ...
            this.cacheMetrics.averageResponseTime =
                (this.cacheMetrics.averageResponseTime * (totalOperations - 1) + executionTime) / totalOperations
        }
    }
