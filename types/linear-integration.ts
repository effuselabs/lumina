...
export interface LinearIntegration {
    ...
    updateIssue(issueId: string, input: UpdateLinearIssueInput): Promise<LinearIssue>;
    ...
    getProjects(teamId?: string): Promise<Array<{ id: string; name: string; teamId: string }>>;
    getLabels(teamId?: string): Promise<Array<{ id: string; name: string; color: string }>>;
...
export interface DocumentationLinearIntegration extends LinearIntegration {
    ...
    addDecisionComment(issueId: string, decision: {
        title: string;
        context: string;
        decision: string;
        rationale: string;
    ...
    createMaintenanceIssue(type: 'broken-link' | 'stale-content' | 'migration', details: {
        title: string;
        description: string;
        filePath?: string;
        url?: string;
    ...
    syncDocumentationStatus(issueId: string, status: 'completed' | 'in-progress' | 'blocked'): Promise<void>;
    updateDocumentationReferences(issueId: string, references: string[]): Promise<void>;
}
...
export const DOCUMENTATION_ISSUE_TEMPLATES: Record<string, LinearIssueTemplate> = {
