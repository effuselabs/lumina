...
describe('Basic Analytics Data Quality Validation', () => {
  ...
  const mockRevenueData = [
    {
      ...
      commissionEarnings: 375.0,
      chairRentalRevenue: 200.0,
      businessRetention: 675.0,
    },
    {
      date: '2025-09-16',
      revenue: 980.0,
      ...
      commissionEarnings: 294.0,
      chairRentalRevenue: 150.0,
      businessRetention: 536.0,
    },
  ];
  ...
  describe('Revenue Data Validation', () => {
    ...
    test('should have realistic revenue patterns', () => {
      ...
      const totalAppointments = mockRevenueData.reduce(
    ...
    test('should calculate average ticket correctly', () => {
      mockRevenueData.forEach((day: any) => {
        if (day.appointments > 0) {
          ...
          expect(Math.abs(day.averageTicket - expectedAverage)).toBeLessThan(
            0.01
          );
        }
      });
  ...
  describe('Client Metrics Validation', () => {
    ...
    test('should provide meaningful top clients data', () => {
      ...
      for (let i = 1; i < mockClientMetrics.topClients.length; i++) {
        expect(
          mockClientMetrics.topClients[i - 1].totalSpent
        ).toBeGreaterThanOrEqual(mockClientMetrics.topClients[i].totalSpent);
      }
    });
  });
  ...
  describe('Staff Performance Validation', () => {
    ...
    test('should have realistic performance distributions', () => {
      const employmentTypes = [
        ...new Set(mockStaffPerformance.map((s: any) => s.employmentType)),
      ];
  ...
  describe('Service Analytics Validation', () => {
    ...
    test('should have proper popularity ranking', () => {
      const ranks = mockServiceAnalytics
  ...
  describe('Data Consistency Validation', () => {
    test('should have consistent revenue calculations', () => {
      const totalRevenueFromRevenue = mockRevenueData.reduce(
        (sum: any, day: any) => sum + day.revenue,
        0
      );
      const totalRevenueFromStaff = mockStaffPerformance.reduce(
        (sum: any, staff: any) => sum + staff.totalRevenue,
        0
      );
      const totalRevenueFromServices = mockServiceAnalytics.reduce(
      ...
      const revenues = [
        totalRevenueFromRevenue,
        totalRevenueFromStaff,
    ...
    test('should have consistent appointment counts', () => {
      const appointmentsFromRevenue = mockRevenueData.reduce(
        (sum: any, day: any) => sum + day.appointments,
        0
      );
      const appointmentsFromStaff = mockStaffPerformance.reduce(
    ...
    test('should have reasonable client data relationships', () => {
      ...
      expect(mockClientMetrics.topClients.length).toBeLessThanOrEqual(
      ...
      if (mockClientMetrics.totalClients > 0) {
        const calculatedRetentionRate =
          (mockClientMetrics.returningClients /
            mockClientMetrics.totalClients) *
          100;
        expect(
          Math.abs(
            mockClientMetrics.clientRetentionRate - calculatedRetentionRate
          )
        ).toBeLessThan(1);
      }
    });
