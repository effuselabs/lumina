...
describe('Analytics Calculation Accuracy', () => {
  ...
  describe('Revenue Calculation Accuracy', () => {
    test('should calculate daily revenue accurately against direct database queries', async () => {
      ...
      for (const dayData of revenueData) {
        ...
        const expectedAverageTicket =
          expectedAppointments > 0 ? expectedRevenue / expectedAppointments : 0;
        ...
        expect(
          Math.abs(dayData.averageTicket - expectedAverageTicket)
        ).toBeLessThan(0.01);
        ...
        expect(
          Math.abs(dayData.commissionEarnings - expectedCommissionEarnings)
        ).toBeLessThan(1);
        expect(
          Math.abs(dayData.chairRentalRevenue - expectedChairRentalRevenue)
        ).toBeLessThan(1);
        expect(
          Math.abs(dayData.businessRetention - expectedBusinessRetention)
        ).toBeLessThan(1);
      }
    });
    ...
    test('should calculate revenue metrics accurately', async () => {
      ...
      const expectedTotalRevenue = revenueData.reduce(
        (sum: any, day: any) => sum + day.revenue,
        0
      );
      const expectedAppointmentCount = revenueData.reduce(
        (sum: any, day: any) => sum + day.appointments,
        0
      );
      const expectedAverageTicket =
        expectedAppointmentCount > 0
          ? expectedTotalRevenue / expectedAppointmentCount
          : 0;

      // Validate metrics
      expect(
        Math.abs(revenueMetrics.totalRevenue - expectedTotalRevenue)
      ).toBeLessThan(0.01);
      expect(revenueMetrics.appointmentCount).toBe(expectedAppointmentCount);
      expect(
        Math.abs(revenueMetrics.averageTicket - expectedAverageTicket)
      ).toBeLessThan(0.01);

      // Validate growth calculations
      const periodLength =
        testDateRange.to.getTime() - testDateRange.from.getTime();
      const previousPeriodStart = new Date(
        testDateRange.from.getTime() - periodLength
      );
      const previousPeriodEnd = new Date(
        testDateRange.to.getTime() - periodLength
      );
      ...
      const previousTotalRevenue = previousRevenueData.reduce(
        (sum: any, day: any) => sum + day.revenue,
        0
      );
      const previousAppointmentCount = previousRevenueData.reduce(
        (sum: any, day: any) => sum + day.appointments,
        0
      );

      if (previousTotalRevenue > 0) {
        const expectedRevenueGrowth =
          ((expectedTotalRevenue - previousTotalRevenue) /
            previousTotalRevenue) *
          100;
        expect(
          Math.abs(revenueMetrics.revenueGrowth - expectedRevenueGrowth)
        ).toBeLessThan(0.1);
      }

      if (previousAppointmentCount > 0) {
        const expectedAppointmentGrowth =
          ((expectedAppointmentCount - previousAppointmentCount) /
            previousAppointmentCount) *
          100;
        expect(
          Math.abs(revenueMetrics.appointmentGrowth - expectedAppointmentGrowth)
        ).toBeLessThan(0.1);
      }
    });
  });
  ...
  describe('Client Metrics Calculation Accuracy', () => {
    test('should calculate client metrics accurately against direct queries', async () => {
      ...
      const expectedRetentionRate =
        totalClients > 0 ? (returningClients / totalClients) * 100 : 0;
      expect(
        Math.abs(clientMetrics.clientRetentionRate - expectedRetentionRate)
      ).toBeLessThan(0.1);
    });
    ...
    test('should calculate top clients accurately', async () => {
      ...
      expect(clientMetrics.topClients.length).toBe(
        Math.min(expectedTopClients.length, 10)
      );
      ...
      clientMetrics.topClients.forEach((client, index) => {
        ...
        expect(
          Math.abs(client.totalSpent - expectedClient.totalSpent)
        ).toBeLessThan(0.01);
    ...
    test('should calculate average lifetime value accurately', async () => {
      ...
      const totalSpentByAllClients = clients.reduce((sum: any, client: any) => {
        const clientSpent = client.appointments.reduce(
          (appointmentSum: any, appointment: any) => {
            return (
              appointmentSum +
              appointment.services.reduce(
                (serviceSum, service) => serviceSum + Number(service.price),
      ...
      const expectedAverageLTV =
        clients.length > 0 ? totalSpentByAllClients / clients.length : 0;

      expect(
        Math.abs(clientMetrics.averageLifetimeValue - expectedAverageLTV)
      ).toBeLessThan(0.01);
    });
  });
  ...
  describe('Staff Performance Calculation Accuracy', () => {
    test('should calculate staff performance accurately', async () => {
      ...
      for (const staffData of staffPerformance) {
        ...
        // Calculate expected values
        const expectedTotalRevenue = appointments.reduce(
          (sum: any, appointment: any) => {
            return (
              sum +
              appointment.services.reduce(
        ...
        // Validate calculations
        expect(
          Math.abs(staffData.totalRevenue - expectedTotalRevenue)
        ).toBeLessThan(0.01);
        expect(staffData.appointmentCount).toBe(expectedAppointmentCount);
        expect(
          Math.abs(staffData.averageTicket - expectedAverageTicket)
        ).toBeLessThan(0.01);
        expect(
          Math.abs(staffData.commissionEarnings - expectedCommissionEarnings)
        ).toBeLessThan(0.01);
        expect(
          Math.abs(staffData.chairRentalPaid - expectedChairRentalPaid)
        ).toBeLessThan(0.01);
      }
    });
  });
    ...
    test('should calculate service analytics accurately', async () => {
      ...
      for (const serviceData of serviceAnalytics) {
        ...
        const expectedAveragePrice =
          expectedBookingCount > 0 ? expectedRevenue / expectedBookingCount : 0;

        // If no bookings, use service base price
        const basePrice =
          serviceBookings.length > 0
            ? Number(serviceBookings[0].service.price)
            : 0;
        const finalExpectedAveragePrice =
          expectedBookingCount > 0 ? expectedAveragePrice : basePrice;

        // Validate calculations
        expect(serviceData.bookingCount).toBe(expectedBookingCount);
        expect(Math.abs(serviceData.revenue - expectedRevenue)).toBeLessThan(
          0.01
        );

        if (expectedBookingCount > 0) {
          expect(
            Math.abs(serviceData.averagePrice - finalExpectedAveragePrice)
          ).toBeLessThan(0.01);
        }
      }
    });
  ...
  describe('Cross-Calculation Consistency', () => {
    test('should have consistent revenue totals across all calculations', async () => {
      const [revenueData, staffPerformance, serviceAnalytics] =
        await Promise.all([
      ...
      const totalFromRevenue = revenueData.reduce(
        (sum: any, day: any) => sum + day.revenue,
        0
      );
      const totalFromStaff = staffPerformance.reduce(
        (sum: any, staff: any) => sum + staff.totalRevenue,
        0
      );
      const totalFromServices = serviceAnalytics.reduce(
    ...
    test('should maintain precision in financial calculations', async () => {
      ...
      staffPerformance.forEach((staff: any) => {
        ...
        if (staff.totalRevenue > 0 && staff.employmentType === 'COMMISSION') {
          ...
          expect(
            Math.abs(staff.commissionEarnings - expectedCommission)
          ).toBeLessThan(0.01);
        }
        ...
        if (staff.appointmentCount > 0) {
          ...
          expect(Math.abs(staff.averageTicket - expectedAverage)).toBeLessThan(
            0.01
          );
        }
      });
