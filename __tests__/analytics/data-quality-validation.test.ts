...
describe('Analytics and Reporting Data Quality Validation', () => {
        ...
        describe('Recent Activity Widget', () => {
            ...
            test('should be sorted by most recent first', () => {
                for (let i = 1; i < recentAppointments.length; i++) {
                    expect(recentAppointments[i - 1].startTime.getTime()).toBeGreaterThanOrEqual(
                        recentAppointments[i].startTime.getTime()
                    );
                }
            });
            ...
            test('should have realistic appointment data', () => {
                recentAppointments.forEach(appointment => {
                    ...
                    expect(['SCHEDULED', 'COMPLETED', 'CANCELLED', 'NO_SHOW']).toContain(appointment.status);
                });
            });
        });
    });
    ...
    describe('Data Consistency and Integrity', () => {
        test('should have consistent data across different widgets', async () => {
            ...
            const [revenueData, clientMetrics, staffPerformance, serviceAnalytics] = await Promise.all([
            ...
            const minRevenue = Math.min(revenueFromRevenueWidget, revenueFromStaffWidget, revenueFromServiceWidget);
