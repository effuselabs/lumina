...
import {
  GET as getStatusInfo,
  PUT as updateStatus,
} from '@/app/api/appointments/[id]/status/route';
...
describe('Appointment Management API', () => {
  ...
  describe('PUT /api/appointments/[id]/status - Update Status', () => {
    ...
    it('should return 404 when appointment not found', async () => {
      ...
      const request = new NextRequest(
        'http://localhost/api/appointments/nonexistent/status?businessId=business-123',
        {
          method: 'PUT',
          body: JSON.stringify(statusUpdateData),
        }
  ...
  describe('GET /api/appointments/[id]/status - Get Status Info', () => {
    it('should return status information successfully', async () => {
      ...
      const response = await getStatusInfo(request, {
        params: { id: 'appointment-123' },
      });
      const data = await response.json();
      ...
      expect(mockStatusManager.getValidTransitions).toHaveBeenCalledWith(
