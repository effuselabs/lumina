...
import { POST as addServices, GET as getServices, DELETE as removeServices } from '@/app/api/appointments/[id]/services/route'
import { POST as checkConflicts, GET as getConflicts } from '@/app/api/appointments/conflicts/route'
...
describe('Advanced Appointment API Endpoints', () => {
    ...
    describe('POST /api/appointments/[id]/services - Add Services', () => {
        ...
        it('should add services to appointment successfully', async () => {
            ...
            const request = new NextRequest('http://localhost/api/appointments/appointment-123/services?businessId=business-123', {
                method: 'POST',
                body: JSON.stringify(addServicesData)
            })

            const response = await addServices(request, { params: { id: 'appointment-123' } })
        ...
        it('should return 404 when appointment not found', async () => {
            ...
            const request = new NextRequest('http://localhost/api/appointments/nonexistent/services?businessId=business-123', {
                method: 'POST',
                body: JSON.stringify(addServicesData)
            })

            const response = await addServices(request, { params: { id: 'nonexistent' } })
            const data = await response.json()
        ...
        it('should return 400 when services not found in business', async () => {
            ...
            expect(data.error).toBe('One or more services not found or not active in this business')
        })
    })
    ...
    describe('DELETE /api/appointments/[id]/services - Remove Services', () => {
        ...
        it('should remove services from appointment successfully', async () => {
            ...
            mockAppointmentService.getAppointmentById.mockResolvedValue(appointmentWithMultipleServices)
            ...
            const response = await removeServices(request, { params: { id: 'appointment-123' } })
            const data = await response.json()
        ...
        it('should return 400 when trying to remove all services', async () => {
            ...
            expect(response.status).toBe(400)
            expect(data.error).toBe('Cannot remove all services from appointment. At least one service is required.')
        })
    })
    ...
    describe('GET /api/appointments/[id]/services - Get Services', () => {
        it('should return appointment services successfully', async () => {
            ...
            const response = await getServices(request, { params: { id: 'appointment-123' } })
            const data = await response.json()
    ...
    describe('GET /api/appointments/conflicts - Check Conflicts', () => {
        it('should check conflicts successfully', async () => {
            ...
            mockCalendarIntegration.detectConflicts.mockResolvedValue(mockConflictResult)

            const url = new URL('http://localhost/api/appointments/conflicts?businessId=business-123&staffId=staff-123&startDate=2024-12-01T10:00:00Z&endDate=2024-12-01T11:00:00Z')
    ...
    describe('POST /api/appointments/conflicts - Check Specific Conflicts', () => {
        ...
        it('should check conflicts for specific appointment data', async () => {
            ...
            mockCalendarIntegration.detectConflicts.mockResolvedValue(mockConflictResult)
            mockCalendarIntegration.checkAvailability.mockResolvedValue(mockAvailabilityResult)

            const request = new NextRequest('http://localhost/api/appointments/conflicts', {
                method: 'POST',
        ...
        it('should return 400 when staff not found', async () => {
            ...
            expect(response.status).toBe(400)
            expect(data.error).toBe('Staff member not found or not active in this business')
        })
    })
    ...
    describe('POST /api/appointments/validate - Validate Appointment', () => {
        ...
        it('should validate appointment successfully', async () => {
            ...
            const request = new NextRequest('http://localhost/api/appointments/validate', {
                method: 'POST',
