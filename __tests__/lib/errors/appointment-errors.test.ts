...
describe('AppointmentError', () => {
    describe('Base AppointmentError class', () => {
        it('should create appointment error with all properties', () => {
            ...
            expect(error.context).toEqual({ appointmentId: 'apt-123', businessId: 'biz-456' })
        })
    ...
    describe('AppointmentNotFoundError', () => {
        ...
        it('should include suggested alternatives', () => {
            ...
            const error = new AppointmentNotFoundError('apt-123', 'biz-456', alternatives)

            expect(error.suggestedAlternatives).toHaveLength(1)
            expect(error.suggestedAlternatives[0].title).toBe('View other appointments')
        })
    })
    ...
    describe('InvalidStatusTransitionError', () => {
        it('should create error with transition details', () => {
            ...
            expect(error.userMessage).toContain('Valid transitions from COMPLETED: CANCELLED')
        ...
        it('should handle empty valid transitions', () => {
            ...
            expect(error.userMessage).toContain('No valid transitions available from COMPLETED')
        })
    })
    ...
    describe('MultiServiceBookingError', () => {
        ...
        it('should create staff skill mismatch error', () => {
            ...
            expect(error.userMessage).toContain('not qualified to perform all requested services')
        })
        ...
        it('should create duration exceeded error', () => {
            ...
            expect(error.userMessage).toContain('exceeds the maximum appointment length of 4 hours')
        })
    ...
    describe('PaymentRequiredError', () => {
        ...
        it('should create deposit payment error', () => {
            const error = new PaymentRequiredError('apt-123', 'confirm', paymentDetails)
        ...
        it('should create full payment error', () => {
            const fullPaymentDetails = { ...paymentDetails, paymentType: 'full_payment' as const }
            const error = new PaymentRequiredError('apt-123', 'complete', fullPaymentDetails)
        ...
        it('should create cancellation fee error', () => {
            const feeDetails = { ...paymentDetails, paymentType: 'cancellation_fee' as const }
            const error = new PaymentRequiredError('apt-123', 'reschedule', feeDetails)
    ...
    describe('AppointmentAlreadyExistsError', () => {
        it('should create error with time formatting', () => {
            ...
            expect(error.userMessage).toContain('already have an appointment scheduled')
    ...
    describe('AppointmentAlreadyCompletedError', () => {
        it('should create error with completion time', () => {
            ...
            const error = new AppointmentAlreadyCompletedError('apt-123', completedAt)
    ...
    describe('AppointmentCancellationError', () => {
        it('should create too late cancellation error', () => {
            ...
            expect(error.userMessage).toContain('cancelled at least 24 hours in advance')
        })

        it('should create already started error', () => {
            const error = new AppointmentCancellationError('apt-123', 'already_started')

            expect(error.userMessage).toContain('already started and cannot be cancelled')
        })

        it('should create payment required error', () => {
            const error = new AppointmentCancellationError('apt-123', 'payment_required')
    ...
    describe('PastAppointmentModificationError', () => {
        ...
        it('should create update error', () => {
            const error = new PastAppointmentModificationError('apt-123', pastTime, 'update')
        ...
        it('should create cancel error', () => {
            const error = new PastAppointmentModificationError('apt-123', pastTime, 'cancel')

            expect(error.userMessage).toContain('Cannot cancel past appointments')
        })

        it('should create reschedule error', () => {
            const error = new PastAppointmentModificationError('apt-123', pastTime, 'reschedule')

            expect(error.userMessage).toContain('Cannot reschedule past appointments')
        })
    })
    ...
    describe('InsufficientNoticeError', () => {
        it('should create cancellation notice error', () => {
            ...
            expect(error.userMessage).toContain('Cancellations require at least 24 hours notice')
            expect(error.userMessage).toContain('You have 12 hours remaining')
        })

        it('should create reschedule notice error', () => {
            const error = new InsufficientNoticeError('apt-123', 'reschedule', 48, 36)

            expect(error.userMessage).toContain('Rescheduling require at least 48 hours notice')
        })

        it('should create modify notice error', () => {
            const error = new InsufficientNoticeError('apt-123', 'modify', 12, 6)

            expect(error.userMessage).toContain('Modifications require at least 12 hours notice')
        })
    })
...
describe('AppointmentErrorFactory', () => {
    describe('createAppointmentNotFoundError', () => {
        it('should create error with alternative appointments', () => {
            ...
            expect(error.suggestedAlternatives).toHaveLength(1)
            expect(error.suggestedAlternatives[0].title).toContain('Haircut appointment')
            expect(error.suggestedAlternatives[0].data?.actionData.appointmentId).toBe('apt-456')
        })

        it('should create error without alternatives', () => {
            const error = AppointmentErrorFactory.createAppointmentNotFoundError('apt-123', 'biz-456')
...
describe('Type guards', () => {
    ...
    it('should identify specific error types correctly', () => {
        ...
        const statusError = new InvalidStatusTransitionError('apt-123', 'COMPLETED', 'SCHEDULED', [])
        const multiServiceError = new MultiServiceBookingError('service_incompatibility', [], [], {})
        const paymentError = new PaymentRequiredError('apt-123', 'confirm', {
            amountDue: 1000,
            currency: 'USD',
            paymentType: 'deposit'
        })
        const calendarError = new CalendarIntegrationFailureError('check_availability', 'calendar_service')
...
describe('Error severity', () => {
    it('should assign correct severity levels', () => {
        const dbError = new DatabaseConstraintViolationError('create', 'constraint', {})
        const calendarError = new CalendarIntegrationFailureError('check', 'service')
        const notFoundError = new AppointmentNotFoundError('apt-123', 'biz-456')
        const statusError = new InvalidStatusTransitionError('apt-123', 'COMPLETED', 'SCHEDULED', [])
        ...
        expect(getAppointmentErrorSeverity(notFoundError)).toBe(ErrorSeverity.MEDIUM)
