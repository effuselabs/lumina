...
describe('StaffUnavailableError', () => {
    it('should create staff unavailable error for time off', () => {
        ...
        const error = new StaffUnavailableError(staffName, requestedTime, 'time_off')
    ...
    it('should create staff unavailable error for not scheduled', () => {
        ...
        const error = new StaffUnavailableError(staffName, requestedTime, 'not_scheduled')
...
describe('SchedulingConflictError', () => {
    it('should create scheduling conflict error with existing appointment', () => {
        ...
        const error = new SchedulingConflictError('overlapping_appointment', conflictDetails)

        expect(error.code).toBe('SCHEDULING_CONFLICT')
        expect(error.userMessage).toContain('conflicts with an existing appointment')
...
describe('InsufficientDurationError', () => {
    it('should create insufficient duration error with service details', () => {
        const error = new InsufficientDurationError(120, 60, ['Haircut', 'Beard Trim'])
...
describe('InvalidTimeSlotError', () => {
    ...
    it('should create end before start error', () => {
        ...
        expect(error.userMessage).toContain('end time cannot be before the start time')
    })
...
describe('CacheFailureError', () => {
    it('should create cache failure error with fallback', () => {
        ...
        expect(error.userMessage).toContain('delays, but your request is being processed')
...
describe('Error Severity', () => {
    it('should return correct severity for different error types', () => {
        expect(getErrorSeverity(new BusinessContextMissingError('test'))).toBe(ErrorSeverity.CRITICAL)
        expect(getErrorSeverity(new CacheFailureError('test'))).toBe(ErrorSeverity.MEDIUM)
        expect(getErrorSeverity(new BusinessClosedError(new Date()))).toBe(ErrorSeverity.LOW)
        expect(getErrorSeverity(new InvalidTimeSlotError('past_time'))).toBe(ErrorSeverity.LOW)
    })
})
...
describe('Type Guards', () => {
    ...
    it('should correctly identify specific error types', () => {
        ...
        const staffUnavailableError = new StaffUnavailableError('John', new Date(), 'time_off')
