...
describe('Dashboard Integration Service', () => {
  ...
  describe('Appointment Booking Engine Integration (LUM-97)', () => {
    it('should get dashboard appointments with enhanced data', async () => {
      ...
      const result =
        await dashboardIntegration.getDashboardAppointments(mockBusinessId);
  ...
  describe('Client Management Integration', () => {
    it('should get client data for appointments', async () => {
      ...
      const result = await dashboardIntegration.getClientData(
        clientId,
        mockBusinessId
      );
  ...
  describe('Service Management Integration', () => {
    it('should get services for dashboard', async () => {
      const result =
        await dashboardIntegration.getServicesForDashboard(mockBusinessId);
  ...
  describe('Staff Management Integration', () => {
    it('should get staff data for dashboard', async () => {
      const result =
        await dashboardIntegration.getStaffForDashboard(mockBusinessId);
...
describe('Appointment Notification Service', () => {
  ...
  describe('Notification System Integration', () => {
    ...
    it('should send real-time WebSocket notifications', async () => {
      ...
      await notificationService.sendRealTimeNotifications(
        mockNotificationData,
        result
      );
      ...
      expect(
        result.sentChannels.includes('realtime') ||
          result.failedChannels.includes('realtime')
      ).toBe(true);
    });
    ...
    it('should handle different notification types', async () => {
      ...
      for (const changeType of notificationTypes) {
        ...
        const result =
          await notificationService.sendAppointmentNotifications(testData);
...
describe('System Integration Manager', () => {
  ...
  describe('System Health Monitoring', () => {
    ...
    it('should check system health', async () => {
      ...
      expect(['healthy', 'degraded', 'critical'].includes(health.overall)).toBe(
        true
      );
      ...
      for (const system of expectedSystems) {
        ...
        expect(
          ['healthy', 'degraded', 'critical', 'offline'].includes(
            health.systems[system].status
          )
        ).toBe(true);
...
describe('Integration Performance Tests', () => {
  ...
  it('should handle concurrent appointment operations', async () => {
    ...
    const operations = Array(concurrentOperations)
      .fill(null)
      .map((_, index) =>
        integrationManager.createAppointment('test-business-id', {
          ...appointmentData,
