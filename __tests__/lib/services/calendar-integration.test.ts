...
>;
const mockConflictDetectionEngine = ConflictDetectionEngine as jest.Mocked<
  typeof ConflictDetectionEngine
>;
const mockServiceDurationValidator = ServiceDurationValidator as jest.Mocked<
  typeof ServiceDurationValidator
>;
const mockAvailabilityCache = AvailabilityCache as jest.Mocked<
  typeof AvailabilityCache
>;

// Add missing mock methods
mockAvailabilityCalculator.batchCalculateAndCache = jest
  .fn()
  .mockResolvedValue(undefined);
mockAvailabilityCalculator.invalidateStaffAvailabilityCache = jest
  .fn()
  .mockResolvedValue(undefined);
mockAvailabilityCalculator.invalidateBusinessHoursCache = jest
  .fn()
  .mockResolvedValue(undefined);
mockAvailabilityCalculator.cleanupExpiredCache = jest.fn().mockResolvedValue(0);

mockAvailabilityCache.invalidateStaffAvailability = jest
  .fn()
  .mockResolvedValue(undefined);
mockAvailabilityCache.invalidateBusinessHours = jest
  .fn()
  .mockResolvedValue(undefined);
...
describe('CalendarIntegration', () => {
  ...
  describe('checkAvailability', () => {
    ...
    it('should successfully check availability when slot is available', async () => {
      ...
      const result =
        await CalendarIntegration.checkAvailability(availabilityRequest);
  ...
  describe('validateServiceDuration', () => {
    ...
    it('should validate single service duration successfully', async () => {
      ...
      const result =
        await CalendarIntegration.validateServiceDuration(durationRequest);
  ...
  describe('invalidateAvailabilityCache', () => {
    it('should invalidate staff availability cache', async () => {
      ...
      expect(
        mockAvailabilityCache.invalidateStaffAvailability
      ).toHaveBeenCalledWith(staffId, businessId);
      expect(
        mockAvailabilityCalculator.invalidateStaffAvailabilityCache
      ).toHaveBeenCalledWith(staffId, businessId);
    });
    ...
    it('should invalidate business hours cache', async () => {
      ...
      expect(
        mockAvailabilityCache.invalidateBusinessHours
      ).toHaveBeenCalledWith(businessId);
      expect(
        mockAvailabilityCalculator.invalidateBusinessHoursCache
      ).toHaveBeenCalledWith(businessId);
    });
    ...
    it('should handle cache invalidation failures gracefully', async () => {
      ...
      await expect(
        CalendarIntegration.invalidateAvailabilityCache(request)
    ...
    it('should update cache metrics on invalidation', async () => {
      ...
      expect(updatedMetrics.coordination.invalidationCount).toBe(
        initialCount + 1
      );
  ...
  describe('warmCache', () => {
    ...
    it('should validate cache warming request parameters', async () => {
      ...
      await expect(
        CalendarIntegration.warmCache(invalidRequest)
      ).resolves.toBeUndefined();
    });
  });
  ...
  describe('cache metrics and management', () => {
    ...
    it('should return comprehensive cache metrics', () => {
      ...
      mockAvailabilityCalculator.getCacheMetrics.mockReturnValue(
        availabilityMetrics
      );
