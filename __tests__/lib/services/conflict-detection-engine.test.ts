...
describe('ConflictDetectionEngine', () => {
  ...
  describe('detectConflicts', () => {
    ...
    it('should detect no conflicts for valid appointment', async () => {
      ...
      const conflicts =
        await ConflictDetectionEngine.detectConflicts(baseRequest);
    ...
    it('should detect staff unavailability', async () => {
      ...
      const staffUnavailableConflicts = conflicts.filter(
        (c: any) => c.type === ConflictType.STAFF_UNAVAILABLE
      );
      expect(staffUnavailableConflicts).toHaveLength(1);
      expect(staffUnavailableConflicts[0].severity).toBe(
    ...
    it('should detect insufficient duration conflict', async () => {
      ...
      const durationConflicts = conflicts.filter(
        (c: any) => c.type === ConflictType.INSUFFICIENT_DURATION
      );
    ...
    it('should invalidate appointment slot with conflicts', async () => {
      ...
      expect(
        result.conflicts.some(
          (c: any) => c.type === ConflictType.BUSINESS_HOURS_VIOLATION
        )
      ).toBe(true);
    });
  });
  ...
  describe('checkOverlappingAppointments', () => {
    ...
    it('should detect precise time overlap', async () => {
      ...
      const conflicts =
        await ConflictDetectionEngine.checkOverlappingAppointments(request);

      expect(conflicts).toHaveLength(1);
      expect(conflicts[0].type).toBe(ConflictType.OVERLAPPING_APPOINTMENT);
      expect(conflicts[0].message).toContain('Jane Smith');
      expect(conflicts[0].details.conflictingAppointment).toBeDefined();
      expect(conflicts[0].details.conflictingAppointment?.services).toContain(
        'Manicure'
      );
    });
    ...
    it('should exclude appointment being rescheduled', async () => {
      ...
      const conflicts =
        await ConflictDetectionEngine.checkOverlappingAppointments(
  ...
  describe('validateAppointmentSlotAdvanced', () => {
    ...
    it('should perform comprehensive validation with all checks enabled', async () => {
      ...
      const result =
        await ConflictDetectionEngine.validateAppointmentSlotAdvanced(request, {
          checkTimeOff: true,
    ...
    it('should generate warnings for scheduling issues', async () => {
      ...
      mockPrisma.appointment.findMany
        ...
        .mockResolvedValueOnce([
          {
            // For adjacent appointment check
      ...
      expect(
        result.warnings.some((w: any) => w.message.includes('Back-to-back'))
      ).toBe(true);
    });
  });
  ...
  describe('checkOverlappingAppointmentsAdvanced', () => {
    ...
    it('should detect conflicts with buffer time consideration', async () => {
      ...
      const conflicts =
        await ConflictDetectionEngine.checkOverlappingAppointmentsAdvanced(
          request,
          10 // 10-minute buffer
        );
  ...
  describe('Error Handling', () => {
    it('should handle database errors gracefully', async () => {
      ...
      await expect(
        ConflictDetectionEngine.detectConflicts(request)
