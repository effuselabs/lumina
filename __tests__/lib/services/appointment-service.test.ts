...
import { AppointmentService, CreateAppointmentServiceRequest, UpdateAppointmentServiceRequest } from '@/lib/services/appointment-service'
...
describe('AppointmentService', () => {
    ...
    beforeEach(() => {
        ...
        mockCalendarIntegration = {
            ...
            ; (appointmentService as any).multiServiceCoordinator = mockMultiServiceCoordinator

        // Mock the CalendarIntegration static methods
        const CalendarIntegration = require('@/lib/services/calendar-integration').CalendarIntegration
    ...
    describe('createAppointment', () => {
        ...
        it('should create appointment successfully with valid data', async () => {
            ...
            mockCalendarIntegration.checkAvailability.mockResolvedValue({
                ...
                conflicts: [],
                warnings: [],
                metadata: { calculationTime: 100, cacheHit: false, validationResults: [] }
            })
            ...
            mockMultiServiceCoordinator.calculateTotalPrice.mockResolvedValue(new Decimal(70))
            mockRepository.create.mockResolvedValue(mockAppointment)
            mockCalendarIntegration.invalidateAvailabilityCache.mockResolvedValue(undefined)
            ...
            expect(mockMultiServiceCoordinator.validateMultiServiceBooking).toHaveBeenCalledWith(
                mockServices,
            ...
            expect(mockCalendarIntegration.validateServiceDuration).toHaveBeenCalled()
            expect(mockRepository.create).toHaveBeenCalled()
            expect(mockCalendarIntegration.invalidateAvailabilityCache).toHaveBeenCalled()
        })
        ...
        it('should fail when time slot is not available', async () => {
            ...
            mockCalendarIntegration.checkAvailability.mockResolvedValue({
                isAvailable: false,
                conflicts: [{ type: 'STAFF_UNAVAILABLE', severity: 'ERROR', message: 'Staff not available', details: {} }],
                warnings: [],
                alternatives: [
                    { startTime: new Date('2024-01-15T14:00:00Z'), endTime: new Date('2024-01-15T15:30:00Z'), staffId: mockStaffId }
                ],
                metadata: { calculationTime: 100, cacheHit: false, validationResults: [] }
            })
            ...
            expect(result.warnings).toContain('Alternative times available: 1 options')
            expect(mockRepository.create).not.toHaveBeenCalled()
        })
    })
    ...
    describe('updateAppointment', () => {
        ...
        it('should update appointment successfully', async () => {
            ...
            // Execute
            const result = await appointmentService.updateAppointment(mockAppointmentId, mockBusinessId, updateRequest)
            ...
            expect(mockRepository.findById).toHaveBeenCalledWith(mockAppointmentId, mockBusinessId)
    ...
    describe('getAppointments', () => {
        ...
        it('should get appointments with basic filtering', async () => {
            ...
            expect(mockRepository.findByBusiness).toHaveBeenCalledWith(mockBusinessId, {
                status: AppointmentStatus.SCHEDULED,
                limit: 10
            })
        })
    ...
    describe('getAppointmentById', () => {
        it('should get appointment by ID successfully', async () => {
            ...
            // Execute
            const result = await appointmentService.getAppointmentById(mockAppointmentId, mockBusinessId)
    ...
    describe('cancelAppointment', () => {
        it('should cancel appointment successfully', async () => {
            ...
            // Execute
            const result = await appointmentService.cancelAppointment(mockAppointmentId, mockBusinessId, {
            ...
            expect(mockStatusManager.validateStatusUpdateWithBusinessRules).toHaveBeenCalledWith(
                mockAppointmentId,
    ...
    describe('error handling', () => {
        it('should handle calendar integration failures gracefully', async () => {
            ...
            mockCalendarIntegration.checkAvailability.mockRejectedValue(new Error('Calendar service unavailable'))
            ...
            // Verify
            expect(result.success).toBe(false)
            expect(result.errors).toContain('Failed to create appointment: Calendar service unavailable')
        })
        ...
        it('should handle cache invalidation failures silently', async () => {
            ...
            mockCalendarIntegration.invalidateAvailabilityCache.mockRejectedValue(new Error('Cache service unavailable'))

            const consoleSpy = jest.spyOn(console, 'error').mockImplementation()

            // Execute
            const result = await appointmentService.updateAppointment(mockAppointmentId, mockBusinessId, {
                notes: 'Updated notes'
            })

            // Verify
            expect(result.success).toBe(true) // Should still succeed
            expect(consoleSpy).toHaveBeenCalledWith('Failed to invalidate caches:', expect.any(Error))
