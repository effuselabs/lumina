...
import {
  MultiServiceAppointmentService,
  ServiceSelection,
} from '../../../lib/services/multi-service-appointment';
...
describe('MultiServiceAppointmentService', () => {
  ...
  describe('validateMultiServiceAppointment', () => {
    ...
    it('should fail validation when services do not exist', async () => {
      ...
      expect(result.errors).toContain(
        'Services not found or inactive: service-2'
      );
    });
    ...
    it('should fail validation when staff cannot perform services', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Staff member cannot perform services: service-2'
      );
    });
    ...
    it('should detect circular dependencies', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Circular dependency detected in service ordering'
      );
    });
  });
  ...
  describe('getQualifiedStaff', () => {
    it('should return staff who can perform all services', async () => {
      ...
      const result = await service.getQualifiedStaff(
        mockBusinessId,
        serviceIds
      );
  ...
  describe('validateServiceSequence', () => {
    ...
    it('should detect duplicate service orders', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Duplicate service order numbers detected'
      );
    });
    ...
    it('should detect gaps in service order', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Gaps in service order sequence detected'
      );
    });
    ...
    it('should detect missing dependencies', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Service Styling depends on service service-2 which is not in the appointment'
      );
    });
  });
