...
describe('Daily Status Decision Tracker', () => {
    ...
    describe('Decision ID Generation', () => {
        ...
        test('handles special characters in titles', () => {
            ...
            expect(id).toMatch(/^DEC-\d{4}-\d{2}-\d{2}-api-endpoint-usersid-implement/);
        });
    });
    ...
    describe('Decision Formatting', () => {
        test('formats decision for markdown correctly', () => {
            ...
            expect(markdown).toContain('- **Alternatives Considered**: Plain text, JSON format');
...
`;
            ...
            const date = new Date('2025-01-10T12:00:00.000Z');
            const decisions = (tracker as any).extractDecisionsFromContent(content, date);
...
describe('Daily Status Blocker Tracker', () => {
    ...
    describe('Blocker Formatting', () => {
        test('formats blocker for markdown correctly', () => {
            ...
            expect(markdown).toContain('- **Description**: Users experiencing 30 second timeouts');
            expect(markdown).toContain('- **Impact**: Blocks user workflow completely');
            expect(markdown).toContain('- **Assignee**: John Doe');
            expect(markdown).toContain('- **Next Steps**: Investigate timeout causes, Implement retry logic');
            expect(markdown).toMatch(/- \*\*Created\*\*: January \d{1,2}, 2025/);
            expect(markdown).toMatch(/- \*\*Target Resolution\*\*: January \d{1,2}, 2025/);
    ...
    describe('Blocker Metrics Calculation', () => {
        test('calculates blocker metrics correctly', async () => {
            ...
            jest.spyOn(tracker, 'getAllBlockers').mockResolvedValue(mockBlockers as any);
...
describe('Integration Tests', () => {
    test('decision and blocker IDs do not conflict', () => {
        ...
        const decisionId = (decisionTracker as any).generateDecisionId(date, 'Test Decision');
