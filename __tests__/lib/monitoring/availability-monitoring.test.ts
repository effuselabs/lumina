...
describe('AvailabilityMonitoring', () => {
  ...
  describe('Performance Monitoring', () => {
    ...
    it('should generate alert for slow availability queries', () => {
      ...
      const performanceAlert = alerts.find(
        (alert: any) => alert.type === AlertType.PERFORMANCE_DEGRADATION
      );
    ...
    it('should generate alert for slow conflict detection', () => {
      ...
      const alerts = availabilityMonitoring.getRecentAlerts(10);
      const conflictAlert = alerts.find(
        (alert: any) => alert.type === AlertType.CONFLICT_DETECTION_SLOW
      );
  ...
  describe('Cache Monitoring', () => {
    ...
    it('should generate alert for low cache hit ratio', () => {
      ...
      const cacheAlert = alerts.find(
        (alert: any) => alert.type === AlertType.CACHE_HIT_RATIO_LOW
      );
  ...
  describe('Business Metrics', () => {
    ...
    it('should generate alert for high conflict detection rate', () => {
      ...
      const alerts = availabilityMonitoring.getRecentAlerts(10);
      const anomalyAlert = alerts.find(
        (alert: any) => alert.type === AlertType.BUSINESS_ANOMALY
      );
  ...
  describe('Alert Management', () => {
    it('should filter alerts by type', () => {
      ...
      const performanceAlerts = availabilityMonitoring.getAlertsByType(
        AlertType.PERFORMANCE_DEGRADATION
      );
      expect(performanceAlerts.length).toBeGreaterThan(0);
      expect(
        performanceAlerts.every(
          (alert: any) => alert.type === AlertType.PERFORMANCE_DEGRADATION
        )
      ).toBe(true);
    });
    ...
    it('should filter alerts by business', () => {
      ...
      const business1Alerts =
        availabilityMonitoring.getAlertsByBusiness(businessId1);
      const business2Alerts =
        availabilityMonitoring.getAlertsByBusiness(businessId2);

      expect(
        business1Alerts.every((alert: any) => alert.businessId === businessId1)
      ).toBe(true);
      expect(
        business2Alerts.every((alert: any) => alert.businessId === businessId2)
      ).toBe(true);
    });
    ...
    it('should detect high error rates', done => {
      ...
      setTimeout(() => {
        ...
        const errorRateAlert = alerts.find(
          (alert: any) => alert.type === AlertType.HIGH_ERROR_RATE
        );
  ...
  describe('System Overload Detection', () => {
    it('should detect system overload conditions', done => {
      ...
      setTimeout(() => {
        const alerts = availabilityMonitoring.getRecentAlerts(10);
        const overloadAlert = alerts.find(
          (alert: any) => alert.type === AlertType.SYSTEM_OVERLOAD
        );
