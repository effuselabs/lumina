...
describe('AppointmentRepository', () => {
  ...
  describe('delete', () => {
    it('should delete appointment with business validation', async () => {
      ...
      // Mock delete
      asMock(mockPrisma.appointment.delete).mockResolvedValue({
        id: 'appointment-123',
      } as any);
  ...
  describe('addServices', () => {
    ...
    it('should add services to an existing appointment', async () => {
      // Mock findById
      asMock(mockPrisma.appointment.findFirst).mockResolvedValue(
        mockExistingAppointment as any
      );
      ...
      asMock(mockPrisma.appointmentService.createMany).mockResolvedValue({
        count: 1,
      } as any);
      // Mock update
      asMock(mockPrisma.appointment.update).mockResolvedValue({
      ...
      const result = await repository.addServices(
        'appointment-123',
        mockBusinessId,
        newServices
      );
    ...
    it('should throw error when appointment not found', async () => {
      ...
      await expect(
        repository.addServices('nonexistent', mockBusinessId, newServices)
  ...
  describe('removeServices', () => {
    ...
    it('should remove services from an existing appointment', async () => {
      ...
      asMock(mockPrisma.appointmentService.deleteMany).mockResolvedValue({
        count: 1,
      } as any);
      ...
      const result = await repository.removeServices(
        'appointment-123',
        mockBusinessId,
        ['service-2']
      );
    ...
    it('should throw error when trying to remove all services', async () => {
      ...
      asMock(mockPrisma.appointment.findFirst).mockResolvedValue(
        singleServiceAppointment as any
      );

      await expect(
        repository.removeServices('appointment-123', mockBusinessId, [
          'service-1',
        ])
  ...
  describe('reorderServices', () => {
    ...
    it('should reorder services within an appointment', async () => {
      ...
      // Mock service reordering
      asMock(mockPrisma.appointmentService.updateMany).mockResolvedValue({
        count: 1,
      } as any);
      // Mock appointment update
      asMock(mockPrisma.appointment.update).mockResolvedValue(
      ...
      const result = await repository.reorderServices(
        'appointment-123',
        mockBusinessId,
        serviceOrders
      );
