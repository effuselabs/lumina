...
describe('StaffAvailabilityRepository', () => {
    ...
    describe('setRecurringAvailability', () => {
        it('should replace existing recurring availability', async () => {
            ...
            const mockCreatedAvailability: StaffAvailability[] = recurringPattern.map((pattern, index) => ({
                id: `avail-${index + 1}`,
                staffId,
                businessId,
                ...pattern,
            ...
            const result = await repository.setRecurringAvailability(staffId, businessId, recurringPattern)
    ...
    describe('error handling', () => {
        ...
        it('should validate required parameters', async () => {
            await expect(repository.getStaffAvailability('', new Date(), new Date()))
                .rejects.toThrow()
        })
        ...
        it('should validate time ranges', async () => {
            ...
            await expect(repository.getStaffAvailability(staffId, invalidStartDate, invalidEndDate))
                .rejects.toThrow()
        })
    })
})