...
describe('StaffAvailabilityRepository', () => {
  ...
  describe('setAvailability', () => {
    it('should create new staff availability', async () => {
      ...
      const result = await repository.setAvailability(
        staffId,
        availabilityData
      );
    ...
    it('should throw error when staff not found', async () => {
      ...
      await expect(
        repository.setAvailability(staffId, availabilityData)
  ...
  describe('getAvailability', () => {
    ...
    it('should throw error when staff not found', async () => {
      ...
      await expect(
        repository.getAvailability(staffId, dateRange)
      ).rejects.toThrow('Staff member not found');
    });
  });
  ...
  describe('setRecurringAvailability', () => {
    it('should replace existing recurring availability', async () => {
      ...
      const mockStaff = { id: staffId, businessId };
      const mockCreatedAvailability: StaffAvailability[] =
        recurringPattern.patterns.map((pattern, index) => ({
          id: `avail-${index + 1}`,
          staffId,
          businessId,
          ...pattern,
  ...
  describe('isStaffAvailable', () => {
    it('should return true when staff is available', async () => {
      ...
      const result = await repository.isStaffAvailable(
        staffId,
        dateTime,
        serviceDuration
      );
  ...
  describe('detectAvailabilityConflicts', () => {
    it('should detect conflicts with appointments', async () => {
      ...
      const result = await repository.detectAvailabilityConflicts(
        staffId,
        dateRange
      );
  ...
  describe('updateRecurringAvailability', () => {
    it('should update recurring availability for specific day', async () => {
      ...
      asMock(mockPrisma.staffAvailability.create).mockResolvedValue(
        mockUpdatedAvailability[0]
      );

      const result = await repository.updateRecurringAvailability(
        staffId,
        dayOfWeek,
        startTime,
        endTime
  ...
  describe('deleteAvailabilityOverride', () => {
    it('should delete availability override', async () => {
      ...
      asMock(mockPrisma.staffAvailabilityOverride.delete).mockResolvedValue(
        {} as any
      );
  ...
  describe('error handling', () => {
    ...
    it('should validate required parameters', async () => {
      ...
      await expect(
        repository.setAvailability('', availabilityData)
      ).rejects.toThrow();
    });
  });
});
