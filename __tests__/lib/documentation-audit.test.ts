...
TODO: Add monitoring for service health.`;
      ...
      mockExecSync.mockReturnValue(
        'abc123|John Doe|2025-01-01|Architecture decision'
      );
      ...
      const decisions = archFile.extractedKnowledge.filter(
        (k: any) => k.type === 'architectural-decision'
      );
      expect(decisions.length).toBeGreaterThan(0);

      const workarounds = archFile.extractedKnowledge.filter(
        (k: any) => k.type === 'workaround'
      );
  ...
  describe('Phase 2: Deep Manual Review', () => {
    ...
    it('should find related files based on content similarity', async () => {
      ...
      expect(reviewedFiles[0].extractedKnowledge[0].relatedFiles).toContain(
        'security.md'
      );
    });
  });
...
ghi789|Bob Wilson|2025-01-03T12:00:00Z|optimize: improve database query performance`;
      ...
      const archaeologyKnowledge =
        await auditor.performKnowledgeArchaeology(mockFiles);

      expect(archaeologyKnowledge.length).toBeGreaterThan(0);

      const architecturalDecisions = archaeologyKnowledge.filter(
        (k: any) => k.type === 'architectural-decision'
      );
      expect(architecturalDecisions.length).toBeGreaterThan(0);

      const performanceKnowledge = archaeologyKnowledge.filter(
        (k: any) => k.type === 'performance'
      );
    ...
    it('should extract knowledge from code comments', async () => {
      ...
      const archaeologyKnowledge = await auditor.performKnowledgeArchaeology([
        codeFile,
      ]);

      expect(archaeologyKnowledge.length).toBeGreaterThan(0);

      const jsdocKnowledge = archaeologyKnowledge.filter(
        (k: any) =>
          k.source === 'code-comment' && k.type === 'architectural-decision'
      );
      expect(jsdocKnowledge.length).toBeGreaterThan(0);

      const todoKnowledge = archaeologyKnowledge.filter(
        (k: any) => k.source === 'code-comment' && k.type === 'workaround'
      );
  ...
  describe('Phase 4: Safe Migration Planning', () => {
    it('should create comprehensive migration plan', async () => {
      ...
      const migrationPlan = await auditor.createMigrationPlan(
        mockFiles,
        mockKnowledge
      );
    ...
    it('should identify duplicate content for consolidation', async () => {
      ...
      const consolidationPhase = migrationPlan.phases.find(
        (p: any) => p.name === 'Consolidation'
      );
