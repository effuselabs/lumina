...
const mockAppointments: DashboardAppointment[] = [
  ...
  },
  {
    ...
    services: [
      { id: 'service-3', name: 'Manicure', duration: 60, price: 3500 },
    ],
    totalPrice: 3500,
    totalDuration: 60,
    client: {
...
describe('highlightText', () => {
  it('highlights matching terms', () => {
    const result = highlightText('John Doe loves haircuts', [
      'john',
      'haircuts',
    ]);
  ...
  it('returns original text when no search terms', () => {
    ...
    expect(result).toEqual([{ text: 'John Doe', isHighlighted: false }]);
  });
...
describe('validateFilters', () => {
  ...
  it('warns about long date ranges', () => {
    ...
    expect(result.warnings).toContain(
      'Date range spans more than a year, results may be slow'
    );
  });
  ...
  it('warns about too many selections', () => {
    ...
    expect(result.warnings).toContain(
      'Many services selected, consider narrowing the selection'
    );
  });
});
