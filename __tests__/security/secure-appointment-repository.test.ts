...
describe('SecureAppointmentRepository', () => {
  ...
  afterEach(async () => {
    ...
    await prisma.user.deleteMany({
      where: { id: { in: [testUserId, managerUserId] } },
    });
  ...
  describe('create', () => {
    ...
    it('should reject appointment with invalid staff assignment', async () => {
      ...
      await expect(
        ...
        'Unauthorized: Staff member not found or access denied'
      );
  ...
  describe('update', () => {
    ...
    it('should allow manager to update any appointment', async () => {
      ...
      expect(auditLogs[0].newValues).toHaveProperty(
        'notes',
        'Updated by manager'
      );
    });
  ...
  describe('findByBusinessSecure', () => {
    ...
    it('should allow manager to see all appointments', async () => {
      ...
      expect(result.appointments.map((a: any) => a.id)).toContain(
        appointment1Id
      );
      expect(result.appointments.map((a: any) => a.id)).toContain(
        appointment2Id
