...
describe('Public Booking Security', () => {
  ...
  beforeEach(() => {
    ...
    asMock(mockPrisma.publicBookingAuditLog.create).mockResolvedValue(
      {} as any
    );
  });
  ...
  describe('Input Sanitization', () => {
    ...
    it('should detect suspicious email patterns', () => {
      ...
      expect(result.violations).toContain(
        'Email appears to be suspicious or temporary'
      );
    });
    ...
    it('should validate service IDs format', () => {
      ...
      expect(
        result.violations.some((v: any) => v.includes('invalid format'))
      ).toBe(true);
    });
    ...
    it('should limit number of services', () => {
      ...
      const result = publicBookingSanitizer.sanitizeBookingData(bookingData);
      expect(result.violations).toContain(
        'Maximum 5 services allowed per booking'
      );
  ...
  describe('Audit Logging', () => {
    it('should log business access event', async () => {
      const mockRequest = new NextRequest(
        'https://example.com/api/public/booking/test',
        {
          method: 'GET',
          headers: {
    ...
    it('should log booking completion with correct risk score', async () => {
      const mockRequest = new NextRequest(
        'https://example.com/api/public/booking/test/book',
        {
          method: 'POST',
          headers: {
            'x-forwarded-for': '192.168.1.1',
  ...
  describe('Security Middleware Integration', () => {
    it('should validate business context for GET requests', async () => {
      const mockRequest = new NextRequest(
        `https://example.com/api/public/booking/${mockBusinessId}`,
        {
          method: 'GET',
          headers: {
            'x-forwarded-for': '192.168.1.1',
  ...
  describe('Business Context Validation', () => {
    ...
    it('should reject invalid business ID format', async () => {
      ...
      const mockRequest = new NextRequest(
        `https://example.com/api/public/booking/${invalidBusinessId}`,
        {
          method: 'GET',
        }
      );
...
describe('Security Integration', () => {
  it('should handle complete security validation flow', async () => {
    ...
    const mockRequest = new NextRequest(
      `https://example.com/api/public/booking/${mockBusinessId}/book`,
      {
        method: 'POST',
        headers: {
