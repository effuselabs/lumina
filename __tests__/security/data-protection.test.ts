...
describe('DataProtectionService', () => {
    ...
    afterEach(async () => {
        // Clean up test data
        await prisma.campaignRecipient.deleteMany({ where: { client: { businessId: testBusinessId } } })
        await prisma.clientReview.deleteMany({ where: { businessId: testBusinessId } })
        await prisma.loyaltyTransaction.deleteMany({ where: { membership: { client: { businessId: testBusinessId } } } })
        await prisma.loyaltyMembership.deleteMany({ where: { client: { businessId: testBusinessId } } })
        await prisma.communicationHistory.deleteMany({ where: { businessId: testBusinessId } })
        await prisma.appointmentService.deleteMany({ where: { appointment: { businessId: testBusinessId } } })
        await prisma.transaction.deleteMany({ where: { businessId: testBusinessId } })
        await prisma.appointment.deleteMany({ where: { businessId: testBusinessId } })
        await prisma.appointmentPreferences.deleteMany({ where: { businessId: testBusinessId } })
    ...
    describe('Encryption and Decryption', () => {
        ...
        it('should fail decryption with tampered data', () => {
            ...
            expect(() => dataProtection.decrypt(tamperedData)).toThrow('Decryption failed')
        })
    })
    ...
    describe('Data Masking', () => {
        it('should mask email addresses correctly', () => {
            expect(dataProtection.maskEmail('john.doe@example.com')).toBe('j***e@e***.com')
        ...
        it('should mask names correctly', () => {
            ...
            expect(dataProtection.maskName('Mary Jane Smith')).toBe('M*** J*** S****')
    ...
    describe('GDPR Data Export', () => {
        ...
        it('should export complete client data', async () => {
            const exportData = await dataProtection.exportClientData(testClientId, testBusinessId)
    ...
    describe('Data Anonymization', () => {
        ...
        it('should anonymize expired data according to retention policy', async () => {
            ...
            const result = await dataProtection.anonymizeExpiredData(testBusinessId, retentionPolicy)
    ...
    describe('Input Validation and Sanitization', () => {
        it('should sanitize malicious input', () => {
            const maliciousInput = '<script>alert("xss")</script>Hello<script>alert("xss2")</script>'
        ...
        it('should validate and sanitize emails', () => {
            expect(dataProtection.validateAndSanitizeEmail('test@example.com')).toBe('test@example.com')
            expect(dataProtection.validateAndSanitizeEmail('Test@Example.COM')).toBe('test@example.com')
            expect(dataProtection.validateAndSanitizeEmail('invalid-email')).toBeNull()
            expect(dataProtection.validateAndSanitizeEmail('<script>@example.com')).toBeNull()
        })

        it('should validate and sanitize phone numbers', () => {
            expect(dataProtection.validateAndSanitizePhone('+1-555-123-4567')).toBe('+15551234567')
            expect(dataProtection.validateAndSanitizePhone('(555) 123-4567')).toBe('5551234567')
            expect(dataProtection.validateAndSanitizePhone('123')).toBeNull()
            expect(dataProtection.validateAndSanitizePhone('abc-def-ghij')).toBeNull()
        })

        it('should validate and sanitize names', () => {
            expect(dataProtection.validateAndSanitizeName('John Doe')).toBe('John Doe')
            expect(dataProtection.validateAndSanitizeName("Mary O'Connor-Smith")).toBe("Mary O'Connor-Smith")
            expect(dataProtection.validateAndSanitizeName('John123 <script>')).toBe('John')
            expect(dataProtection.validateAndSanitizeName('   Multiple   Spaces   ')).toBe('Multiple Spaces')
    ...
    describe('Error Handling', () => {
        it('should handle encryption errors gracefully', () => {
            ...
            expect(() => new DataProtectionService()).toThrow('Encryption key must be 32 bytes')
        })
        ...
        it('should handle missing encryption key', () => {
            ...
            expect(() => new DataProtectionService()).toThrow('DATA_ENCRYPTION_KEY environment variable is required')
        ...
        it('should handle database errors during GDPR operations', async () => {
            ...
            prisma.client.findFirst = jest.fn().mockRejectedValue(new Error('Database error'))
