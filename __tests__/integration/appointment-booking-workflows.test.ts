...
import {
  DELETE as cancelAppointment,
  GET as getAppointmentById,
...
>;
const mockAppointmentStatusManager =
  AppointmentStatusManager as jest.MockedClass<typeof AppointmentStatusManager>;
...
describe('End-to-End Appointment Booking Workflows', () => {
  ...
  beforeEach(() => {
    ...
    addMockMethod(
      multiServiceCoordinatorInstance,
      'validateMultiServiceBooking'
    );
    ...
    mockCalendarIntegration.mockImplementation(
      () => calendarIntegrationInstance
    );
    mockMultiServiceCoordinator.mockImplementation(
      () => multiServiceCoordinatorInstance
    );
    mockAppointmentService.mockImplementation(() => appointmentServiceInstance);
    mockAppointmentStatusManager.mockImplementation(
  ...
  describe('Complete Single-Service Appointment Workflow', () => {
    it('should complete full appointment lifecycle: create → confirm → start → complete', async () => {
      ...
      const confirmRequest = new NextRequest(
        'http://localhost/api/appointments/appointment-123/status?businessId=business-123',
        {
          method: 'PUT',
          body: JSON.stringify({
            status: AppointmentStatus.CONFIRMED,
      ...
      const confirmResponse = await updateStatus(confirmRequest, {
      ...
      const startResponse = await updateStatus(startRequest, {
        params: { id: 'appointment-123' },
      });
      ...
      const completeResponse = await updateStatus(completeRequest, {
        params: { id: 'appointment-123' },
      });
      ...
      console.log(
        `Complete Single-Service Workflow: ${totalWorkflowTime.toFixed(2)}ms`
      );
    });
    ...
    it('should handle appointment cancellation workflow with proper cleanup', async () => {
      ...
      appointmentServiceInstance.cancelAppointment
        .mockResolvedValue({
          success: true,
          appointment: {
            ...mockAppointment,
        ...
        .invalidateAvailabilityCache.mockResolvedValue();

      const cancelRequest = new NextRequest(
        'http://localhost/api/appointments/appointment-456?businessId=business-123',
        {
          method: 'DELETE',
          body: JSON.stringify({
      ...
      const response = await cancelAppointment(cancelRequest, {
  ...
  describe('Multi-Service Appointment Workflows', () => {
    it('should complete complex multi-service appointment booking with staff coordination', async () => {
      const multiServiceData = {
        ...
        .validateMultiServiceBooking.mockResolvedValue({
          isValid: true,
          errors: [],
        ...
        .calculateTotalDuration.mockResolvedValue(210)(
          multiServiceCoordinatorInstance as any
        )
        .calculateTotalPrice.mockResolvedValue(200)(
      ...
      expect(
        multiServiceCoordinatorInstance.validateMultiServiceBooking
    ...
    it('should handle adding services to existing appointment', async () => {
      ...
      const addServicesRequest = new NextRequest(
        'http://localhost/api/appointments/appointment-789/services?businessId=business-123',
        {
          method: 'POST',
      ...
      const response = await addServices(addServicesRequest, {
  ...
  describe('Calendar Infrastructure Integration Workflows', () => {
    it('should integrate with LUM-96 availability calculation throughout booking process', async () => {
      const appointmentData = {
        ...
        .validateServiceDuration.mockResolvedValue({
          isValid: true,
      ...
      expect(
        calendarIntegrationInstance.checkAvailability
      ).toHaveBeenCalledWith('staff-123', expect.any(Object), ['service-123']);
      expect(calendarIntegrationInstance.detectConflicts).toHaveBeenCalled();
      expect(
        calendarIntegrationInstance.validateServiceDuration
      ).toHaveBeenCalled();
    });
    ...
    it('should handle LUM-96 conflict detection and provide alternatives', async () => {
      ...
      expect(data.availability.alternatives[0].reason).toBe(
        'Next available slot'
      );
    });
    ...
    it('should coordinate cache invalidation with LUM-96 after appointment changes', async () => {
      ...
      appointmentServiceInstance.updateAppointment
        .mockResolvedValue({
          success: true,
      ...
      const response = await updateAppointment(updateRequest, {
      ...
      expect(
        calendarIntegrationInstance.invalidateAvailabilityCache
      ).toHaveBeenCalledWith('staff-123', expect.any(Object));
    });
  });
  ...
  describe('Concurrent Appointment Operations', () => {
    it('should handle concurrent appointment creation without conflicts', async () => {
      ...
      const appointmentRequests = Array.from(
      ...
      console.log(
        `Concurrent Creation Test: ${concurrentRequests} requests in ${totalTime.toFixed(2)}ms`
      );
    });
    ...
    it('should handle concurrent status updates with proper synchronization', async () => {
      ...
      const statusUpdateRequests = Array.from(
        { length: concurrentStatusUpdates },
        () => ({
          status: AppointmentStatus.CONFIRMED,
      ...
      const responses = await Promise.all(
        statusUpdateRequests.map((updateData: any) => {
          const request = new NextRequest(
            `http://localhost/api/appointments/${appointmentId}/status?businessId=business-123`,
            {
              method: 'PUT',
              body: JSON.stringify(updateData),
            }
  ...
  describe('Performance Validation', () => {
      ...
      const operations = [
        ...
        },
        {
          ...
          operation: async () => {
            ...
            const request = new NextRequest(
              'http://localhost/api/appointments?businessId=business-123&limit=20'
            );
        ...
        },
        {
          ...
          operation: async () => {
            ...
            const request = new NextRequest(
              'http://localhost/api/appointments/perf-test-123?businessId=business-123'
            );
            return getAppointmentById(request, {
              params: { id: 'perf-test-123' },
            });
          },
        },
      ];
    ...
    it('should handle load testing scenarios efficiently', async () => {
      ...
      console.log(
        `Load Test: ${loadTestRequests} requests in ${totalTime.toFixed(2)}ms (avg: ${averageResponseTime.toFixed(2)}ms)`
      );
    });
  });
});
