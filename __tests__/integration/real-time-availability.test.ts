...
describe('Real-Time Availability Updates Integration Tests', () => {
  ...
  describe('Business Hours Updates', () => {
    it('should immediately reflect business hours changes in availability', async () => {
      ...
      availabilityCalculator.getAvailableSlots.mockResolvedValueOnce(
        mockInitialSlots as any
      );
      ...
      expect(
        updatedSlots.some((slot: any) => slot.startTime.getHours() >= 15)
      ).toBe(false);
    });
  });
  ...
  describe('Staff Availability Updates', () => {
    it('should immediately reflect staff availability changes', async () => {
      ...
      await staffAvailabilityRepo.setStaffAvailability(
        staffId,
        businessId,
        initialAvailability
      );
      ...
      expect(
        updatedSlots.some((slot: any) => slot.startTime.getHours() >= 13)
      ).toBe(false);
    });
  });
  ...
  describe('Time-Off Request Updates', () => {
    it('should immediately reflect time-off requests in availability', async () => {
      ...
      expect(
        updatedSlots.every(
          (slot: any) => slot.startTime.getHours() < 13 // Only morning slots available
        )
      ).toBe(true);
      expect(
        updatedSlots.some(
          (slot: any) => slot.startTime.getHours() >= 13 // No afternoon slots
        )
      ).toBe(false);
    });
  });
  ...
  describe('Concurrent Updates', () => {
    it('should handle concurrent availability updates correctly', async () => {
      ...
      const businessHoursUpdate = businessHoursRepo.setBusinessHours(
        businessId,
        {
          dayOfWeek: 1,
      ...
      const staffAvailabilityUpdate =
        staffAvailabilityRepo.setStaffAvailability(staffId, businessId, {
          dayOfWeek: 1,
          startTime: '09:00:00',
