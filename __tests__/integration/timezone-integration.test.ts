...
import { MultiLocationTimeZoneManager, TimeZoneHandler } from '../../lib/services/timezone-handler'
...
describe('Timezone Integration Tests', () => {
    describe('Cross-timezone appointment booking flow', () => {
        it('should handle booking from different timezone than business', async () => {
            ...
            const utcTime = TimeZoneHandler.localToUTC(clientLocalTime, appointmentDate, clientTimezone)
            const businessLocalTime = TimeZoneHandler.utcToLocal(utcTime, businessTimezone)
        ...
        it('should validate appointment times across multiple timezones', async () => {
            ...
            for (const testCase of testCases) {
                ...
                const businessTime = TimeZoneHandler.utcToLocal(utcTime, testCase.businessTimezone)
    ...
    describe('Multi-location business scenarios', () => {
        ...
        it('should normalize availability across all locations', async () => {
            const availability = [
                { locationId: 'nyc', startTime: '09:00', endTime: '17:00', date: '2024-07-15' },
                { locationId: 'la', startTime: '09:00', endTime: '17:00', date: '2024-07-15' },
                { locationId: 'london', startTime: '09:00', endTime: '17:00', date: '2024-07-15' }
            ]

            const normalized = multiLocationManager.normalizeAvailabilityAcrossLocations(availability)
    ...
    describe('DST transition edge cases', () => {
        it('should handle spring forward transition correctly', async () => {
            ...
            for (const test of testTimes) {
                const isValid = TimeZoneHandler.isValidTimeInZone(test.time, transitionDate, timezone)
        ...
        it('should handle fall back transition correctly', async () => {
            ...
            for (const time of testTimes) {
                const isValid = TimeZoneHandler.isValidTimeInZone(time, transitionDate, timezone)
                expect(isValid).toBe(true)
            }

            // Check DST transition detection
            const dstInfo = TimeZoneHandler.handleDSTTransition('01:00', '03:00', transitionDate, timezone)
            expect(dstInfo.dstTransition).toBe(true)
        ...
        it('should handle timezone without DST', async () => {
            ...
            const springDST = TimeZoneHandler.handleDSTTransition('02:00', '03:00', springDate, timezone)
            const fallDST = TimeZoneHandler.handleDSTTransition('01:00', '02:00', fallDate, timezone)
    ...
    describe('Business hours validation across timezones', () => {
        it('should validate business hours in different timezones', async () => {
            ...
            const utcOpen = TimeZoneHandler.localToUTC(businessOpen, date, businessTimezone)
            const utcClose = TimeZoneHandler.localToUTC(businessClose, date, businessTimezone)
    ...
    describe('Performance and edge cases', () => {
        ...
        it('should handle edge dates correctly', async () => {
            ...
            for (const date of edgeDates) {
                const utc = TimeZoneHandler.localToUTC('12:00', date, 'America/New_York')
    ...
    describe('Real-world scenarios', () => {
        it('should handle international client booking scenario', async () => {
            ...
            const utcTime = TimeZoneHandler.localToUTC(clientPreferredTime, appointmentDate, clientTimezone)
            const businessTime = TimeZoneHandler.utcToLocal(utcTime, businessTimezone)
            ...
            const businessHoursStart = TimeZoneHandler.localToUTC('09:00', appointmentDate, businessTimezone)
            const clientEquivalentTime = TimeZoneHandler.utcToLocal(businessHoursStart, clientTimezone)
        ...
        it('should handle daylight saving time boundary appointments', async () => {
            ...
            const regularUtc = TimeZoneHandler.localToUTC(appointmentTime, regularDate, timezone)
            const dstUtc = TimeZoneHandler.localToUTC(appointmentTime, dstDate, timezone)
            const postDstUtc = TimeZoneHandler.localToUTC(appointmentTime, postDstDate, timezone)
