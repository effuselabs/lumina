...
describe('Multi-Tenant Data Isolation Integration Tests', () => {
    ...
    describe('Business Hours Isolation', () => {
        it('should only return business hours for authenticated business', async () => {
            ...
            expect(data.data.every((h: any) => h.businessId === businessA)).toBe(true)
    ...
    describe('Staff Availability Isolation', () => {
        it('should only return staff and availability for authenticated business', async () => {
            ...
            expect(data.data.every((s: any) => s.staff.id.startsWith('staff-a'))).toBe(true)
    ...
    describe('Time-Off Request Isolation', () => {
        it('should only return time-off requests for authenticated business', async () => {
            ...
            const mixedTimeOffRequests = [
                {
                    ...
                    reason: 'Vacation A1',
                    status: 'PENDING',
                    staff: { id: staffA1, name: 'Staff A1', email: 'staff-a1@example.com' },
                },
                {
                    id: 'request-a2',
                    staffId: staffA2,
                    ...
                    reason: 'Vacation A2',
                    status: 'APPROVED',
                    staff: { id: staffA2, name: 'Staff A2', email: 'staff-a2@example.com' },
                },
                {
                    id: 'request-b1',
                    staffId: staffB1,
                    ...
                    reason: 'Vacation B1',
                    status: 'PENDING',
                    staff: { id: staffB1, name: 'Staff B1', email: 'staff-b1@example.com' },
                },
                {
                    id: 'request-c1',
                    staffId: staffC1,
                    ...
                    reason: 'Vacation C1',
                    status: 'DENIED',
                    staff: { id: staffC1, name: 'Staff C1', email: 'staff-c1@example.com' },
                },
            ]
            ...
            expect(data.data.every((r: any) => r.staff.id.startsWith('staff-a'))).toBe(true)
    ...
    describe('Cross-Business Access Prevention', () => {
        it('should prevent any cross-business data access attempts', async () => {
            ...
            const staffRequest = new NextRequest('http://localhost:3000/api/availability/staff')
            const timeOffRequest = new NextRequest('http://localhost:3000/api/availability/time-off')

            const [businessHoursResponse, staffResponse, timeOffResponse] = await Promise.all([
    ...
    describe('Data Integrity Validation', () => {
        it('should ensure all returned data belongs to the authenticated business', async () => {
            ...
            expect(businessHoursData.data.every((h: any) => h.businessId === businessA)).toBe(true)
            expect(staffData.data.every((s: any) => s.staff.id === staffA1)).toBe(true)
            expect(timeOffData.data.every((r: any) => r.staff.id === staffA1)).toBe(true)

            // Verify no data from other businesses is present
            expect(businessHoursData.data.some((h: any) => h.businessId !== businessA)).toBe(false)
            expect(staffData.data.some((s: any) => s.staff.id.startsWith('staff-b') || s.staff.id.startsWith('staff-c'))).toBe(false)
            expect(timeOffData.data.some((r: any) => r.staff.id.startsWith('staff-b') || r.staff.id.startsWith('staff-c'))).toBe(false)
        })
    })
})