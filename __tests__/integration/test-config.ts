...
export const performanceUtils = {
    ...
    expectPerformance: (duration: number, maxDuration: number, operation: string) => {
        if (duration > maxDuration) {
            console.warn(`Performance warning: ${operation} took ${duration.toFixed(2)}ms (expected < ${maxDuration}ms)`)
        }
...
export const accessibilityUtils = {
    ...
    checkTouchTargetSize: (element: HTMLElement): boolean => {
        ...
        return rect.width >= testConfig.accessibility.touchTargetSize &&
...
export const mobileUtils = {
    ...
    simulateSwipeGesture: (element: HTMLElement, direction: 'left' | 'right' | 'up' | 'down') => {
...
export const reportingUtils = {
    logPerformanceMetrics: (testName: string, metrics: Record<string, number>) => {
    ...
    logAccessibilityResults: (testName: string, violations: any[]) => {
        if (violations.length === 0) {
            ...
            console.log(`❌ ${testName}: ${violations.length} accessibility violations found`)
    ...
    generateTestSummary: (results: { passed: number; failed: number; total: number }) => {
