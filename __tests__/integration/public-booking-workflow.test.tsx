...
import { mockAvailableSlots, mockBusiness, mockServices } from '@/test-utils/booking-mocks'
...
describe('Public Booking Workflow Integration', () => {
    ...
    describe('Multi-Service Booking Flow', () => {
        test('should handle multiple service selection with duration calculation', async () => {
            ...
            expect(screen.getByText('Total Duration: 90 minutes')).toBeInTheDocument()
    ...
    describe('Error Handling Integration', () => {
        test('should handle booking conflicts gracefully', async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.getByText('Alternative Times Available')).toBeInTheDocument()
        ...
        test('should handle network errors with retry options', async () => {
            ...
            // Mock network error
            mockBookingApi.getBusinessInfo.mockRejectedValue(new Error('Network error'))

            render(<PublicBookingInterface businessId={businessId} />)

            await waitFor(() => {
                expect(screen.getByText('Unable to load booking information')).toBeInTheDocument()
    ...
    describe('Real-time Availability Updates', () => {
        test('should update availability when slots become unavailable', async () => {
            ...
            mockBookingApi.getAvailableSlots.mockResolvedValue({
                availableSlots: mockAvailableSlots.filter(slot =>
            ...
            await waitFor(() => {
                expect(screen.queryByTestId('slot-2024-01-15-10:00')).not.toBeInTheDocument()
            })
        })
    })
})