...
const {
  AvailabilityCalculator,
} = require('@/lib/services/availability-calculator');
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
describe('/api/availability/slots Integration Tests', () => {
  ...
  describe('GET /api/availability/slots', () => {
    ...
    it('should filter slots by staff member', async () => {
      ...
      asMock(mockPrisma.staff.findFirst).mockResolvedValue(
        mockStaff as unknown as Staff
      );
      AvailabilityCalculator.getAvailableSlots.mockResolvedValue(mockSlots);

      const request = new NextRequest(
        `http://localhost:3000/api/availability/slots?date=2024-01-15&staffId=${staffId}`
      );
    ...
    it('should filter slots by service', async () => {
      ...
      asMock(mockPrisma.service.findFirst).mockResolvedValue(
        mockService as Service
      );
      AvailabilityCalculator.getAvailableSlots.mockResolvedValue(mockSlots);

      const request = new NextRequest(
        `http://localhost:3000/api/availability/slots?date=2024-01-15&serviceId=${serviceId}`
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should validate timezone parameter', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots?date=2024-01-15&timezone=Invalid/Timezone'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should return 401 when user is not authenticated', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots?date=2024-01-15'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should return 400 when date parameter is missing', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should return 400 when date format is invalid', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots?date=invalid-date'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should enforce business context isolation', async () => {
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/availability/slots?date=2024-01-15&staffId=${otherBusinessStaffId}`
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle duration parameter validation', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots?date=2024-01-15&duration=500'
      ); // Too long
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should include unavailable slots when requested', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/availability/slots?date=2024-01-15&includeUnavailable=true'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should handle database errors gracefully', async () => {
      asMock(mockPrisma.business.findUnique).mockRejectedValue(
    ...
    it('should handle availability calculator errors gracefully', async () => {
      ...
      AvailabilityCalculator.getAvailableSlots.mockRejectedValue(
  ...
  describe('Multi-tenant data isolation', () => {
    it('should only access staff from authenticated business', async () => {
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/availability/slots?date=2024-01-15&staffId=other-staff-123`
      );
      const response = await GET(request);
    ...
    it('should only access services from authenticated business', async () => {
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/availability/slots?date=2024-01-15&serviceId=other-service-123`
      );
      const response = await GET(request);
