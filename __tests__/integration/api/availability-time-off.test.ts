...
describe('/api/availability/time-off Integration Tests', () => {
  ...
  describe('GET /api/availability/time-off', () => {
    ...
    it('should filter by status when provided', async () => {
      ...
      const request = new NextRequest(
        'http://localhost:3000/api/availability/time-off?status=approved'
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it('should filter by staff when provided', async () => {
      ...
      const request = new NextRequest(
        `http://localhost:3000/api/availability/time-off?staffId=${staffId}`
      );
      const response = await GET(request);
  ...
  describe('Error handling and edge cases', () => {
    it('should handle database transaction failures', async () => {
      ...
      asMock(mockPrisma.appointment.findMany).mockResolvedValue([]);
      asMock(mockPrisma.timeOffRequest.findMany).mockResolvedValue([]);
      asMock(mockPrisma.timeOffRequest.create).mockRejectedValue(
