...
const {
  ConflictDetectionEngine,
} = require('@/lib/services/conflict-detection-engine');
...
describe('Availability API Integration Tests', () => {
  ...
  describe('Conflict Detection Integration', () => {
    it('should integrate conflict detection with database queries', async () => {
      ...
      // Conflict detection
      const conflicts =
        await ConflictDetectionEngine.detectConflicts(appointmentRequest);
    ...
    it('should validate appointment slots with service duration', async () => {
      ...
      ConflictDetectionEngine.validateAppointmentSlot.mockResolvedValue(
  ...
  describe('Multi-Tenant Data Isolation', () => {
    it('should enforce business context across all operations', async () => {
      ...
      expect(staff.every((s: any) => s.businessId === userBusinessId)).toBe(
        true
      );
      expect(
        timeOffRequests.every((r: any) => r.businessId === userBusinessId)
      ).toBe(true);
      expect(
        businessHours.every((h: any) => h.businessId === userBusinessId)
      ).toBe(true);
  ...
  describe('Real-Time Updates Integration', () => {
    it('should reflect immediate changes across related operations', async () => {
      ...
      const initialAvailability =
        await AvailabilityCalculator.getAvailableSlots({
          businessId,
