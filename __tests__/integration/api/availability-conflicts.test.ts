...
describe('/api/availability/conflicts Integration Tests', () => {
  ...
  describe('GET /api/availability/conflicts', () => {
    ...
    it('should return 400 when required parameters are missing', async () => {
      const request = new NextRequest(
        'http://localhost:3000/api/availability/conflicts'
      );
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(data.error).toBe(
        'staffId, startTime, and endTime parameters are required'
      );
    });
    ...
    it('should handle conflict detection engine errors gracefully', async () => {
      ...
      ConflictDetectionEngine.detectConflicts.mockRejectedValue(
    ...
    it('should provide detailed conflict information with resolutions', async () => {
      ...
      expect(data.conflicts[0].suggestedResolutions[0].type).toBe(
        'ALTERNATIVE_TIME'
      );
      expect(data.conflicts[0].suggestedResolutions[1].type).toBe(
        'ALTERNATIVE_STAFF'
      );
    });
  });
