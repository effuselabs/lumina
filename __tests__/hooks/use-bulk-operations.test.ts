...
describe('useBulkOperations', () => {
  ...
  describe('cancelAppointments', () => {
    it('should successfully cancel appointments', async () => {
      ...
      await act(async () => {
        const results = await result.current.cancelAppointments(
          ['apt-1'],
          'Test reason'
        );
  ...
  describe('loadOperationHistory', () => {
    ...
    it('should handle history loading errors', async () => {
      mockService.getOperationHistory.mockRejectedValueOnce(
  ...
  describe('progress callbacks', () => {
    it('should call progress callback during operations', async () => {
      ...
      mockService.cancelAppointments.mockImplementation(
        async (ids, reason, notify, onProgress) => {
          if (onProgress) {
      ...
      await act(async () => {
        await result.current.cancelAppointments(
          ['apt-1', 'apt-2'],
          'Test',
          true,
          progressCallback
