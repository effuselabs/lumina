...
export const handlers = [
  ...
  rest.get('/api/appointments', (req: any, res: any, ctx: any) => {
    ...
    let filteredAppointments = mockAppointments.filter(
      (apt: any) => apt.businessId === businessId
    );
    ...
    if (startDate && endDate) {
      ...
      filteredAppointments = filteredAppointments.filter(
        (apt: any) => apt.startTime >= start && apt.startTime <= end
      );
    }
  ...
  rest.get('/api/appointments/:id', (req: any, res: any, ctx: any) => {
    ...
    if (!appointment) {
      return res(ctx.status(404), ctx.json({ error: 'Appointment not found' }));
    }
  ...
  rest.put('/api/appointments/:id', async (req: any, res: any, ctx: any) => {
    ...
    if (updateData.startTime || updateData.endTime) {
      const hasConflict = mockAppointments.some(
        apt =>
          ...
          apt.staffId ===
            (updateData.staffId ||
              mockAppointments[appointmentIndex].staffId) &&
          new Date(apt.startTime) <=
            new Date(
              updateData.endTime || mockAppointments[appointmentIndex].endTime
            ) &&
          new Date(apt.endTime) >=
            new Date(
              updateData.startTime ||
                mockAppointments[appointmentIndex].startTime
            )
      );
    ...
    return res(ctx.status(200), ctx.json(updatedAppointment));
  }),
  ...
  rest.post(
    ...
    async (req: any, res: any, ctx: any) => {
      const { staffId, startTime, endTime, excludeAppointmentId } =
        await req.json();
  ...
  rest.get('/api/staff', (req: any, res: any, ctx: any) => {
    ...
    return res(ctx.status(200), ctx.json(filteredStaff));
  }),
  ...
  rest.get('/api/services', (req: any, res: any, ctx: any) => {
    ...
    return res(ctx.status(200), ctx.json(filteredServices));
  }),
  ...
  rest.get('/api/clients', (req: any, res: any, ctx: any) => {
    ...
    let filteredClients = mockClients.filter(
      client => client.businessId === businessId
    );
    ...
    return res(ctx.status(200), ctx.json(filteredClients));
  }),
  ...
  rest.post(
    '/api/appointments/network-error',
  ...
  rest.get(
    '/api/appointments/websocket-token',
    (req: any, res: any, ctx: any) => {
      return res(ctx.status(200), ctx.json({ token: 'mock-websocket-token' }));
    }
  ),
];
