...
describe('Report Types Data Quality Validation', () => {
    ...
    describe('Profit & Loss (P&L) Report Validation', () => {
        test('should calculate accurate revenue and expenses', async () => {
            ...
            if (businessRevenue.transactionCount > 0) {
                const expectedAverage = businessRevenue.totalRevenue / businessRevenue.transactionCount;
                expect(Math.abs(businessRevenue.averageTransactionAmount - expectedAverage)).toBeLessThan(0.01);
            }
        });

        test('should break down revenue by employment type', async () => {
            const [commissionRevenue, chairRentalRevenue, hybridRevenue] = await Promise.all([
                calculateBusinessRevenue(testBusinessId, testDateRange.from, testDateRange.to, { employmentType: 'COMMISSION' }),
                calculateBusinessRevenue(testBusinessId, testDateRange.from, testDateRange.to, { employmentType: 'CHAIR_RENTAL' }),
                calculateBusinessRevenue(testBusinessId, testDateRange.from, testDateRange.to, { employmentType: 'HYBRID' }),
            ]);
            ...
            const totalByEmploymentType = commissionRevenue.totalRevenue +
            ...
            if (overallRevenue.totalRevenue > 0) {
                const variance = Math.abs(totalByEmploymentType - overallRevenue.totalRevenue) / overallRevenue.totalRevenue;
        ...
        test('should calculate business retention accurately', async () => {
            ...
            const expectedRetention = businessRevenue.totalRevenue - totalCommissionsPaid + totalChairRentalReceived;

            // Validate that retention calculation is reasonable
            expect(expectedRetention).toBeGreaterThanOrEqual(0);
            expect(expectedRetention).toBeLessThanOrEqual(businessRevenue.totalRevenue * 1.5); // Allow for chair rental income
        });
    });
        ...
        test('should calculate accurate staff earnings by employment type', async () => {
            ...
            for (const staffMember of staff) {
                ...
                if (earnings.totalRevenue > 0) {
                    const calculatedRate = earnings.totalCommissions / earnings.totalRevenue;
        ...
        test('should calculate commission rates accurately', async () => {
            ...
            for (const staffMember of staff) {
                ...
                if (earnings.totalRevenue > 0) {
                    ...
                    expect(Math.abs(earnings.averageCommissionRate - actualRate)).toBeLessThan(0.01);
                }
            }
        });
    });
    ...
    describe('Client Report Validation', () => {
        test('should provide accurate client analytics', async () => {
            ...
            if (clientMetrics.totalClients > 0) {
                const calculatedRetentionRate = (clientMetrics.returningClients / clientMetrics.totalClients) * 100;
                expect(Math.abs(clientMetrics.clientRetentionRate - calculatedRetentionRate)).toBeLessThan(1);
            }
            ...
            for (let i = 1; i < clientMetrics.topClients.length; i++) {
                expect(clientMetrics.topClients[i - 1].totalSpent).toBeGreaterThanOrEqual(
                    clientMetrics.topClients[i].totalSpent
                );
            }
        });
        ...
        test('should calculate lifetime value accurately', async () => {
            ...
            if (clientMetrics.topClients.length > 0) {
                ...
                const averageOfTopClients = totalSpentByTopClients / clientMetrics.topClients.length;

                // Average LTV should be reasonable compared to top clients
                expect(clientMetrics.averageLifetimeValue).toBeGreaterThan(0);
                expect(clientMetrics.averageLifetimeValue).toBeLessThan(averageOfTopClients * 2);
            }
        });
    ...
    describe('Cross-Report Data Consistency', () => {
        test('should have consistent revenue across all report types', async () => {
            const [revenueData, businessRevenue, staffPerformance, serviceAnalytics] = await Promise.all([
                dataService.getRevenueData(testDateRange),
                calculateBusinessRevenue(testBusinessId, testDateRange.from, testDateRange.to),
        ...
        test('should have consistent client data across reports', async () => {
            ...
            expect(clientMetrics.totalClients).toBeGreaterThan(0);
            expect(clientMetrics.newClients + clientMetrics.returningClients).toBeLessThanOrEqual(
                clientMetrics.totalClients * 2 // Allow for some overlap in date ranges
            );

            // Top clients should be a subset of total clients
            expect(clientMetrics.topClients.length).toBeLessThanOrEqual(clientMetrics.totalClients);
        });
    });
