...
const steeringComplianceHook = JSON.parse(
    readFileSync(join(process.cwd(), '.kiro/hooks/steering-compliance.json'), 'utf-8')
)
...
async function executeSteeringComplianceHook(context: AgentHookContext): Promise<any> {
    const prompt = steeringComplianceHook.prompt.replace('{{changedFiles}}', context.changedFiles.join(', '))

    return await analyzeSteeringCompliance(context.changedFiles, context.workspaceRoot)
}

// Mock compliance analysis function
async function analyzeSteeringCompliance(changedFiles: string[], workspaceRoot: string) {
    const results = []
...
async function analyzeFileCompliance(filePath: string, workspaceRoot: string) {
    ...
    if (filePath.includes('app/api/') && !fileContent.includes('businessId')) {
        violations.push({
            ...
            suggestion: 'Add businessId validation to ensure multi-tenant data isolation'
        })
    }

    if (filePath.endsWith('.tsx') && !fileContent.includes('interface') && !fileContent.includes('type')) {
        violations.push({
    ...
    if (!fileContent.includes('try') && !fileContent.includes('catch') && filePath.endsWith('.ts')) {
        violations.push({
...
describe('Steering Compliance Agent Hook', () => {
    ...
    describe('Hook Configuration', () => {
        it('should have correct trigger configuration', () => {
            ...
            expect(steeringComplianceHook.trigger.patterns).toContain('app/api/**/*.ts')
            expect(steeringComplianceHook.trigger.patterns).toContain('components/**/*.tsx')
            expect(steeringComplianceHook.trigger.patterns).toContain('lib/**/*.ts')
            expect(steeringComplianceHook.trigger.patterns).toContain('prisma/schema.prisma')
        })
    ...
    describe('Steering File Application', () => {
        it('should apply API standards to API routes', async () => {
            ...
            if (results.length > 0) {
                expect(results[0].applicableSteeringFiles).toContain('api-standards.md')
            }
        })
        ...
        it('should apply database standards to Prisma files', async () => {
            ...
            if (results.length > 0) {
                expect(results[0].applicableSteeringFiles).toContain('database-standards.md')
            }
        })
        ...
        it('should apply general coding standards to all files', async () => {
            ...
            if (results.length > 0) {
                expect(results[0].applicableSteeringFiles).toContain('coding-approach-and-standards.md')
            }
        })
    })
    ...
    describe('Violation Detection', () => {
        it('should detect missing business scoping in API routes', async () => {
            mockReadFile.mockReturnValue('export async function GET() { return Response.json({}) }')
        ...
        it('should detect missing TypeScript types in components', async () => {
            mockReadFile.mockReturnValue('export default function Button() { return <button>Click</button> }')
        ...
        it('should detect missing business scoping in database models', async () => {
            mockReadFile.mockReturnValue('model Client { id String @id name String }')
        ...
        it('should detect missing error handling', async () => {
            mockReadFile.mockReturnValue('export function processData() { return data.process() }')
    ...
    describe('Compliance Scoring', () => {
      ...
      `)
        ...
        it('should reduce score based on number of violations', async () => {
            mockReadFile.mockReturnValue('export function badFunction() { return data }')
    ...
    describe('Error Handling', () => {
        it('should handle file read errors gracefully', async () => {
            ...
            await expect(executeSteeringComplianceHook(context)).resolves.not.toThrow()
        })
    ...
    describe('Integration Requirements', () => {
        it('should fulfill requirement 2.1 - Steering Compliance', () => {
            ...
            expect(steeringComplianceHook.prompt).toContain('API routes: Apply api-standards.md')
            expect(steeringComplianceHook.prompt).toContain('React components: Apply ui-standards.md')
        ...
        it('should fulfill requirement 2.3 - Automated Issue Updates', () => {
            ...
            expect(steeringComplianceHook.prompt).toContain('Create Linear issues with "compliance" label')

            // Detailed violation descriptions
            expect(steeringComplianceHook.prompt).toContain('Include violation details')

            // Fix suggestions and remediation guidance
            expect(steeringComplianceHook.prompt).toContain('Fix Suggestions')
            expect(steeringComplianceHook.prompt).toContain('step-by-step remediation instructions')
        })
    })
})