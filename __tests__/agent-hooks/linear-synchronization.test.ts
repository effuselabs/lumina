...
async function createDocumentationIssue(issueData: LinearIssueData): Promise<string> {
...
async function createComplianceIssue(issueData: LinearIssueData): Promise<string> {
    const issue = await mockLinearCreateIssue({
        title: issueData.title,
        description: issueData.description,
        labelIds: issueData.labels,
...
async function updateExistingIssue(issueId: string, updates: Partial<LinearIssueData>): Promise<void> {
    await mockLinearUpdateIssue(issueId, updates)
}

async function syncMaintenanceTasks(tasks: Array<{
    title: string
    description: string
...
describe('Linear Synchronization Agent Hook Integration', () => {
    ...
    describe('Documentation Issue Creation', () => {
        it('should create Linear issues for missing documentation', async () => {
            const issueData: LinearIssueData = {
                ...
                description: 'Create documentation for the Button component including usage examples and props interface.',
        ...
        it('should create Linear issues for API documentation', async () => {
            const issueData: LinearIssueData = {
                ...
                description: 'Document the clients API endpoint including request/response formats, authentication, and error codes.',
        ...
        it('should create Linear issues for utility documentation', async () => {
            const issueData: LinearIssueData = {
                ...
                description: 'Update authentication utility documentation to reflect recent security enhancements.',
    ...
    describe('Compliance Issue Creation', () => {
        it('should create Linear issues for steering compliance violations', async () => {
            const issueData: LinearIssueData = {
                title: 'Compliance Violation: Missing businessId validation in /api/clients',
                description: 'The clients API route is missing businessId validation required for multi-tenant data isolation.\n\nFix: Add businessId validation middleware and ensure all queries are scoped to the business context.',
        ...
        it('should create Linear issues for TypeScript compliance violations', async () => {
            const issueData: LinearIssueData = {
                title: 'Compliance Violation: Missing TypeScript types in Button component',
                description: 'The Button component is missing proper TypeScript interface definitions for props.\n\nFix: Add ButtonProps interface with proper type definitions.',
        ...
        it('should create Linear issues for security compliance violations', async () => {
            const issueData: LinearIssueData = {
                ...
                description: 'The authentication endpoint is missing proper input validation and sanitization.\n\nFix: Implement Zod schema validation and input sanitization.',
            ...
            const maintenanceTasks = [
                {
                    ...
                    description: 'Document the /api/clients endpoint including request/response formats.',
                    priority: 'critical' as const,
                    labels: ['documentation', 'api', 'critical']
                },
                {
                    title: 'Update component documentation',
                    description: 'Update all component documentation with latest prop interfaces.',
                    priority: 'high' as const,
                    labels: ['documentation', 'components']
                },
                {
                    title: 'Add utility function examples',
                    description: 'Add usage examples to all utility function documentation.',
    ...
    describe('Error Handling', () => {
        it('should handle Linear API errors gracefully', async () => {
            ...
            await expect(createDocumentationIssue(issueData)).rejects.toThrow('Linear API error')
        })
        ...
        it('should handle network errors during issue creation', async () => {
            ...
            await expect(createDocumentationIssue(issueData)).rejects.toThrow('Network error')
        })
        ...
        it('should handle invalid issue data', async () => {
            ...
            await expect(createDocumentationIssue(issueData)).rejects.toThrow('Invalid issue data')
        })
    })
    ...
    describe('Batch Operations', () => {
        ...
        it('should continue processing after individual failures', async () => {
            ...
            const tasks = [
                ...
                { title: 'Task 2', description: 'Desc 2', priority: 'medium' as const, labels: ['doc'] },
                { title: 'Task 3', description: 'Desc 3', priority: 'medium' as const, labels: ['doc'] }
            ]
