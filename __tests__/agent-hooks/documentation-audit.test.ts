...
const documentationAuditHook = JSON.parse(
  readFileSync(
    join(process.cwd(), '.kiro/hooks/documentation-audit.json'),
    'utf-8'
  )
);
...
async function executeDocumentationAuditHook(
...
async function performDocumentationAudit(
  workspaceRoot: string
): Promise<DocumentationAuditResult> {
  // Mock file discovery
  ...
  const maintenanceTasks = [
    {
      ...
      description:
        'Document the /api/clients endpoint including request/response formats, authentication requirements, and error handling.',
      priority: 'critical' as const,
    ...
    },
    {
      ...
      description:
        'Update auth.md to reflect recent changes in authentication flow and add new security features.',
      priority: 'high' as const,
    ...
    },
    {
      ...
      description:
        'Add comprehensive usage examples to all component documentation files.',
      priority: 'medium' as const,
...
describe('Documentation Audit Agent Hook', () => {
  ...
  describe('Hook Configuration', () => {
    it('should have correct trigger configuration', () => {
      ...
      expect(documentationAuditHook.trigger.buttonText).toBe(
    ...
    it('should have a comprehensive prompt template', () => {
      expect(documentationAuditHook.prompt).toContain(
  ...
  describe('Documentation Inventory', () => {
    it('should catalog existing documentation files', async () => {
      ...
      expect(result.inventory.existingDocs).toContain(
        'docs/components/ui/button.md'
      );
      expect(result.inventory.existingDocs).toContain(
        'docs/api/auth/signin.md'
    ...
    it('should identify missing documentation files', async () => {
      ...
      expect(result.inventory.missingDocs).toContain(
        'docs/components/ui/input.md'
      );
  ...
  describe('Health Assessment', () => {
    ...
    it('should assess completeness of documentation', async () => {
      ...
      expect(result.healthAssessment.completenessScore).toBeGreaterThanOrEqual(
        0
      );
      expect(result.healthAssessment.completenessScore).toBeLessThanOrEqual(
        100
      );
    });
  ...
  describe('Gap Analysis', () => {
    ...
    it('should identify outdated documentation', async () => {
      ...
      const outdatedDocs = result.gaps.filter(
        (gap: any) => gap.type === 'outdated-docs'
      );
  ...
  describe('Compliance Check', () => {
    ...
    it('should categorize compliance issues by severity', async () => {
      ...
      const severities = result.complianceIssues.map(
        (issue: any) => issue.severity
      );
  ...
  describe('Maintenance Task Creation', () => {
    it('should create maintenance tasks for critical issues', async () => {
      ...
      const criticalTasks = result.maintenanceTasks.filter(
        (task: any) => task.priority === 'critical'
      );
    ...
    it('should create maintenance tasks for high priority issues', async () => {
      ...
      const highPriorityTasks = result.maintenanceTasks.filter(
        (task: any) => task.priority === 'high'
      );
      expect(highPriorityTasks.length).toBeGreaterThan(0);
      expect(highPriorityTasks[0].title).toContain(
  ...
  describe('Error Handling', () => {
    it('should handle file system errors gracefully', async () => {
      ...
      await expect(
        executeDocumentationAuditHook(context)
  ...
  describe('Manual Trigger Functionality', () => {
    ...
    it('should provide comprehensive audit scope', () => {
      expect(documentationAuditHook.prompt).toContain(
        'Full project documentation review'
      );
      expect(documentationAuditHook.prompt).toContain(
        'API documentation, component guides, database schema, development processes'
      );
    });
