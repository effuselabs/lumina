...
describe('StatCard Component', () => {
    ...
    describe('Change Indicators', () => {
        it('renders increase change indicator', () => {
            const change = { value: 12, type: 'increase' as const, period: 'this month' };
            render(<StatCard {...defaultProps} change={change} />);

            const changeElement = screen.getByLabelText(/increased by 12% this month/i);
            expect(changeElement).toBeInTheDocument();
            expect(screen.getByText(/↗ 12% this month/)).toBeInTheDocument();
        });

        it('renders decrease change indicator', () => {
            const change = { value: 8, type: 'decrease' as const, period: 'this week' };
            render(<StatCard {...defaultProps} change={change} />);
        ...
        it('renders placeholder when no change provided', () => {
            render(<StatCard {...defaultProps} />);
            const placeholder = screen.getByRole('article').querySelector('.stat-card-change-placeholder');
    ...
    describe('Action Links', () => {
        ...
        it('renders action link when provided', () => {
            ...
            const link = screen.getByRole('link', { name: /view details for total revenue/i });
        ...
        it('does not render action section when no action provided', () => {
            render(<StatCard {...defaultProps} />);
            const footer = screen.getByRole('article').querySelector('.stat-card-footer');
    ...
    describe('Loading State', () => {
        ...
        it('renders loading skeleton with action when action is provided', () => {
            ...
            const skeletonAction = screen.getByRole('status').querySelector('.stat-card-skeleton-action');
    ...
    describe('Accessibility', () => {
        ...
        it('hides decorative elements from screen readers', () => {
            ...
            const iconContainer = screen.getByRole('article').querySelector('.stat-card-icon-container');
            expect(iconContainer).toHaveAttribute('aria-hidden', 'true');

            const actionIcon = screen.getByRole('article').querySelector('.stat-card-action-icon');
            expect(actionIcon).toHaveAttribute('aria-hidden', 'true');
        });

        it('generates unique IDs for multiple cards', () => {
            const { rerender } = render(<StatCard {...defaultProps} title="Revenue" />);
    ...
    describe('Custom Styling', () => {
        ...
        it('maintains base classes with custom className', () => {
            render(<StatCard {...defaultProps} className="custom-class" size="compact" />);
            const card = screen.getByRole('article');
            expect(card).toHaveClass('stat-card', 'stat-card-compact', 'custom-class');
        });
    });
