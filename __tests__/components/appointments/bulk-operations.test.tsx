...
import {
  ...
  useBulkSelection,
} from '@/components/appointments/bulk-selection-provider';
import {
  AppointmentStatus,
...
function TestBulkSelectionComponent() {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => selectAppointment('1')}
        data-testid="select-appointment-1"
      >
        Select Appointment 1
      </button>
      <button
        onClick={() => selectAll(mockAppointments)}
...
describe('BulkSelectionProvider', () => {
  const renderWithProvider = (component: React.ReactNode) => {
    return render(<BulkSelectionProvider>{component}</BulkSelectionProvider>);
  };
...
describe('BulkOperationConfirmationDialog', () => {
  ...
  it('should render cancel confirmation dialog', () => {
    ...
    expect(
      screen.getByText(/Are you sure you want to cancel 2 appointments/)
    ).toBeInTheDocument();
  });
  ...
  it('should render status update confirmation dialog', () => {
    ...
    expect(
      screen.getByText(/Update 2 appointments to "confirmed"/)
    ).toBeInTheDocument();
  });
