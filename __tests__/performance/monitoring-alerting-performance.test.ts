...
describe('Monitoring and Alerting Performance Tests', () => {
    ...
    mockGetConnectionMetrics.mockImplementation(() => {
      ...
      return {
        metrics: {
          ...
          slowQueries: performanceMetrics.filter(
            (m: any) => m.duration > ALERT_THRESHOLDS.averageResponseTime
          ).length,
        ...
        performance: {
          errorRate:
            performanceMetrics.filter((m: any) => !m.success).length /
            Math.max(performanceMetrics.length, 1),
          slowQueryRate:
            performanceMetrics.filter(
              (m: any) => m.duration > ALERT_THRESHOLDS.averageResponseTime
            ).length / Math.max(performanceMetrics.length, 1),
  ...
  const calculateAverageResponseTime = (): number => {
    ...
    return (
      performanceMetrics.reduce(
        (sum: any, metric: any) => sum + metric.duration,
        0
  ...
  const calculateP95ResponseTime = (): number => {
    if (performanceMetrics.length === 0) return 0;
    const sortedDurations = performanceMetrics
  ...
  const checkAlertThresholds = (metric: PerformanceMetrics) => {
    ...
    // Check error rate
    const errorRate =
      recentMetrics.filter((m: any) => !m.success).length /
  ...
  const triggerAlert = (
    ...
    console.log(
      `   Threshold: ${threshold}, Actual: ${actualValue.toFixed(2)}`
    );
  };
  ...
  describe('Real-time Performance Monitoring', () => {
    it('should monitor appointment operation performance in real-time', async () => {
      ...
      console.log(
        `Starting real-time monitoring of ${monitoringOperations} operations...`
      );
        ...
        try {
          switch (operationType) {
            case 'create':
              ...
              break;
            case 'read':
              await repositoryInstance.findById(
                `appointment-${i}`,
                testBusinessId
              );
              break;
            case 'update':
              await repositoryInstance.update(
                `appointment-${i}`,
                testBusinessId,
                { notes: `Updated ${i}` }
              );
    ...
    it('should detect performance degradation patterns over time', async () => {
      ...
      console.log(
        `Testing performance degradation detection over ${degradationTestPeriods} periods...`
      );
      ...
      for (let period = 0; period < degradationTestPeriods; period++) {
        ...
        const periodOperations = performanceMetrics.slice(
          periodStartMetrics,
          periodEndMetrics
        );
        const avgTime =
          periodOperations.reduce((sum: any, m: any) => sum + m.duration, 0) /
          periodOperations.length;
        const errorRate =
          periodOperations.filter((m: any) => !m.success).length /
          periodOperations.length;
      ...
      const firstHalfAvg =
        firstHalf.reduce((sum: any, p: any) => sum + p.avgTime, 0) /
        firstHalf.length;
      const secondHalfAvg =
        secondHalf.reduce((sum: any, p: any) => sum + p.avgTime, 0) /
        secondHalf.length;
      ...
      periodMetrics.forEach(({ period, avgTime, errorRate }) => {
        console.log(
          `  Period ${period}: ${avgTime.toFixed(2)}ms avg, ${(errorRate * 100).toFixed(1)}% errors`
        );
      });
    });
  });
  ...
  describe('Alert System Validation', () => {
    it('should trigger alerts when performance thresholds are exceeded', async () => {
      ...
      console.log(
        `Testing alert system with ${alertTestOperations} operations...`
      );
      ...
      const alertsByType = alertEvents.reduce(
        (acc: any, alert: any) => {
      ...
      const alertsBySeverity = alertEvents.reduce(
        (acc: any, alert: any) => {
    ...
    it('should escalate alerts based on severity and frequency', async () => {
      ...
      criticalAlerts = alertEvents.filter(
      ...
      const errorRateAlerts = alertEvents.filter(
        (a: any) => a.alertType === 'ERROR_RATE'
      );
      if (errorRateAlerts.length > 0) {
        expect(errorRateAlerts.some((a: any) => a.severity === 'HIGH')).toBe(
          true
        );
      }
    });
  ...
  describe('Resource Monitoring', () => {
    it('should monitor database connection pool utilization', async () => {
      ...
      console.log(
        `Monitoring connection pool utilization over ${connectionMonitoringOperations} operations...`
      );
      ...
      const avgUtilization =
        connectionMetricsHistory.reduce(
          (sum: any, m: any) => sum + m.utilization,
          0
        ) / connectionMetricsHistory.length;
      const maxUtilization = Math.max(
        ...connectionMetricsHistory.map((m: any) => m.utilization)
      );
      const utilizationSpikes = connectionMetricsHistory.filter(
        (m: any) => m.utilization > ALERT_THRESHOLDS.connectionUtilization
      ).length;

      expect(avgUtilization).toBeLessThan(
        ALERT_THRESHOLDS.connectionUtilization
      );
      ...
      console.log(
        `- Alert threshold: ${(ALERT_THRESHOLDS.connectionUtilization * 100).toFixed(1)}%`
      );
    });
    ...
    it('should monitor cache performance and hit rates', async () => {
      ...
      console.log(
        `Monitoring cache performance over ${cacheMonitoringOperations} operations...`
      );
      ...
      const overallHitRate =
        cacheMetricsHistory.reduce((sum: any, m: any) => sum + m.hitRate, 0) /
        cacheMetricsHistory.length;
      const avgResponseTime =
        cacheMetricsHistory.reduce(
          (sum: any, m: any) => sum + m.responseTime,
          0
        ) / cacheMetricsHistory.length;
    ...
    it('should detect memory leaks and resource exhaustion', async () => {
      ...
      console.log(
        `Monitoring memory usage over ${memoryMonitoringOperations} operations...`
      );
      ...
      console.log(
        `- Memory increase: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB (${memoryIncreasePercentage.toFixed(1)}%)`
      );
      ...
      memorySnapshots.forEach((snapshot, index) => {
        if (index % 2 === 0) {
          // Log every other snapshot
          console.log(
            `  Operation ${snapshot.operation}: ${(snapshot.heapUsed / 1024 / 1024).toFixed(2)}MB`
          );
        }
      });
    });
  ...
  describe('Performance Trend Analysis', () => {
    it('should identify performance trends and predict degradation', async () => {
      ...
      console.log(
        `Analyzing performance trends over ${trendAnalysisOperations} operations...`
      );
      ...
      for (
        let window = 0;
        window < trendAnalysisOperations / trendWindow;
        window++
      ) {
        ...
        const avgResponseTime =
          windowMetrics.reduce((sum: any, m: any) => sum + m.duration, 0) /
          windowMetrics.length;
        const errorRate =
          windowMetrics.filter((m: any) => !m.success).length /
          windowMetrics.length;
      ...
      console.log(
        `- Response time trend: ${responseTimeTrend > 0 ? 'Increasing' : 'Decreasing'} (${responseTimeTrend.toFixed(3)})`
      );
      console.log(
        `- Throughput trend: ${throughputTrend > 0 ? 'Increasing' : 'Decreasing'} (${throughputTrend.toFixed(3)})`
      );

      trendData.forEach(
        ({ window, avgResponseTime, errorRate, throughput }) => {
          console.log(
            `  Window ${window}: ${avgResponseTime.toFixed(2)}ms avg, ${(errorRate * 100).toFixed(1)}% errors, ${throughput.toFixed(1)} ops/sec`
          );
        }
      );
