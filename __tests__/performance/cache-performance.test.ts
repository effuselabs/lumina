...
describe('Cache Performance and Invalidation Tests', () => {
  ...
  describe('Cache Hit Performance', () => {
    ...
    it('should achieve >80% cache hit ratio under normal load', async () => {
      ...
      console.log(
        `- Average time per request: ${(totalExecutionTime / totalRequests).toFixed(2)}ms`
      );
    });
    ...
    it('should handle cache warming efficiently', async () => {
      ...
      expect(results.filter((r: any) => r !== null)).toHaveLength(
        warmingRequests.length
      );
      ...
      console.log(
        `- Average time per request: ${(totalExecutionTime / warmingRequests.length).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Cache Invalidation Performance', () => {
    ...
    it('should invalidate staff availability cache quickly when staff availability changes', async () => {
      ...
      expect(mockRedis.keys).toHaveBeenCalledWith(
        `availability:${businessId}:${staffIds[0]}:*`
      );
      expect(mockRedis.keys).toHaveBeenCalledWith(
        `availability:${businessId}:*`
      );
    ...
    it('should maintain performance during cache invalidation storms', async () => {
      ...
      console.log(`- Total execution time: ${totalExecutionTime.toFixed(2)}ms`);
      console.log(
        `- Average time per update: ${(totalExecutionTime / simultaneousUpdates.length).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Real-Time Update Performance', () => {
    it('should propagate availability changes in under 500ms', async () => {
      ...
      console.log(
        `- Initial query time: ${(initialEndTime - initialStartTime).toFixed(2)}ms`
      );
    ...
    it('should maintain cache consistency during rapid updates', async () => {
      ...
      expect(mockPrisma.businessHours.upsert).toHaveBeenCalledTimes(
        rapidUpdates.length
      );
      ...
      console.log(`- Total execution time: ${totalExecutionTime.toFixed(2)}ms`);
      console.log(
        `- Average time per update+query: ${(totalExecutionTime / rapidUpdates.length).toFixed(2)}ms`
      );
    });
  });
});
