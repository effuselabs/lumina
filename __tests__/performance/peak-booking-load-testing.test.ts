...
>;
const mockCacheCoordinator = CalendarCacheCoordinator as jest.MockedClass<
  typeof CalendarCacheCoordinator
>;
...
describe('Peak Booking Scenarios Load Testing', () => {
  ...
  describe('Monday Morning Rush Load Testing', () => {
    it('should handle Monday morning booking rush (200 concurrent bookings)', async () => {
      ...
      console.log(
        `Starting Monday Morning Rush Test: ${mondayRushLoad} bookings in ${timeWindow / 60000} minutes`
      );

      const bookingPromises = Array.from(
        { length: mondayRushLoad },
        async (_, index) => {
          // Simulate realistic booking pattern - more bookings at the start
          const delay =
            Math.random() *
            timeWindow *
            (index < mondayRushLoad / 2 ? 0.3 : 0.7);
      ...
      const warningsCount = results.reduce(
        (sum: any, result: any) => sum + result.warnings.length,
        0
      );
      ...
      console.log(
        `- Success rate: ${((successfulBookings.length / mondayRushLoad) * 100).toFixed(1)}%`
      );
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time per booking: ${(totalTime / mondayRushLoad).toFixed(2)}ms`
      );
    ...
    it('should maintain cache performance during Monday rush', async () => {
      ...
      cacheInstance.get.mockImplementation(async (key: any) => {
        await new Promise(resolve =>
          setTimeout(resolve, 2 + Math.random() * 6)
        );
        if (Math.random() < 0.6) {
          // 60% hit rate during high load
          cacheHits++;
  ...
  describe('Holiday Booking Surge Load Testing', () => {
    it('should handle holiday booking surge (500 concurrent bookings)', async () => {
      ...
      console.log(
        `Starting Holiday Surge Test: ${holidaySurgeLoad} bookings in ${surgeWindow / 60000} minutes`
      );
      ...
      const totalTime = burstTimes.reduce(
        (sum: any, time: any) => sum + time,
        0
      );
      expect(totalTime).toBeLessThan(LOAD_THRESHOLDS.HOLIDAY_BOOKING_SURGE);

      const successfulBookings = allResults.filter(
        (result: any) => result.success
      );
      const failedBookings = allResults.filter(
        (result: any) => !result.success
      );

      expect(successfulBookings.length).toBeGreaterThan(
        holidaySurgeLoad * 0.85
      ); // 85% success rate
      ...
      console.log(`- Failed bookings: ${failedBookings.length}`);
      console.log(
        `- Success rate: ${((successfulBookings.length / holidaySurgeLoad) * 100).toFixed(1)}%`
      );
      console.log(`- Total processing time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average burst time: ${(totalTime / numberOfBursts).toFixed(2)}ms`
      );
    });
    ...
    it('should handle calendar integration under holiday surge load', async () => {
      ...
      const calendarPromises = Array.from(
        { length: calendarOperations },
        async (_, index) => {
      ...
      expect(totalTime).toBeLessThan(
        LOAD_THRESHOLDS.HOLIDAY_BOOKING_SURGE * 0.8
      ); // Calendar ops should be faster
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time per operation: ${(totalTime / calendarOperations).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Promotional Campaign Load Testing', () => {
    it('should handle promotional campaign traffic spike (300 concurrent bookings with validation)', async () => {
      ...
      console.log(
        `Starting Promotional Campaign Test: ${campaignLoad} bookings with ${validationIntensity * 100}% validation`
      );
      ...
      console.log(
        `- Validation rate: ${(validationIntensity * 100).toFixed(0)}%`
      );
      console.log(`- Successful bookings: ${successfulBookings.length}`);
      console.log(`- Failed bookings: ${failedBookings.length}`);
      console.log(
        `- Success rate: ${((successfulBookings.length / campaignLoad) * 100).toFixed(1)}%`
      );
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
    ...
    it('should maintain performance during promotional validation surge', async () => {
      ...
      expect(totalTime).toBeLessThan(
        LOAD_THRESHOLDS.PROMOTIONAL_CAMPAIGN * 0.7
      ); // Validation should be faster
      expect(validationSuccesses + validationFailures).toBe(
      ...
      console.log(
        `- Success rate: ${((validationSuccesses / validationOperations) * 100).toFixed(1)}%`
      );
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average validation time: ${(totalTime / validationOperations).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Sustained High Load Testing', () => {
    it('should handle sustained high load (1000 operations over 30 seconds)', async () => {
      ...
      console.log(
        `Starting Sustained Load Test: ${sustainedOperations} operations over ${sustainedDuration / 1000} seconds`
      );
      ...
      for (let batch = 0; batch < numberOfBatches; batch++) {
        const batchOperations = Array.from(
          ...
          async (_, index) => {
            ...
            const operationType =
              operationTypes[globalIndex % operationTypes.length];

            switch (operationType) {
              case 'create':
                return serviceInstance.createAppointment(
                  createLoadTestAppointment(globalIndex)
                );
              case 'query':
              ...
              case 'update':
                return repositoryInstance.update(
                  `appointment-${globalIndex}`,
                  testBusinessId,
                  { notes: `Updated ${globalIndex}` }
                );
              case 'status':
      ...
      const totalTime = batchMetrics.reduce(
        (sum: any, batch: any) => sum + batch.time,
        0
      );
      ...
      const averageBatchTime =
        batchTimes.reduce((sum: any, time: any) => sum + time, 0) /
        batchTimes.length;
      ...
      console.log(
        `- Operations per second: ${(sustainedOperations / (totalTime / 1000)).toFixed(1)}`
      );
    });
    ...
    it('should handle database connection pool stress', async () => {
      ...
      for (let i = 0; i < dbStressOperations; i += maxConcurrentConnections) {
        const batch = Array.from(
          {
            length: Math.min(maxConcurrentConnections, dbStressOperations - i),
          },
          (_: any, j: any) => {
            const index = i + j;
            ...
            switch (operationType) {
              case 0:
                return repositoryInstance.create(
                  createLoadTestAppointment(index)
                );
              case 1:
                return repositoryInstance.findByBusinessOptimized(
                  testBusinessId,
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time per operation: ${(totalTime / dbStressOperations).toFixed(2)}ms`
      );
    });
  });
