...
describe('Appointment Performance Tests', () => {
  ...
  const measurePerformance = async <T>(
    ...
    console.log(
      `${operation}: ${duration.toFixed(2)}ms (threshold: ${threshold}ms) - ${withinThreshold ? 'PASS' : 'FAIL'}`
    );
  ...
  describe('Create Appointment Performance', () => {
    ...
    it('should handle concurrent appointment creation efficiently', async () => {
      ...
      console.log(
        `Concurrent Creation: ${totalDuration.toFixed(2)}ms total, ${averageDuration.toFixed(2)}ms average`
      );

      expect(averageDuration).toBeLessThanOrEqual(
        PERFORMANCE_THRESHOLDS.CREATE_APPOINTMENT
      );
  ...
  describe('Read Operation Performance', () => {
    ...
    it('should find appointments by staff within 300ms threshold', async () => {
      ...
      const { result, duration, withinThreshold } = await measurePerformance(
        ...
        () =>
          repository.findByStaffOptimized(
            testStaffId,
            testBusinessId,
            dateRange,
            { limit: 50 }
  ...
  describe('Pagination Performance', () => {
    ...
    it('should handle cursor-based pagination within 350ms threshold', async () => {
      ...
      expect(firstDuration).toBeLessThanOrEqual(
        PERFORMANCE_THRESHOLDS.PAGINATION
      );
      ...
      if (firstPage.nextCursor) {
        ...
        expect(secondDuration).toBeLessThanOrEqual(
          PERFORMANCE_THRESHOLDS.PAGINATION
        );
  ...
  describe('Conflict Detection Performance', () => {
    it('should detect conflicts within 250ms threshold', async () => {
      ...
      const { result, duration, withinThreshold } = await measurePerformance(
        ...
        () =>
          repository.findConflictsOptimized(
            testStaffId,
            testBusinessId,
            timeSlot
          )
  ...
  describe('Connection Pool Performance', () => {
    it('should maintain connection pool metrics within acceptable ranges', async () => {
      ...
      console.log('Connection Pool Metrics:', {
        ...
        slowQueryRate:
          (metrics.performance.slowQueryRate * 100).toFixed(2) + '%',
      });

      // Validate performance metrics
      expect(metrics.metrics.averageQueryTime).toBeLessThanOrEqual(
        PERFORMANCE_THRESHOLDS.FIND_BY_ID
      );
    ...
    it('should handle concurrent database operations efficiently', async () => {
      ...
      const operations = Array.from(
        ...
        async (_, index) => {
          ...
          if (index % 3 === 0) {
            return repository.findByBusinessOptimized(testBusinessId, {
              limit: 5,
            });
          } else if (index % 3 === 1) {
            return repository.findByStaffOptimized(
              testStaffId,
              testBusinessId,
              undefined,
              { limit: 5 }
      ...
      console.log(
        `Concurrent Operations: ${totalDuration.toFixed(2)}ms total, ${averageDuration.toFixed(2)}ms average`
      );
  ...
  describe('Performance Regression Tests', () => {
    it('should maintain consistent performance across multiple operations', async () => {
      ...
      console.log(
        `Performance Consistency: avg=${averageDuration.toFixed(2)}ms, min=${minDuration.toFixed(2)}ms, max=${maxDuration.toFixed(2)}ms, variance=${variance.toFixed(2)}ms`
      );
      ...
      expect(variance).toBeLessThanOrEqual(
        PERFORMANCE_THRESHOLDS.FIND_BY_BUSINESS * 0.5
      ); // Variance should be less than 50% of threshold
    });
  });
});
