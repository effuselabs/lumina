...
describe('Concurrent Booking Performance Tests', () => {
  ...
  describe('Concurrent Availability Requests', () => {
    test('should handle multiple simultaneous availability requests efficiently', async () => {
      ...
      expect(mockBookingApi.getAvailableSlots).toHaveBeenCalledTimes(
        concurrentRequests
      );
    });
    ...
    test('should maintain performance under high load', async () => {
      ...
      console.log(
        `High load test: ${highLoadRequests} requests in ${totalTime.toFixed(2)}ms`
      );
  ...
  describe('Concurrent Booking Attempts', () => {
    test('should handle race conditions in booking creation', async () => {
      ...
      const bookingRequests = Array.from(
        { length: concurrentBookings },
        (_, index) =>
          mockBookingApi
      ...
      const conflicts = results.filter(
        (result: any) => result.type === 'BOOKING_CONFLICT'
      );
    ...
    test('should maintain data consistency under concurrent load', async () => {
      ...
      console.log(
        `Concurrent booking test: ${concurrentUsers} users in ${totalTime.toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Memory and Resource Usage', () => {
    ...
    test('should handle timeout scenarios gracefully', async () => {
      ...
      await expect(Promise.race([apiPromise, timeoutPromise])).rejects.toThrow(
        'Request timeout'
      );
    });
  });
  ...
  describe('Performance Benchmarks', () => {
    test('should meet performance SLA requirements', async () => {
      ...
      // Calculate averages
      const avgBusinessInfo =
        performanceMetrics.businessInfoLoad.reduce((a: any, b: any) => a + b) /
        testRuns;
      const avgAvailability =
        performanceMetrics.availabilityCheck.reduce((a: any, b: any) => a + b) /
        testRuns;
      const avgBooking =
        performanceMetrics.bookingCreation.reduce((a: any, b: any) => a + b) /
        testRuns;
