...
describe('Availability System Load Testing', () => {
  ...
  describe('Concurrent User Load Testing (100+ Users)', () => {
    it('should handle 100 concurrent availability requests', async () => {
      ...
      console.log(
        `- Average time per request: ${(totalExecutionTime / concurrentRequests).toFixed(2)}ms`
      );
    });
    ...
    it('should handle mixed concurrent operations', async () => {
      ...
      const availabilityResults = results.filter(
        (r: any) => r.type === 'availability' && !('error' in r)
      );
      const conflictResults = results.filter(
        (r: any) => r.type === 'conflict' && !('error' in r)
      );
      const validationResults = results.filter(
        (r: any) => r.type === 'validation' && !('error' in r)
      );
      ...
      console.log(`- Failed: ${failedRequests.length}`);
      console.log(
        `- Availability operations: ${availabilityResults.length}/50`
      );
  ...
  describe('Sustained Load Testing', () => {
    it('should maintain performance under sustained load', async () => {
      ...
      const performanceDegradation =
        (lastBatchTime - firstBatchTime) / firstBatchTime;
    ...
    it('should handle peak load scenarios', async () => {
      ...
      const availabilityOps = results.filter(
        (r: any) => r.type === 'availability'
      );
      ...
      console.log(`- Failed: ${failedRequests.length}`);
      console.log(
        `- Availability ops: ${availabilityOps.filter((op: any) => op.success).length}/${availabilityOps.length}`
      );
      console.log(
        `- Conflict ops: ${conflictOps.filter((op: any) => op.success).length}/${conflictOps.length}`
      );
      console.log(
        `- Validation ops: ${validationOps.filter((op: any) => op.success).length}/${validationOps.length}`
      );
      console.log(`- Total time: ${totalExecutionTime.toFixed(2)}ms`);
      console.log(
        `- Average time per request: ${(totalExecutionTime / peakRequests.length).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Resource Utilization Under Load', () => {
    ...
    it('should handle database connection pooling efficiently', async () => {
      ...
      const batchPromises = Array.from(
        ...
        async (_, batchIndex) => {
          const requests = Array.from({ length: requestsPerBatch }, (_, i) => ({
            ...
            date: new Date('2024-01-15'),
            staffId:
              staffIds[(batchIndex * requestsPerBatch + i) % staffIds.length],
            duration: 60,
            includeUnavailable: false,
          }));
      ...
      batchResults.forEach((batch: any) => {
        console.log(
          `  Batch ${batch.batchIndex + 1}: ${batch.executionTime.toFixed(2)}ms, ${batch.successCount}/${requestsPerBatch} successful`
        );
      });
    });
  });
