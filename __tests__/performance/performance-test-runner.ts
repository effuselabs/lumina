...
export class PerformanceTestRunner {
    ...
    async runSustainedLoad<T>(
        ...
        const { durationMs, requestsPerSecond, name = 'Sustained Load Test' } = options

        const intervalMs = 1000 / requestsPerSecond
        const totalOperations = Math.floor(durationMs / intervalMs)

        console.log(`Starting ${name} for ${durationMs}ms at ${requestsPerSecond} req/sec...`)
     ...
     */
    private logMetrics(testName: string, metrics: PerformanceMetrics, operationCount: number): void {
        ...
        const peakIncrease = metrics.memoryUsage.peak.heapUsed - metrics.memoryUsage.initial.heapUsed
    ...
    assertPerformanceRequirements(
        ...
        if (maxMemoryIncreaseMB !== undefined) {
            const memoryIncreaseMB = (metrics.memoryUsage.final.heapUsed - metrics.memoryUsage.initial.heapUsed) / 1024 / 1024
...
export const PerformanceTestUtils = {
     ...
     */
    validateMemoryUsage: (metrics: PerformanceMetrics, maxIncreaseMB: number = 50) => {
        const memoryIncreaseMB = (metrics.memoryUsage.final.heapUsed - metrics.memoryUsage.initial.heapUsed) / 1024 / 1024
