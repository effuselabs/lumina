...
describe('Calendar Infrastructure Performance Benchmark Suite', () => {
  ...
  describe('Mixed Operation Performance Benchmarks', () => {
    ...
    it('should maintain performance under sustained mixed load', async () => {
        ...
        switch (opType) {
          ...
          case 1:
            return conflictEngine.detectConflicts({
              ...
              staffId: staffIds[operationCounter % staffIds.length],
              startTime: new Date(
                `2024-01-15T${10 + (operationCounter % 8)}:00:00Z`
              ),
              endTime: new Date(
                `2024-01-15T${11 + (operationCounter % 8)}:00:00Z`
              ),
      ...
      const metrics = await testRunner.runSustainedLoad(createMixedOperation, {
  ...
  describe('Scalability Benchmarks', () => {
    it('should scale linearly with increased staff count', async () => {
      ...
      results.forEach(({ staffCount, metrics }) => {
        ...
        console.log(
          `Staff Count ${staffCount}: Avg ${metrics.averageTime.toFixed(2)}ms, P95 ${metrics.p95Time.toFixed(2)}ms`
        );
      });
      ...
      const performanceDegradation =
        (largeStaffMetrics.averageTime - smallStaffMetrics.averageTime) /
        smallStaffMetrics.averageTime;
    ...
    it('should handle increased service complexity efficiently', async () => {
      ...
      results.forEach(({ duration, metrics }) => {
        ...
        console.log(
          `Duration ${duration}min: Avg ${metrics.averageTime.toFixed(2)}ms, P95 ${metrics.p95Time.toFixed(2)}ms`
        );
      });
    });
  });
  ...
  describe('Memory and Resource Benchmarks', () => {
    it('should maintain stable memory usage under load', async () => {
      ...
      const memoryIncrease =
        ...
        1024;
      console.log(
        `Memory increase: ${memoryIncrease.toFixed(2)}MB for ${operations.length} operations`
      );
    });
    ...
    it('should handle garbage collection efficiently', async () => {
      ...
      console.log(
        `Memory growth over ${rounds} rounds: ${memoryGrowth.toFixed(2)}MB`
      );
    });
  });
});
