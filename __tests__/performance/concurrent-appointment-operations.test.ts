...
>;
const mockAppointmentService = AppointmentService as jest.MockedClass<
  typeof AppointmentService
>;
const mockCalendarIntegration = CalendarIntegration as jest.MockedClass<
  typeof CalendarIntegration
>;
const mockMultiServiceCoordinator = MultiServiceCoordinator as jest.MockedClass<
  typeof MultiServiceCoordinator
>;
const mockStatusManager = AppointmentStatusManager as jest.MockedClass<
  typeof AppointmentStatusManager
>;
const mockAppointmentCache = AppointmentCache as jest.MockedClass<
  typeof AppointmentCache
>;
...
describe('Concurrent Appointment Operations Performance Tests', () => {
  ...
  const setupDefaultMocks = () => {
    ...
    repositoryInstance.updateStatus.mockImplementation(
      async (id, businessId, status) =>
        ({
          id,
          businessId,
          status,
  ...
  describe('Concurrent Appointment Creation', () => {
    it('should handle 50 concurrent appointment creations within performance threshold', async () => {
      ...
      const appointmentPromises = Array.from(
        { length: concurrentCreations },
        async (_, index) => {
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_CREATION
      );
    ...
    it('should maintain performance with calendar integration under concurrent load', async () => {
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_CREATION * 1.5
      ); // Allow 50% more time for calendar integration
      expect(results.filter((r: any) => r.success)).toHaveLength(
        concurrentRequests
      );
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentRequests).toFixed(2)}ms`
      );
    });
    ...
    it('should handle concurrent multi-service appointment creation', async () => {
      ...
      coordinatorInstance.validateMultiServiceBooking.mockImplementation(
        async services => {
          await new Promise(resolve =>
      ...
      const multiServicePromises = Array.from(
        { length: concurrentMultiService },
        async (_, index) => {
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_CREATION * 2
      ); // Allow more time for multi-service
      expect(results.filter((r: any) => r.success)).toHaveLength(
        concurrentMultiService
      );
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentMultiService).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Concurrent Appointment Updates', () => {
    it('should handle 30 concurrent appointment updates efficiently', async () => {
      ...
      const appointmentIds = Array.from(
        ...
        (_, i) => `appointment-update-${i}`
      );

      const updatePromises = appointmentIds.map(
        async (appointmentId, index) => {
          const updateData = {
          ...
          return serviceInstance.updateAppointment(
            appointmentId,
            testBusinessId,
            updateData
          );
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentUpdates).toFixed(2)}ms`
      );
    });
    ...
    it('should coordinate cache invalidation during concurrent updates', async () => {
      ...
      calendarInstance.invalidateAvailabilityCache.mockImplementation(
        async () => {
          await new Promise(resolve =>
            setTimeout(resolve, 5 + Math.random() * 15)
          ); // 5-20ms delay
      ...
      const updatePromises = Array.from(
        ...
        async (_, index) => {
          ...
          const result = await serviceInstance.updateAppointment(
            appointmentId,
            testBusinessId,
            updateData
          );
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_UPDATES * 1.5
      ); // Allow extra time for cache operations
      expect(results.filter((r: any) => r.success)).toHaveLength(
        concurrentUpdates
      );
  ...
  describe('Concurrent Status Changes', () => {
    it('should handle 25 concurrent status changes with proper synchronization', async () => {
      ...
      const appointmentIds = Array.from(
        { length: concurrentStatusChanges },
        (_, i) => `appointment-status-${i}`
      );
      ...
      statusManagerInstance.updateStatus.mockImplementation(
      ...
      const statusChangePromises = appointmentIds.map(
        async (appointmentId, index) => {
          const transition =
            statusTransitions[index % statusTransitions.length];
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_STATUS_CHANGES
      );
      expect(results.filter((r: any) => r.success)).toHaveLength(
        concurrentStatusChanges
      );
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentStatusChanges).toFixed(2)}ms`
      );
    });
    ...
    it('should handle batch status updates efficiently', async () => {
      ...
      const appointmentIds = Array.from(
        { length: batchSize },
        (_, i) => `batch-appointment-${i}`
      );

      repositoryInstance.batchUpdateStatus.mockImplementation(
        async (ids, businessId, status, reason) => {
          await new Promise(resolve =>
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Time per appointment: ${(totalTime / batchSize).toFixed(2)}ms`
      );
    });
  });
  ...
  describe('Concurrent Query Operations', () => {
    it('should handle 100 concurrent appointment queries efficiently', async () => {
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentQueries).toFixed(2)}ms`
      );
      console.log(
        `- Throughput: ${(concurrentQueries / (totalTime / 1000)).toFixed(1)} queries/second`
      );
    });
    ...
    it('should optimize cache usage during concurrent queries', async () => {
      ...
      cacheInstance.get.mockImplementation(async (key: any) => {
        await new Promise(resolve =>
          setTimeout(resolve, 1 + Math.random() * 3)
        ); // 1-4ms delay
        ...
        if (Math.random() < 0.7) {
          ...
          return {
            appointments: [
              { id: 'cached-appointment', businessId: testBusinessId },
            ],
            total: 1,
            hasMore: false,
          };
      ...
      const queryPromises = Array.from(
        ...
        async (_, index) => {
          ...
          if (!result) {
            ...
            result = await repositoryInstance.findByBusinessOptimized(
              testBusinessId,
              {
                limit: 20,
                offset: index * 20,
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONCURRENT_QUERIES * 0.8
      ...
      console.log(`- Queries: ${concurrentQueries}`);
      console.log(
        `- Cache hits: ${cacheHits} (${((cacheHits / concurrentQueries) * 100).toFixed(1)}%)`
      );
      console.log(
        `- Cache misses: ${cacheMisses} (${((cacheMisses / concurrentQueries) * 100).toFixed(1)}%)`
      );
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
    });
  });
  ...
  describe('Mixed Concurrent Operations', () => {
    it('should handle 100 mixed concurrent operations efficiently', async () => {
      ...
      const mixedOperations = Array.from(
          ...
          switch (operationType) {
            case 'create':
              return {
                type: 'create',
                operation: () =>
                  serviceInstance.createAppointment(
                    createTestAppointmentData(index)
                  ),
              };
            case 'update':
              return {
                type: 'update',
                operation: () =>
                  serviceInstance.updateAppointment(
                    `appointment-${index}`,
                    testBusinessId,
                    { notes: `Updated ${index}` }
                  ),
            ...
            case 'status':
              return {
                type: 'status',
                operation: () =>
                  statusManagerInstance.updateStatus(
                    `appointment-${index}`,
                    AppointmentStatus.CONFIRMED,
                    testBusinessId,
                    { changedBy: 'user-123' }
      ...
      const operationStats = operationTypes.map((type: any) => {
        ...
        const typeSuccessful = typeResults.filter(
          (result: any) => !('error' in result)
        );
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / totalOperations).toFixed(2)}ms`
      );
      console.log(
        `- Overall success rate: ${((successfulOperations.length / totalOperations) * 100).toFixed(1)}%`
      );

      operationStats.forEach((stat: any) => {
        console.log(
          `- ${stat.type}: ${stat.successful}/${stat.total} (${stat.successRate.toFixed(1)}%)`
        );
      });
    });
  });
  ...
  describe('Conflict Resolution Under Concurrency', () => {
    it('should handle concurrent conflict detection efficiently', async () => {
      ...
      const conflictCheckPromises = Array.from(
        { length: concurrentConflictChecks },
        async (_, index) => {
          const request = {
            businessId: testBusinessId,
            staffId: testStaffIds[index % testStaffIds.length],
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONFLICT_RESOLUTION
      );
      expect(results).toHaveLength(concurrentConflictChecks);

      const conflictsFound = results.filter(
        (result: any) => result.hasConflicts
      ).length;
      const noConflicts = results.filter(
        (result: any) => !result.hasConflicts
      ).length;
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average time: ${(totalTime / concurrentConflictChecks).toFixed(2)}ms`
      );
    ...
    it('should resolve conflicts and provide alternatives under load', async () => {
      ...
      calendarInstance.checkAvailability.mockImplementation(
        async (staffId, timeSlot) => {
          await new Promise(resolve =>
          ...
          return {
            ...
            alternatives: isAvailable
              ...
              : [
                  {
                    startTime: new Date(
                      timeSlot.startTime.getTime() + 60 * 60 * 1000
                    ),
                    endTime: new Date(
                      timeSlot.endTime.getTime() + 60 * 60 * 1000
                    ),
                    staffId,
      ...
      const resolutionPromises = Array.from(
        { length: concurrentResolutions },
        async (_, index) => {
          const timeSlot = {
          ...
          return calendarInstance.checkAvailability(
            staffId,
            timeSlot,
            serviceIds
          );
      ...
      expect(totalTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.CONFLICT_RESOLUTION * 1.5
      ); // Allow more time for alternatives
      expect(results).toHaveLength(concurrentResolutions);

      const availableSlots = results.filter(
        (result: any) => result.isAvailable
      ).length;
      const unavailableSlots = results.filter(
        (result: any) => !result.isAvailable
      ).length;
      const alternativesProvided = results.filter(
        (result: any) => result.alternatives.length > 0
      ).length;
  ...
  describe('Performance Monitoring and Metrics', () => {
    it('should maintain connection pool efficiency under concurrent load', async () => {
      ...
      console.log(`- Total time: ${totalTime.toFixed(2)}ms`);
      console.log(
        `- Average query time: ${metrics.metrics.averageQueryTime.toFixed(2)}ms`
      );
      console.log(
        `- Error rate: ${(metrics.performance.errorRate * 100).toFixed(2)}%`
      );
      console.log(
        `- Slow query rate: ${(metrics.performance.slowQueryRate * 100).toFixed(2)}%`
      );
    });
    ...
    it('should track performance degradation over sustained load', async () => {
      ...
      for (let round = 0; round < sustainedRounds; round++) {
        const roundOperations = Array.from(
          { length: operationsPerRound },
          async (_, index) => {
            const globalIndex = round * operationsPerRound + index;
            return serviceInstance.createAppointment(
              createTestAppointmentData(globalIndex)
            );
          }
        );
      ...
      const firstHalfAvg =
        performanceMetrics
          .slice(0, sustainedRounds / 2)
          .reduce((sum: any, time: any) => sum + time, 0) /
        (sustainedRounds / 2);

      const secondHalfAvg =
        performanceMetrics
          .slice(sustainedRounds / 2)
          .reduce((sum: any, time: any) => sum + time, 0) /
        (sustainedRounds / 2);
