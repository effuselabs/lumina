...
describe('DataResetManager', () => {
  ...
  describe('cleanReset', () => {
    beforeEach(() => {
      ...
      (prisma.staffService.deleteMany as jest.Mock).mockResolvedValue({
        count: 25,
      });
      (prisma.appointmentService.deleteMany as jest.Mock).mockResolvedValue({
        count: 75,
      });
      (prisma.businessUser.deleteMany as jest.Mock).mockResolvedValue({
        count: 3,
      });
    ...
    it('should handle business not found error', async () => {
      ...
      expect(result.errors).toContain(
        `Business with ID ${mockBusinessId} not found`
      );
    });
    ...
    it('should detect appointments without services', async () => {
      ...
      expect(
        result.errors.some((e: any) =>
          e.message.includes('appointments without services')
        )
      ).toBe(true);
    });
    ...
    it('should detect staff without services as warning', async () => {
      ...
      expect(
        result.warnings.some((w: any) =>
          w.message.includes('staff members without assigned services')
        )
      ).toBe(true);
    });
  });
  ...
  describe('validateConfiguration', () => {
    ...
    it('should detect invalid age range distribution', () => {
      ...
      expect(
        result.errors.some((e: any) =>
          e.message.includes('Age range distribution')
        )
      ).toBe(true);
    });
    ...
    it('should detect performance warnings for large datasets', () => {
      ...
      expect(
        result.warnings.some((w: any) =>
          w.message.includes('Large client count')
        )
      ).toBe(true);
      expect(
        result.warnings.some((w: any) =>
          w.message.includes('Long historical period')
        )
      ).toBe(true);
    });
  });
