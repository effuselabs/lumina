...
import {
  appointmentCache,
  useAppointmentCache,
} from '@/lib/cache/appointment-cache';
...
describe('AppointmentCacheManager', () => {
  ...
  describe('Cache Operations', () => {
    it('should get appointment from cache', async () => {
      ...
      expect(mockRedisInstance.get).toHaveBeenCalledWith(
        CacheKeyGenerator.appointmentById(
          mockAppointment.id,
          mockAppointment.businessId
        )
      );
  ...
  describe('List Caching', () => {
    ...
    it('should set appointment list with custom TTL', async () => {
      ...
      await cacheManager.setAppointmentList(
        cacheKey,
        appointmentList,
        customTTL
      );
  ...
  describe('Statistics Caching', () => {
    it('should cache and retrieve appointment statistics', async () => {
      ...
      const result = await cacheManager.getStatistics(
        'test-business-1',
        dateRange
      );
    ...
    it('should set statistics with appropriate TTL', async () => {
      ...
      await cacheManager.setStatistics(
        'test-business-1',
        dateRange,
        statistics
      );
  ...
  describe('Conflict Check Caching', () => {
    ...
    it('should set conflict check results with short TTL', async () => {
      ...
      expect(mockRedisInstance.setex).toHaveBeenCalledWith(
        CacheKeyGenerator.appointmentConflicts(
          'test-staff-1',
          'test-business-1',
          timeSlot
        ),
  ...
  describe('Cache Invalidation', () => {
    ...
    it('should invalidate staff appointments by pattern', async () => {
      ...
      await cacheManager.invalidateStaffAppointments(
        'test-staff-1',
        'test-business-1'
      );

      expect(mockRedisInstance.keys).toHaveBeenCalledWith(
        CacheKeyGenerator.staffAppointmentList(
          'test-staff-1',
          'test-business-1'
        )
      );
...
describe('CacheKeyGenerator', () => {
  describe('Key Generation', () => {
    ...
    it('should generate business appointment list keys with filters', () => {
      ...
      const key = CacheKeyGenerator.appointmentsByBusiness(
        'business-1',
        filters
      );
    ...
    it('should generate staff appointment keys with date range', () => {
      ...
      const key = CacheKeyGenerator.appointmentsByStaff(
        'staff-1',
        'business-1',
        dateRange
      );
    ...
    it('should generate conflict check keys', () => {
      ...
      const key = CacheKeyGenerator.appointmentConflicts(
        'staff-1',
        'business-1',
        timeSlot
      );
    ...
    it('should generate statistics keys', () => {
      ...
      const key = CacheKeyGenerator.appointmentStatistics(
        'business-1',
        dateRange
      );
  ...
  describe('Related Keys Generation', () => {
    it('should generate all related keys for appointment', () => {
      const keys = CacheKeyGenerator.appointmentRelatedKeys(
        'app-1',
        'business-1',
        'staff-1'
      );
...
describe('CalendarCacheCoordinator', () => {
  ...
  describe('Event Handling', () => {
    ...
    it('should handle appointment updated events', async () => {
      ...
      await coordinator.onAppointmentUpdated('app-1', 'business-1', 'staff-1', {
        startTime: new Date('2024-01-15T10:00:00Z'),
        endTime: new Date('2024-01-15T11:00:00Z'),
      });
    ...
    it('should handle appointment deleted events', async () => {
      ...
      await coordinator.onAppointmentDeleted('app-1', 'business-1', 'staff-1', {
        startTime: new Date('2024-01-15T10:00:00Z'),
        endTime: new Date('2024-01-15T11:00:00Z'),
      });
...
describe('Cache Integration', () => {
  it('should coordinate cache invalidation between systems', async () => {
    ...
    // Test coordination
    await coordinator.onAppointmentCreated('app-1', 'business-1', 'staff-1', {
      startTime: new Date('2024-01-15T10:00:00Z'),
      endTime: new Date('2024-01-15T11:00:00Z'),
    });
