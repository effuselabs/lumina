...
class PerformanceTestRunner {
  ...
  async measureComponentRender(selector: string): Promise<PerformanceMetrics> {
    ...
    const performanceMetrics = await this.page.evaluate(() => {
      ...
      return {
        renderTime:
          entries.find(e => e.name === 'component-render')?.duration || 0,
        paintTime:
          paintEntries.find(e => e.name === 'first-contentful-paint')
            ?.startTime || 0,
        layoutTime:
          navigationEntries[0]?.domContentLoadedEventEnd -
            navigationEntries[0]?.domContentLoadedEventStart || 0,
        scriptTime:
          navigationEntries[0]?.loadEventEnd -
            navigationEntries[0]?.loadEventStart || 0,
      };
    });
  ...
  async measureThemeSwitch(): Promise<ThemeSwitchMetrics> {
    ...
    droppedFrames = frames.filter(
      (duration: number) =>
        duration > PERFORMANCE_THRESHOLDS.animationFrame * 1.5
    ).length;
  ...
  getAverageMetrics(): PerformanceMetrics {
    ...
    return {
      renderTime:
        ...
        this.metrics.length,
      paintTime:
        this.metrics.reduce((sum, m) => sum + m.paintTime, 0) /
        this.metrics.length,
      layoutTime:
        this.metrics.reduce((sum, m) => sum + m.layoutTime, 0) /
        this.metrics.length,
      scriptTime:
        this.metrics.reduce((sum, m) => sum + m.scriptTime, 0) /
        this.metrics.length,
      memoryUsage:
...
test.describe('Performance Regression Tests', () => {
  ...
  test.describe('Component Rendering Performance', () => {
    const components = [
      {
        ...
        selector: '[data-testid="button-primary-md"]',
      },
      {
        name: 'StatCard',
        route: '/design-system/stat-cards',
        selector: '[data-testid="stat-card-default"]',
      },
      {
        name: 'Input',
        route: '/design-system/forms',
        selector: '[data-testid="input-default"]',
      },
      {
        name: 'Select',
        route: '/design-system/forms',
        selector: '[data-testid="select-default"]',
      },
      {
        name: 'PageHeader',
        route: '/design-system/page-header',
        selector: '[data-testid="page-header-default"]',
      },
    ];
    ...
    components.forEach(component => {
      test(`${component.name} rendering performance`, async ({ page }) => {
        ...
        const metrics = await performanceRunner.measureComponentRender(
          component.selector
        );
        ...
        expect(metrics.paintTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.paintTime
        );
        expect(metrics.layoutTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.layoutTime
        );
        expect(metrics.scriptTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.scriptTime
        );
    ...
    test('Multiple component rendering performance', async ({ page }) => {
      ...
      expect(averageMetrics.renderTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.componentRender
      );
    });
  });
  ...
  test.describe('Theme Switching Performance', () => {
    ...
    routes.forEach(route => {
      test(`Theme switching performance - ${route}`, async ({ page }) => {
        ...
        // Assert performance thresholds
        expect(metrics.switchTime).toBeLessThan(
          PERFORMANCE_THRESHOLDS.themeSwitch
        );
    ...
    test('Rapid theme switching performance', async ({ page }) => {
      ...
      expect(averageSwitchTime).toBeLessThan(
        PERFORMANCE_THRESHOLDS.themeSwitch / 2
      ); // Even faster for rapid switches
    });
  });
  ...
  test.describe('Animation Performance', () => {
    ...
    test('Loading animation performance', async ({ page }) => {
      ...
      for (const element of loadingElements) {
        ...
        const goodFrames = (frameRates as number[]).filter(
          duration => duration <= PERFORMANCE_THRESHOLDS.animationFrame * 1.2
        );
        const framePerformance =
          (goodFrames.length / (frameRates as number[]).length) * 100;
  ...
  test.describe('Memory Usage Performance', () => {
    ...
    test('Theme switching memory usage', async ({ page }) => {
      ...
      expect(memoryIncrease).toBeLessThan(
        PERFORMANCE_THRESHOLDS.memoryUsage / 4
      ); // 25% of threshold
    });
  });
  ...
  test.describe('Responsive Performance', () => {
    ...
    breakpoints.forEach(breakpoint => {
      test(`Responsive layout performance - ${breakpoint.name}`, async ({
        page,
      }) => {
