...
test.describe('Appointment Management E2E', () => {
    ...
    test('should complete full appointment creation workflow', async ({ page }) => {
        ...
        await page.selectOption('[data-testid="service-select"]', testAppointment.service)
        await page.selectOption('[data-testid="staff-select"]', testAppointment.staff)

        // Verify price and duration are calculated
        await expect(page.locator('[data-testid="total-price"]')).toContainText('$50.00')
        await expect(page.locator('[data-testid="total-duration"]')).toContainText('60 minutes')
        ...
        const appointmentBlock = page.locator('[data-testid="appointment-block"]').filter({ hasText: testAppointment.client })
    ...
    test('should handle appointment rescheduling via drag-and-drop', async ({ page }) => {
        // Wait for existing appointment to be visible
        const appointment = page.locator('[data-testid="appointment-block"]').first()
        await expect(appointment).toBeVisible()

        // Get original position
        const originalSlot = await appointment.locator('..').getAttribute('data-testid')
        ...
        // Verify success message
        await expect(page.locator('[data-testid="success-message"]')).toContainText('Appointment rescheduled successfully')

        // Verify appointment moved to new slot
        const rescheduledAppointment = page.locator('[data-testid="time-slot-11-00"] [data-testid="appointment-block"]')
        await expect(rescheduledAppointment).toBeVisible()

        // Verify original slot is empty
        const originalSlotElement = page.locator(`[data-testid="${originalSlot}"] [data-testid="appointment-block"]`)
        await expect(originalSlotElement).not.toBeVisible()
    })

    test('should manage appointment conflicts appropriately', async ({ page }) => {
        ...
        await page.selectOption('[data-testid="service-select"]', 'Color Treatment')
        ...
        await expect(page.locator('text=Time slot conflicts with existing appointment')).toBeVisible()
    ...
    test('should sync updates across multiple user sessions', async ({ page, context }) => {
        ...
        await expect(page.locator('[data-testid="appointment-block"]').filter({ hasText: 'Multi Session Test' })).toBeVisible()

        // Verify appointment appears in second session (real-time sync)
        await expect(secondPage.locator('[data-testid="appointment-block"]').filter({ hasText: 'Multi Session Test' })).toBeVisible({ timeout: 10000 })

        // Edit appointment in second session
        await secondPage.click('[data-testid="appointment-block"]').filter({ hasText: 'Multi Session Test' })
        await secondPage.click('[data-testid="edit-button"]')
        await secondPage.fill('[data-testid="notes-input"]', 'Updated from second session')
        await secondPage.click('[data-testid="save-button"]')

        // Verify update appears in first session
        await page.click('[data-testid="appointment-block"]').filter({ hasText: 'Multi Session Test' })
        await expect(page.locator('[data-testid="appointment-notes"]')).toContainText('Updated from second session')
    ...
    test('should handle search and filtering workflows', async ({ page }) => {
        ...
        const visibleAppointments = page.locator('[data-testid="appointment-block"]:visible')
        ...
        const aliceAppointments = page.locator('[data-testid="appointment-block"]:visible')
        ...
        const filteredAppointments = page.locator('[data-testid="appointment-block"]:visible')
        ...
        const todayAppointments = page.locator('[data-testid="appointment-block"]:visible')
    ...
    test('should support keyboard navigation and accessibility', async ({ page }) => {
        ...
        const firstAppointment = page.locator('[data-testid="appointment-block"]').first()
        await expect(firstAppointment).toBeFocused()

        // Navigate with arrow keys
        await page.keyboard.press('ArrowRight')
        const secondAppointment = page.locator('[data-testid="appointment-block"]').nth(1)
        ...
        await expect(page.locator('[role="main"]')).toHaveAttribute('aria-label', 'Appointment Dashboard')
        await expect(page.locator('[role="tablist"]')).toHaveAttribute('aria-label', 'Calendar Views')
        await expect(page.locator('[role="grid"]')).toHaveAttribute('aria-label', 'Calendar Grid')
    })
    ...
    test('should handle bulk operations correctly', async ({ page }) => {
        ...
        // Verify completion
        await expect(page.locator('[data-testid="bulk-success"]')).toContainText('2 appointments rescheduled successfully')
        ...
        await expect(page.locator('[data-testid="bulk-success"]')).toContainText('1 appointment cancelled successfully')
    })
    ...
    test('should handle error scenarios gracefully', async ({ page }) => {
        ...
        await expect(page.locator('[data-testid="error-message"]')).toContainText('Failed to create appointment')
