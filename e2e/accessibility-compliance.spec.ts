...
test.describe('Accessibility Compliance', () => {
  ...
  test('Design system page passes axe accessibility audit', async ({
    page,
  }) => {
    ...
    for (const theme of themes) {
      ...
      if (accessibilityScanResults.incomplete.length > 0) {
        console.log(
          `⚠️  Incomplete accessibility tests in ${theme} theme:`,
  ...
  test('Color contrast meets WCAG AA standards', async ({ page }) => {
    ...
    for (const theme of themes) {
      ...
      const textElements = await page
        .locator('h1, h2, h3, h4, h5, h6, p, span, a, button, label')
        .all();
  ...
  test('Keyboard navigation works correctly', async ({ page }) => {
    ...
    if (dialogCount > 0) {
      ...
      const dialogTrigger = page.locator(
        '[data-testid*="dialog-trigger"], [data-testid*="modal-trigger"]'
      );
  ...
  test('Form elements have proper labels and ARIA attributes', async ({
    page,
  }) => {
    await page.goto('/design-system');
    await page.waitForLoadState('networkidle');
  ...
  test('Interactive elements have appropriate ARIA roles and states', async ({
    page,
  }) => {
    await page.goto('/design-system');
    await page.waitForLoadState('networkidle');
    ...
    for (let i = 0; i < linkCount; i++) {
      ...
      if (href && href.startsWith('http') && !href.includes(page.url())) {
        ...
        if (!indicatesExternal) {
          console.warn(
            `External link may need accessibility indicator: ${href}`
          );
        }
      }
    }
    ...
    const customInteractive = page.locator(
  ...
  test('Focus management in dynamic content', async ({ page }) => {
    ...
    if ((await themeToggle.count()) > 0) {
      ...
      const focusedElement = page.locator(':focus');
      const isSameElement = await focusedElement.evaluate(
        (focused, toggle) => {
    ...
    const modalTrigger = page.locator(
      '[data-testid*="modal-trigger"], [data-testid*="dialog-trigger"]'
    );
    if ((await modalTrigger.count()) > 0) {
      ...
      if ((await modal.count()) > 0) {
        ...
        if (focusableCount > 1) {
          ...
          const isFirstFocused = await firstFocusable.evaluate(
        ...
        const triggerFocused = await modalTrigger
          .first()
    ...
    if (skipLinkCount > 0) {
      ...
      if (href) {
        ...
        if ((await targetElement.count()) > 0) {
          const targetFocused = await targetElement.evaluate(
            el => el === document.activeElement
          );
