...
test.describe('Keyboard Navigation Tests', () => {
    ...
    test.describe(`${config.name} keyboard navigation`, () => {
      test('Tab navigation forward and backward', async ({ page }) => {
        ...
        if (config.expectedFocusableCount) {
          expect(focusableElements.length).toBeGreaterThanOrEqual(
            config.expectedFocusableCount - 5
          );
          expect(focusableElements.length).toBeLessThanOrEqual(
            config.expectedFocusableCount + 5
          );
        }
      ...
      test('Skip links functionality', async ({ page }) => {
        ...
        expect(
          focusedId?.toLowerCase().includes('main') || focusedRole === 'main'
        ).toBe(true);
      });
      ...
      test('Arrow key navigation in component groups', async ({ page }) => {
        ...
        const buttonGroups = await page
          .locator(
            '[role="group"], .button-group, [data-testid*="button-group"]'
          )
          .all();
      ...
      test('Enter and Space key activation', async ({ page }) => {
        ...
        for (let i = 0; i < Math.min(buttons.length, 5); i++) {
          ...
          if (isClickable) {
            ...
            const wasActivated = await button.getAttribute(
              'data-test-activated'
            );
            ...
            const wasActivatedBySpace = await button.getAttribute(
              'data-test-activated'
            );
      ...
      test('Focus trap in modals and dropdowns', async ({ page }) => {
        ...
        const dropdownTriggers = await page
          .locator('[aria-haspopup="true"], select, [role="combobox"]')
          .all();
        ...
        for (const trigger of dropdownTriggers) {
          ...
          if (isOpen) {
            ...
            const focusStayedInDropdown = await newFocus.evaluate(el => {
              return (
                el.closest('[role="listbox"], [role="menu"], select') !== null
              );
            });
      ...
      test('Custom keyboard shortcuts', async ({ page }) => {
        ...
        for (const shortcut of shortcuts) {
          ...
          if (focusableElements.length > 0) {
            await focusableElements[
              Math.floor(focusableElements.length / 2)
            ].focus();
  ...
  test.describe('Cross-theme keyboard navigation', () => {
    test('Keyboard navigation consistency across themes', async ({ page }) => {
      ...
      expect(darkThemeFocusableElements.length).toBe(
        lightThemeFocusableElements.length
      );
  ...
  test.describe('Responsive keyboard navigation', () => {
    ...
    breakpoints.forEach(breakpoint => {
      test(`Keyboard navigation at ${breakpoint.name} breakpoint`, async ({
        page,
      }) => {
