...
export async function testAccessibilityStates(
    ...
    await element.page().emulateMedia({ colorScheme: 'dark', reducedMotion: 'no-preference' });
    await expect(element).toHaveScreenshot(
    ...
    for (const theme of defaultVisualConfig.themes) {
        for (const breakpoint of defaultVisualConfig.breakpoints) {
            ...
            if (testAccessibility) {
                const element = page.locator(`[data-testid="${componentName}-default"]`);
                if (await element.count() > 0) {
                    await testAccessibilityStates(element, componentName, theme, breakpoint.name);
                }
            }
        }
    }
