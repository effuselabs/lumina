...
mobileDevices.forEach(device => {
    test.describe(`Mobile Appointment Management - ${device.name}`, () => {
        ...
        test('should display mobile-optimized calendar interface', async ({ page }) => {
        ...
        test('should support touch gestures for navigation', async ({ page }) => {
            ...
            // Verify date changed
            await expect(page.locator('[data-testid="current-date"]')).not.toContainText('15')
        ...
        test('should handle touch interactions for appointments', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="mobile-appointment-modal"]')).toBeVisible()
        ...
        test('should support long press for context menu', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="context-reschedule"]')).toBeVisible()
            await expect(page.locator('[data-testid="context-cancel"]')).toBeVisible()

            // Test quick reschedule
            await page.click('[data-testid="context-reschedule"]')
            await expect(page.locator('[data-testid="quick-reschedule-picker"]')).toBeVisible()
        })
        ...
        test('should handle mobile search and filters', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="staff-filter-options"]')).toBeVisible()

            await page.click('[data-testid="staff-alice"]')
            await expect(page.locator('[data-testid="filter-chip-alice"]')).toBeVisible()

            // Test clearing filters
            await page.click('[data-testid="clear-filters"]')
            await expect(page.locator('[data-testid="filter-chip-alice"]')).not.toBeVisible()
        })

        test('should support mobile appointment creation', async ({ page }) => {
            // Test floating action button for new appointment
            const fabButton = page.locator('[data-testid="mobile-fab-new-appointment"]')
            ...
            await page.fill('[data-testid="mobile-client-input"]', 'Mobile Test Client')

            // Test mobile service picker
            await page.click('[data-testid="mobile-service-picker"]')
            await expect(page.locator('[data-testid="service-picker-modal"]')).toBeVisible()
            ...
            await expect(page.locator('[data-testid="datetime-picker-modal"]')).toBeVisible()
            ...
            await expect(page.locator('[data-testid="appointment-block"]').filter({ hasText: 'Mobile Test Client' })).toBeVisible()
        })

        test('should handle mobile drag and drop for rescheduling', async ({ page }) => {
            const appointment = page.locator('[data-testid="appointment-block"]').first()
            ...
            if (appointmentBox && targetBox) {
                ...
                await expect(page.locator('[data-testid="mobile-reschedule-confirm"]')).toBeVisible()
                await page.click('[data-testid="confirm-reschedule"]')

                // Verify appointment moved
                await expect(page.locator('[data-testid="time-slot-11-00"] [data-testid="appointment-block"]')).toBeVisible()
            }
        })

        test('should support pinch-to-zoom for calendar views', async ({ page }) => {
            const calendarView = page.locator('[data-testid="mobile-calendar-view"]')
        ...
        test('should handle mobile orientation changes', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="landscape-calendar-view"]')).toBeVisible()
        ...
        test('should handle mobile performance optimization', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="loading-more"]')).not.toBeVisible({ timeout: 5000 })
        ...
        test('should support offline functionality', async ({ page }) => {
            ...
            await expect(page.locator('[data-testid="appointment-block"]').first()).toBeVisible()
            ...
            await expect(page.locator('[data-testid="queued-appointment"]')).toBeVisible()
...
test.describe('Cross-Device Mobile Compatibility', () => {
    test('should maintain consistency across different mobile devices', async ({ browser }) => {
