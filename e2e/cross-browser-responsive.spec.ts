...
import { defaultVisualConfig, setupVisualTest } from './utils/visual-test-helpers';
    ...
    browsers.forEach(browserName => {
        test.describe(`${browserName} browser tests`, () => {
            ...
            testRoutes.forEach(route => {
                test(`${route} renders correctly in ${browserName}`, async ({ page }) => {
            ...
            test(`Interactive elements work correctly in ${browserName}`, async ({ page }) => {
                await page.goto('/design-system');
                await page.waitForLoadState('networkidle');

                // Test button interactions
                const primaryButton = page.locator('[data-testid="button-primary"]').first();
...
test.describe('Responsive Design Testing', () => {
    ...
    breakpoints.forEach(breakpoint => {
        test.describe(`${breakpoint.name} (${breakpoint.width}x${breakpoint.height})`, () => {
            ...
            testRoutes.forEach(route => {
                test(`${route} responsive layout at ${breakpoint.name}`, async ({ page }) => {
                    await page.goto(route);
                    await page.waitForLoadState('networkidle');
                ...
                const metrics = await page.evaluate(() => {
                    return new Promise(resolve => {
                        new PerformanceObserver((list) => {
                            ...
                            const navigation = entries.find(entry => entry.entryType === 'navigation') as any;
