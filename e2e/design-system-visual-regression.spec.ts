...
test.describe('Design System Visual Regression', () => {
  ...
  test('Color palette visual regression', async ({ page }) => {
    ...
    for (const theme of defaultVisualConfig.themes) {
      for (const breakpoint of defaultVisualConfig.breakpoints) {
        ...
        const complementaryColors = page.locator(
          '[data-testid="complementary-colors"]'
        );
    ...
    for (const theme of defaultVisualConfig.themes) {
      for (const breakpoint of defaultVisualConfig.breakpoints) {
        ...
        for (const element of typographyElements) {
          const typographyElement = page.locator(
  ...
  test('Accessibility visual states', async ({ page }) => {
    ...
    for (const theme of defaultVisualConfig.themes) {
      ...
      for (let i = 0; i < Math.min(count, 5); i++) {
        ...
        const testId =
          (await element.getAttribute('data-testid')) || `focusable-${i}`;
        await expect(element).toHaveScreenshot(`focus-${testId}-${theme}.png`);
      }
    }
  ...
  test('Loading and error states visual regression', async ({ page }) => {
    ...
    const componentsWithStates = ['button', 'card', 'form', 'data-table'];
  ...
  test.afterAll(async () => {
    ...
    if (summary.failed > 0) {
      ...
      console.log(
        'Run with --update-snapshots to update baseline images if changes are intentional.'
      );
    }
